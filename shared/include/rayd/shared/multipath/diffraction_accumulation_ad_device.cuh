// Shared device body for the four diffraction accumulation AD kernels
// (dfr_direct_accum_jvp/vjp, dfr_chain_accum_jvp/vjp).
//
// This is a TEXTUAL DEVICE-BODY FRAGMENT, not a standalone header. Each backend
// includes it INSIDE its own anonymous namespace, after declaring its Params
// structs and its storage-access layer. The algorithm text below is the single
// owner of the AD arithmetic; every load/store whose spelling differs between
// the backends (dense Dr.Jit reads vs nullable strided Torch reads) is routed
// through an includer-provided macro so that each instantiation reproduces the
// backend's pre-dedup expression tree exactly. Both consuming translation units
// compile under the nvcc_default numeric profile (see
// shared/contracts/compile_policy.json); the SASS of all four kernels was
// verified bitwise identical against the pre-dedup binaries in both backends.
//
// The includer MUST define, before including this fragment:
//
//   types
//     DfrDirectAccumADParams, DfrChainAccumADParams  (backend Params structs)
//
//   float3 primitives (each backend keeps its own historical spellings)
//     operator+(float3, float3), operator-(float3, float3),
//     operator*(float, float3), dot3, norm3, cross3, normalize3
//
//   storage-access macros (P = params, I = index, D = default)
//     RAYD_DFR_AD_READ_F32(P, field, stride_field, I)      raw primal f32 read
//     RAYD_DFR_AD_READ_I32(P, field, stride_field, I)      raw primal i32 read
//     RAYD_DFR_AD_READ_I32_OR(P, field, stride_field, I, D) guarded i32 read
//     RAYD_DFR_AD_READ_VEC(P, x, y, z, stride_field, I)    raw primal vec3 read
//     RAYD_DFR_AD_READ_VEC_GUARDED(P, x, y, z, stride_field, I)
//                                     per-component nullable vec3 read (wi)
//     RAYD_DFR_AD_READ_DOT_F32(P, field, stride_field, I)  nullable tangent f32
//     RAYD_DFR_AD_READ_DOT_VEC(P, x, y, z, stride_field, I) nullable tangent vec3
//     RAYD_DFR_AD_MATERIAL_VALID_ENTRY(P, I)  material_valid[] test, non-null ptr
//     RAYD_DFR_AD_MATERIAL_GAIN_TAPE(P, I)    gain read on the direct tape path
//     RAYD_DFR_AD_MATERIAL_GAIN_EVENT(P, I)   gain read inside chain_event_weight
//     RAYD_DFR_AD_MATERIAL_GAIN_OR_ONE(P, I)  gain read defaulting to 1.f
//     RAYD_DFR_AD_SUFFIX_FACE_PRIM(P, field, stride_field, HAS_THIRD, SECOND, THIRD)
//                                     terminal-event face prim selection
//
// The unit-VJP enumerations (vjp_by_unit_jvps / chain_vjp_by_unit_jvps) live in
// the companion fragment diffraction_accumulation_ad_vjp_device.cuh, included
// by the backend AFTER it defines its gradient-write helpers (add_unit_vjp and
// friends) against the types declared here. Keeping those helpers backend-local
// preserves each backend's exact gradient-write expression shape, which the
// bitwise SASS gate depends on.
//
// No #include directives may appear here: the fragment expands inside a
// namespace. The __global__ kernels, launch wrappers, and Params structs stay
// backend-local.
#pragma once
...
static __forceinline__ __device__ float3 stable_perpendicular(float3 axis,
    ...
    const float3 fallback = fabsf(axis.z) < 0.9f
                                ? dfr_make3(0.f, 0.f, 1.f)
                                : dfr_make3(0.f, 1.f, 0.f);
...
static __forceinline__ __device__ bool keller_target_from_state(
    ...
    sincosf(2.f * kDfrAdPi * uniform01(static_cast<unsigned int>(lane),
                                       1u,
...
static __forceinline__ __device__ float3 keller_target_jvp(
    ...
    const float dot_radial =
        radial > kDfrAdEps ? (-(axial / radial) * dot_axial) : 0.f;
...
static __forceinline__ __device__ bool material_valid_for_prim(
    ...
    return prim >= 0 &&
           ...
           (params.material_valid == nullptr ||
            RAYD_DFR_AD_MATERIAL_VALID_ENTRY(params, prim));
}
...
static __forceinline__ __device__ bool load_triangle(
    ...
    p0 = dfr_make3(params.tri_p0_x[prim],
                   params.tri_p0_y[prim],
                   params.tri_p0_z[prim]);
    e1 = dfr_make3(params.tri_e1_x[prim],
                   params.tri_e1_y[prim],
                   params.tri_e1_z[prim]);
    e2 = dfr_make3(params.tri_e2_x[prim],
                   params.tri_e2_y[prim],
                   params.tri_e2_z[prim]);
    normal = dfr_make3(params.tri_fn_x[prim],
...
static __forceinline__ __device__ bool load_primal(
    ...
    p.edge_pos = RAYD_DFR_AD_READ_VEC(params,
                                      state_edge_pos_x,
                                      state_edge_pos_y,
                                      state_edge_pos_z,
                                      state_edge_pos_stride,
                                      p.state_idx);
    p.edge_dir_raw = RAYD_DFR_AD_READ_VEC(params,
                                          state_edge_dir_x,
                                          state_edge_dir_y,
                                          state_edge_dir_z,
                                          state_edge_dir_stride,
                                          p.state_idx);
    ...
    p.source = RAYD_DFR_AD_READ_VEC(params,
                                    state_src_x,
                                    state_src_y,
                                    state_src_z,
                                    state_src_stride,
                                    p.state_idx);
    p.wi_raw = RAYD_DFR_AD_READ_VEC_GUARDED(params,
                                            state_wi_x,
                                            state_wi_y,
                                            state_wi_z,
                                            state_wi_stride,
    ...
    p.exterior_angle = RAYD_DFR_AD_READ_F32(params,
                                            state_exterior_angle,
                                            state_exterior_angle_stride,
                                            p.state_idx);
    const float exterior_clamped = fmaxf(p.exterior_angle, 0.25f * kDfrAdPi);
    p.wedge_scale = fminf(exterior_clamped / (2.f * kDfrAdPi), 2.f);
    p.wedge_active = p.exterior_angle > 0.25f * kDfrAdPi &&
                     exterior_clamped / (2.f * kDfrAdPi) < 2.f;
    p.material_gain = 1.f;
    ...
    if (p.is_suffix) {
        const int prim0 = RAYD_DFR_AD_READ_I32_OR(params,
                                                  state_prim0,
                                                  state_prim0_stride,
                                                  p.state_idx,
                                                  -1);
        const int prim1 = RAYD_DFR_AD_READ_I32_OR(params,
                                                  state_prim1,
                                                  state_prim1_stride,
                                                  p.state_idx,
                                                  -1);
    ...
    const float wave_gain =
        (params.wavelength * (1.f / (4.f * kDfrAdPi))) *
        (params.wavelength * (1.f / (4.f * kDfrAdPi)));
...
static __forceinline__ __device__ float3 read_vec_or_zero(
    ...
    const float *z,
    int index) {
    return dfr_make3(read_or_zero(x, index),
...
static __forceinline__ __device__ float chain_event_weight(
    ...
    event.wedge_active = exterior_angle > 0.25f * kDfrAdPi &&
                         exterior_clamped / (2.f * kDfrAdPi) < 2.f;
...
static __forceinline__ __device__ bool load_chain_event_initial(
    ...
    event.state_idx = idx;
    event.edge_u = u;
    event.edge_pos = RAYD_DFR_AD_READ_VEC(params,
                                          state_edge_pos_x,
                                          state_edge_pos_y,
                                          state_edge_pos_z,
                                          state_edge_pos_stride,
                                          idx);
    event.edge_dir_raw = RAYD_DFR_AD_READ_VEC(params,
                                              state_edge_dir_x,
                                              state_edge_dir_y,
                                              state_edge_dir_z,
                                              state_edge_dir_stride,
    ...
    const float t_min =
        RAYD_DFR_AD_READ_F32(params, state_edge_t_min, state_edge_t_min_stride, idx);
    const float t_max =
        RAYD_DFR_AD_READ_F32(params, state_edge_t_max, state_edge_t_max_stride, idx);
...
static __forceinline__ __device__ bool load_chain_event_recursive(
    ...
    event.edge_u = u;
    event.edge_pos = RAYD_DFR_AD_READ_VEC(params,
                                          recursive_state_edge_pos_x,
                                          recursive_state_edge_pos_y,
                                          recursive_state_edge_pos_z,
                                          recursive_state_edge_pos_stride,
                                          idx);
    event.edge_dir_raw = RAYD_DFR_AD_READ_VEC(params,
                                              recursive_state_edge_dir_x,
                                              recursive_state_edge_dir_y,
                                              recursive_state_edge_dir_z,
                                              recursive_state_edge_dir_stride,
                                              idx);
    event.edge_dir_norm = norm3(event.edge_dir_raw);
    if (!(event.edge_dir_norm > kDfrAdEps) || !isfinite(event.edge_dir_norm)) {
        return false;
    ...
    event.edge_dir = (1.f / event.edge_dir_norm) * event.edge_dir_raw;
    const float t_min = RAYD_DFR_AD_READ_F32(params,
                                             recursive_state_edge_t_min,
                                             recursive_state_edge_t_min_stride,
                                             idx);
    const float t_max = RAYD_DFR_AD_READ_F32(params,
                                             recursive_state_edge_t_max,
                                             recursive_state_edge_t_max_stride,
                                             idx);
    event.edge_t = t_min + u * (t_max - t_min);
    event.edge_point = event.edge_pos + event.edge_t * event.edge_dir;
    return true;
...
static __forceinline__ __device__ bool load_chain_primal(
    ...
    const float3 source = RAYD_DFR_AD_READ_VEC(params,
                                               state_src_x,
                                               state_src_y,
                                               state_src_z,
                                               state_src_stride,
    ...
    if (p.is_suffix) {
        const ChainEventPrimal &terminal = p.has_third ? p.third : p.second;
        const int face0_prim = RAYD_DFR_AD_SUFFIX_FACE_PRIM(params,
                                                            recursive_state_prim0,
                                                            recursive_state_prim0_stride,
                                                            p.has_third,
                                                            p.second_idx,
                                                            p.third_idx);
        const int face1_prim = RAYD_DFR_AD_SUFFIX_FACE_PRIM(params,
                                                            recursive_state_prim1,
                                                            recursive_state_prim1_stride,
                                                            p.has_third,
                                                            p.second_idx,
                                                            p.third_idx);
        if (!suffix_reflection_connection(params,
    ...
    const float first_weight = chain_event_weight(
        ...
        RAYD_DFR_AD_READ_F32(params, state_exterior_angle, state_exterior_angle_stride, p.first_idx));
    ...
    const float second_weight = chain_event_weight(
        ...
        RAYD_DFR_AD_READ_F32(params,
                             ...
                             recursive_state_exterior_angle_stride,
                             p.second_idx));
...
static __forceinline__ __device__ float chain_event_weight_jvp(
    ...
    if (event.wedge_active && event.wedge_scale != 0.f) {
        dot_weight +=
            event.contribution *
            ((tangent.exterior_angle / (2.f * kDfrAdPi)) / event.wedge_scale);
    }
...
static __forceinline__ __device__ ChainTangent chain_read_tangent(
    ...
    tangent.first.edge_pos =
        RAYD_DFR_AD_READ_DOT_VEC(params,
                                 dot_state_edge_pos_x,
                                 dot_state_edge_pos_y,
                                 dot_state_edge_pos_z,
                                 dot_state_edge_pos_stride,
                                 p.first_idx);
    tangent.first.edge_dir_raw =
        RAYD_DFR_AD_READ_DOT_VEC(params,
                                 dot_state_edge_dir_x,
                                 dot_state_edge_dir_y,
                                 dot_state_edge_dir_z,
                                 dot_state_edge_dir_stride,
                                 p.first_idx);
    ...
    tangent.first.exterior_angle =
        RAYD_DFR_AD_READ_DOT_F32(params,
                                 dot_state_exterior_angle,
                                 dot_state_exterior_angle_stride,
                                 p.first_idx);
    ...
    tangent.second.edge_pos =
        RAYD_DFR_AD_READ_DOT_VEC(params,
                                 dot_recursive_state_edge_pos_x,
                                 dot_recursive_state_edge_pos_y,
                                 dot_recursive_state_edge_pos_z,
                                 dot_recursive_state_edge_pos_stride,
                                 p.second_idx);
    tangent.second.edge_dir_raw =
        RAYD_DFR_AD_READ_DOT_VEC(params,
                                 dot_recursive_state_edge_dir_x,
                                 dot_recursive_state_edge_dir_y,
                                 dot_recursive_state_edge_dir_z,
                                 dot_recursive_state_edge_dir_stride,
                                 p.second_idx);
    tangent.second.edge_t_min =
        RAYD_DFR_AD_READ_DOT_F32(params,
                                 dot_recursive_state_edge_t_min,
                                 dot_recursive_state_edge_t_min_stride,
                                 p.second_idx);
    tangent.second.edge_t_max =
        RAYD_DFR_AD_READ_DOT_F32(params,
                                 dot_recursive_state_edge_t_max,
                                 dot_recursive_state_edge_t_max_stride,
                                 p.second_idx);
...
static __forceinline__ __device__ float contribution_jvp(
    ...
    const float dot_wedge =
        p.wedge_active ? tangent.exterior_angle / (2.f * kDfrAdPi) : 0.f;
    const float3 dot_edge_dir =
...
static __forceinline__ __device__ float direct_jvp(
    ...
    tangent.wi_raw =
        RAYD_DFR_AD_READ_DOT_VEC(params,
                                 ...
                                 p.state_idx);
    tangent.src_power =
        RAYD_DFR_AD_READ_DOT_F32(params, dot_state_src_power, dot_state_src_power_stride, p.state_idx);
