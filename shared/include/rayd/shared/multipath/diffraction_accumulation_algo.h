...
// Host-compilable diffraction-accumulation algorithm. This is the de-CUDA-ised
// body of the former DiffractionAccumulationDevice<Policy>: math is math::Vec3f
// throughout (the arithmetic operators below mirror the exact op order of the old
// local CUDA vector operators, so every geometry expression is preserved verbatim
// and device codegen stays bit-identical), the two ray-cast families go through
// an rt::is_traverser Traverser (so no OptiX ray-cast intrinsic, payload
// register, or launch-index query appears here), and the lane index is a plain
// parameter. The local 4-field HitPayload dissolves into rt::TriangleHit (the
// barycentrics stay unread). diffraction_accumulation_device.cuh instantiates it
// with the pipeline's own OptixTraverser (closest + terminate-on-first-hit
// occlusion casts); the CUDA fused executor (P4d) will reuse it with
// CudaBvhTraverser. The Policy still supplies storage accessors, staging, and the
// per-cell atomics; the debug-counter atomics stay in the algorithm behind a
// host non-atomic fallback (the host path exists only for the compile gate).
//
// UTD math (host-safe utd_math.h) speaks its own `utd::float3a` POD at the pair
// boundary; the algorithm carries geometry in math::Vec3f and converts at the
// call sites.

namespace rayd::shared::math {

// Vec3f arithmetic operators (found via ADL). Each mirrors the exact op order of
...
/// Diffraction-accumulation algorithm for one lane, templated over the backend
/// storage/atomics Policy and the acceleration-structure Traverser. It owns a
...
struct DiffractionAccumulationAlgo {
    ...
    // (Namespace alias kept local so the body reads exactly like the original.)
    ...
    // ---- Vec3f helpers -----------------------------------------------------
    RAYD_HOST_DEVICE static Vec3f make_f3(float x, float y, float z) {
        return ::rayd::shared::math::make_vec3(x, y, z);
    }
    RAYD_HOST_DEVICE static float dot3(Vec3f a, Vec3f b) {
        return ::rayd::shared::math::dot(a, b);
    }
    RAYD_HOST_DEVICE static float norm3(Vec3f a) {
        return sqrtf(fmaxf(::rayd::shared::math::dot(a, a), 0.f));
    }
    RAYD_HOST_DEVICE static Vec3f cross3(Vec3f a, Vec3f b) {
        return ::rayd::shared::math::cross(a, b);
    }
    RAYD_HOST_DEVICE static Vec3f normalize3(Vec3f v) {
        return ::rayd::shared::math::scale(
            v, reciprocal_sqrt(fmaxf(::rayd::shared::math::dot(v, v), 1e-12f)));
    }
    RAYD_HOST_DEVICE static ::rayd::shared::utd::float3a to_utd(Vec3f value) {
        return ::rayd::shared::utd::make_f3(value.x, value.y, value.z);
    }
    RAYD_HOST_DEVICE static TriangleHit choose_hit(TriangleHit a, TriangleHit b) {
        if (a.hit == 0u) {
            return b;
        }
        if (b.hit == 0u) {
    ...
    RAYD_HOST_DEVICE static unsigned int hash_u32(unsigned int x) {
        x ^= x >> 16;
        x *= 0x7feb352du;
        x ^= x >> 15;
        x *= 0x846ca68bu;
    ...
    RAYD_HOST_DEVICE static float uniform01(unsigned int lane, unsigned int stream, unsigned int seed) {
        const unsigned int h = hash_u32(lane ^ (stream * 0x9e3779b9u) ^ seed);
        return static_cast<float>(h & 0x00ffffffu) * (1.f / 16777216.f);
    }

    // ---- Policy storage forwarders ----------------------------------------
    RAYD_HOST_DEVICE static int sample_state_index_for_lane(unsigned int lane) {
        return Policy::sample_state_index_for_lane(lane);
    }
    RAYD_HOST_DEVICE static int state_edge_index_at(int i) { return Policy::state_edge_index_at(i); }
    RAYD_HOST_DEVICE static Vec3f state_edge_pos_at(int i) { return Policy::state_edge_pos_at(i); }
    RAYD_HOST_DEVICE static Vec3f state_edge_dir_at(int i) { return Policy::state_edge_dir_at(i); }
    RAYD_HOST_DEVICE static float state_edge_t_min_at(int i) { return Policy::state_edge_t_min_at(i); }
    RAYD_HOST_DEVICE static float state_edge_t_max_at(int i) { return Policy::state_edge_t_max_at(i); }
    RAYD_HOST_DEVICE static float sample_edge_weight_for_lane(int i, unsigned int lane, int n) {
        return Policy::sample_edge_weight_for_lane(i, lane, n);
    }
    RAYD_HOST_DEVICE static int state_prim0_at(int i) { return Policy::state_prim0_at(i); }
    RAYD_HOST_DEVICE static int state_prim1_at(int i) { return Policy::state_prim1_at(i); }
    ...
    // ---- ray casts (instance: use primary / secondary traversers) ---------
    template <bool PrimaryOnly>
    RAYD_DEVICE TriangleHit trace_scene_impl(Vec3f origin, Vec3f direction, float tmax) const {
        const TriangleHit p = primary.trace_closest(origin, direction, kRayTMin, tmax);
        if constexpr (PrimaryOnly) {
            return p;
        } else if (Policy::params().split_mode == 0) {
            return p;
        }
        const TriangleHit s = secondary.trace_closest(origin, direction, kRayTMin, tmax);
    ...
    RAYD_DEVICE bool visible_segment_impl(Vec3f start, Vec3f end) const {
        ...
        const Vec3f origin = start + kDfrRayBias * dir;
        const float tmax = fmaxf(dist - 2.f * kDfrRayBias, 0.f);
        if (primary.trace_occluded(origin, dir, kRayTMin, tmax)) {
            return false;
        }
        if constexpr (!PrimaryOnly) {
    ...
    RAYD_DEVICE bool point_inside_one_ray_impl(Vec3f point, Vec3f ray_dir) const {
        ...
        const Vec3f normal = face_normal_for_global_prim(global_primitive_id(hit));
        return dot3(normal, ray_dir) > 0.f;
    }

    template <bool PrimaryOnly>
    RAYD_DEVICE bool point_inside_closed_mesh_robust_impl(Vec3f point) const {
        const Vec3f d0 = normalize3(make_f3(0.81234133f, 0.52311241f, 0.25843197f));
        const Vec3f d1 = normalize3(make_f3(-0.37139068f, 0.60114462f, 0.70757474f));
        return point_inside_one_ray_impl<PrimaryOnly>(point, d0) &&
               point_inside_one_ray_impl<PrimaryOnly>(point, d1);
    }

    template <bool PrimaryOnly>
    RAYD_DEVICE bool visible_segment_ignore_prim_impl(Vec3f start, Vec3f end, int ignore_prim) const {
        const Vec3f delta = end - start;
        const float dist = norm3(delta);
        if (dist <= 1e-5f) {
            return true;
    ...
    template <bool PrimaryOnly>
    RAYD_DEVICE bool visible_segment_ignore_two_prims_impl(Vec3f start, Vec3f end, int ignore0, int ignore1) const {
        const Vec3f delta = end - start;
        const float dist = norm3(delta);
        if (dist <= 1e-5f) {
            return true;
    ...
    RAYD_HOST_DEVICE static Vec3f grid_cell_center(int cell) {
        ...
        const float c1 = Policy::params().grid_coord1_min +
                         v * (Policy::params().grid_coord1_max - Policy::params().grid_coord1_min);
    ...
    RAYD_HOST_DEVICE static float component(Vec3f value, int axis) {
    ...
    RAYD_HOST_DEVICE static bool grid_cell_from_point(Vec3f point, int &cell) {
        ...
        if (c0 < Policy::params().grid_coord0_min || c0 >= Policy::params().grid_coord0_max ||
            c1 < Policy::params().grid_coord1_min || c1 >= Policy::params().grid_coord1_max) {
            return false;
        }
        const float u = (c0 - Policy::params().grid_coord0_min) /
                        fmaxf(Policy::params().grid_coord0_max - Policy::params().grid_coord0_min, kDfrEps);
        const float v = (c1 - Policy::params().grid_coord1_min) /
                        fmaxf(Policy::params().grid_coord1_max - Policy::params().grid_coord1_min, kDfrEps);
        const int i = imin(imax(static_cast<int>(u * Policy::params().grid_resolution0), 0),
                           Policy::params().grid_resolution0 - 1);
        const int j = imin(imax(static_cast<int>(v * Policy::params().grid_resolution1), 0),
    ...
    RAYD_HOST_DEVICE static float first_order_diffraction_parameter(
        ...
        return ::rayd::shared::utd::first_order_diffraction_parameter(
            ::rayd::shared::utd::make_f3(source.x, source.y, source.z),
            ::rayd::shared::utd::make_f3(target.x, target.y, target.z),
            ::rayd::shared::utd::make_f3(edge_origin.x, edge_origin.y, edge_origin.z),
            ::rayd::shared::utd::make_f3(edge_dir.x, edge_dir.y, edge_dir.z));
    }

    RAYD_HOST_DEVICE static float shadow_decay_span_from_wedge_n(float wedge_n) {
    ...
    RAYD_HOST_DEVICE static ::rayd::shared::utd::MaterialParams coherent_material_params() {
        ::rayd::shared::utd::MaterialParams mat;
        mat.useFresnel = 0;
        mat.etaR = 0.f;
        mat.muR = 1.f;
        mat.sigma = 0.f;
    ...
    RAYD_HOST_DEVICE static ::rayd::shared::utd::PairInputs load_coherent_pair_inputs(int sIdx) {
        namespace utd = ::rayd::shared::utd;
        utd::PairInputs p;
        p.edgePos = utd::make_f3(Policy::params().utd_epx[sIdx], Policy::params().utd_epy[sIdx],
                                 Policy::params().utd_epz[sIdx]);
        p.edgeDir = utd::make_f3(Policy::params().utd_edx[sIdx], Policy::params().utd_edy[sIdx],
                                 Policy::params().utd_edz[sIdx]);
        p.n0 = utd::make_f3(Policy::params().utd_n0x[sIdx], Policy::params().utd_n0y[sIdx],
                            Policy::params().utd_n0z[sIdx]);
        p.nn = utd::make_f3(Policy::params().utd_nnx[sIdx], Policy::params().utd_nny[sIdx],
        ...
        p.sourcePos = utd::make_f3(Policy::params().utd_spx[sIdx], Policy::params().utd_spy[sIdx],
                                   Policy::params().utd_spz[sIdx]);
        p.incidentField = utd::cplx(Policy::params().utd_ifr[sIdx], Policy::params().utd_ifi[sIdx]);
        p.incidentNormalDerivative = utd::cplx(Policy::params().utd_inr[sIdx], Policy::params().utd_ini[sIdx]);
        p.r0 = utd::cplx(Policy::params().utd_r0r[sIdx], Policy::params().utd_r0i[sIdx]);
        p.rn = utd::cplx(Policy::params().utd_rnr[sIdx], Policy::params().utd_rni[sIdx]);
        p.incidentVector = {utd::cplx(Policy::params().utd_vxr[sIdx], Policy::params().utd_vxi[sIdx]),
                            utd::cplx(Policy::params().utd_vyr[sIdx], Policy::params().utd_vyi[sIdx]),
                            utd::cplx(Policy::params().utd_vzr[sIdx], Policy::params().utd_vzi[sIdx])};
        p.incidentDerivativeVector = {utd::cplx(Policy::params().utd_dxr[sIdx], Policy::params().utd_dxi[sIdx]),
                                      utd::cplx(Policy::params().utd_dyr[sIdx], Policy::params().utd_dyi[sIdx]),
                                      utd::cplx(Policy::params().utd_dzr[sIdx], Policy::params().utd_dzi[sIdx])};
        p.incidentJones = {utd::cplx(Policy::params().utd_jur[sIdx], Policy::params().utd_jui[sIdx]),
                           utd::cplx(Policy::params().utd_jvr[sIdx], Policy::params().utd_jvi[sIdx])};
        p.incidentDerivativeJones = {utd::cplx(Policy::params().utd_djur[sIdx], Policy::params().utd_djui[sIdx]),
                                     utd::cplx(Policy::params().utd_djvr[sIdx], Policy::params().utd_djvi[sIdx])};
        p.incidentBasis = {utd::make_f3(Policy::params().utd_bux[sIdx], Policy::params().utd_buy[sIdx],
                                        Policy::params().utd_buz[sIdx]),
                           utd::make_f3(Policy::params().utd_bvx[sIdx], Policy::params().utd_bvy[sIdx],
                                        Policy::params().utd_bvz[sIdx]),
                           utd::make_f3(Policy::params().utd_bkx[sIdx], Policy::params().utd_bky[sIdx],
                                        Policy::params().utd_bkz[sIdx])};
        p.face0Operator = {utd::cplx(Policy::params().utd_f0m00r[sIdx], Policy::params().utd_f0m00i[sIdx]),
                           utd::cplx(Policy::params().utd_f0m01r[sIdx], Policy::params().utd_f0m01i[sIdx]),
                           utd::cplx(Policy::params().utd_f0m10r[sIdx], Policy::params().utd_f0m10i[sIdx]),
                           utd::cplx(Policy::params().utd_f0m11r[sIdx], Policy::params().utd_f0m11i[sIdx])};
        p.face1Operator = {utd::cplx(Policy::params().utd_f1m00r[sIdx], Policy::params().utd_f1m00i[sIdx]),
                           utd::cplx(Policy::params().utd_f1m01r[sIdx], Policy::params().utd_f1m01i[sIdx]),
                           utd::cplx(Policy::params().utd_f1m10r[sIdx], Policy::params().utd_f1m10i[sIdx]),
                           utd::cplx(Policy::params().utd_f1m11r[sIdx], Policy::params().utd_f1m11i[sIdx])};
        p.face0Material = {Policy::params().utd_f0er[sIdx], Policy::params().utd_f0mu[sIdx],
                           Policy::params().utd_f0sg[sIdx], Policy::params().utd_f0g[sIdx],
                           Policy::params().utd_f0uf[sIdx], 1.f};
        p.face1Material = {Policy::params().utd_f1er[sIdx], Policy::params().utd_f1mu[sIdx],
    ...
    RAYD_DEVICE bool coherent_visibility_and_support(
        ::rayd::shared::utd::PairInputs state, Vec3f target_f3, bool selected_valid,
    ...
    RAYD_HOST_DEVICE static bool coherent_selected_visibility_point(
        ::rayd::shared::utd::PairInputs original, Vec3f target,
        ::rayd::shared::utd::PairInputs &selected, Vec3f &visibility_point) {
        namespace utd = ::rayd::shared::utd;
        selected = original;
        ...
        const Vec3f edge_dir =
            normalize3(make_f3(original.edgeDir.x, original.edgeDir.y, original.edgeDir.z));
        const Vec3f edge_pos = make_f3(original.edgePos.x, original.edgePos.y, original.edgePos.z);
    ...
    RAYD_DEVICE void run_coherent_utd_lane(int state_idx, int cell) const {
        ...
        if (Policy::params().prefilter_visibility != 0) {
            const int ignore0 = Policy::params().coherent_adjacent_face0 != nullptr
                                    ...
                                    : -1;
            const int ignore1 = Policy::params().coherent_adjacent_face1 != nullptr
            ...
            if (!visible_segment_ignore_two_prims_impl<PrimaryOnly>(visibility_point, target, ignore0, ignore1)) {
                if (Policy::params().collect_debug_counts != 0 &&
                    ...
                    atomic_add(Policy::params().out_visibility_reject_count + cell, 1);
                }
                return;
            }
        }
        ...
        if (owner == utd::OWNERSHIP_MIXED) {
            ...
            Policy::atomic_add_same_cell(Policy::params().out_multi_field_x_re, cell, out.vectorField.x.re, cell_group);
            Policy::atomic_add_same_cell(Policy::params().out_multi_field_x_im, cell, out.vectorField.x.im, cell_group);
            Policy::atomic_add_same_cell(Policy::params().out_multi_field_y_re, cell, out.vectorField.y.re, cell_group);
            Policy::atomic_add_same_cell(Policy::params().out_multi_field_y_im, cell, out.vectorField.y.im, cell_group);
            Policy::atomic_add_same_cell(Policy::params().out_multi_field_z_re, cell, out.vectorField.z.re, cell_group);
            Policy::atomic_add_same_cell(Policy::params().out_multi_field_z_im, cell, out.vectorField.z.im, cell_group);
    ...
    RAYD_HOST_DEVICE static Vec3f stable_perpendicular(Vec3f axis, Vec3f preferred) {
        Vec3f projected = preferred - dot3(preferred, axis) * axis;
        if (dot3(projected, projected) > 1e-12f) {
            return normalize3(projected);
        }
        const Vec3f fallback =
            fabsf(axis.z) < 0.9f ? make_f3(0.f, 0.f, 1.f) : make_f3(0.f, 1.f, 0.f);
    ...
    RAYD_HOST_DEVICE static bool keller_grid_hit_from_incident(
        ...
        const Vec3f incident = normalize3(incident_vec);
        const float axial = fminf(fmaxf(dot3(incident, edge_dir), -1.f), 1.f);
        const float radial = sqrtf(fmaxf(1.f - axial * axial, 0.f));
        const Vec3f basis0 = stable_perpendicular(edge_dir, incident);
        const Vec3f basis1 = normalize3(cross3(edge_dir, basis0));
        float s;
        float c;
        sincos_f(2.f * kPi * uniform01(lane, stream, static_cast<unsigned int>(Policy::params().seed)), &s, &c);
        const Vec3f ko = normalize3(axial * edge_dir + radial * (c * basis0 + s * basis1));
    ...
    RAYD_HOST_DEVICE static int material_index_for_faces(int face0_prim, int face1_prim) {
        if (Policy::params().material_gain == nullptr || Policy::params().material_count <= 0) {
            return -1;
        }
        int prim = face0_prim;
        if (prim < 0 || prim >= Policy::params().material_count || !material_valid_at(prim)) {
            prim = face1_prim;
        }
        if (prim < 0 || prim >= Policy::params().material_count || !material_valid_at(prim)) {
            return -1;
    ...
    RAYD_HOST_DEVICE static float material_gain_for_faces(int face0_prim, int face1_prim) {
    ...
    RAYD_HOST_DEVICE static float material_gain_for_state(int state_idx) {
        return material_gain_for_faces(state_prim0_at(state_idx), state_prim1_at(state_idx));
    }
    ...
    RAYD_HOST_DEVICE static bool select_local_suffix_candidate(
        ...
        const unsigned int candidate_hash =
            hash_u32(lane ^ (stream * 0x9e3779b9u) ^ static_cast<unsigned int>(Policy::params().seed));
    ...
    RAYD_HOST_DEVICE static bool load_triangle(
        ...
        p0 = make_f3(Policy::params().tri_p0_x[prim], Policy::params().tri_p0_y[prim], Policy::params().tri_p0_z[prim]);
        e1 = make_f3(Policy::params().tri_e1_x[prim], Policy::params().tri_e1_y[prim], Policy::params().tri_e1_z[prim]);
        e2 = make_f3(Policy::params().tri_e2_x[prim], Policy::params().tri_e2_y[prim], Policy::params().tri_e2_z[prim]);
        normal = make_f3(Policy::params().tri_fn_x[prim], Policy::params().tri_fn_y[prim], Policy::params().tri_fn_z[prim]);
    ...
    RAYD_HOST_DEVICE static bool intersect_reflection_triangle(
        ...
        const Vec3f delta = target - image_source;
        const float dist = norm3(delta);
        if (!(dist > kDfrRayBias) || !is_finite(dist)) {
            return false;
        }
        const Vec3f dir = (1.f / dist) * delta;
        const Vec3f h = cross3(dir, e2);
        const float a = dot3(e1, h);
        if (fabsf(a) <= 1e-7f) {
            return false;
        }
        const float f = 1.f / a;
        const Vec3f s = image_source - p0;
        const float u = f * dot3(s, h);
        if (u < -1e-5f || u > 1.f + 1e-5f) {
            return false;
        }
        const Vec3f q = cross3(s, e1);
        const float v = f * dot3(dir, q);
        if (v < -1e-5f || u + v > 1.f + 1e-5f) {
            return false;
        }
        const float t = f * dot3(e2, q);
        if (!(t > kDfrRayBias) || !(t < dist - kDfrRayBias) || !is_finite(t)) {
            return false;
        }
    ...
    RAYD_HOST_DEVICE static bool suffix_reflection_connection(
        ...
        const Vec3f image_source = diff_point - 2.f * plane_distance * normal;
        if (!intersect_reflection_triangle(image_source, target, prim, reflection_point, normal)) {
            return false;
        }

        const Vec3f incoming = reflection_point - diff_point;
        const Vec3f outgoing = target - reflection_point;
        ...
        const Vec3f incoming_hat = (1.f / incoming_dist) * incoming;
        const Vec3f oriented_normal = dot3(incoming_hat, normal) > 0.f ? (-1.f * normal) : normal;
        const Vec3f reflected_hat =
            incoming_hat - 2.f * dot3(incoming_hat, oriented_normal) * oriented_normal;
        const Vec3f outgoing_hat = (1.f / outgoing_dist) * outgoing;
        ...
        if (!(is_finite(reflection_gain) && is_finite(suffix_fspl))) {
            return false;
        }
        return true;
    }
    ...
    RAYD_DEVICE void run_diffraction_order1_accumulation_algo(std::uint32_t lane) const {
        ...
        const Vec3f edge_pos = state_edge_pos_at(state_idx);
        const Vec3f edge_dir = normalize3(state_edge_dir_at(state_idx));
        ...
        if (Policy::params().tape_active != nullptr) {
            ...
            if (Policy::params().tape_material_idx != nullptr) {
                Policy::params().tape_material_idx[lane] =
                    material_index_for_faces(state_prim0_at(state_idx), state_prim1_at(state_idx));
            }
        ...
        if (Policy::stage_order1(lane, cell, contribution, field_x_re, is_direct, is_keller)) {
            return;
        }

        const typename Policy::CellGroup cell_group = Policy::cell_group(cell);
        Policy::atomic_add_same_cell(Policy::params().out_power, cell, contribution, cell_group);
        Policy::atomic_add_same_cell(Policy::params().out_field_x_re, cell, field_x_re, cell_group);
        if (is_direct) {
    ...
    RAYD_DEVICE void run_diffraction_order1_coherent_accumulation_algo(std::uint32_t lane) const {
        ...
        const int grid_cell_count = Policy::params().grid_resolution0 * Policy::params().grid_resolution1;
        const int state_idx = sample_state_index_for_lane(lane);
        const int cell = static_cast<int>(lane / static_cast<unsigned int>(Policy::params().state_count));
        ...
        const Vec3f edge_point = edge_pos + edge_t * edge_dir;
        const Vec3f visibility_edge_point = edge_pos + visibility_edge_t * edge_dir;
        ...
        const float source_distance = fmaxf(norm3(edge_point - source), kDfrEps);
        const float target_distance = fmaxf(norm3(target - edge_point), kDfrEps);
        const float phase = -Policy::params().k * (source_distance + target_distance);
        ...
        if (is_multi) {
            ...
            if (Policy::params().out_multi_field_x_re != nullptr) {
                Policy::atomic_add_same_cell(Policy::params().out_multi_field_x_re, cell, field_re, cell_group);
                Policy::atomic_add_same_cell(Policy::params().out_multi_field_x_im, cell, field_im, cell_group);
            }
            if (Policy::params().out_multi_count != nullptr) {
                Policy::atomic_add_same_cell(Policy::params().out_multi_count, cell, 1, cell_group);
            }
            ...
            if (Policy::params().out_direct_field_x_re != nullptr) {
                Policy::atomic_add_same_cell(Policy::params().out_direct_field_x_re, cell, field_re, cell_group);
                Policy::atomic_add_same_cell(Policy::params().out_direct_field_x_im, cell, field_im, cell_group);
            }
            if (Policy::params().out_direct_count != nullptr) {
                Policy::atomic_add_same_cell(Policy::params().out_direct_count, cell, 1, cell_group);
            }
    ...
    RAYD_DEVICE void run_diffraction_chain_accumulation_algo(std::uint32_t lane) const {
        ...
        const Vec3f first_edge_pos = state_edge_pos_at(first_idx);
        const Vec3f first_edge_dir = normalize3(state_edge_dir_at(first_idx));
        ...
        const Vec3f first_point = first_edge_pos + first_t * first_edge_dir;

        const Vec3f second_edge_pos = recursive_state_edge_pos_at(second_idx);
        const Vec3f second_edge_dir = normalize3(recursive_state_edge_dir_at(second_idx));
        ...
        const Vec3f second_point = second_edge_pos + second_t * second_edge_dir;

        const Vec3f source = state_src_at(first_idx);
        const Vec3f target = grid_cell_center(cell);
        Vec3f third_point = second_point;
        Vec3f third_edge_dir = second_edge_dir;
        if (Policy::params().max_order == 3) {
            const float third_u = uniform01(lane, 4u, static_cast<unsigned int>(Policy::params().seed));
            const Vec3f third_edge_pos = recursive_state_edge_pos_at(third_idx);
        ...
        const Vec3f terminal_point = Policy::params().max_order == 3 ? third_point : second_point;
        const Vec3f terminal_edge_dir = Policy::params().max_order == 3 ? third_edge_dir : second_edge_dir;
        ...
        if (!first_inter_edge_visible || !second_inter_edge_visible) {
            if (Policy::params().collect_debug_counts != 0) {
                atomic_add(Policy::params().out_edge_vis_rejects, 1);
            }
            return;
        }
        ...
        if (Policy::params().tape_active != nullptr) {
            ...
            if (Policy::params().tape_material_idx != nullptr) {
                Policy::params().tape_material_idx[lane] =
                    material_index_for_faces(state_prim0_at(first_idx), state_prim1_at(first_idx));
            }
            if (Policy::params().tape_edge_u != nullptr) {
