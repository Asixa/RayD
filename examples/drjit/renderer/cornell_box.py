...
def box(lo, hi):
    ...
    f = dr.cuda.Array3i(
        [0, 0, 4, 4, 0, 0, 1, 1, 0, 0, 3, 3], [2, 3, 5, 6, 7, 4, 2, 6, 1, 5, 7, 6], [1, 2, 6, 7, 3, 7, 6, 5, 5, 4, 6, 2]
    )
    m = rd.Mesh(v, f)
...
def build_scene(tx):
    ...
    add(quad([-1, -1, 1], [-1, -1, 5], [1, -1, 5], [1, -1, 1]), [0.78, 0.78, 0.78])  # floor
    add(quad([-1, 1, 1], [1, 1, 1], [1, 1, 5], [-1, 1, 5]), [0.78, 0.78, 0.78])  # ceiling
    add(quad([-1, -1, 5], [-1, 1, 5], [1, 1, 5], [1, -1, 5]), [0.75, 0.75, 0.75])  # back
    add(quad([-1, -1, 1], [-1, 1, 1], [-1, 1, 5], [-1, -1, 5]), [0.74, 0.14, 0.12])  # left (red)
    add(quad([1, -1, 1], [1, -1, 5], [1, 1, 5], [1, 1, 1]), [0.12, 0.55, 0.16])  # right (green)
    ...
    add(box([-0.72, -1.0, 2.70], [-0.18, 0.25, 3.62]), [0.82, 0.82, 0.82])  # tall block
...
def trace(scene, alb, emi, light_id, primary_ray, rng, max_depth=4, diff=False):
    ...
    if diff:
        ...
        F3, B, MkR = dr.cuda.Array3f, dr.cuda.Bool, rd.Ray
        ray = primary_ray
    ...
    for depth in range(max_depth):
        ...
        L = dr.select(
            ...
            + beta
            * kd
            * (F3(*LIGHT_E.tolist()) * (LIGHT_AREA * cos_s * cos_l * dr.rcp(PI * dr.maximum(d2, 1e-5)) * vis)),
            L,
        )
        # bounce
...
def render_edge_ad(scene, camera, alb, emi, lid, tx, seed=7, spp=16, md=4):
    ...
    c = dr.select(valid, c / float(spp), 0.0)
    c -= dr.cuda.ad.Float(dr.detach(c))
...
def main():
    ...
    args = ap.parse_args()
    w, h, n = args.width, args.height, args.width * args.height
