# RayD Torch CUDA / OptiX Performance Analysis

> **Historical record, frozen 2026-06-12.** Predates the 0.6.0 dual-backend
> monorepo; "RayDN" / `raydn` below is what is now `rayd.torch`. Paths and
> commands have been re-rooted to their current locations.
...
>   [torch_performance.md](torch_performance.md):
>   **warm-started local scene build** and **public static AD/VJP overhead**.

## Current implementation status (2026-06-12)

Implemented in the 2026-06-12 pass:

- `CMakeLists.txt` saves the requested `CMAKE_CUDA_ARCHITECTURES` before
  `find_package(Torch)` and restores it afterwards: PyTorch's consumed
  Caffe2 config (`Caffe2/public/cuda.cmake`) sets the variable to OFF, which
  previously stripped every `-gencode` flag on reconfigure (silent sm_52
  fallback). Configure now fails hard if the architecture list resolves empty.
- Diffraction/paths boolean segment-visibility traces use
  `TERMINATE_ON_FIRST_HIT | DISABLE_ANYHIT | DISABLE_CLOSESTHIT` (item 20).
- Diffraction accumulation forward initializes outputs/tape/stage/temp buffers
  with one fused kernel (`init_dfr_accum_outputs_cuda`) instead of ~16
  `at::zeros`/`at::full` launches per call; same-script direct diffraction
  measured 0.40 -> 0.23 ms static.
- Point nearest-edge queries keep only the tightest radius tier on the OptiX
  path and resolve the rest with an exact shared-memory tiled scan
  (`edge_point_bruteforce_kernel`), preserving the full-radius cutoff. Native
  grid-192/65,536 random-query nearest-edge: 230.8 -> 27.8 ms. Ray queries
  keep the previous tier loop (follow-up below).
- Shared multipath pipelines: exact stack sizes (item 18), SBT sized to the
  actual record count (item 32), pinned params staging ring (item 31).

Implemented earlier (2026-06-08 worktree):
...
### 18. Shared multipath pipeline uses a hardcoded oversized stack `[implemented] - Low`

- Original problem: `optixPipelineSetStackSize(pipeline_, 0, 0, 4096, 2)` while the context
  pipelines computed exact sizes via `optixUtilComputeStackSizes`.
- Implemented (2026-06-12): the shared multipath pipeline accumulates stack sizes over its
  program groups and calls `optixUtilComputeStackSizes` with the link-time trace depth.
...
### 20. Audit occlusion/visibility ray flags for `TERMINATE_ON_FIRST_HIT | DISABLE_ANYHIT` `[implemented] - High`

- Already correct before: `visibility_optix.cu` and `epc_optix.cu` used
  `OPTIX_RAY_FLAG_TERMINATE_ON_FIRST_HIT` for their visibility traces.
- Implemented (2026-06-12): the boolean `visible_segment` tests in
  `diffraction/accum_optix.cu` and `diffraction/paths_optix.cu` now trace with
  `TERMINATE_ON_FIRST_HIT | DISABLE_ANYHIT | DISABLE_CLOSESTHIT` and resolve in the miss
  program (payload initialized to blocked). The `visible_segment_ignore_prim` /
  `ignore_two_prims` variants keep closest-hit semantics because they inspect the winning
  primitive id; nearest-reflection-point searches are unchanged.
...
### 31. Every OptiX launch re-copies the full params struct to device `[implemented shared pipeline] - Low`

- Implemented (2026-06-12): the shared multipath pipeline stages launch params through a
  4-slot pinned host ring with per-slot CUDA events, so the H2D upload is a true async DMA
  and the host never overwrites a slot still being read. The edge/intersect context
  pipelines keep their existing persistent params buffers with pageable staging.

### 32. `hitgroup_record_capacity` rounds up to a minimum of 64 SBT records `[implemented] - Low`

- Implemented (2026-06-12): capacity is the next power of two over the actual record count
  with no 64-record floor; single-mesh pipelines now carry a single hitgroup record.
