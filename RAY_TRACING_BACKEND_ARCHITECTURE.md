...
- **[已核查]** 该后端**运行时可选**：`Scene(edge_bvh_backend=...)`，枚举 `DrJit`/`Optix`(默认)/`OptixDrJit` 定义在 `include/rayd/jit/scene.h:45-50`，解析在 `src/scene/scene.cpp:66-82`，Python 入口 `src/rayd.cpp:1760` → `rd.Scene(edge_bvh_backend="drjit")`。是构造参数，**不是环境变量**。
...
- **[已核查]** Dr.Jit 后端**不需要 OptiX SDK**：`include/rayd/jit/optix.h` 手写声明 host API；PTX 已签入仓库（`.target sm_70`），仅在 `RAYD_REGENERATE_*_PTX`（默认 `OFF`，`CMakeLists.txt:14-21`）时才需要 SDK 重新生成。§14 中「CUDA-only build 不需要 OptiX SDK」这条硬性要求，**对 drjit 后端已经成立**。
