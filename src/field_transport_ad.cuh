...
// Defines internal field transport support for AD.
...
__device__ __forceinline__ utd::Complex3 fold_output_cotangents(const c10::complex<float>* grad_field_vector,
    ...
    g_value.x = utd::cplx_mul_real(g_scalar, rx_axis.x);
    g_value.y = utd::cplx_mul_real(g_scalar, rx_axis.y);
    g_value.z = utd::cplx_mul_real(g_scalar, rx_axis.z);
    if (grad_field_vector != nullptr) {
        const int64_t base = index * 3;
        const c10::complex<float> gx = grad_field_vector[base];
        const c10::complex<float> gy = grad_field_vector[base + 1];
        const c10::complex<float> gz = grad_field_vector[base + 2];
        g_value.x = utd::cplx_add(g_value.x, utd::cplx(gx.real(), gx.imag()));
        g_value.y = utd::cplx_add(g_value.y, utd::cplx(gy.real(), gy.imag()));
        g_value.z = utd::cplx_add(g_value.z, utd::cplx(gz.real(), gz.imag()));
    }
    return g_value;
}
