...
// Implements the Dr.Jit surfel OptiX programs.
...
static __forceinline__ __device__ float3 load_center(int surfel) {
    return make_vec3(params.center_x[surfel], params.center_y[surfel], params.center_z[surfel]);
}

static __forceinline__ __device__ float3 load_tangent_u(int surfel) {
    return make_vec3(params.tangent_u_x[surfel], params.tangent_u_y[surfel], params.tangent_u_z[surfel]);
}

static __forceinline__ __device__ float3 load_tangent_v(int surfel) {
    return make_vec3(params.tangent_v_x[surfel], params.tangent_v_y[surfel], params.tangent_v_z[surfel]);
}
...
static __forceinline__ __device__ bool evaluate_gaussian_hit(float3 origin, float3 direction, float trace_tmax,
                                                             int surfel, float& plane_t, float* out_alpha = nullptr) {
    ...
    if (!isfinite(plane_t) || !(plane_t > params.ray_epsilon) || !(plane_t < trace_tmax)) {
        return false;
    }
...
static __forceinline__ __device__ float sh_basis(int basis, float3 view) {
    ...
    case 1:
        return 0.4886025119029199f * view.y;
    case 2:
        return 0.4886025119029199f * view.z;
    case 3:
        return 0.4886025119029199f * view.x;
    case 4:
        return 1.0925484305920792f * view.x * view.y;
    case 5:
        return 1.0925484305920792f * view.y * view.z;
    case 6:
        return 0.31539156525252005f * (3.0f * view.z * view.z - 1.0f);
    case 7:
        return 1.0925484305920792f * view.x * view.z;
    case 8:
        return 0.5462742152960396f * (view.x * view.x - view.y * view.y);
    case 9:
        return 0.5900435899266435f * view.y * (3.0f * view.x * view.x - view.y * view.y);
    case 10:
        return 2.890611442640554f * view.x * view.y * view.z;
    case 11:
        return 0.4570457994644658f * view.y * (5.0f * view.z * view.z - 1.0f);
    case 12:
        return 0.3731763325901154f * view.z * (5.0f * view.z * view.z - 3.0f);
    case 13:
        return 0.4570457994644658f * view.x * (5.0f * view.z * view.z - 1.0f);
    case 14:
        return 1.445305721320277f * view.z * (view.x * view.x - view.y * view.y);
    case 15:
        return 0.5900435899266435f * view.x * (view.x * view.x - 3.0f * view.y * view.y);
    default:
        return 0.0f;
    }
}
...
static __forceinline__ __device__ float3 oriented_surfel_normal(int surfel, float3 direction) {
    const float3 tangent_u =
        make_float3(params.tangent_u_x[surfel], params.tangent_u_y[surfel], params.tangent_u_z[surfel]);
    const float3 tangent_v =
        make_float3(params.tangent_v_x[surfel], params.tangent_v_y[surfel], params.tangent_v_z[surfel]);
    const float3 raw_normal = cross3(tangent_u, tangent_v);
    const float normal_len_sq = squared_norm(raw_normal);
    if (!(normal_len_sq > 1.0e-16f)) {
...
static __forceinline__ __device__ float evaluate_appearance_channel(int surfel, int channel, float3 view) {
    ...
    if (params.color_model == 1) {
        ...
        const int storage_degree =
            params.appearance_sh_degree < 0 ? 0 : (params.appearance_sh_degree > 3 ? 3 : params.appearance_sh_degree);
        ...
        for (int basis = 0; basis < basis_count; ++basis) {
            value += params.appearance_values[base + basis * 3 + rgb_channel] * sh_basis(basis, view);
        }
        return value;
    }

    const int channel_count = params.appearance_channel_count > 0 ? params.appearance_channel_count : 1;
...
static __forceinline__ __device__ void insert_composite_hit(unsigned int ray, int surfel, float t, float alpha,
                                                            float value) {
    const int k = params.composite_hit_capacity;
    if (k <= 0 || params.scratch_surfel_id == nullptr || params.scratch_t == nullptr ||
        params.scratch_alpha == nullptr || params.scratch_value == nullptr) {
        return;
    }
    ...
    const bool before_tail =
        tail_id < 0 || t < tail_t - kSortEpsilon || (fabsf(t - tail_t) <= kSortEpsilon && surfel < tail_id);
    if (!before_tail) {
        return;
    }
    ...
    while (insert > 0) {
        ...
        const bool before_prev =
            prev_id < 0 || t < prev_t - kSortEpsilon || (fabsf(t - prev_t) <= kSortEpsilon && surfel < prev_id);
        if (!before_prev) {
            break;
        }
...
static __forceinline__ __device__ void composite_candidate_buffer_for_ray(
    unsigned int ray, int k, int channel_count, int channel_base, float3 view, float3 direction, float& intensity,
    float& alpha_accum, float& transmittance, float& depth_numerator, float3& normal_numerator) {
    if (k <= 0 || params.scratch_surfel_id == nullptr || params.scratch_t == nullptr ||
...
static __forceinline__ __device__ void consider_intersection_hit(unsigned int ray, int triangle_id, int surfel,
                                                                 float plane_t) {
    ...
    const bool take =
        current_triangle < 0 || plane_t < current_t - kSortEpsilon ||
        (fabsf(plane_t - current_t) <= kSortEpsilon && (current_surfel < 0 || surfel < current_surfel ||
...
extern "C" __global__ void __anyhit__surfel_composite() {
    ...
    if (ray >= static_cast<unsigned int>(params.ray_count) ||
        primitive >= static_cast<unsigned int>(params.triangle_count) || params.triangle_to_surfel_id == nullptr) {
    ...
    float plane_t = 0.0f;
    float alpha = 0.0f;
    if (evaluate_gaussian_hit(optixGetWorldRayOrigin(), optixGetWorldRayDirection(), ray_tmax(ray), surfel, plane_t,
                              &alpha)) {
...
extern "C" __global__ void __raygen__surfel_trace() {
    const unsigned int ray = optixGetLaunchIndex().x;
    if (ray >= static_cast<unsigned int>(params.ray_count) || !is_active(ray) || params.handle == 0ull ||
        params.triangle_count <= 0 || params.surfel_count <= 0 || params.triangle_to_surfel_id == nullptr ||
        params.out_triangle_id == nullptr || params.out_proxy_t == nullptr) {
        write_invalid(ray);
        return;
    }
    ...
    uint32_t dummy = 0u;
    optixTrace(static_cast<OptixTraversableHandle>(params.handle), load_ray_origin(ray), load_ray_direction(ray),
               fmaxf(params.ray_epsilon, 0.0f), trace_tmax, 0.0f, 255u,
               OPTIX_RAY_FLAG_DISABLE_CLOSESTHIT | OPTIX_RAY_FLAG_ENFORCE_ANYHIT, 1, 1, 0, dummy);

    const bool valid = params.out_triangle_id[ray] >= 0 && params.out_triangle_id[ray] < params.triangle_count &&
                       isfinite(params.out_proxy_t[ray]) && params.out_proxy_t[ray] < trace_tmax;
    if (params.out_valid != nullptr) {
...
extern "C" __global__ void __raygen__surfel_composite() {
    ...
    const bool trace_enabled = is_active(ray) && params.handle != 0ull && params.triangle_count > 0 &&
                               params.surfel_count > 0 && params.triangle_to_surfel_id != nullptr;
    ...
    if (trace_enabled) {
        ...
        for (int segment = 0;
             segment < segment_limit && transmittance > params.transmittance_min && t_start < trace_tmax; ++segment) {
            clear_candidate_buffer_for_ray(ray, k);
            uint32_t dummy = 0u;
            optixTrace(static_cast<OptixTraversableHandle>(params.handle), load_ray_origin(ray),
                       load_ray_direction(ray), t_start, trace_tmax, 0.0f, 255u,
                       OPTIX_RAY_FLAG_DISABLE_CLOSESTHIT | OPTIX_RAY_FLAG_ENFORCE_ANYHIT, 0, 1, 0, dummy);
            ...
            composite_candidate_buffer_for_ray(ray, k, channel_count, channel_base, view, ray_direction, intensity,
                                               alpha_accum, transmittance, depth_numerator, normal_numerator);
    ...
    if (params.output_normal != 0 && params.out_normal_x != nullptr && params.out_normal_y != nullptr &&
