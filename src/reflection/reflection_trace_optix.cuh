...
// Defines shared reflection support for trace optix device.
...
// host-compilable src/reflection/reflection_algorithms.cuh; this header
...
template <bool AllowAoSInputs, bool AllowPackedTriangles, bool HonorOutputLayout, bool ClearEmptySlots,
          bool NullableRayTMax, bool AllowExtendedOutputs>
...
using DrJitReflectionTracePolicy = ReflectionTracePolicy<false, false, false, false, false, false>;
using TorchReflectionTracePolicy = ReflectionTracePolicy<true, true, true, true, true, true>;
...
static __forceinline__ __device__ OptixTraverser make_reflection_traverser(::OptixTraversableHandle handle) {
    return OptixTraverser{handle, static_cast<unsigned int>(OPTIX_RAY_FLAG_DISABLE_ANYHIT),           0u, 1u,
                          0u,     ::rayd::shared::multipath::reflection_trace_algo_detail::kTraceTMax};
}

template <typename Policy>
static __forceinline__ __device__ void reflection_trace_raygen(const ReflectionTraceParams& params) {
