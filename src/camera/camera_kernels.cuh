...
__global__ void sample_to_world_kernel(int64_t count, const float* sample, int64_t s0, int64_t s1, float* world,
...
__global__ void sample_to_world_backward_kernel(int64_t count, const float* grad_world, int64_t s0, int64_t s1,
...
__global__ void world_to_sample_kernel(int64_t count, const float* point, int64_t s0, int64_t s1, float* sample,
...
__global__ void world_to_sample_backward_kernel(int64_t count, const float* point, int64_t p0, int64_t p1,
    ...
    grad_point[i * 3 + 2] =
        z_raw > 1.0e-12f ? gu * (-0.5f * x * inv_z2 / tan_x) + gv * (0.5f * y * inv_z2 / tan_y) : 0.0f;
}

__global__ void sample_ray_kernel(int64_t count, const float* sample, int64_t s0, int64_t s1, float* origin,
                                  float* direction, float tan_x, float tan_y) {
    const int64_t i = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (i >= count)
        return;
...
__global__ void sample_ray_backward_kernel(int64_t count, const float* sample, int64_t s0, int64_t s1,
