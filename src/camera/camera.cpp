...
void require_optional_grad(const c10::optional<at::Tensor>& grad, int64_t rows, int64_t cols, const char* name) {
    if (!grad.has_value())
        return;
...
at::Tensor camera_sample_to_world_op(at::Tensor sample, double tan_x, double tan_y, double depth) {
    require_sample(sample, "sample");
    return camera_sample_to_world_cuda(sample, tan_x, tan_y, depth);
}

at::Tensor camera_sample_to_world_backward_op(at::Tensor grad_world, int64_t sample_count, double tan_x, double tan_y,
                                              double depth) {
    if (sample_count < 0)
...
at::Tensor camera_world_to_sample_op(at::Tensor point, double tan_x, double tan_y) {
    require_point(point, "point");
    return camera_world_to_sample_cuda(point, tan_x, tan_y);
}

at::Tensor camera_world_to_sample_backward_op(at::Tensor point, at::Tensor grad_sample, double tan_x, double tan_y) {
    require_point(point, "point");
...
py::tuple camera_sample_ray_op(at::Tensor sample, double tan_x, double tan_y) {
    require_sample(sample, "sample");
    auto [origin, direction] = camera_sample_ray_cuda(sample, tan_x, tan_y);
    return py::make_tuple(origin, direction);
}

at::Tensor camera_sample_ray_backward_op(at::Tensor sample, c10::optional<at::Tensor> grad_direction, double tan_x,
                                         double tan_y) {
    require_sample(sample, "sample");
    require_optional_grad(grad_direction, sample.size(0), 3, "grad_direction");
    return camera_sample_ray_backward_cuda(sample, grad_direction.has_value() ? &(*grad_direction) : nullptr, tan_x,
                                           tan_y);
}
