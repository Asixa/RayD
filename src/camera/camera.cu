...
at::Tensor camera_sample_to_world_backward_cuda(const at::Tensor& grad, int64_t count, double tan_x, double tan_y,
                                                double depth) {
...
at::Tensor camera_world_to_sample_backward_cuda(const at::Tensor& point, const at::Tensor& grad, double tan_x,
                                                double tan_y) {
    c10::cuda::CUDAGuard guard(point.device());
    ...
    camera_detail::launch_1d(stream_for(point), point.size(0), camera_detail::world_to_sample_backward_kernel,
                             ...
                             grad.stride(0), grad.stride(1), out.data_ptr<float>(), static_cast<float>(tan_x),
...
at::Tensor camera_sample_ray_backward_cuda(const at::Tensor& sample, const at::Tensor* grad, double tan_x,
                                           double tan_y) {
    c10::cuda::CUDAGuard guard(sample.device());
