...
struct DiffractionAccumulationPolicy {
    ...
    static __forceinline__ __device__ const DfrAccumParams& params() { return ::rayd::torch_backend::params; }
    ...
    static __forceinline__ __device__ void atomic_add_same_cell(float* base, int i, float v, CellGroup g) {
        ::rayd::torch_backend::atomic_add_same_cell(base, i, v, g);
    }
    static __forceinline__ __device__ void atomic_add_same_cell(int* base, int i, int v, CellGroup g) {
        ::rayd::torch_backend::atomic_add_same_cell(base, i, v, g);
    }
    ...
    static __forceinline__ __device__ bool stage_order1(unsigned int lane, int cell, float power, float field_x_re,
        ...
        params().stage_value[lane] = make_float4(power, field_x_re, is_direct ? 1.f : 0.f, is_keller ? 1.f : 0.f);
        return true;
    }
    static __forceinline__ __device__ bool stage_coherent(int cell, int state_idx, bool is_multi, float xr, float xi,
    ...
    static __forceinline__ __device__ bool active_for_state(const uint8_t* mask, int width, int stride, int state_idx) {
        if (mask == nullptr) {
            return true;
        }
    ...
    static __forceinline__ __device__ ::rayd::shared::math::Vec3f state_vec(const float* x, const float* y,
                                                                            const float* z, int stride, int idx) {
        const int offset = idx * stride;
        return ::rayd::shared::math::make_vec3(x[offset], y[offset], z[offset]);
    }

    static __forceinline__ __device__ float3 optional_state_vec(const float* x, const float* y, const float* z,
                                                                int stride, int idx) {
        const int offset = idx * stride;
        return make_f3(x != nullptr ? x[offset] : 0.f, y != nullptr ? y[offset] : 0.f, z != nullptr ? z[offset] : 0.f);
    }

    static __forceinline__ __device__ ::rayd::shared::math::Vec3f recursive_state_vec(const float* x, const float* y,
    ...
    static __forceinline__ __device__ int read_i32(const int* ptr, int stride, int idx) { return ptr[idx * stride]; }
    ...
    static __forceinline__ __device__ int sample_state_index_for_lane(unsigned int lane) {
        ...
        const int state_idx =
            read_i32(params().sample_state_index, params().sample_state_index_stride, static_cast<int>(lane));
    ...
    static __forceinline__ __device__ ::rayd::shared::math::Vec3f state_edge_pos_at(int idx) {
        return state_vec(params().state_edge_pos_x, params().state_edge_pos_y, params().state_edge_pos_z,
                         params().state_edge_pos_stride, idx);
    }

    static __forceinline__ __device__ ::rayd::shared::math::Vec3f state_edge_dir_at(int idx) {
        return state_vec(params().state_edge_dir_x, params().state_edge_dir_y, params().state_edge_dir_z,
                         params().state_edge_dir_stride, idx);
    }
    ...
    static __forceinline__ __device__ float sample_edge_weight_for_lane(int state_idx, unsigned int lane,
                                                                        int sample_count) {
        if (params().sample_edge_weight != nullptr) {
            return fmaxf(read_f32(params().sample_edge_weight, params().sample_edge_weight_stride,
                                  static_cast<int>(lane)),
                         0.f);
        }
        const float edge_length = fmaxf(state_edge_t_max_at(state_idx) - state_edge_t_min_at(state_idx), 0.f);
        return edge_length * static_cast<float>(params().state_count) / fmaxf(static_cast<float>(sample_count), 1.f);
    }
    ...
    static __forceinline__ __device__ ::rayd::shared::math::Vec3f state_src_at(int idx) {
        return state_vec(params().state_src_x, params().state_src_y, params().state_src_z, params().state_src_stride,
                         idx);
    }

    static __forceinline__ __device__ float3 state_wi_at(int idx) {
        return optional_state_vec(params().state_wi_x, params().state_wi_y, params().state_wi_z,
                                  params().state_wi_stride, idx);
    }
    ...
    static __forceinline__ __device__ ::rayd::shared::math::Vec3f recursive_state_edge_pos_at(int idx) {
        return recursive_state_vec(params().recursive_state_edge_pos_x, params().recursive_state_edge_pos_y,
                                   params().recursive_state_edge_pos_z, params().recursive_state_edge_pos_stride, idx);
    }

    static __forceinline__ __device__ ::rayd::shared::math::Vec3f recursive_state_edge_dir_at(int idx) {
        return recursive_state_vec(params().recursive_state_edge_dir_x, params().recursive_state_edge_dir_y,
                                   params().recursive_state_edge_dir_z, params().recursive_state_edge_dir_stride, idx);
    }
...
using Device =
    ::rayd::shared::multipath::diffraction_accumulation::DiffractionAccumulationDevice<DiffractionAccumulationPolicy>;
...
extern "C" __global__ void __raygen__diffraction_order1_accumulation() {
    Device::make_algo().run_diffraction_order1_accumulation_algo<false, false, true, true, true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_accumulation_primary() {
    Device::make_algo().run_diffraction_order1_accumulation_algo<true, false, true, true, true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_accumulation_no_suffix() {
    Device::make_algo().run_diffraction_order1_accumulation_algo<false, false, true, true, false>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_accumulation_no_suffix_primary() {
    Device::make_algo().run_diffraction_order1_accumulation_algo<true, false, true, true, false>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_accumulation_suffix() {
    Device::make_algo().run_diffraction_order1_accumulation_algo<false, false, false, false, true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_accumulation_suffix_primary() {
    Device::make_algo().run_diffraction_order1_accumulation_algo<true, false, false, false, true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_source_visibility_primary() {
    Device::make_algo().run_diffraction_order1_source_visibility_algo<true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_no_suffix_target_accumulation_primary() {

    Device::make_algo().run_diffraction_order1_no_suffix_target_accumulation_algo<true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_suffix_first_visibility_primary() {
    Device::make_algo().run_diffraction_order1_suffix_first_visibility_algo<true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_suffix_target_accumulation_primary() {
    Device::make_algo().run_diffraction_order1_suffix_target_accumulation_algo<true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_coherent_accumulation() {
    Device::make_algo().run_diffraction_order1_coherent_accumulation_algo<false>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_order1_coherent_accumulation_primary() {
    Device::make_algo().run_diffraction_order1_coherent_accumulation_algo<true>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_chain_accumulation() {
    Device::make_algo().run_diffraction_chain_accumulation_algo<false>(global_lane());
}

extern "C" __global__ void __raygen__diffraction_chain_accumulation_primary() {
    Device::make_algo().run_diffraction_chain_accumulation_algo<true>(global_lane());
}
