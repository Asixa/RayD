...
#define RAYD_DFR_AD_READ_I32_OR(P, F, S, I, D) ((P).F != nullptr ? (P).F[(I)] : (D))
#define RAYD_DFR_AD_READ_VEC(P, X, Y, Z, S, I) make_vec3((P).X[(I)], (P).Y[(I)], (P).Z[(I)])
#define RAYD_DFR_AD_READ_VEC_GUARDED(P, X, Y, Z, S, I)                                                                 \
    make_vec3((P).X != nullptr ? (P).X[(I)] : 0.f, (P).Y != nullptr ? (P).Y[(I)] : 0.f,                                \
              (P).Z != nullptr ? (P).Z[(I)] : 0.f)
#define RAYD_DFR_AD_READ_DOT_F32(P, F, S, I) read_or_zero((P).F, (I))
#define RAYD_DFR_AD_READ_DOT_VEC(P, X, Y, Z, S, I) read_vec_or_zero((P).X, (P).Y, (P).Z, (I))
...
#define RAYD_DFR_AD_ADD_UNIT_VJP(P, PR, G, F, S, I, T) add_unit_vjp((P), (PR), (G), (P).F, (I), (T))
...
__global__ void dfr_direct_accum_vjp_kernel(DfrDirectAccumADParams params) {
    ...
    float grad_contribution = read_or_zero(params.grad_out_power, p.cell);
    const float amp = sqrtf(fmaxf(p.contribution, 0.f));
    if (amp > kSmallEps) {
        grad_contribution += read_or_zero(params.grad_out_field_x_re, p.cell) * 0.5f / amp;
    }
    ...
    if (p.material_active && p.material_idx >= 0 && params.grad_material_gain != nullptr) {
        const float grad_gain = grad_contribution * p.contribution / fmaxf(p.material_gain, kSmallEps);
    ...
    if (p.wedge_active && params.grad_state_exterior_angle != nullptr) {
        const float grad_wedge = grad_contribution * p.contribution / fmaxf(p.wedge_scale, kSmallEps);
        atomicAdd(params.grad_state_exterior_angle + p.state_idx, grad_wedge / (2.f * kPi));
    }
    ...
    atomic_add_vec(params.grad_state_src_x, params.grad_state_src_y, params.grad_state_src_z, p.state_idx, grad_source);
    atomic_add_vec(params.grad_state_edge_pos_x, params.grad_state_edge_pos_y, params.grad_state_edge_pos_z,
                   p.state_idx, grad_edge_point);

    const float grad_edge_t = dot3(grad_edge_point, p.edge_dir);
    if (params.grad_state_edge_t_min != nullptr) {
        atomicAdd(params.grad_state_edge_t_min + p.state_idx, (1.f - p.edge_u) * grad_edge_t - grad_edge_length);
    }
    if (params.grad_state_edge_t_max != nullptr) {
        atomicAdd(params.grad_state_edge_t_max + p.state_idx, p.edge_u * grad_edge_t + grad_edge_length);
    }
    ...
    atomic_add_vec(params.grad_state_edge_dir_x, params.grad_state_edge_dir_y, params.grad_state_edge_dir_z,
                   p.state_idx, grad_edge_dir_raw);
}
...
void launch_ad_kernel(const char* name, Kernel kernel, const Params& params) {
    ...
    require(stream != nullptr, "dfr_direct_accum_ad_gpu(): CUDA stream is unavailable.");
