...
DfrPathsRaw alloc_dfr_paths_raw(int capacity) {
    ...
    raw.p0 = Vector3f(empty<Float>(capacity), empty<Float>(capacity), empty<Float>(capacity));
    raw.p1 = Vector3f(empty<Float>(capacity), empty<Float>(capacity), empty<Float>(capacity));
    raw.p2 = Vector3f(empty<Float>(capacity), empty<Float>(capacity), empty<Float>(capacity));
    return raw;
}
...
DfrPathsT<Detached> Scene::trace_dfr_paths(const Vector3fT<Detached>& tx_positions,
    ...
    ScopedNativeLaunchStage native_launch_stage(NativeLaunchStage::AccumDfr);
    require(is_ready(), "Scene::trace_dfr_paths(): scene is not built.");
    require(!pending_updates_, "Scene::trace_dfr_paths(): scene has pending updates. Call Scene::sync() first.");
    require(options.wavelength > 0.f, "Scene::trace_dfr_paths(): wavelength must be positive.");
    require(options.max_order == 1, "Scene::trace_dfr_paths(): only max_order == 1 is supported.");
    require(options.max_paths > 0, "Scene::trace_dfr_paths(): max_paths must be positive.");
    ...
    if constexpr (!Detached) {
        ...
        const int rx_count = options.max_rx > 0 ? std::min(rx_width, options.max_rx) : rx_width;
        ...
        require(state_count > 0 && state_count <= state_width, "Scene::trace_dfr_paths(): invalid state count.");
        ...
        require(rx_count <= rx_width, "Scene::trace_dfr_paths(): invalid receiver count.");

        const int material_count = static_cast<int>(slices(material.eta_r));
        require(material_count > 0, "Scene::trace_dfr_paths(): material payload must not be empty.");
        ...
        const int64_t capacity64 =
            static_cast<int64_t>(tx_count) * static_cast<int64_t>(rx_count) * static_cast<int64_t>(state_limit);
        ...
        auto material_gain_for_faces = [&](const IntAD& prim0, const IntAD& prim1,
                                           const MaskAD& path_active) -> FloatAD {
            const MaskAD prim0_in_range = path_active && (prim0 >= IntAD(0)) && (prim0 < IntAD(material_count));
            const IntAD safe0 = select(prim0_in_range, prim0, IntAD(0));
            const MaskAD prim0_valid = prim0_in_range && gather<MaskAD>(material.valid, safe0, prim0_in_range);
            const MaskAD prim1_in_range = path_active && (prim1 >= IntAD(0)) && (prim1 < IntAD(material_count));
            const IntAD safe1 = select(prim1_in_range, prim1, IntAD(0));
            const MaskAD prim1_valid = prim1_in_range && gather<MaskAD>(material.valid, safe1, prim1_in_range);
            const IntAD chosen = select(prim0_valid, safe0, safe1);
            const MaskAD chosen_valid = prim0_valid || prim1_valid;
            const FloatAD gain = gather<FloatAD>(material.gain, chosen, chosen_valid);
        ...
        const MaskAD state_active = gather<MaskAD>(active_ad, state_idx, lane_active);

        const Vector3fAD edge_pos = gather<Vector3fAD>(states.edge_pos, state_idx, state_active);
        const Vector3fAD edge_dir = normalize(gather<Vector3fAD>(states.edge_dir, state_idx, state_active));
        const FloatAD edge_t_min = gather<FloatAD>(states.edge_t_min, state_idx, state_active);
        const FloatAD edge_t_max = gather<FloatAD>(states.edge_t_max, state_idx, state_active);
        const FloatAD edge_t = FloatAD(0.5f) * (edge_t_min + edge_t_max);
        const Vector3fAD edge_point = edge_pos + edge_t * edge_dir;
        const Vector3fAD source = gather<Vector3fAD>(states.src, state_idx, state_active);
        const Vector3fAD receiver = gather<Vector3fAD>(rx_positions, rx_idx, lane_active);
        const FloatAD src_power = gather<FloatAD>(states.src_power, state_idx, state_active);
        ...
        const FloatAD exterior_angle = gather<FloatAD>(states.exterior_angle, state_idx, state_active);
        const IntAD edge_index = gather<IntAD>(states.edge_index, state_idx, state_active);

        const MaskAD finite_active = state_active && drjit::isfinite(source.x()) && drjit::isfinite(source.y()) &&
                                     drjit::isfinite(source.z()) && drjit::isfinite(edge_point.x()) &&
                                     drjit::isfinite(edge_point.y()) && drjit::isfinite(edge_point.z()) &&
                                     drjit::isfinite(receiver.x()) && drjit::isfinite(receiver.y()) &&
                                     drjit::isfinite(receiver.z()) && drjit::isfinite(src_power);
        const SegmentPairVisibilityAD visibility =
            this->template visible_pair<false>(edge_point, source, receiver, Int(), finite_active);
        const MaskAD visible = visibility.visible_a && visibility.visible_b;
        const FloatAD source_distance = maximum(norm(edge_point - source), FloatAD(Epsilon));
        const FloatAD receiver_distance = maximum(norm(receiver - edge_point), FloatAD(Epsilon));
        const FloatAD edge_length = maximum(edge_t_max - edge_t_min, FloatAD(0.f));
        const FloatAD wedge_scale =
            minimum(maximum(exterior_angle, FloatAD(0.25f * Pi)) / FloatAD(2.f * Pi), FloatAD(2.f));
        const FloatAD material_gain = material_gain_for_faces(prim0, prim1, finite_active);
        const FloatAD wave_gain = FloatAD(options.wavelength) / FloatAD(4.f * Pi);
        const FloatAD contribution = src_power * material_gain * edge_length * wedge_scale * wave_gain * wave_gain /
                                     (source_distance * source_distance * receiver_distance * receiver_distance);
        const MaskAD path_active = visible && (contribution > FloatAD(0.f)) && drjit::isfinite(contribution);
        ...
        result.edge1 = full<IntAD>(-1, capacity);
        result.edge2 = full<IntAD>(-1, capacity);
        result.delay = select(path_active, path_length / FloatAD(299792458.f), FloatAD(0.f));
        result.field_x = drjit::Complex<FloatAD>(select(path_active, amplitude * cos(phase), FloatAD(0.f)),
        ...
        result.p1 = zeros<Vector3fAD>(capacity);
        result.p2 = zeros<Vector3fAD>(capacity);
        scatter_reduce(ReduceOp::Add, result.count, IntAD(1), zeros<IntAD>(capacity), path_active);
        return result;
    } else {
        const int tx_count = static_cast<int>(slices(tx_positions));
        const int rx_width = static_cast<int>(slices(rx_positions));
        ...
        active_detached &= drjit::isfinite(states.src.x()) && drjit::isfinite(states.src.y()) &&
                           drjit::isfinite(states.src.z()) && drjit::isfinite(states.edge_pos.x()) &&
                           drjit::isfinite(states.edge_pos.y()) && drjit::isfinite(states.edge_pos.z()) &&
        ...
        if (!cuda_trace) {
            ...
            require(hitgroup_record_count > 0, "Scene::trace_dfr_paths(): invalid hitgroup record count.");

            if (split_mode != 0) {
                ensure_pipeline(diffraction_paths_pipeline_, primary_scene->context(), hitgroup_record_count,
                                diffraction_paths_pipeline_config());
            }
        }

        drjit::eval(tx_positions, rx_positions, states.edge_index, states.edge_pos, states.edge_dir, states.edge_t_min,
                    states.edge_t_max, states.n0, states.n1, states.prim0, states.prim1, states.exterior_angle,
                    states.src, states.src_power, active_detached, material.eta_r, material.sigma, material.mu_r,
                    material.gain, material.valid);
        ...
        if (split_mode == 0) {
            ...
            params.temp_visibility = reinterpret_cast<uint8_t*>(temp_visibility.data());

            if (cuda_trace) {
                cuda_backend().run_dfr_paths(params, capacity);
            } else {
                ensure_pipeline(diffraction_paths_source_visibility_primary_pipeline_, primary_scene->context(),
                                hitgroup_record_count, diffraction_paths_source_visibility_primary_pipeline_config());
                diffraction_paths_source_visibility_primary_pipeline_->launch(0, params);

                ensure_pipeline(diffraction_paths_target_export_primary_pipeline_, primary_scene->context(),
                                hitgroup_record_count, diffraction_paths_target_export_primary_pipeline_config());
...
template DfrPaths Scene::trace_dfr_paths<true>(const Vector3f& tx_positions, const Vector3f& rx_positions,
                                               const DfrStates& states, const DfrMaterial& material,
                                               const DfrPathOptions& options, Mask active) const;
template DfrPathsAD Scene::trace_dfr_paths<false>(const Vector3fAD& tx_positions, const Vector3fAD& rx_positions,
                                                  const DfrStatesAD& states, const DfrMaterialAD& material,
                                                  const DfrPathOptions& options, MaskAD active) const;
...
void init_dfr_accum_raw(DfrAccumRaw& raw) {
    const int zero_i = 0;
    const float zero_f = 0.f;
    jit_memset_async(JitBackend::CUDA, raw.power.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    ...
    jit_memset_async(JitBackend::CUDA, raw.vis_rejects.data(), 1, sizeof(int), &zero_i);
    jit_memset_async(JitBackend::CUDA, raw.edge_vis_rejects.data(), 1, sizeof(int), &zero_i);
...
int dfr_accum_launch_count(const DfrOptions& options) {
    return dfr_accum_direct_sample_count(options) + dfr_accum_keller_sample_count(options) +
...
void Scene::ensure_dfr_order1_accumulation_pipeline() const {
    ...
    require(scenes.hitgroup_record_count > 0, "Scene::accum_dfr_direct(): invalid hitgroup record count.");
    auto& pipeline = scenes.split_mode == 0 ? diffraction_order1_accumulation_primary_pipeline_
    ...
    ensure_pipeline(pipeline, primary_scene->context(), scenes.hitgroup_record_count, config);
}

void Scene::ensure_dfr_chain_accumulation_pipeline() const {
    const OptixSceneSelection scenes = select_optix_scenes();
    const OptixScene* primary_scene = scenes.primary;
    require(primary_scene != nullptr && primary_scene->is_ready(), "Scene::accum_dfr(): OptiX scene is not ready.");
    require(scenes.hitgroup_record_count > 0, "Scene::accum_dfr(): invalid hitgroup record count.");
    auto& pipeline = scenes.split_mode == 0 ? diffraction_chain_accumulation_primary_pipeline_
                                            : diffraction_chain_accumulation_pipeline_;
    const OptixPipelineConfig config = scenes.split_mode == 0 ? diffraction_chain_accumulation_primary_pipeline_config()
                                                              : diffraction_chain_accumulation_pipeline_config();
    ensure_pipeline(pipeline, primary_scene->context(), scenes.hitgroup_record_count, config);
}

template <bool Detached>
DfrAccumT<Detached> Scene::accum_dfr_direct(const DfrStatesT<Detached>& states, const DfrGrid& grid,
                                            const DfrMaterialT<Detached>& material, const DfrOptions& options,
                                            MaskT<Detached> active) const {
    ScopedNativeLaunchStage native_launch_stage(NativeLaunchStage::AccumDfr);
    require(is_ready(), "Scene::accum_dfr_direct(): scene is not built.");
    require(!pending_updates_, "Scene::accum_dfr_direct(): scene has pending updates. Call Scene::sync() first.");
    require(grid.axis >= 0 && grid.axis <= 2, "Scene::accum_dfr_direct(): grid.axis must be 0, 1, or 2.");
    ...
    require(grid.cell_area > 0.f, "Scene::accum_dfr_direct(): grid.cell_area must be positive.");
    require(options.wavelength > 0.f, "Scene::accum_dfr_direct(): wavelength must be positive.");
    require(options.max_order == 1, "Scene::accum_dfr_direct(): only max_order == 1 is supported.");
    ...
    if constexpr (!Detached) {
        ...
        return dfr_direct_accum_custom_op(this, states, grid, material, options, triangle_info_.p0,
                                          triangle_info_.face_normal, global_geometry_.vertices, global_geometry_.faces,
                                          active);

        result.power = zeros<FloatAD>(grid_cell_count);
        result.field_x = drjit::Complex<FloatAD>(zeros<FloatAD>(grid_cell_count), zeros<FloatAD>(grid_cell_count));
        result.field_y = drjit::Complex<FloatAD>(zeros<FloatAD>(grid_cell_count), zeros<FloatAD>(grid_cell_count));
        result.field_z = drjit::Complex<FloatAD>(zeros<FloatAD>(grid_cell_count), zeros<FloatAD>(grid_cell_count));
        ...
        require(state_count > 0 && state_count <= state_width, "Scene::accum_dfr_direct(): invalid state count.");
        require(static_cast<int>(slices(states.edge_pos)) >= state_count &&
                    static_cast<int>(slices(states.edge_dir)) >= state_count &&
                    static_cast<int>(slices(states.edge_t_min)) >= state_count &&
                    static_cast<int>(slices(states.edge_t_max)) >= state_count &&
        ...
        const int material_count = static_cast<int>(slices(material.eta_r));
        require(material_count > 0, "Scene::accum_dfr_direct(): material payload must not be empty.");
        require(static_cast<int>(slices(material.sigma)) == material_count &&
                    static_cast<int>(slices(material.mu_r)) == material_count &&
                    static_cast<int>(slices(material.gain)) == material_count &&
                    static_cast<int>(slices(material.valid)) == material_count,
                "Scene::accum_dfr_direct(): material payload fields must have matching widths.");
        {
            ...
            require(primary_scene != nullptr && primary_scene->is_ready(),
                    "Scene::accum_dfr_direct(): OptiX scene is not ready.");
            require(scenes.hitgroup_record_count > 0, "Scene::accum_dfr_direct(): invalid hitgroup record count.");
            ensure_pipeline(diffraction_order1_accumulation_pipeline_, primary_scene->context(),
                            scenes.hitgroup_record_count, diffraction_order1_accumulation_pipeline_config());
        }

        const int direct_samples = (options.strategy_mask & RAYD_DFR_DIRECT) != 0
                                       ? (options.direct_samples > 0 ? options.direct_samples : options.samples)
                                       : 0;
        const int keller_samples = (options.strategy_mask & RAYD_DFR_KELLER) != 0 ? options.keller_samples : 0;
        const int suffix_samples = (options.strategy_mask & RAYD_DFR_SUFFIX_REFL) != 0 ? options.suffix_samples : 0;
        ...
        auto grid_cell_center = [](const DfrGrid& grid_desc, const IntAD& cell) -> Vector3fAD {
            const IntAD ix = cell % IntAD(grid_desc.resolution0);
            const IntAD iy = cell / IntAD(grid_desc.resolution0);
            const FloatAD u = (FloatAD(ix) + FloatAD(0.5f)) / maximum(FloatAD(grid_desc.resolution0), FloatAD(1.f));
            const FloatAD v = (FloatAD(iy) + FloatAD(0.5f)) / maximum(FloatAD(grid_desc.resolution1), FloatAD(1.f));
            const FloatAD c0 = FloatAD(grid_desc.coord0_min) + u * FloatAD(grid_desc.coord0_max - grid_desc.coord0_min);
            const FloatAD c1 = FloatAD(grid_desc.coord1_min) + v * FloatAD(grid_desc.coord1_max - grid_desc.coord1_min);
            if (grid_desc.axis == 0) {
        ...
        auto uniform01 = [&](const UIntAD& sample_lane, unsigned int stream) -> FloatAD {
            const UIntAD h = hash_u32(sample_lane ^ (UIntAD(stream) * UIntAD(0x9e3779b9u)) ^
        ...
        auto material_gain_for_faces = [&](const IntAD& prim0, const IntAD& prim1,
            ...
            const MaskAD prim0_in_range = sample_active && (prim0 >= IntAD(0)) && (prim0 < IntAD(material_count));
            const IntAD safe0 = select(prim0_in_range, prim0, IntAD(0));
            const MaskAD prim0_valid = prim0_in_range && gather<MaskAD>(material.valid, safe0, prim0_in_range);
            const MaskAD prim1_in_range = sample_active && (prim1 >= IntAD(0)) && (prim1 < IntAD(material_count));
            const IntAD safe1 = select(prim1_in_range, prim1, IntAD(0));
            const MaskAD prim1_valid = prim1_in_range && gather<MaskAD>(material.valid, safe1, prim1_in_range);
            const IntAD chosen = select(prim0_valid, safe0, safe1);
            const MaskAD chosen_valid = prim0_valid || prim1_valid;
        ...
        const MaskAD is_keller = !is_direct && (lane_i < IntAD(direct_samples + keller_samples));
        const MaskAD is_suffix = !is_direct && !is_keller && (lane_i < IntAD(launch_count));
        const IntAD state_idx = IntAD(lane % UIntAD(state_count));
        const IntAD cell = IntAD((lane / UIntAD(state_count)) % UIntAD(grid_cell_count));
        ...
        const FloatAD edge_t_min = gather<FloatAD>(states.edge_t_min, state_idx, state_active);
        const FloatAD edge_t_max = gather<FloatAD>(states.edge_t_max, state_idx, state_active);
        const FloatAD edge_t = edge_t_min + uniform01(lane, 0u) * (edge_t_max - edge_t_min);
        const Vector3fAD edge_point = edge_pos + edge_t * edge_dir;
        const Vector3fAD source = gather<Vector3fAD>(states.src, state_idx, state_active);
        const FloatAD src_power = gather<FloatAD>(states.src_power, state_idx, state_active);
        const IntAD prim0 = gather<IntAD>(states.prim0, state_idx, state_active);
        ...
        const SegmentPairVisibilityAD visibility =
            this->template visible_pair<false>(edge_point, source, target, Int(), finite_active);
        const MaskAD visible = visibility.visible_a && visibility.visible_b;

        const FloatAD source_distance = maximum(norm(edge_point - source), FloatAD(Epsilon));
        const FloatAD target_distance = maximum(norm(target - edge_point), FloatAD(Epsilon));
        const FloatAD edge_length = maximum(edge_t_max - edge_t_min, FloatAD(0.f));
        const FloatAD wedge_scale =
            minimum(maximum(exterior_angle, FloatAD(0.25f * Pi)) / FloatAD(2.f * Pi), FloatAD(2.f));
        const FloatAD material_gain = material_gain_for_faces(prim0, prim1, finite_active);
        const IntAD strategy_samples =
            select(is_direct, IntAD(std::max(direct_samples, 1)),
                   select(is_keller, IntAD(std::max(keller_samples, 1)), IntAD(std::max(suffix_samples, 1))));
        const FloatAD contribution = src_power * material_gain * edge_length * FloatAD(grid.cell_area) * wedge_scale /
                                     FloatAD(strategy_samples) /
                                     (source_distance * source_distance * target_distance * target_distance);
        const MaskAD contribution_active = visible && (contribution > FloatAD(0.f)) && drjit::isfinite(contribution);
        scatter_reduce(ReduceOp::Add, result.power, contribution, cell, contribution_active);
        const FloatAD amplitude = sqrt(maximum(contribution, FloatAD(0.f)));
        scatter_reduce(ReduceOp::Add, result.field_x.x(), amplitude, cell, contribution_active);
        scatter_reduce(ReduceOp::Add, result.direct_count, IntAD(1), zeros<IntAD>(launch_count),
                       contribution_active && is_direct);
        scatter_reduce(ReduceOp::Add, result.keller_count, IntAD(1), zeros<IntAD>(launch_count),
                       contribution_active && is_keller);
        scatter_reduce(ReduceOp::Add, result.suffix_count, IntAD(1), zeros<IntAD>(launch_count),
                       contribution_active && is_suffix);
        scatter_reduce(ReduceOp::Add, result.edge_uses, IntAD(1), zeros<IntAD>(launch_count),
                       contribution_active && options.collect_edge_use);
        scatter_reduce(ReduceOp::Add, result.vis_rejects, IntAD(1), zeros<IntAD>(launch_count),
                       finite_active && !visible && options.collect_debug_counts);
        return result;
    } else {
        result.power = zeros<Float>(grid_cell_count);
        result.field_x = drjit::Complex<Float>(zeros<Float>(grid_cell_count), zeros<Float>(grid_cell_count));
        result.field_y = drjit::Complex<Float>(zeros<Float>(grid_cell_count), zeros<Float>(grid_cell_count));
        result.field_z = drjit::Complex<Float>(zeros<Float>(grid_cell_count), zeros<Float>(grid_cell_count));
        ...
        if (!cuda_trace) {
            ...
            require(primary_scene != nullptr && primary_scene->is_ready(),
                    "Scene::accum_dfr_direct(): OptiX scene is not ready.");
            require(hitgroup_record_count > 0, "Scene::accum_dfr_direct(): invalid hitgroup record count.");
        }
        if (suffix_samples > 0) {
            require(triangle_count > 0, "Scene::accum_dfr_direct(): suffix reflection requires scene triangles.");
        ...
        const bool has_non_suffix_strategy = direct_samples > 0 || keller_samples > 0;
        ...
        if (has_suffix_strategy && has_non_suffix_strategy) {
            dfr_pipeline = split_mode == 0 ? &diffraction_order1_accumulation_primary_pipeline_
                                           : &diffraction_order1_accumulation_pipeline_;
            dfr_pipeline_config = split_mode == 0 ? diffraction_order1_accumulation_primary_pipeline_config()
                                                  : diffraction_order1_accumulation_pipeline_config();
        } else if (has_suffix_strategy) {
            dfr_pipeline = split_mode == 0 ? &diffraction_order1_accumulation_suffix_primary_pipeline_
                                           : &diffraction_order1_accumulation_suffix_pipeline_;
            dfr_pipeline_config = split_mode == 0 ? diffraction_order1_accumulation_suffix_primary_pipeline_config()
                                                  : diffraction_order1_accumulation_suffix_pipeline_config();
        } else {
            dfr_pipeline = split_mode == 0 ? &diffraction_order1_accumulation_no_suffix_primary_pipeline_
                                           : &diffraction_order1_accumulation_no_suffix_pipeline_;
            dfr_pipeline_config = split_mode == 0 ? diffraction_order1_accumulation_no_suffix_primary_pipeline_config()
                                                  : diffraction_order1_accumulation_no_suffix_pipeline_config();
        }

        drjit::eval(states.edge_index, states.edge_pos, states.edge_dir, states.edge_t_min, states.edge_t_max,
                    states.n0, states.n1, states.prim0, states.prim1, states.exterior_angle, states.src,
                    states.src_power, states.wi, states.d0, states.prefix_depth, active_detached, material.eta_r,
                    material.sigma, material.mu_r, material.gain, material.valid);
        if (suffix_samples > 0) {
        ...
        if (!staged_primary) {
            ensure_pipeline(*dfr_pipeline, primary_scene->context(), hitgroup_record_count, dfr_pipeline_config);
        }
        ...
        params.out_edge_vis_rejects = raw.edge_vis_rejects.data();
        params.out_utd_rejects = raw.utd_rejects.data();
        params.out_edge_uses = raw.edge_uses.data();
        if (active_dfr_direct_tape_capture != nullptr && active_dfr_direct_tape_capture->launch_count == launch_count) {
            params.tape_active = reinterpret_cast<uint8_t*>(active_dfr_direct_tape_capture->active.data());
            params.tape_state_idx = active_dfr_direct_tape_capture->state_idx.data();
            params.tape_cell = active_dfr_direct_tape_capture->cell.data();
            params.tape_material_idx = active_dfr_direct_tape_capture->material_idx.data();
            params.tape_edge_u = active_dfr_direct_tape_capture->edge_u.data();
        }
        ...
        if (staged_primary) {
            ...
            params.temp_visibility = reinterpret_cast<uint8_t*>(temp_visibility.data());
            if (cuda_trace) {
                cuda_backend().run_dfr_accum_direct(params, has_non_suffix_strategy, has_suffix_strategy, launch_count);
            } else {
                ensure_pipeline(diffraction_order1_source_visibility_primary_pipeline_, primary_scene->context(),
                                hitgroup_record_count, diffraction_order1_source_visibility_primary_pipeline_config());
                diffraction_order1_source_visibility_primary_pipeline_->launch(0, params);
                if (has_non_suffix_strategy) {
                    ensure_pipeline(diffraction_order1_no_suffix_target_primary_pipeline_, primary_scene->context(),
                ...
                if (has_suffix_strategy) {
                    ensure_pipeline(diffraction_order1_suffix_first_visibility_primary_pipeline_,
                                    primary_scene->context(), hitgroup_record_count,
                                    diffraction_order1_suffix_first_visibility_primary_pipeline_config());
                    ensure_pipeline(diffraction_order1_suffix_target_primary_pipeline_, primary_scene->context(),
                                    hitgroup_record_count, diffraction_order1_suffix_target_primary_pipeline_config());
        ...
        result.edge_vis_rejects = raw.edge_vis_rejects;
...
template <bool Detached>
DfrAccumT<Detached> Scene::accum_dfr(const DfrStatesT<Detached>& initial_states,
                                     const DfrStatesT<Detached>& recursive_states, const DfrGrid& grid,
                                     const DfrMaterialT<Detached>& material, const DfrOptions& options,
                                     MaskT<Detached> active) const {
    ScopedNativeLaunchStage native_launch_stage(NativeLaunchStage::AccumDfr);
    require(is_ready(), "Scene::accum_dfr(): scene is not built.");
    require(!pending_updates_, "Scene::accum_dfr(): scene has pending updates. Call Scene::sync() first.");
    require(grid.axis >= 0 && grid.axis <= 2, "Scene::accum_dfr(): grid.axis must be 0, 1, or 2.");
    require(grid.resolution0 > 0 && grid.resolution1 > 0, "Scene::accum_dfr(): grid resolution must be positive.");
    require(grid.coord0_min < grid.coord0_max && grid.coord1_min < grid.coord1_max,
            "Scene::accum_dfr(): grid bounds must be ordered.");
    require(grid.cell_area > 0.f, "Scene::accum_dfr(): grid.cell_area must be positive.");
    require(options.wavelength > 0.f, "Scene::accum_dfr(): wavelength must be positive.");
    ...
    if constexpr (!Detached) {
        ...
        return dfr_chain_accum_custom_op(this, initial_states, recursive_states, grid, material, options,
                                         triangle_info_.p0, triangle_info_.face_normal, global_geometry_.vertices,
                                         global_geometry_.faces, active);
        ...
        const int initial_count = initial_states.count > 0 ? initial_states.count : initial_width;
        const int recursive_width = static_cast<int>(slices(recursive_states.edge_index));
        const int recursive_count = recursive_states.count > 0 ? recursive_states.count : recursive_width;
        ...
        const int material_count = static_cast<int>(slices(material.eta_r));
        require(material_count > 0, "Scene::accum_dfr(): material payload must not be empty.");
        require(static_cast<int>(slices(material.sigma)) == material_count &&
                    static_cast<int>(slices(material.mu_r)) == material_count &&
                    static_cast<int>(slices(material.gain)) == material_count &&
                    static_cast<int>(slices(material.valid)) == material_count,
                "Scene::accum_dfr(): material payload fields must have matching widths.");
        {
            ...
            require(scenes.hitgroup_record_count > 0, "Scene::accum_dfr(): invalid hitgroup record count.");
            ensure_pipeline(diffraction_chain_accumulation_pipeline_, primary_scene->context(),
                            scenes.hitgroup_record_count, diffraction_chain_accumulation_pipeline_config());
        }
        ...
        auto chain_event_weight =
            [&](const FloatAD& src_power, const IntAD& face0_prim, const IntAD& face1_prim, const FloatAD& edge_t_min,
                const FloatAD& edge_t_max, const FloatAD& exterior_angle, const Vector3fAD& source,
                const Vector3fAD& edge_point, const Vector3fAD& target, const MaskAD& sample_active) -> FloatAD {
            const FloatAD source_distance = maximum(norm(edge_point - source), FloatAD(Epsilon));
            const FloatAD target_distance = maximum(norm(target - edge_point), FloatAD(Epsilon));
            const FloatAD edge_length = maximum(edge_t_max - edge_t_min, FloatAD(0.f));
            const FloatAD wedge_scale =
                minimum(maximum(exterior_angle, FloatAD(0.25f * Pi)) / FloatAD(2.f * Pi), FloatAD(2.f));
            const FloatAD material_gain = material_gain_for_faces(face0_prim, face1_prim, sample_active);
        ...
        const UIntAD second_hash = hash_u32(
            lane ^ (UIntAD(static_cast<unsigned int>(options.seed)) * UIntAD(0x9e3779b9u)) ^ UIntAD(0x51ed270bu));
        const IntAD second_idx = IntAD(second_hash % UIntAD(recursive_count));
        const UIntAD third_hash = hash_u32(
            lane ^ (UIntAD(static_cast<unsigned int>(options.seed)) * UIntAD(0x85ebca6bu)) ^ UIntAD(0xc2b2ae35u));
        const IntAD third_idx = IntAD(third_hash % UIntAD(recursive_count));
        const IntAD cell = IntAD((lane / UIntAD(initial_count)) % UIntAD(grid_cell_count));
        const MaskAD lane_active = full<MaskAD>(true, launch_count);
        const MaskAD first_active = gather<MaskAD>(active_ad, first_idx, lane_active);
        const IntAD first_edge_index = gather<IntAD>(initial_states.edge_index, first_idx, first_active);
        const IntAD second_edge_index = gather<IntAD>(recursive_states.edge_index, second_idx, first_active);
        const IntAD third_edge_index = gather<IntAD>(recursive_states.edge_index, third_idx, first_active);
        ...
        const Vector3fAD first_edge_pos = gather<Vector3fAD>(initial_states.edge_pos, first_idx, first_active);
        const Vector3fAD first_edge_dir =
            normalize(gather<Vector3fAD>(initial_states.edge_dir, first_idx, first_active));
        const FloatAD first_t_min = gather<FloatAD>(initial_states.edge_t_min, first_idx, first_active);
        const FloatAD first_t_max = gather<FloatAD>(initial_states.edge_t_max, first_idx, first_active);
        const FloatAD first_t = first_t_min + uniform01(lane, 0u) * (first_t_max - first_t_min);
        const Vector3fAD first_point = first_edge_pos + first_t * first_edge_dir;

        const Vector3fAD second_edge_pos = gather<Vector3fAD>(recursive_states.edge_pos, second_idx, first_active);
        const Vector3fAD second_edge_dir =
            normalize(gather<Vector3fAD>(recursive_states.edge_dir, second_idx, first_active));
        const FloatAD second_t_min = gather<FloatAD>(recursive_states.edge_t_min, second_idx, first_active);
        const FloatAD second_t_max = gather<FloatAD>(recursive_states.edge_t_max, second_idx, first_active);
        const FloatAD second_t = second_t_min + uniform01(lane, 2u) * (second_t_max - second_t_min);
        const Vector3fAD second_point = second_edge_pos + second_t * second_edge_dir;

        const Vector3fAD third_edge_pos = gather<Vector3fAD>(recursive_states.edge_pos, third_idx, first_active);
        const Vector3fAD third_edge_dir =
            normalize(gather<Vector3fAD>(recursive_states.edge_dir, third_idx, first_active));
        const FloatAD third_t_min = gather<FloatAD>(recursive_states.edge_t_min, third_idx, first_active);
        const FloatAD third_t_max = gather<FloatAD>(recursive_states.edge_t_max, third_idx, first_active);
        const FloatAD third_t = third_t_min + uniform01(lane, 4u) * (third_t_max - third_t_min);
        const Vector3fAD third_point = third_edge_pos + third_t * third_edge_dir;

        const Vector3fAD source = gather<Vector3fAD>(initial_states.src, first_idx, first_active);
        const FloatAD src_power = gather<FloatAD>(initial_states.src_power, first_idx, first_active);
        const Vector3fAD target = grid_cell_center(grid, cell);
        const Vector3fAD terminal_point = select(IntAD(options.max_order) == IntAD(3), third_point, second_point);

        const MaskAD finite_active =
            first_active && distinct_edges && drjit::isfinite(source.x()) && drjit::isfinite(source.y()) &&
            drjit::isfinite(source.z()) && drjit::isfinite(first_point.x()) && drjit::isfinite(first_point.y()) &&
            drjit::isfinite(first_point.z()) && drjit::isfinite(second_point.x()) &&
            drjit::isfinite(second_point.y()) && drjit::isfinite(second_point.z()) && drjit::isfinite(src_power);

        const SegmentPairVisibilityAD first_visibility =
            this->template visible_pair<false>(first_point, source, second_point, Int(), finite_active);
        const SegmentPairVisibilityAD terminal_visibility =
            this->template visible_pair<false>(terminal_point,
                                               select(IntAD(options.max_order) == IntAD(3), second_point, target),
                                               target, Int(), finite_active);
        const MaskAD source_visible = first_visibility.visible_a;
        const MaskAD first_edge_visible = first_visibility.visible_b;
        const MaskAD second_edge_visible = select(IntAD(options.max_order) == IntAD(3), terminal_visibility.visible_a,
                                                  full<MaskAD>(true, launch_count));
        const MaskAD target_visible =
            select(IntAD(options.max_order) == IntAD(3), terminal_visibility.visible_b, terminal_visibility.visible_a);
        const MaskAD visible = source_visible && first_edge_visible && second_edge_visible && target_visible;

        const IntAD first_prim0 = gather<IntAD>(initial_states.prim0, first_idx, finite_active);
        const IntAD first_prim1 = gather<IntAD>(initial_states.prim1, first_idx, finite_active);
        const FloatAD first_exterior = gather<FloatAD>(initial_states.exterior_angle, first_idx, finite_active);
        const FloatAD first_weight =
            chain_event_weight(src_power, first_prim0, first_prim1, first_t_min, first_t_max, first_exterior, source,
                               first_point, second_point, finite_active);

        const IntAD second_prim0 = gather<IntAD>(recursive_states.prim0, second_idx, finite_active);
        const IntAD second_prim1 = gather<IntAD>(recursive_states.prim1, second_idx, finite_active);
        const FloatAD second_exterior = gather<FloatAD>(recursive_states.exterior_angle, second_idx, finite_active);
        const Vector3fAD second_target = select(IntAD(options.max_order) == IntAD(3), third_point, target);
        const FloatAD second_weight =
            chain_event_weight(FloatAD(1.f), second_prim0, second_prim1, second_t_min, second_t_max, second_exterior,
                               first_point, second_point, second_target, finite_active);

        const IntAD third_prim0 = gather<IntAD>(recursive_states.prim0, third_idx, finite_active);
        const IntAD third_prim1 = gather<IntAD>(recursive_states.prim1, third_idx, finite_active);
        const FloatAD third_exterior = gather<FloatAD>(recursive_states.exterior_angle, third_idx, finite_active);
        const FloatAD third_weight =
            chain_event_weight(FloatAD(1.f), third_prim0, third_prim1, third_t_min, third_t_max, third_exterior,
                               second_point, third_point, target, finite_active);
        FloatAD chain_weight = first_weight * second_weight;
        chain_weight = select(IntAD(options.max_order) == IntAD(3), chain_weight * third_weight, chain_weight);

        const FloatAD wave_gain_per_event =
            (FloatAD(options.wavelength) / FloatAD(4.f * Pi)) * (FloatAD(options.wavelength) / FloatAD(4.f * Pi));
        const FloatAD wave_gain = select(IntAD(options.max_order) == IntAD(3),
                                         wave_gain_per_event * wave_gain_per_event, wave_gain_per_event);
        const IntAD strategy_samples =
            select(is_direct, IntAD(std::max(direct_samples, 1)),
                   select(is_keller, IntAD(std::max(keller_samples, 1)), IntAD(std::max(suffix_samples, 1))));
        const FloatAD contribution = chain_weight * wave_gain * FloatAD(grid.cell_area) / FloatAD(strategy_samples);
        const MaskAD contribution_active = visible && (contribution > FloatAD(0.f)) && drjit::isfinite(contribution);
        scatter_reduce(ReduceOp::Add, result.power, contribution, cell, contribution_active);
        const FloatAD amplitude = sqrt(maximum(contribution, FloatAD(0.f)));
        scatter_reduce(ReduceOp::Add, result.field_x.x(), amplitude, cell, contribution_active);
        ...
        scatter_reduce(ReduceOp::Add, result.edge_vis_rejects, IntAD(1), zeros<IntAD>(launch_count),
                       finite_active && target_visible &&
                           (!source_visible || !first_edge_visible || !second_edge_visible) &&
                           options.collect_debug_counts);
        scatter_reduce(ReduceOp::Add, result.utd_rejects, IntAD(1), zeros<IntAD>(launch_count),
        ...
        active_detached &= drjit::isfinite(initial_states.src.x()) && drjit::isfinite(initial_states.src.y()) &&
                           drjit::isfinite(initial_states.src.z()) && drjit::isfinite(initial_states.edge_pos.x()) &&
                           drjit::isfinite(initial_states.edge_pos.y()) &&
                           drjit::isfinite(initial_states.edge_pos.z()) && drjit::isfinite(initial_states.src_power);
        Mask recursive_active =
            drjit::isfinite(recursive_states.edge_pos.x()) && drjit::isfinite(recursive_states.edge_pos.y()) &&
            drjit::isfinite(recursive_states.edge_pos.z()) && drjit::isfinite(recursive_states.edge_dir.x()) &&
            drjit::isfinite(recursive_states.edge_dir.y()) && drjit::isfinite(recursive_states.edge_dir.z());
        ...
        if (!cuda_trace) {
            ...
            require(primary_scene != nullptr && primary_scene->is_ready(),
                    "Scene::accum_dfr(): OptiX scene is not ready.");
            require(hitgroup_record_count > 0, "Scene::accum_dfr(): invalid hitgroup record count.");
        }
        if (suffix_samples > 0) {
            require(triangle_count > 0, "Scene::accum_dfr(): suffix reflection requires scene triangles.");
            require(material_count >= triangle_count,
                    "Scene::accum_dfr(): suffix reflection requires per-triangle materials.");
        }

        if (!cuda_trace) {
            auto& dfr_pipeline = split_mode == 0 ? diffraction_chain_accumulation_primary_pipeline_
                                                 : diffraction_chain_accumulation_pipeline_;
            const OptixPipelineConfig dfr_pipeline_config =
                split_mode == 0 ? diffraction_chain_accumulation_primary_pipeline_config()
                                : diffraction_chain_accumulation_pipeline_config();

            ensure_pipeline(dfr_pipeline, primary_scene->context(), hitgroup_record_count, dfr_pipeline_config);
        }

        drjit::eval(initial_states.edge_index, initial_states.edge_pos, initial_states.edge_dir,
                    initial_states.edge_t_min, initial_states.edge_t_max, initial_states.prim0, initial_states.prim1,
                    initial_states.exterior_angle, initial_states.src, initial_states.src_power,
                    recursive_states.edge_index, recursive_states.edge_pos, recursive_states.edge_dir,
                    recursive_states.edge_t_min, recursive_states.edge_t_max, recursive_states.prim0,
                    recursive_states.prim1, recursive_states.exterior_angle, active_detached, recursive_active,
                    material.eta_r, material.sigma, material.mu_r, material.gain, material.valid);
        if (suffix_samples > 0) {
            drjit::eval(triangle_info_detached_.p0, triangle_info_detached_.e1, triangle_info_detached_.e2,
                        triangle_info_detached_.face_normal, face_offsets_);
        }
        ...
        params.recursive_active_mask = reinterpret_cast<const uint8_t*>(recursive_active.data());
...
template DfrAccum Scene::accum_dfr_direct<true>(const DfrStates& states, const DfrGrid& grid,
                                                const DfrMaterial& material, const DfrOptions& options,
                                                Mask active) const;
template DfrAccumAD Scene::accum_dfr_direct<false>(const DfrStatesAD& states, const DfrGrid& grid,
                                                   const DfrMaterialAD& material, const DfrOptions& options,
                                                   MaskAD active) const;

template DfrAccum Scene::accum_dfr<true>(const DfrStates& initial_states, const DfrStates& recursive_states,
                                         const DfrGrid& grid, const DfrMaterial& material, const DfrOptions& options,
                                         Mask active) const;
template DfrAccumAD Scene::accum_dfr<false>(const DfrStatesAD& initial_states, const DfrStatesAD& recursive_states,
                                            const DfrGrid& grid, const DfrMaterialAD& material,
                                            const DfrOptions& options, MaskAD active) const;
...
void init_dfr_coherent_accum_raw(DfrCoherentAccumRaw& raw) {
    const int zero_i = 0;
    const float zero_f = 0.f;
    jit_memset_async(JitBackend::CUDA, raw.direct_field_x_re.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.direct_field_x_im.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.direct_field_y_re.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.direct_field_y_im.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.direct_field_z_re.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.direct_field_z_im.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.multi_field_x_re.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.multi_field_x_im.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.multi_field_y_re.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.multi_field_y_im.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.multi_field_z_re.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.multi_field_z_im.data(), raw.grid_cell_count, sizeof(float), &zero_f);
    jit_memset_async(JitBackend::CUDA, raw.direct_count.data(), raw.grid_cell_count, sizeof(int), &zero_i);
    jit_memset_async(JitBackend::CUDA, raw.multi_count.data(), raw.grid_cell_count, sizeof(int), &zero_i);
    jit_memset_async(JitBackend::CUDA, raw.visibility_reject_count.data(), raw.grid_cell_count, sizeof(int), &zero_i);
    jit_memset_async(JitBackend::CUDA, raw.utd_reject_count.data(), raw.grid_cell_count, sizeof(int), &zero_i);
}
...
Vector3f normalize_with_fallback(const Vector3f& value, const Vector3f& fallback) {
    ...
    return select(value_norm > Float(1.0e-8f), value / (value_norm + Float(1.0e-12f)),
...
Vector3f stable_perpendicular_basis_jit(const Vector3f& ray_dir, const Vector3f& preferred) {
    ...
    const Vector3f alt_axis =
        select(use_z, Vector3f(Float(0.f), Float(0.f), Float(1.f)), Vector3f(Float(0.f), Float(1.f), Float(0.f)));
    const Vector3f alt_proj = alt_axis - dot(alt_axis, ray_dir) * ray_dir;
    return normalize_with_fallback(proj, alt_proj);
}

Mask wedge_exterior_mask_jit(const Vector3f& direction_from_edge, const Vector3f& edge_dir, const Vector3f& n0,
                             const Vector3f& nn) {
    const Vector3f direction_proj = direction_from_edge - dot(direction_from_edge, edge_dir) * edge_dir;
    return (norm(direction_proj) > Float(1.0e-8f)) &&
           ((dot(direction_proj, n0) >= Float(-1.0e-8f)) || (dot(direction_proj, nn) >= Float(-1.0e-8f)));
}
...
Int interleave_four_ignore_slots(const Int& slot0, const Int& slot1, const Int& slot2, const Int& slot3, int width) {
    if (width <= 0) {
        return zeros<Int>(0);
    }
...
Float gather_material_float(const Float& values, const Int& face, const Mask& valid, float fallback) {
...
template <bool Detached>
DfrCoherentUtdStatesT<Detached> Scene::build_dfr_coherent_tx_states(const DfrCoherentEdgeT<Detached>& edges,
    ...
    require(options.wavelength > 0.f, "Scene::build_dfr_coherent_tx_states(): wavelength must be positive.");
    require(options.k > 0.f, "Scene::build_dfr_coherent_tx_states(): k must be positive.");
    if constexpr (!Detached) {
        (void)edges;
        (void)tx_position;
        (void)material;
        (void)active;
        throw std::runtime_error("Scene::build_dfr_coherent_tx_states(): AD inputs are not supported yet.");
    } else {
        const int edge_count = edges.count;
        require(edge_count >= 0, "Scene::build_dfr_coherent_tx_states(): invalid edge count.");
        ...
        const int material_count = static_cast<int>(slices(material.eta_r));
        require(material_count > 0, "Scene::build_dfr_coherent_tx_states(): material payload must not be empty.");
        require(static_cast<int>(slices(material.sigma)) == material_count &&
                    static_cast<int>(slices(material.mu_r)) == material_count &&
                    static_cast<int>(slices(material.gain)) == material_count &&
                    static_cast<int>(slices(material.valid)) == material_count,
        ...
        const Vector3f edge_dir = normalize_with_fallback(edges.edge_dir, Vector3f(Float(0.f), Float(0.f), Float(1.f)));
        ...
        const SegmentVisibility visibility_result =
            trace_segment_visibility_native<true>(optix_scene(), *segment_visibility_pipeline_, face_offsets_,
                                                  mesh_count_, source_pos, edges.edge_pos, edges.ignore_prim_ids,
        ...
        const Mask finite_line = (edges.edge_line_max - edges.edge_line_min) > Float(1.0e-8f);
        ...
        result.incident_normal_derivative = drjit::Complex<Float>(zeros<Float>(state_count), zeros<Float>(state_count));
        ...
        result.incident_basis_v =
            normalize_with_fallback(cross(ray_dir, result.incident_basis_u),
                                    stable_perpendicular_basis_jit(ray_dir,
        ...
        const Mask valid0 = result.adjacent_face0 >= Int(0) && result.adjacent_face0 < Int(material_count);
        const Mask valid1 = result.adjacent_face1 >= Int(0) && result.adjacent_face1 < Int(material_count);
...
DfrCoherentCandidatePairsT<Detached> Scene::build_dfr_coherent_higher_candidates(
    ...
    const IntT<Detached>& global_to_local_edge_index, const DfrCoherentOptions& options, MaskT<Detached> active) const {
    ...
    if constexpr (!Detached) {
        ...
        (void)active;
        throw std::runtime_error("Scene::build_dfr_coherent_higher_candidates(): AD inputs are not supported yet.");
    } else {
        ...
        const Float probe_radius = minimum(maximum(unclamped_radius, Float(options.higher_probe_radius_min)),
                                           Float(options.higher_probe_radius_max));
        const Vector3f ray_origin = edge_pos + basis_u * (probe_radius * probe_u) + basis_v * (probe_radius * probe_v);
        const Vector3f ray_dir = basis_k * probe_sign;

        const NearestRayEdge nearest = this->template nearest_edge<true>(Ray(ray_origin, ray_dir), probe_active);
        Mask valid = nearest.global_edge_id >= Int(0);
        const Int safe_global_edge = select(valid, nearest.global_edge_id, Int(0));
        valid &= safe_global_edge < Int(static_cast<int>(slices(global_to_local_edge_index)));
        const Int local_edge_idx = gather<Int>(global_to_local_edge_index, UInt(safe_global_edge), valid);
        ...
        if (options.higher_filter_visibility) {
            require(static_cast<int>(slices(edges.edge_pos)) >= edge_count &&
                    ...
                    "Scene::build_dfr_coherent_higher_candidates(): visibility filtering requires edge positions and "
                    "adjacent faces.");
            require(trace_backend_ != nullptr && optix_scene().is_ready(),
            ...
            const Int ignore_prim_ids =
                interleave_four_ignore_slots(prev_adjacent_face0, prev_adjacent_face1, next_adjacent_face0,
                                             next_adjacent_face1, probe_lane_count);
            ensure_pipeline(segment_visibility_pipeline_, optix_scene().context(), mesh_count_,
                            segment_visibility_pipeline_config());
            const SegmentVisibility visibility_result =
                trace_segment_visibility_native<true>(optix_scene(), *segment_visibility_pipeline_, face_offsets_,
                                                      mesh_count_, edge_pos, next_edge_pos, ignore_prim_ids, 4, valid);
...
template <bool Detached>
DfrCoherentAccumT<Detached> Scene::accum_dfr_coherent_direct(const DfrCoherentUtdStatesT<Detached>& states,
    ...
    require(grid.axis >= 0 && grid.axis <= 2, "Scene::accum_dfr_coherent_direct(): grid.axis must be 0, 1, or 2.");
    require(grid.resolution0 > 0 && grid.resolution1 > 0,
    ...
    require(options.wavelength > 0.f, "Scene::accum_dfr_coherent_direct(): wavelength must be positive.");
    require(options.max_order == 1, "Scene::accum_dfr_coherent_direct(): only max_order == 1 is supported.");
    ...
    if constexpr (!Detached) {
        (void)states;
        (void)active;
        throw std::runtime_error("Scene::accum_dfr_coherent_direct(): AD inputs are not supported yet.");
    } else {
        const int state_count = states.count;
        require(state_count > 0, "Scene::accum_dfr_coherent_direct(): invalid state count.");
        require(static_cast<int>(slices(states.edge_pos)) >= state_count &&
                    static_cast<int>(slices(states.edge_dir)) >= state_count &&
                    static_cast<int>(slices(states.n0)) >= state_count &&
        ...
        require(launch_count > 0, "Scene::accum_dfr_coherent_direct(): invalid launch count.");
        ...
        active_detached &= drjit::isfinite(states.source_pos.x()) && drjit::isfinite(states.source_pos.y()) &&
                           drjit::isfinite(states.source_pos.z()) && drjit::isfinite(states.edge_pos.x()) &&
                           drjit::isfinite(states.edge_pos.y()) && drjit::isfinite(states.edge_pos.z());
        ...
        if (!cuda_trace) {
            ...
            auto& dfr_pipeline = split_mode == 0 ? diffraction_coherent_accumulation_primary_pipeline_
                                                 : diffraction_coherent_accumulation_pipeline_;
            const OptixPipelineConfig dfr_pipeline_config =
                split_mode == 0 ? diffraction_coherent_accumulation_primary_pipeline_config()
                                : diffraction_coherent_accumulation_pipeline_config();

            ensure_pipeline(dfr_pipeline, primary_scene->context(), scenes.hitgroup_record_count, dfr_pipeline_config);
        }

        drjit::eval(states.edge_pos, states.edge_dir, states.n0, states.n_face_n, states.source_pos,
                    states.incident_basis_u, states.incident_basis_v, states.incident_basis_k, states.wedge_n,
                    states.edge_line_min, states.edge_line_max, states.face0_eta_r, states.face0_mu_r,
                    states.face0_sigma, states.face0_gain, states.face0_use_fresnel, states.face1_eta_r,
                    states.face1_mu_r, states.face1_sigma, states.face1_gain, states.face1_use_fresnel,
                    states.select_stationary_point, states.incident_field, states.incident_normal_derivative,
                    states.r_face0, states.r_face_n, states.incident_vector_x, states.incident_vector_y,
                    states.incident_vector_z, states.incident_normal_derivative_vector_x,
                    states.incident_normal_derivative_vector_y, states.incident_normal_derivative_vector_z,
                    states.incident_jones_u, states.incident_jones_v, states.incident_derivative_jones_u,
                    states.incident_derivative_jones_v, states.face0_operator_m00, states.face0_operator_m01,
                    states.face0_operator_m10, states.face0_operator_m11, states.face1_operator_m00,
                    states.face1_operator_m01, states.face1_operator_m10, states.face1_operator_m11, states.owner_code,
                    states.adjacent_face0, states.adjacent_face1, active_detached, triangle_info_detached_.face_normal,
                    face_offsets_);
...
template <bool Detached>
DfrCoherentAccumT<Detached> Scene::accum_dfr_coherent_direct(const DfrStatesT<Detached>& states, const DfrGrid& grid,
                                                             const DfrMaterialT<Detached>& material,
                                                             const DfrCoherentOptions& options,
                                                             MaskT<Detached> active) const {
    ScopedNativeLaunchStage native_launch_stage(NativeLaunchStage::AccumDfr);
    ...
    require(grid.coord0_min < grid.coord0_max && grid.coord1_min < grid.coord1_max,
            "Scene::accum_dfr_coherent_direct(): grid bounds must be ordered.");
    require(grid.cell_area > 0.f, "Scene::accum_dfr_coherent_direct(): grid.cell_area must be positive.");
    require(options.wavelength > 0.f, "Scene::accum_dfr_coherent_direct(): wavelength must be positive.");
    require(options.max_order == 1, "Scene::accum_dfr_coherent_direct(): only max_order == 1 is supported.");
    require(options.receiver_model == RAYD_DFR_MATCHED_ISO,
            "Scene::accum_dfr_coherent_direct(): only matched isotropic receivers are supported.");
    if constexpr (!Detached) {
        ...
        const int material_count = static_cast<int>(slices(material.eta_r));
        require(material_count > 0, "Scene::accum_dfr_coherent_direct(): material payload must not be empty.");
        require(static_cast<int>(slices(material.sigma)) == material_count &&
                    static_cast<int>(slices(material.mu_r)) == material_count &&
                    static_cast<int>(slices(material.gain)) == material_count &&
                    static_cast<int>(slices(material.valid)) == material_count,
...
template DfrCoherentAccum Scene::accum_dfr_coherent_direct<true>(const DfrCoherentUtdStates& states,
                                                                 const DfrGrid& grid, const DfrCoherentOptions& options,
                                                                 Mask active) const;
template DfrCoherentAccumAD Scene::accum_dfr_coherent_direct<false>(const DfrCoherentUtdStatesAD& states,
                                                                    const DfrGrid& grid,
                                                                    const DfrCoherentOptions& options,
                                                                    MaskAD active) const;
template DfrCoherentUtdStates Scene::build_dfr_coherent_tx_states<true>(const DfrCoherentEdge& edges,
                                                                        const Vector3f& tx_position,
                                                                        const DfrMaterial& material,
                                                                        const DfrCoherentOptions& options,
                                                                        Mask active) const;
template DfrCoherentUtdStatesAD Scene::build_dfr_coherent_tx_states<false>(const DfrCoherentEdgeAD& edges,
...
template DfrCoherentCandidatePairs Scene::build_dfr_coherent_higher_candidates<true>(
    const DfrCoherentUtdStates& prev_states, const DfrCoherentEdge& edges, const Int& global_to_local_edge_index,
    const DfrCoherentOptions& options, Mask active) const;
template DfrCoherentCandidatePairsAD Scene::build_dfr_coherent_higher_candidates<false>(
    const DfrCoherentUtdStatesAD& prev_states, const DfrCoherentEdgeAD& edges, const IntAD& global_to_local_edge_index,
    const DfrCoherentOptions& options, MaskAD active) const;
template DfrCoherentAccum Scene::accum_dfr_coherent_direct<true>(const DfrStates& states, const DfrGrid& grid,
                                                                 const DfrMaterial& material,
                                                                 const DfrCoherentOptions& options, Mask active) const;
template DfrCoherentAccumAD Scene::accum_dfr_coherent_direct<false>(const DfrStatesAD& states, const DfrGrid& grid,
                                                                    const DfrMaterialAD& material,
                                                                    const DfrCoherentOptions& options,
                                                                    MaskAD active) const;
