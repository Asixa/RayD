...
// Implements diffraction support for diffraction kernels.
...
__global__ void init_dfr_path_outputs_kernel(
    int64_t capacity, int* __restrict__ out_count, uint8_t* __restrict__ out_valid, int* __restrict__ out_tx_id,
    int* __restrict__ out_rx_id, int* __restrict__ out_order, int* __restrict__ out_edge0, int* __restrict__ out_edge1,
    int* __restrict__ out_edge2, float* __restrict__ out_delay, float* __restrict__ out_field_x_re,
    float* __restrict__ out_field_x_im, float* __restrict__ out_field_y_re, float* __restrict__ out_field_y_im,
    float* __restrict__ out_field_z_re, float* __restrict__ out_field_z_im, float* __restrict__ out_p0,
...
void init_dfr_path_outputs_cuda(int64_t capacity, at::Tensor& out_count, at::Tensor& out_valid, at::Tensor& out_tx_id,
                                at::Tensor& out_rx_id, at::Tensor& out_order, at::Tensor& out_edge0,
                                at::Tensor& out_edge1, at::Tensor& out_edge2, at::Tensor& out_delay,
                                at::Tensor& out_field_x_re, at::Tensor& out_field_x_im, at::Tensor& out_field_y_re,
                                at::Tensor& out_field_y_im, at::Tensor& out_field_z_re, at::Tensor& out_field_z_im,
                                at::Tensor& out_p0, at::Tensor& out_p1, at::Tensor& out_p2) {
    ...
    init_dfr_path_outputs_kernel<<<blocks, threads, 0, stream>>>(
        capacity, out_count.data_ptr<int>(), reinterpret_cast<uint8_t*>(out_valid.data_ptr<bool>()),
        out_tx_id.data_ptr<int>(), out_rx_id.data_ptr<int>(), out_order.data_ptr<int>(), out_edge0.data_ptr<int>(),
        out_edge1.data_ptr<int>(), out_edge2.data_ptr<int>(), out_delay.data_ptr<float>(),
        ...
        out_p0.data_ptr<float>(), out_p1.data_ptr<float>(), out_p2.data_ptr<float>());
...
struct AddCoherentValue {
    __host__ __device__ DfrCoherentStagedValue operator()(DfrCoherentStagedValue x, DfrCoherentStagedValue y) const {
...
__global__ void scatter_dfr_accum_reduced_kernel(const int* __restrict__ num_runs, const int* __restrict__ unique_cells,
                                                 const float4* __restrict__ reduced_values,
                                                 float* __restrict__ out_power, float* __restrict__ out_field_x_re,
                                                 int* __restrict__ out_direct_count, int* __restrict__ out_keller_count,
...
__global__ void scatter_dfr_coherent_accum_reduced_kernel(
    int cell_count, const int* __restrict__ num_runs, const int* __restrict__ unique_keys,
    const DfrCoherentStagedValue* __restrict__ reduced_values, float* __restrict__ out_direct_field_x_re,
    float* __restrict__ out_direct_field_x_im, float* __restrict__ out_direct_field_y_re,
    float* __restrict__ out_direct_field_y_im, float* __restrict__ out_direct_field_z_re,
    float* __restrict__ out_direct_field_z_im, float* __restrict__ out_multi_field_x_re,
    float* __restrict__ out_multi_field_x_im, float* __restrict__ out_multi_field_y_re,
    float* __restrict__ out_multi_field_y_im, float* __restrict__ out_multi_field_z_re,
    float* __restrict__ out_multi_field_z_im, int* __restrict__ out_direct_count, int* __restrict__ out_multi_count) {
    const int n = *num_runs;
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < n; idx += blockDim.x * gridDim.x) {
        ...
        if (x_re != nullptr)
            atomicAdd(x_re + cell, value.a.x);
        if (x_im != nullptr)
            atomicAdd(x_im + cell, value.a.y);
        if (y_re != nullptr)
            atomicAdd(y_re + cell, value.a.z);
        if (y_im != nullptr)
            atomicAdd(y_im + cell, value.a.w);
        if (z_re != nullptr)
            atomicAdd(z_re + cell, value.b.x);
        if (z_im != nullptr)
            atomicAdd(z_im + cell, value.b.y);
...
void reduce_dfr_accum_staged_cuda(int64_t sample_count, const at::Tensor& stage_cell, const at::Tensor& stage_value,
    ...
    cuda_check(cub::DeviceReduce::ReduceByKey(nullptr, reduce_temp_bytes, sorted_cells.data_ptr<int>(),
                                              ...
                                              num_runs.data_ptr<int>(), AddFloat4{}, sample_count_i, stream),
    ...
    cuda_check(cub::DeviceReduce::ReduceByKey(reduce_temp.data_ptr<uint8_t>(), reduce_temp_bytes,
                                              ...
                                              reduced_values_ptr, num_runs.data_ptr<int>(), AddFloat4{}, sample_count_i,
                                              stream),
               "cub::DeviceReduce::ReduceByKey(dfr accum)");

    constexpr int block_size = 256;
    const int block_count = static_cast<int>((sample_count + block_size - 1) / block_size);
    scatter_dfr_accum_reduced_kernel<<<block_count, block_size, 0, stream>>>(num_runs.data_ptr<int>(),
...
void reduce_dfr_coherent_accum_staged_cuda(int64_t sample_count, int64_t cell_count, const at::Tensor& stage_key,
    ...
    auto* values_in = reinterpret_cast<DfrCoherentStagedValue*>(stage_value.data_ptr<float>());
    auto* values_sorted = reinterpret_cast<DfrCoherentStagedValue*>(sorted_values.data_ptr<float>());

    size_t sort_temp_bytes = 0;
    cuda_check(cub::DeviceRadixSort::SortPairs(nullptr, sort_temp_bytes, stage_key.data_ptr<int>(),
                                               sorted_keys.data_ptr<int>(), values_in, values_sorted, sample_count_i, 0,
                                               sizeof(int) * 8, stream),
               "cub::DeviceRadixSort::SortPairs(dfr coherent accum size)");
    at::Tensor sort_temp = at::empty({std::max<int64_t>(1, static_cast<int64_t>(sort_temp_bytes))}, byte_options);
    cuda_check(cub::DeviceRadixSort::SortPairs(sort_temp.data_ptr<uint8_t>(), sort_temp_bytes,
                                               stage_key.data_ptr<int>(), sorted_keys.data_ptr<int>(), values_in,
                                               values_sorted, sample_count_i, 0, sizeof(int) * 8, stream),
    ...
    at::Tensor num_runs = at::empty({1}, key_options);
    auto* reduced_values_ptr = reinterpret_cast<DfrCoherentStagedValue*>(reduced_values.data_ptr<float>());

    size_t reduce_temp_bytes = 0;
    cuda_check(cub::DeviceReduce::ReduceByKey(nullptr, reduce_temp_bytes, sorted_keys.data_ptr<int>(),
                                              unique_keys.data_ptr<int>(), values_sorted, reduced_values_ptr,
                                              num_runs.data_ptr<int>(), AddCoherentValue{}, sample_count_i, stream),
               "cub::DeviceReduce::ReduceByKey(dfr coherent accum size)");
    at::Tensor reduce_temp = at::empty({std::max<int64_t>(1, static_cast<int64_t>(reduce_temp_bytes))}, byte_options);
    cuda_check(cub::DeviceReduce::ReduceByKey(reduce_temp.data_ptr<uint8_t>(), reduce_temp_bytes,
                                              sorted_keys.data_ptr<int>(), unique_keys.data_ptr<int>(), values_sorted,
                                              reduced_values_ptr, num_runs.data_ptr<int>(), AddCoherentValue{},
    ...
    scatter_dfr_coherent_accum_reduced_kernel<<<block_count, block_size, 0, stream>>>(
        cell_count_i, num_runs.data_ptr<int>(), unique_keys.data_ptr<int>(), reduced_values_ptr,
        out_direct_field_x_re.data_ptr<float>(), out_direct_field_x_im.data_ptr<float>(),
        out_direct_field_y_re.data_ptr<float>(), out_direct_field_y_im.data_ptr<float>(),
        out_direct_field_z_re.data_ptr<float>(), out_direct_field_z_im.data_ptr<float>(),
        out_multi_field_x_re.data_ptr<float>(), out_multi_field_x_im.data_ptr<float>(),
        out_multi_field_y_re.data_ptr<float>(), out_multi_field_y_im.data_ptr<float>(),
        out_multi_field_z_re.data_ptr<float>(), out_multi_field_z_im.data_ptr<float>(),
        out_direct_count.data_ptr<int>(), out_multi_count.data_ptr<int>());
...
static __forceinline__ __device__ float read_f32_strided_or_zero(const float* ptr, int stride, int index) {
    return ptr != nullptr ? ptr[index * stride] : 0.f;
}

static __forceinline__ __device__ int read_i32_strided_or_default(const int* ptr, int stride, int index,
                                                                  int default_value) {
    return ptr != nullptr ? ptr[index * stride] : default_value;
}

static __forceinline__ __device__ bool read_u8_strided_or_false(const uint8_t* ptr, int stride, int index) {
    return ptr != nullptr && ptr[index * stride] != 0u;
}

static __forceinline__ __device__ float3 read_vec_strided_or_zero(const float* x, const float* y, const float* z,
                                                                  int stride, int index) {
    return make_f3(read_f32_strided_or_zero(x, stride, index), read_f32_strided_or_zero(y, stride, index),
                   read_f32_strided_or_zero(z, stride, index));
}

static __forceinline__ __device__ float read_grid_or_zero(const float* ptr, int rank, int stride0, int stride1,
                                                          int resolution0, int cell) {
    if (ptr == nullptr) {
        return 0.f;
    }
    if (rank == 2) {
...
static __forceinline__ __device__ void atomic_add_strided(float* ptr, int stride, int index, float value) {
    if (ptr != nullptr) {
        atomicAdd(ptr + index * stride, value);
    }
}

static __forceinline__ __device__ void atomic_add_vec_strided(float* x, float* y, float* z, int stride, int index,
                                                              float3 value) {
...
static __forceinline__ __device__ float material_gain_for_prim(const DfrDirectAccumADParams& params, int prim) {
    return params.material_gain != nullptr
               ? read_f32_strided_or_zero(params.material_gain, params.material_gain_stride, prim)
               : 1.f;
}

static __forceinline__ __device__ float material_gain_for_prim(const DfrChainAccumADParams& params, int prim) {
    return params.material_gain != nullptr
               ? read_f32_strided_or_zero(params.material_gain, params.material_gain_stride, prim)
               : 1.f;
}
...
#define RAYD_DFR_AD_READ_F32(P, F, S, I) read_f32_strided_or_zero((P).F, (P).S, (I))
#define RAYD_DFR_AD_READ_I32(P, F, S, I) read_i32_strided_or_default((P).F, (P).S, (I), -1)
#define RAYD_DFR_AD_READ_I32_OR(P, F, S, I, D) read_i32_strided_or_default((P).F, (P).S, (I), (D))
#define RAYD_DFR_AD_READ_VEC(P, X, Y, Z, S, I) read_vec_strided_or_zero((P).X, (P).Y, (P).Z, (P).S, (I))
#define RAYD_DFR_AD_READ_VEC_GUARDED(P, X, Y, Z, S, I) read_vec_strided_or_zero((P).X, (P).Y, (P).Z, (P).S, (I))
#define RAYD_DFR_AD_READ_DOT_F32(P, F, S, I) read_f32_strided_or_zero((P).F, (P).S, (I))
#define RAYD_DFR_AD_READ_DOT_VEC(P, X, Y, Z, S, I) read_vec_strided_or_zero((P).X, (P).Y, (P).Z, (P).S, (I))
#define RAYD_DFR_AD_MATERIAL_VALID_ENTRY(P, I)                                                                         \
    read_u8_strided_or_false((P).material_valid, (P).material_valid_stride, (I))
#define RAYD_DFR_AD_MATERIAL_GAIN_TAPE(P, I) read_f32_strided_or_zero((P).material_gain, (P).material_gain_stride, (I))
...
static __forceinline__ __device__ void add_unit_vjp(const DfrDirectAccumADParams& params, const DirectPrimal& p,
...
static __forceinline__ __device__ void add_unit_vjp_strided(const DfrDirectAccumADParams& params, const DirectPrimal& p,
                                                            float grad_contribution, float* ptr, int stride, int index,
                                                            const DfrTangent& tangent) {
    if (ptr != nullptr) {
        const float partial = contribution_jvp(params, p, tangent);
...
#define RAYD_DFR_AD_ADD_UNIT_VJP(P, PR, G, F, S, I, T) add_unit_vjp_strided((P), (PR), (G), (P).F, (P).S, (I), (T))
#define RAYD_DFR_AD_ADD_UNIT_VJP_DENSE(P, PR, G, F, I, T) add_unit_vjp((P), (PR), (G), (P).F, (I), (T))
...
__global__ void dfr_direct_accum_jvp_kernel(DfrDirectAccumADParams params) {
    ...
    if (params.dot_out_field_x_re != nullptr) {
        ...
        if (amp > kDfrEps) {
            atomicAdd(params.dot_out_field_x_re + p.cell, 0.5f * dot_contribution / amp);
        }
    }
}
...
__global__ void dfr_direct_accum_vjp_kernel(DfrDirectAccumADParams params) {
    ...
    float grad_contribution =
        read_grid_or_zero(params.grad_out_power, params.grad_out_power_rank, params.grad_out_power_stride0,
                          params.grad_out_power_stride1, params.grid_resolution0, p.cell);
    const float amp = sqrtf(fmaxf(p.contribution, 0.f));
    if (amp > kDfrEps) {
        grad_contribution += read_grid_or_zero(params.grad_out_field_x_re, params.grad_out_field_x_re_rank,
                                               params.grad_out_field_x_re_stride0, params.grad_out_field_x_re_stride1,
                                               params.grid_resolution0, p.cell) *
                             0.5f / amp;
    }
    ...
    atomic_add_strided(params.grad_state_src_power, params.grad_state_src_power_stride, p.state_idx, grad_src_power);

    if (p.material_active && p.material_idx >= 0) {
        const float grad_gain = grad_contribution * p.contribution / fmaxf(p.material_gain, kDfrEps);
        atomic_add_strided(params.grad_material_gain, params.grad_material_gain_stride, p.material_idx, grad_gain);
    }
    ...
    if (p.wedge_active) {
        const float grad_wedge = grad_contribution * p.contribution / fmaxf(p.wedge_scale, kDfrEps);
        atomic_add_strided(params.grad_state_exterior_angle, params.grad_state_exterior_angle_stride, p.state_idx,
                           grad_wedge / (2.f * kPi));
    }
    ...
    const float3 d_contribution_d_edge =
        p.contribution * ((-2.f / p.source_dist2) * source_delta + (2.f / p.target_dist2) * target_delta);
    const float3 d_contribution_d_source = p.contribution * ((2.f / p.source_dist2) * source_delta);
    ...
    atomic_add_vec_strided(params.grad_state_src_x, params.grad_state_src_y, params.grad_state_src_z,
                           params.grad_state_src_stride, p.state_idx, grad_source);
    atomic_add_vec_strided(params.grad_state_edge_pos_x, params.grad_state_edge_pos_y, params.grad_state_edge_pos_z,
                           params.grad_state_edge_pos_stride, p.state_idx, grad_edge_point);

    const float grad_edge_t = dot3(grad_edge_point, p.edge_dir);
    atomic_add_strided(params.grad_state_edge_t_min, params.grad_state_edge_t_min_stride, p.state_idx,
                       (1.f - p.edge_u) * grad_edge_t - grad_edge_length);
    atomic_add_strided(params.grad_state_edge_t_max, params.grad_state_edge_t_max_stride, p.state_idx,
                       p.edge_u * grad_edge_t + grad_edge_length);

    const float3 grad_edge_dir = p.edge_t * grad_edge_point;
    const float3 grad_edge_dir_raw =
        (1.f / p.edge_dir_norm) * (grad_edge_dir - dot3(p.edge_dir, grad_edge_dir) * p.edge_dir);
    atomic_add_vec_strided(params.grad_state_edge_dir_x, params.grad_state_edge_dir_y, params.grad_state_edge_dir_z,
                           params.grad_state_edge_dir_stride, p.state_idx, grad_edge_dir_raw);
}
...
void launch_ad_kernel(const char* name, Kernel kernel, const Params& params) {
    ...
    audit_cuda_kernel_launch(name, static_cast<uint32_t>(block_count), 1, 1, static_cast<uint32_t>(block_size), 1, 1,
...
void dfr_direct_accum_jvp_gpu(const DfrDirectAccumADParams& params) {
    launch_ad_kernel("dfr_direct_accum_jvp_kernel", dfr_direct_accum_jvp_kernel, params);
}

void dfr_direct_accum_vjp_gpu(const DfrDirectAccumADParams& params) {
    launch_ad_kernel("dfr_direct_accum_vjp_kernel", dfr_direct_accum_vjp_kernel, params);
}

void dfr_chain_accum_jvp_gpu(const DfrChainAccumADParams& params) {
    launch_ad_kernel("dfr_chain_accum_jvp_kernel", dfr_chain_accum_jvp_kernel, params);
}

void dfr_chain_accum_vjp_gpu(const DfrChainAccumADParams& params) {
    launch_ad_kernel("dfr_chain_accum_vjp_kernel", dfr_chain_accum_vjp_kernel, params);
}
