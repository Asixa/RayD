...
// Implements the shared BVH build kernels.
...
__host__ __device__ inline BvhBounds3 merge_bounds(const BvhBounds3& a, const BvhBounds3& b) {
...
__host__ __device__ inline std::uint32_t morton_code_3d(const BvhFloat3& point, const BvhBounds3& scene_bounds) {
    BvhFloat3 normalized{0.5f, 0.5f, 0.5f};
    const BvhFloat3 extent{scene_bounds.max.x - scene_bounds.min.x, scene_bounds.max.y - scene_bounds.min.y,
...
__device__ inline bool node_in_treelet(int node_index, const int* treelet_nodes, int treelet_node_count) {
    for (int i = 0; i < treelet_node_count; ++i) {
        if (treelet_nodes[i] == node_index)
            return true;
    }
    return false;
}

__device__ inline void update_internal_node(int node_index, const int* left_child, const int* right_child,
                                            MutableAabbSoAView node_bounds, float* node_cost, float inflation) {
    const int left = left_child[node_index];
    const int right = right_child[node_index];
    const BvhBounds3 merged = merge_bounds(load_bounds(left, node_bounds), load_bounds(right, node_bounds));
    store_bounds(node_index, merged, node_bounds);
    node_cost[node_index] = bbox_cost_inflated(merged, inflation) + node_cost[left] + node_cost[right];
}
...
__device__ inline void treelet_optimize_node(int root_index, const int* is_leaf, int* left_child, int* right_child,
                                             int* parent, MutableAabbSoAView node_bounds, int* leaf_primitive,
                                             float* node_cost, float inflation) {
    ...
    for (int subset_size = 2; subset_size <= frontier_count; ++subset_size) {
        for (std::uint32_t subset = 1u; subset <= full_mask; ++subset) {
            ...
            for (std::uint32_t left_subset = (subset - 1u) & subset; left_subset > 0u;
            ...
            if (best_partition == 0u) {
                ...
                best_children_cost = optimal_cost[best_partition] + optimal_cost[subset ^ best_partition];
            }
    ...
    while (stack_size > 0) {
        ...
        if (__popc(static_cast<std::uint32_t>(partition)) == 1) {
            ...
            if (entry.child_slot == 0u)
                left_child[entry.parent_node] = child;
            else
                right_child[entry.parent_node] = child;
        ...
        if (entry.child_slot == 0u)
            left_child[entry.parent_node] = internal;
        else
            right_child[entry.parent_node] = internal;
...
__global__ void compute_morton_codes_kernel(MortonCodeParams params) {
    ...
    if (primitive >= static_cast<int>(params.primitive_bounds.count))
        return;
...
__device__ inline int longest_common_prefix(const std::uint32_t* morton_codes, const std::int32_t* sorted_primitives,
                                            int primitive_count, int first, int second) {
    ...
    if (first_code != second_code)
        return clz_u32(first_code ^ second_code);
...
__global__ void build_radix_tree_kernel(RadixTreeParams params) {
    ...
    if (node_index >= params.primitive_count - 1)
        return;
    const int delta_next = longest_common_prefix(params.morton_codes, params.sorted_primitives, params.primitive_count,
                                                 node_index, node_index + 1);
    const int delta_prev = longest_common_prefix(params.morton_codes, params.sorted_primitives, params.primitive_count,
                                                 node_index, node_index - 1);
    const int direction = (delta_next - delta_prev) >= 0 ? 1 : -1;
    const int delta_min = longest_common_prefix(params.morton_codes, params.sorted_primitives, params.primitive_count,
                                                node_index, node_index - direction);
    int max_length = 2;
    while (longest_common_prefix(params.morton_codes, params.sorted_primitives, params.primitive_count, node_index,
    ...
    for (int step = max_length / divider; step >= 1;) {
        if (longest_common_prefix(params.morton_codes, params.sorted_primitives, params.primitive_count, node_index,
    ...
    const int node_prefix =
        longest_common_prefix(params.morton_codes, params.sorted_primitives, params.primitive_count, node_index, other);
    int split = 0;
    divider = 2;
...
__global__ void finalize_leaves_and_bounds_kernel(LeafBoundsFinalizeParams params) {
    ...
    if (leaf_index >= params.primitive_count)
        return;
    ...
    while (current >= 0) {
        __threadfence();
        if (atomicAdd(params.merge_counters + current, 1) == 0)
            return;
        const int left = params.left_child[current];
        const int right = params.right_child[current];
        store_bounds(current,
                     merge_bounds(load_bounds(left, params.node_bounds), load_bounds(right, params.node_bounds)),
                     params.node_bounds);
...
__global__ void initialize_leaf_costs_kernel(LeafCostParams params) {
    ...
    if (leaf_index >= params.primitive_count)
        return;
    const int node = params.primitive_count - 1 + leaf_index;
    params.node_cost[node] = bbox_cost_inflated(load_bounds(node, params.node_bounds), params.inflation);
}
...
__global__ void initialize_internal_costs_kernel(InternalCostParams params) {
    ...
    while (current >= 0) {
        if (atomicAdd(params.arrival_counter + current, 1) == 0)
            return;
        params.node_cost[current] = bbox_cost_inflated(load_bounds(current, params.node_bounds), params.inflation) +
...
__global__ void optimize_selected_treelets_kernel(TreeletOptimizeParams params) {
    ...
    if (item >= params.selected_count)
        return;
    treelet_optimize_node(params.selected_nodes[item], params.is_leaf, params.left_child, params.right_child,
                          params.parent, params.node_bounds, params.leaf_primitive, params.node_cost, params.inflation);
}

__global__ void mark_dirty_ancestors_kernel(int leaf_count, const std::int32_t* leaf_nodes,
                                            const std::int32_t* node_parent, std::int32_t* dirty_marks) {
    const int leaf_index = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
...
__global__ void compact_dirty_level_kernel(int level_count, const std::int32_t* level_nodes,
                                           const std::int32_t* dirty_marks, std::int32_t* selected_nodes,
...
__global__ void refit_selected_internal_nodes_kernel(int max_selected_count, const std::int32_t* selected_count,
                                                     const std::int32_t* selected_nodes, const std::int32_t* left_child,
                                                     const std::int32_t* right_child, MutableAabbSoAView node_bounds) {
    const int item_index = static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x);
...
void launch_init_sequence_async(const SequenceInitParams& params) {
    if (params.count == 0)
        return;
    init_sequence_kernel<<<(params.count + kBlockSize - 1) / kBlockSize, kBlockSize, 0, params.stream>>>(params.count,
                                                                                                         params.values);
}
...
void launch_compute_morton_codes_async(const MortonCodeParams& params) {
    ...
    if (count == 0)
        return;
    compute_morton_codes_kernel<<<(count + kBlockSize - 1) / kBlockSize, kBlockSize, 0, params.stream>>>(params);
}
...
void launch_build_radix_tree_async(const RadixTreeParams& params) {
    ...
    if (count <= 0)
        return;
    build_radix_tree_kernel<<<(count + kBlockSize - 1) / kBlockSize, kBlockSize, 0, params.stream>>>(params);
}

void launch_finalize_leaves_and_bounds_async(const LeafBoundsFinalizeParams& params) {
    if (params.primitive_count == 0)
        return;
    finalize_leaves_and_bounds_kernel<<<(params.primitive_count + kBlockSize - 1) / kBlockSize, kBlockSize, 0,
...
void launch_initialize_leaf_costs_async(const LeafCostParams& params) {
    if (params.primitive_count == 0)
        return;
    initialize_leaf_costs_kernel<<<(params.primitive_count + kBlockSize - 1) / kBlockSize, kBlockSize, 0,
                                   params.stream>>>(params);
}
...
void launch_initialize_internal_costs_async(const InternalCostParams& params) {
    if (params.primitive_count == 0)
        return;
    initialize_internal_costs_kernel<<<(params.primitive_count + kBlockSize - 1) / kBlockSize, kBlockSize, 0,
                                       params.stream>>>(params);
}

void launch_optimize_selected_treelets_async(const TreeletOptimizeParams& params) {
    if (params.selected_count == 0)
        return;
    optimize_selected_treelets_kernel<<<(params.selected_count + kBlockSize - 1) / kBlockSize, kBlockSize, 0,
                                        params.stream>>>(params);
}
...
void launch_mark_dirty_ancestors_async(const DirtyAncestorMarkParams& params) {
    ...
    mark_dirty_ancestors_kernel<<<blocks, kBlockSize, 0, params.stream>>>(count, params.leaf_nodes, params.node_parent,
                                                                          params.dirty_marks);
}
...
void launch_compact_dirty_level_async(const DirtyLevelCompactParams& params) {
    ...
    const int blocks = (count + kBlockSize - 1) / kBlockSize;
    compact_dirty_level_kernel<<<blocks, kBlockSize, 0, params.stream>>>(count, params.level_nodes, params.dirty_marks,
                                                                         params.selected_nodes, params.selected_count);
}
...
void launch_refit_selected_internal_nodes_async(const InternalNodeRefitParams& params) {
    ...
    const int blocks = (count + kBlockSize - 1) / kBlockSize;
    refit_selected_internal_nodes_kernel<<<blocks, kBlockSize, 0, params.stream>>>(count, params.selected_count,
