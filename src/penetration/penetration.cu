...
const std::uint8_t *optional_byte_mask_ptr(const at::Tensor *tensor) {
    return tensor == nullptr
        ? nullptr
        : reinterpret_cast<const std::uint8_t *>(tensor->data_ptr<bool>());
}
...
__device__ float3 load3(const float *values, int64_t row, int64_t stride0, int64_t stride1) {
    return values == nullptr
        ...
        : make_float3(
              values[row * stride0 + 0 * stride1],
              values[row * stride0 + 1 * stride1],
              values[row * stride0 + 2 * stride1]);
}

__device__ float3 load3_3d(
    const float *values,
    int64_t row,
    int64_t slot,
    int64_t stride0,
    ...
    return values == nullptr
        ? make_float3(0.0f, 0.0f, 0.0f)
        : make_float3(
              values[row * stride0 + slot * stride1 + 0 * stride2],
...
__device__ float3 solve_columns(float3 c0, float3 c1, float3 c2, float3 rhs) {
    ...
    return make_float3(
        determinant(rhs, c1, c2) * inverse,
        determinant(c0, rhs, c2) * inverse,
...
__device__ float3 normal_jvp(float3 e1, float3 e2, float3 de1, float3 de2) {
    ...
    return mul3(
        1.0f / length,
        sub3(draw, mul3(dot3(normal, draw), normal)));
}
...
__device__ float3 normalized_vjp(float3 value, float floor, float3 gradient) {
    ...
    return mul3(
        1.0f / norm,
        sub3(gradient, mul3(dot3(normalized, gradient), normalized)));
}
...
__device__ float3 normalized_jvp(float3 value, float floor, float3 tangent) {
    ...
    return mul3(
        1.0f / norm,
        sub3(tangent, mul3(dot3(normalized, tangent), normalized)));
}
...
__global__ void initialize_kernel(
    ...
    float *position,
    float *normal,
    float *geometric_normal,
    int *global_primitive_id,
    int *tape_primitive_id,
    float *tape_barycentric,
    float *tape_restart_epsilon,
    std::uint8_t *tape_restart_branch,
    std::uint8_t *tape_restart_tie_mask,
    std::uint8_t *tape_direction_denominator_branch,
    const std::uint8_t *input_active,
    int *capacity_failure_state,
    int failure_bit,
...
__global__ void sanitize_kernel(
    ...
    std::uint8_t *tape_restart_tie_mask,
    std::uint8_t *tape_direction_denominator_branch,
    const int *capacity_failure_state,
    int64_t segment_count,
    int64_t hit_capacity) {
...
__global__ void backward_kernel(
    ...
    const float *geometric_normal,
    const int *primitive_id,
    const float *barycentric,
    const float *restart_epsilon,
    const std::uint8_t *restart_branch,
    const std::uint8_t *restart_tie_mask,
    const std::uint8_t *direction_denominator_branch,
    ...
    const float *grad_position,
    int64_t grad_position_stride0,
    int64_t grad_position_stride1,
    int64_t grad_position_stride2,
    const float *grad_normal,
    int64_t grad_normal_stride0,
    int64_t grad_normal_stride1,
    int64_t grad_normal_stride2,
    const float *grad_geometric_normal,
    int64_t grad_geometric_normal_stride0,
    int64_t grad_geometric_normal_stride1,
    int64_t grad_geometric_normal_stride2,
    int policy,
    int64_t segment_count,
    int64_t hit_capacity,
    float *grad_vertices,
    ...
    const float direction_floor =
        policy == SegmentPenetrationEnumeratedFullDistance ? 1.0e-9f : 1.0e-6f;
    const float denominator = direction_denominator_branch[segment] != 0u
        ? distance
        : direction_floor;
    const float3 direction = mul3(1.0f / denominator, delta);
    float3 direction_gradient = load3(
        grad_direction,
        segment,
    ...
    for (int64_t slot = hit_capacity; slot-- > 0;) {
        ...
        float3 position_gradient = load3_3d(
            grad_position,
            segment,
            slot,
            grad_position_stride0,
            grad_position_stride1,
            grad_position_stride2);
        position_gradient = add3(position_gradient, current_origin_gradient);
        direction_gradient = add3(
            direction_gradient,
            mul3(restart_epsilon[row], current_origin_gradient));
        ...
        if (restart_branch[row] == SegmentPenetrationRestartPosition) {
            if (policy == SegmentPenetrationEnumeratedFullDistance) {
                ...
                if (position_norm > 0.0f) {
                    position_gradient = add3(
                        position_gradient,
                        mul3(restart_gradient * 1.0e-6f / position_norm, hit_position));
                }
            } else {
                ...
                if (tie_count > 0) {
                    ...
                    position_gradient.x += (tie_mask & 1u) != 0u
                        ? share * copysignf(1.0f, hit_position.x)
                        : 0.0f;
                    position_gradient.y += (tie_mask & 2u) != 0u
                        ? share * copysignf(1.0f, hit_position.y)
                        : 0.0f;
                    position_gradient.z += (tie_mask & 4u) != 0u
                        ? share * copysignf(1.0f, hit_position.z)
                        : 0.0f;
                }
            }
        }

        float3 selected_normal_gradient = load3_3d(
            grad_normal,
            segment,
            slot,
            grad_normal_stride0,
        ...
        if (policy == SegmentPenetrationEnumeratedFullDistance) {
            selected_normal_gradient = normalized_vjp(
                geo, 1.0e-9f, selected_normal_gradient);
        }
        const float3 normal_gradient = add3(
            selected_normal_gradient,
            load3_3d(
                grad_geometric_normal,
                segment,
                slot,
                grad_geometric_normal_stride0,
                grad_geometric_normal_stride1,
                grad_geometric_normal_stride2));
        float normal_length = 0.0f;
        const float3 face_n = unit_normal(e1, e2, &normal_length);
        const float3 raw_normal_gradient = mul3(
            1.0f / normal_length,
            sub3(normal_gradient, mul3(dot3(face_n, normal_gradient), face_n)));
        ...
        const float3 lambda = solve_transpose(
            mul3(-1.0f, direction),
            e1,
            e2,
            make_float3(hit_gradient, 0.0f, 0.0f));
    ...
    if (direction_denominator_branch[segment] != 0u) {
        delta_gradient = mul3(
            1.0f / distance,
            sub3(direction_gradient, mul3(dot3(direction, direction_gradient), direction)));
    } else {
        delta_gradient = mul3(1.0f / direction_floor, direction_gradient);
    }
    const float distance_gradient = grad_distance == nullptr
        ? 0.0f
        : grad_distance[segment * grad_distance_stride0];
    if (distance > 0.0f)
...
__global__ void jvp_kernel(
    ...
    const std::uint8_t *restart_tie_mask,
    const std::uint8_t *direction_denominator_branch,
    const float *tangent_vertices,
    int64_t tangent_vertices_stride0,
    int64_t tangent_vertices_stride1,
    const float *tangent_origins,
    int64_t tangent_origins_stride0,
    int64_t tangent_origins_stride1,
    const float *tangent_targets,
    int64_t tangent_targets_stride0,
    int64_t tangent_targets_stride1,
    int policy,
    int64_t segment_count,
    int64_t hit_capacity,
    ...
    float *tangent_t,
    float *tangent_position,
    float *tangent_normal,
    ...
    const float3 delta = sub3(target, source);
    const float distance = sqrtf(fmaxf(dot3(delta, delta), 0.0f));
    const float3 dsource = load3(
        tangent_origins,
        segment,
        tangent_origins_stride0,
        tangent_origins_stride1);
    const float3 dtarget = load3(
        tangent_targets,
        segment,
        tangent_targets_stride0,
        tangent_targets_stride1);
    ...
    for (int64_t slot = 0; slot < hit_capacity; ++slot) {
        ...
        const float3 v1 = load3(vertices, i1, 3, 1);
        const float3 v2 = load3(vertices, i2, 3, 1);
        const float3 dv0 = load3(
            tangent_vertices,
            i0,
            tangent_vertices_stride0,
            tangent_vertices_stride1);
        const float3 dv1 = load3(
            tangent_vertices,
            i1,
            tangent_vertices_stride0,
            tangent_vertices_stride1);
        const float3 dv2 = load3(
            tangent_vertices,
            i2,
            tangent_vertices_stride0,
            tangent_vertices_stride1);
        ...
        const float v = barycentric[row * 2 + 1];
        const float hit_t = t[row];
        const float3 vertex_tangent = add3(
            add3(mul3(1.0f - u - v, dv0), mul3(u, dv1)),
            mul3(v, dv2));
        const float3 rhs = sub3(
            add3(current_origin_tangent, mul3(hit_t, ddirection)),
            vertex_tangent);
        const float hit_tangent = solve_columns(
            mul3(-1.0f, direction), e1, e2, rhs).x;
        const float3 position_tangent = add3(
            current_origin_tangent,
            add3(mul3(hit_tangent, direction), mul3(hit_t, ddirection)));
        ...
        const float3 selected_normal_tangent =
            ...
            ? normalized_jvp(
                  load3(geometric_normal, row, 3, 1),
                  1.0e-9f,
                  geo_tangent)
            : geo_tangent;
        ...
        if (restart_branch[row] == SegmentPenetrationRestartPosition) {
            ...
            if (policy == SegmentPenetrationEnumeratedFullDistance) {
                const float position_norm = sqrtf(fmaxf(dot3(hit_position, hit_position), 0.0f));
                restart_tangent = position_norm > 0.0f
                    ? 1.0e-6f * dot3(hit_position, position_tangent) / position_norm
                    : 0.0f;
            } else {
                const std::uint8_t tie_mask = restart_tie_mask[row];
                const int tie_count = population3(tie_mask);
                if (tie_count > 0) {
                    ...
                    restart_tangent = 1.0e-6f * inverse_count * (
                        ((tie_mask & 1u) != 0u
                            ? copysignf(1.0f, hit_position.x) * position_tangent.x
                            : 0.0f) +
                        ((tie_mask & 2u) != 0u
                            ? copysignf(1.0f, hit_position.y) * position_tangent.y
                            : 0.0f) +
                        ((tie_mask & 4u) != 0u
                            ? copysignf(1.0f, hit_position.z) * position_tangent.z
                            : 0.0f));
                }
            }
        }
        current_origin_tangent = add3(
            position_tangent,
            add3(
                mul3(restart_epsilon[row], ddirection),
                mul3(restart_tangent, direction)));
    }
}
...
void segment_penetration_initialize_cuda(
    ...
    const cudaStream_t stream = at::cuda::getCurrentCUDAStream(
        capacity_failure_state.get_device()).stream();
    initialize_kernel<<<blocks, threads, 0, stream>>>(
        byte_mask_ptr(outputs.result.valid),
        outputs.result.num_hits.data_ptr<int>(),
        byte_mask_ptr(outputs.result.reached_target),
        byte_mask_ptr(outputs.result.overflow),
        outputs.result.distance.data_ptr<float>(),
        outputs.result.direction.data_ptr<float>(),
        outputs.result.t.data_ptr<float>(),
        outputs.result.position.data_ptr<float>(),
        outputs.result.normal.data_ptr<float>(),
        outputs.result.geometric_normal.data_ptr<float>(),
        ...
        outputs.tape.direction_denominator_branch.defined()
            ? byte_mask_ptr(outputs.tape.direction_denominator_branch)
            : nullptr,
        optional_byte_mask_ptr(input_active),
        capacity_failure_state.data_ptr<int>(),
        failure_bit,
        input_active_any,
        segment_count,
        hit_capacity);
...
SegmentPenetrationBackwardOutputs segment_penetration_backward_cuda(
    ...
    const auto *active = request.primal.input_active.has_value()
        ? &*request.primal.input_active
        : nullptr;
    ...
    outputs.grad_vertices = request.need_grad_vertices
        ? at::empty_like(scene.global_vertices)
        : at::Tensor();
    outputs.grad_origins = request.need_grad_origins
        ? at::empty_like(request.primal.origins)
        : at::Tensor();
    outputs.grad_targets = request.need_grad_targets
        ? at::empty_like(request.primal.targets)
        : at::Tensor();
    const cudaStream_t stream = at::cuda::getCurrentCUDAStream(
        request.primal.origins.get_device()).stream();
    ...
    backward_kernel<<<blocks, threads, 0, stream>>>(
        ...
        request.primal.origins.data_ptr<float>(),
        request.primal.targets.data_ptr<float>(),
        optional_byte_mask_ptr(active),
        request.primal.capacity_failure_state.data_ptr<int>(),
        reinterpret_cast<const std::uint8_t *>(request.tape.result.valid.data_ptr<bool>()),
        request.tape.result.t.data_ptr<float>(),
        request.tape.result.position.data_ptr<float>(),
        request.tape.result.geometric_normal.data_ptr<float>(),
        request.tape.tape_primitive_id.data_ptr<int>(),
        request.tape.tape_barycentric.data_ptr<float>(),
        request.tape.tape_restart_epsilon.data_ptr<float>(),
        request.tape.tape_restart_branch.data_ptr<std::uint8_t>(),
        request.tape.tape_restart_tie_mask.data_ptr<std::uint8_t>(),
        reinterpret_cast<const std::uint8_t *>(
            request.tape.tape_direction_denominator_branch.data_ptr<bool>()),
        grad_distance == nullptr ? nullptr : grad_distance->data_ptr<float>(),
        optional_stride(grad_distance, 0),
        grad_direction == nullptr ? nullptr : grad_direction->data_ptr<float>(),
        optional_stride(grad_direction, 0),
        optional_stride(grad_direction, 1),
        grad_t == nullptr ? nullptr : grad_t->data_ptr<float>(),
        optional_stride(grad_t, 0),
        optional_stride(grad_t, 1),
        grad_position == nullptr ? nullptr : grad_position->data_ptr<float>(),
        optional_stride(grad_position, 0),
        optional_stride(grad_position, 1),
        optional_stride(grad_position, 2),
        grad_normal == nullptr ? nullptr : grad_normal->data_ptr<float>(),
        optional_stride(grad_normal, 0),
        optional_stride(grad_normal, 1),
        optional_stride(grad_normal, 2),
        grad_geometric_normal == nullptr ? nullptr : grad_geometric_normal->data_ptr<float>(),
        optional_stride(grad_geometric_normal, 0),
        optional_stride(grad_geometric_normal, 1),
        optional_stride(grad_geometric_normal, 2),
        static_cast<int>(request.primal.policy),
        segment_count,
        hit_capacity,
...
SegmentPenetrationJvpOutputs segment_penetration_jvp_cuda(
    ...
    for (const at::Tensor *tensor : {
             &outputs.tangent_distance,
             &outputs.tangent_direction,
             &outputs.tangent_t,
             &outputs.tangent_position,
             &outputs.tangent_normal,
             &outputs.tangent_geometric_normal}) {
        if (tensor->numel() != 0)
            cuda_check(
                cudaMemsetAsync(
                    tensor->data_ptr<float>(),
    ...
    jvp_kernel<<<blocks, threads, 0, stream>>>(
        ...
        tangent_origins == nullptr ? nullptr : tangent_origins->data_ptr<float>(),
        optional_stride(tangent_origins, 0),
        optional_stride(tangent_origins, 1),
        tangent_targets == nullptr ? nullptr : tangent_targets->data_ptr<float>(),
        optional_stride(tangent_targets, 0),
        optional_stride(tangent_targets, 1),
        static_cast<int>(request.primal.policy),
        segment_count,
        hit_capacity,
        outputs.tangent_distance.data_ptr<float>(),
        outputs.tangent_direction.data_ptr<float>(),
        outputs.tangent_t.data_ptr<float>(),
        outputs.tangent_position.data_ptr<float>(),
        outputs.tangent_normal.data_ptr<float>(),
        outputs.tangent_geometric_normal.data_ptr<float>());
