...
void require_contiguous_tensor(
    const at::Tensor &tensor,
    at::ScalarType dtype,
    at::IntArrayRef shape,
    const at::Tensor &device_reference,
    const char *name) {
...
ValidatedRequest validate_request(const rayd::torch::SegmentPenetrationRequest &request) {
    ...
    require_same_device(
        request.origins, request.capacity_failure_state, "capacity_failure_state");
    validate_failure_bit(request.failure_bit);
    const std::int32_t segment_count = checked_i32(
        request.origins.size(0), "segment_count");
    const std::int32_t hit_capacity = checked_i32(
        request.hit_capacity, "hit_capacity");
    if (hit_capacity != 0 &&
        segment_count > std::numeric_limits<std::int32_t>::max() / hit_capacity) {
...
SegmentPenetrationNativeOutputs allocate_outputs(
    const rayd::torch::SegmentPenetrationRequest &request,
    bool export_tape) {
...
SegmentPenetrationNativeOutputs forward_native(
    const rayd::torch::SegmentPenetrationRequest &request,
    bool export_tape) {
    ...
    segment_penetration_initialize_cuda(
        outputs,
        validated.input_active,
        request.capacity_failure_state,
        request.failure_bit,
        request.input_active_any);
    ...
    if (validated.segment_count > 0 && request.input_active_any) {
        ...
        params.reached_target = reinterpret_cast<std::uint8_t *>(
            outputs.result.reached_target.data_ptr<bool>());
        params.overflow = reinterpret_cast<std::uint8_t *>(
            outputs.result.overflow.data_ptr<bool>());
        ...
        if (export_tape) {
            ...
            params.tape_direction_denominator_branch = reinterpret_cast<std::uint8_t *>(
                outputs.tape.direction_denominator_branch.data_ptr<bool>());
        }
...
void validate_tape(
    const rayd::torch::SegmentPenetrationRequest &primal,
    ...
    require_contiguous_tensor(
        tape.result.valid, at::kBool, {segment_count, hit_capacity}, device, "tape.result.valid");
    require_contiguous_tensor(
        tape.result.num_hits, at::kInt, {segment_count}, device, "tape.result.num_hits");
    require_contiguous_tensor(
        tape.result.reached_target, at::kBool, {segment_count}, device, "tape.result.reached_target");
    require_contiguous_tensor(
        tape.result.overflow, at::kBool, {segment_count}, device, "tape.result.overflow");
    require_contiguous_tensor(
        tape.result.distance, at::kFloat, {segment_count}, device, "tape.result.distance");
    require_contiguous_tensor(
        tape.result.direction, at::kFloat, {segment_count, 3}, device, "tape.result.direction");
    require_contiguous_tensor(
        tape.result.t, at::kFloat, {segment_count, hit_capacity}, device, "tape.result.t");
    for (const auto &entry : {
             std::pair<const at::Tensor *, const char *>{&tape.result.position, "tape.result.position"},
             {&tape.result.normal, "tape.result.normal"},
             {&tape.result.geometric_normal, "tape.result.geometric_normal"}}) {
        require_contiguous_tensor(
            *entry.first,
            at::kFloat,
            {segment_count, hit_capacity, 3},
            device,
            entry.second);
    }
    require_contiguous_tensor(
...
SegmentPenetrationBackwardResult segment_penetration_backward(
    ...
    torch_backend::require_strided_float(
        optional(request.grad_distance), {segment_count}, device, "grad_distance");
    torch_backend::require_strided_float(
        optional(request.grad_direction), {segment_count, 3}, device, "grad_direction");
    torch_backend::require_strided_float(
        optional(request.grad_t), {segment_count, hit_capacity}, device, "grad_t");
    torch_backend::require_strided_float(
        optional(request.grad_position),
        {segment_count, hit_capacity, 3},
        device,
        "grad_position");
    torch_backend::require_strided_float(
        optional(request.grad_normal),
        {segment_count, hit_capacity, 3},
        device,
        "grad_normal");
    torch_backend::require_strided_float(
        optional(request.grad_geometric_normal),
        {segment_count, hit_capacity, 3},
        device,
        "grad_geometric_normal");
    c10::cuda::CUDAGuard guard(validated.scene.device_index);
    auto outputs = torch_backend::segment_penetration_backward_cuda(
...
SegmentPenetrationJvpResult segment_penetration_jvp(
    ...
    torch_backend::require_strided_float(
        optional(request.tangent_vertices),
        validated.scene.global_vertices.sizes(),
        device,
        "tangent_vertices");
    torch_backend::require_strided_float(
        optional(request.tangent_origins), {segment_count, 3}, device, "tangent_origins");
    torch_backend::require_strided_float(
        optional(request.tangent_targets), {segment_count, 3}, device, "tangent_targets");
    c10::cuda::CUDAGuard guard(validated.scene.device_index);
