...
#include <rayd/torch/transmission/transmission.h>
...
rayd::torch::LayerStackResult rayd::torch::em_layer_stack_eval(
    const rayd::torch::LayerStackRequest& request) {
...
void check_stack_primal(
    ...
    for (const auto& tensor : {material_id, layer_offset, layer_count,
                               ...
                               layer_mu_r})
        TORCH_CHECK(
            tensor.get_device() == cos_theta.get_device(),
            "em_layer_stack tensors must share one CUDA device");
    TORCH_CHECK(frequency_hz > 0.0, "frequency_hz must be positive");
}
...
rayd::torch::LayerStackBackwardResult rayd::torch::em_layer_stack_backward(
    ...
    for (int field = 0; field < 12; ++field) {
        grads.p[field] = stack_optional_grad(
            request.grad_outputs[field], kStackFields[field], count,
            cos_theta);
    }
...
rayd::torch::LayerStackResult rayd::torch::em_layer_stack_jvp(
    const rayd::torch::LayerStackJvpRequest& request) {
    const auto& cos_theta = request.primal.cos_theta;
    const auto& material_id = request.primal.material_id;
    const auto& layer_offset = request.primal.layer_offset;
    const auto& layer_count = request.primal.layer_count;
    ...
    const float* t_cos = stack_optional_grad(
        request.tangent_cos_theta, "tangent_cos_theta", count, cos_theta);
    const float* t_thickness = stack_optional_grad(
        request.tangent_layer_thickness_m, "tangent_layer_thickness_m",
        layer_total, cos_theta);
    const float* t_eps = stack_optional_grad(
        request.tangent_layer_eps_r, "tangent_layer_eps_r", layer_total,
        cos_theta);
    const float* t_sigma = stack_optional_grad(
        request.tangent_layer_sigma_e, "tangent_layer_sigma_e",
        layer_total, cos_theta);
...
std::pair<int64_t, int64_t> check_transmission_ad_primal(
    const rayd::torch::TransmissionSequenceRequest& request) {
...
rayd::torch::field_transmission_sequence_backward(
    ...
    const auto [depth, material_count] =
        check_transmission_ad_primal(request.primal);
