...
__global__ void reflection_trace_stats_kernel(const bool* __restrict__ valid, const float* __restrict__ t,
                                              int64_t ray_count, int64_t max_bounces, int64_t* __restrict__ counts,
    ...
    if (valid_for_ray == max_bounces) {
        atomicAdd(reinterpret_cast<unsigned long long*>(counts + 1), static_cast<unsigned long long>(1));
    }
}

__global__ void diffraction_path_stats_kernel(const int* __restrict__ count, const bool* __restrict__ valid,
                                              const float* __restrict__ delay, int64_t capacity,
                                              int64_t* __restrict__ valid_count, double* __restrict__ checksum) {
...
__global__ void default_dfr_material_kernel(int64_t count, float* __restrict__ eta_r, float* __restrict__ sigma,
                                            float* __restrict__ mu_r, float* __restrict__ gain,
...
__global__ void intersection_valid_from_t_kernel(const float* __restrict__ t, int64_t count, int64_t t_stride,
...
__global__ void intersection_valid_from_shape_kernel(const int* __restrict__ shape_id, int64_t count,
                                                     int64_t shape_stride, bool* __restrict__ out_valid) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= count)
        return;
...
std::tuple<at::Tensor, at::Tensor> reflection_trace_stats_cuda(const at::Tensor& valid, const at::Tensor& t) {
    ...
    reflection_trace_stats_kernel<<<blocks, threads, 0, stream>>>(valid.data_ptr<bool>(), t.data_ptr<float>(),
                                                                  ray_count, max_bounces, counts.data_ptr<int64_t>(),
...
std::tuple<at::Tensor, at::Tensor> diffraction_path_stats_cuda(const at::Tensor& count, const at::Tensor& valid,
    ...
    diffraction_path_stats_kernel<<<blocks, threads, 0, stream>>>(count.data_ptr<int>(), valid.data_ptr<bool>(),
                                                                  delay.data_ptr<float>(), capacity,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> default_dfr_material_cuda(
    ...
    if (count > 0) {
        ...
        default_dfr_material_kernel<<<blocks, threads, 0, stream>>>(count, eta_r.data_ptr<float>(),
                                                                    sigma.data_ptr<float>(), mu_r.data_ptr<float>(),
                                                                    gain.data_ptr<float>(), valid.data_ptr<bool>());
    }
...
at::Tensor intersection_valid_cuda(const at::Tensor& t, const at::Tensor& shape_id) {
    ...
    if (shape_id.defined() && shape_id.numel() == count) {
        intersection_valid_from_shape_kernel<<<blocks, threads, 0, stream>>>(shape_id.data_ptr<int>(), count,
                                                                             shape_id.stride(0),
                                                                             valid.data_ptr<bool>());
    } else {
        intersection_valid_from_t_kernel<<<blocks, threads, 0, stream>>>(t.data_ptr<float>(), count, t.stride(0),
                                                                         valid.data_ptr<bool>());
    }
    return valid;
}
