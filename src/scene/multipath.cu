...
void cuda_check(cudaError_t result, const char* where) {
    if (result != cudaSuccess)
        throw std::runtime_error(std::string("CUDA error in ") + where + ": " + cudaGetErrorString(result));
}

__device__ shared::bvh::CudaBvhView make_view(shared::bvh::TriangleSoAView triangles, shared::bvh::AabbSoAView bounds,
...
__device__ shared::bvh::CudaBvhTraverser make_traverser(shared::bvh::CudaBvhView view, int* stack_nodes, int* overflow,
...
struct TorchCudaDiffractionAccumulationPolicy {
    ...
    static __forceinline__ __device__ bool active_for_state(const uint8_t* mask, int width, int stride, int idx) {
        if (mask == nullptr)
            return true;
    ...
    static __forceinline__ __device__ bool stage_order1(unsigned int lane, int cell, float power, float field_x_re,
                                                        bool direct, bool keller) {
        if (params().stage_cell == nullptr || params().stage_value == nullptr)
            return false;
    ...
    static __forceinline__ __device__ bool stage_coherent(int cell, int state, bool multi, float xr, float xi, float yr,
                                                          float yi, float zr, float zi) {
    ...
    static __forceinline__ __device__ shared::math::Vec3f vec(const float* x, const float* y, const float* z, int s,
                                                              int i) {
        const int o = i * s;
        return shared::math::make_vec3(x[o], y[o], z[o]);
    }
    static __forceinline__ __device__ float3 opt_vec(const float* x, const float* y, const float* z, int s, int i) {
        const int o = i * s;
        return make_float3(x ? x[o] : 0.f, y ? y[o] : 0.f, z ? z[o] : 0.f);
    }
    static __forceinline__ __device__ int sample_state_index_for_lane(unsigned int lane) {
        if (!params().sample_state_index)
            return int(lane % unsigned(params().state_count));
    ...
    static __forceinline__ __device__ int state_edge_index_at(int i) {
        return read_i32(params().state_edge_index, params().state_edge_index_stride, i);
    }
    static __forceinline__ __device__ shared::math::Vec3f state_edge_pos_at(int i) {
        return vec(params().state_edge_pos_x, params().state_edge_pos_y, params().state_edge_pos_z,
                   params().state_edge_pos_stride, i);
    }
    static __forceinline__ __device__ shared::math::Vec3f state_edge_dir_at(int i) {
        return vec(params().state_edge_dir_x, params().state_edge_dir_y, params().state_edge_dir_z,
                   params().state_edge_dir_stride, i);
    }
    static __forceinline__ __device__ float state_edge_t_min_at(int i) {
        return read_f32(params().state_edge_t_min, params().state_edge_t_min_stride, i);
    }
    static __forceinline__ __device__ float state_edge_t_max_at(int i) {
        return read_f32(params().state_edge_t_max, params().state_edge_t_max_stride, i);
    }
    static __forceinline__ __device__ float sample_edge_weight_for_lane(int i, unsigned int lane, int n) {
        if (params().sample_edge_weight)
            return fmaxf(read_f32(params().sample_edge_weight, params().sample_edge_weight_stride, int(lane)), 0.f);
        return fmaxf(state_edge_t_max_at(i) - state_edge_t_min_at(i), 0.f) * float(params().state_count) /
               fmaxf(float(n), 1.f);
    }
    static __forceinline__ __device__ int state_prim0_at(int i) {
        return read_i32(params().state_prim0, params().state_prim0_stride, i);
    }
    static __forceinline__ __device__ int state_prim1_at(int i) {
        return read_i32(params().state_prim1, params().state_prim1_stride, i);
    }
    static __forceinline__ __device__ float state_exterior_angle_at(int i) {
        return read_f32(params().state_exterior_angle, params().state_exterior_angle_stride, i);
    }
    static __forceinline__ __device__ float state_src_power_at(int i) {
        return read_f32(params().state_src_power, params().state_src_power_stride, i);
    }
    static __forceinline__ __device__ shared::math::Vec3f state_src_at(int i) {
        return vec(params().state_src_x, params().state_src_y, params().state_src_z, params().state_src_stride, i);
    }
    static __forceinline__ __device__ float3 state_wi_at(int i) {
        return opt_vec(params().state_wi_x, params().state_wi_y, params().state_wi_z, params().state_wi_stride, i);
    }
    static __forceinline__ __device__ int recursive_state_edge_index_at(int i) {
        return read_i32(params().recursive_state_edge_index, params().recursive_state_edge_index_stride, i);
    }
    static __forceinline__ __device__ shared::math::Vec3f recursive_state_edge_pos_at(int i) {
        return vec(params().recursive_state_edge_pos_x, params().recursive_state_edge_pos_y,
                   params().recursive_state_edge_pos_z, params().recursive_state_edge_pos_stride, i);
    }
    static __forceinline__ __device__ shared::math::Vec3f recursive_state_edge_dir_at(int i) {
        return vec(params().recursive_state_edge_dir_x, params().recursive_state_edge_dir_y,
                   params().recursive_state_edge_dir_z, params().recursive_state_edge_dir_stride, i);
    }
    static __forceinline__ __device__ float recursive_state_edge_t_min_at(int i) {
        return read_f32(params().recursive_state_edge_t_min, params().recursive_state_edge_t_min_stride, i);
    }
    static __forceinline__ __device__ float recursive_state_edge_t_max_at(int i) {
        return read_f32(params().recursive_state_edge_t_max, params().recursive_state_edge_t_max_stride, i);
    }
    static __forceinline__ __device__ int recursive_state_prim0_at(int i) {
        return read_i32(params().recursive_state_prim0, params().recursive_state_prim0_stride, i);
    }
    static __forceinline__ __device__ int recursive_state_prim1_at(int i) {
        return read_i32(params().recursive_state_prim1, params().recursive_state_prim1_stride, i);
    }
    static __forceinline__ __device__ float recursive_state_exterior_angle_at(int i) {
        return read_f32(params().recursive_state_exterior_angle, params().recursive_state_exterior_angle_stride, i);
    }
    static __forceinline__ __device__ bool material_valid_at(int i) {
        return !params().material_valid || read_u8(params().material_valid, params().material_valid_stride, i) != 0u;
    }
    static __forceinline__ __device__ float material_gain_at(int i) {
        return read_f32(params().material_gain, params().material_gain_stride, i);
    }
};
...
__device__ __forceinline__ bool epc_prim_ignored(const ReflEpcParams& params, int prim, int ig0, int ig1, int ig2) {
    ...
    return (ig0 >= 0 && candidate == ig0) || (ig1 >= 0 && candidate == ig1) || (ig2 >= 0 && candidate == ig2);
}

__device__ __forceinline__ int epc_first_blocker(const shared::bvh::CudaBvhView& view,
                                                 const shared::bvh::TriangleTraversalScratchView& scratch, size_t lane,
                                                 const ReflEpcParams& params, shared::math::Vec3f origin,
                                                 shared::math::Vec3f direction, float tmin, float tmax, int ig0,
                                                 int ig1, int ig2) {
    ...
    for (;;) {
        while (node >= 0 && !shared::bvh::is_leaf_node(view.topology, node)) {
            ...
            float t_right = 0.0f;
            const bool hit_left =
                shared::bvh::intersect_node_bounds(view.node_bounds, left, origin.x, origin.y, origin.z, inv_dx, inv_dy,
                                                   inv_dz, tmin, best_t, t_left);
            const bool hit_right =
                shared::bvh::intersect_node_bounds(view.node_bounds, right, origin.x, origin.y, origin.z, inv_dx,
                                                   inv_dy, inv_dz, tmin, best_t, t_right);
        ...
        if (node >= 0) {
            ...
            for (int slot = 0; slot < leaf_count; ++slot) {
                ...
                const auto hit =
                    shared::bvh::intersect_triangle_watertight(origin.x, origin.y, origin.z, direction.x, direction.y,
                                                               direction.z, v.ax, v.ay, v.az, v.bx, v.by, v.bz, v.cx,
...
struct TorchCudaEpcTraverser {
    ...
    __device__ __forceinline__ shared::rt::TriangleHit trace_closest(shared::math::Vec3f origin,
                                                                     shared::math::Vec3f direction, float tmin,
                                                                     float tmax) const {
        return shared::bvh::CudaBvhTraverser{view, scratch, lane}.trace_closest(origin, direction, tmin, tmax);
    }
    __device__ __forceinline__ shared::rt::TriangleHit trace_first_blocker(shared::math::Vec3f origin,
                                                                           shared::math::Vec3f direction, float tmin,
                                                                           float tmax, const int32_t* ignore,
    ...
    __device__ __forceinline__ bool trace_occluded_ignore(shared::math::Vec3f origin, shared::math::Vec3f direction,
                                                          float tmin, float tmax, const int32_t* ignore,
                                                          int ignore_count) const {
        return trace_first_blocker(origin, direction, tmin, tmax, ignore, ignore_count).hit != 0u;
    }
    __device__ __forceinline__ bool trace_occluded(shared::math::Vec3f origin, shared::math::Vec3f direction,
...
__global__ void reflection_kernel(shared::optix::ReflectionTraceParams params, shared::bvh::TriangleSoAView triangles,
                                  shared::bvh::AabbSoAView bounds, shared::bvh::CompactBvhTopologyView topology,
                                  shared::bvh::TrianglePrimIdMapView prim_map, int* stack_nodes, int* overflow,
                                  int lane_count) {
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    if (lane >= static_cast<unsigned int>(lane_count))
        return;
    const auto traverser =
        make_traverser(make_view(triangles, bounds, topology, prim_map), stack_nodes, overflow, lane_count, lane);
    using Config = shared::rt::TraceConfig<TorchCudaReflectionPolicy, shared::bvh::CudaBvhTraverser>;
    shared::multipath::reflection_trace_algo<Config>(params, lane, traverser, traverser);
}

__global__ void visibility_kernel(shared::optix::SegmentVisibilityParams params, int variant,
                                  shared::bvh::TriangleSoAView triangles, shared::bvh::AabbSoAView bounds,
                                  shared::bvh::CompactBvhTopologyView topology,
                                  shared::bvh::TrianglePrimIdMapView prim_map, int* stack_nodes, int* overflow,
                                  int lane_count) {
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    const auto traverser =
        make_traverser(make_view(triangles, bounds, topology, prim_map), stack_nodes, overflow, lane_count, lane);
    using Config = shared::rt::TraceConfig<TorchCudaVisibilityPolicy, shared::bvh::CudaBvhTraverser>;
...
__global__ void reflection_accumulation_kernel(AccumParams params, shared::bvh::TriangleSoAView triangles,
                                               shared::bvh::AabbSoAView bounds,
                                               shared::bvh::CompactBvhTopologyView topology,
                                               shared::bvh::TrianglePrimIdMapView prim_map, int* stack_nodes,
    ...
    const auto traverser =
        make_traverser(make_view(triangles, bounds, topology, prim_map), stack_nodes, overflow, lane_count, lane);
    shared::multipath::reflection_accumulation_algo<AccumParams, TorchCudaReflectionAccumulationPolicy,
                                                    shared::bvh::CudaBvhTraverser>(params, lane, traverser, traverser);
}

template <bool SourcePass>
__global__ void diffraction_paths_kernel(DfrPathParams params, shared::bvh::TriangleSoAView triangles,
                                         shared::bvh::AabbSoAView bounds, shared::bvh::CompactBvhTopologyView topology,
                                         shared::bvh::TrianglePrimIdMapView prim_map, int* stack_nodes, int* overflow,
                                         int lane_count) {
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if constexpr (SourcePass)
        shared::multipath::trace_paths_source_visibility_algo<DfrPathParams>(params, lane, traverser, traverser);
    else
        shared::multipath::trace_paths_target_export_algo<DfrPathParams>(params, lane, traverser, traverser);
}

__global__ void diffraction_accumulation_kernel(DfrAccumParams kernel_params, int variant,
                                                shared::bvh::TriangleSoAView triangles, shared::bvh::AabbSoAView bounds,
                                                shared::bvh::CompactBvhTopologyView topology,
                                                shared::bvh::TrianglePrimIdMapView prim_map, int* stack_nodes,
                                                int* overflow, int lane_count) {
    ...
    const auto traverser =
        make_traverser(make_view(triangles, bounds, topology, prim_map), stack_nodes, overflow, lane_count, lane);
    using Algo = shared::multipath::DiffractionAccumulationAlgo<TorchCudaDiffractionAccumulationPolicy,
                                                                shared::bvh::CudaBvhTraverser>;
...
__global__ void reflection_epc_kernel(ReflEpcParams params, shared::bvh::TriangleSoAView triangles,
                                      shared::bvh::AabbSoAView bounds, shared::bvh::CompactBvhTopologyView topology,
                                      shared::bvh::TrianglePrimIdMapView prim_map, int* stack_nodes, int* overflow,
                                      int lane_count) {
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    shared::bvh::TriangleTraversalScratchView scratch = {stack_nodes,
                                                         overflow,
                                                         static_cast<size_t>(lane_count),
                                                         static_cast<size_t>(shared::bvh::kBvhTraversalStackDepth),
                                                         static_cast<size_t>(lane_count) *
                                                             shared::bvh::kBvhTraversalStackDepth,
                                                         static_cast<size_t>(lane_count)};
    ...
    shared::multipath::run_reflection_epc_algo<Config, DirectOnly, PrimaryOnly>(params, lane, traverser, traverser);
}
...
void launch_reflection_trace_cuda(const SceneCache& scene, const shared::optix::ReflectionTraceParams& params,
    ...
    at::Tensor stack = at::empty({shared::bvh::kBvhTraversalStackDepth, lane_count}, iopts);
    ...
    reflection_kernel<<<blocks, kBlockSize, 0, stream>>>(params, scene_triangle_view(scene),
                                                         scene_triangle_bvh_bounds_view(scene),
...
void launch_segment_visibility_cuda(const SceneCache& scene, const shared::optix::SegmentVisibilityParams& params,
    ...
    visibility_kernel<<<blocks, kBlockSize, 0, stream>>>(cuda_params, static_cast<int>(variant),
                                                         scene_triangle_view(scene),
                                                         scene_triangle_bvh_bounds_view(scene),
                                                         scene_triangle_bvh_topology_view(scene), scene_prim_map(scene),
                                                         stack.data_ptr<int>(), overflow.data_ptr<int>(), lane_count);
...
void launch_reflection_accumulation_cuda(const SceneCache& scene, const AccumParams& params, int lane_count) {
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream(scene.device_index).stream();
    const int blocks = (lane_count + kBlockSize - 1) / kBlockSize;
    reflection_accumulation_kernel<<<blocks, kBlockSize, 0, stream>>>(params, scene_triangle_view(scene),
                                                                      scene_triangle_bvh_bounds_view(scene),
                                                                      scene_triangle_bvh_topology_view(scene),
                                                                      scene_prim_map(scene), stack.data_ptr<int>(),
                                                                      overflow.data_ptr<int>(), lane_count);
    cuda_check(cudaGetLastError(), "launch_reflection_accumulation_cuda");
}

void launch_reflection_epc_cuda(const SceneCache& scene, const ReflEpcParams& params, bool direct_only,
...
#define RAYD_LAUNCH_EPC(DIRECT, PRIMARY)                                                                               \
    reflection_epc_kernel<DIRECT, PRIMARY>                                                                             \
        <<<blocks, kBlockSize, 0, stream>>>(params, scene_triangle_view(scene), scene_triangle_bvh_bounds_view(scene), \
                                            scene_triangle_bvh_topology_view(scene), scene_prim_map(scene),            \
                                            stack.data_ptr<int>(), overflow.data_ptr<int>(), lane_count)
...
#define RAYD_LAUNCH_DFR_PATH(SOURCE)                                                                                   \
    diffraction_paths_kernel<SOURCE>                                                                                   \
        <<<blocks, kBlockSize, 0, stream>>>(params, scene_triangle_view(scene), scene_triangle_bvh_bounds_view(scene), \
                                            scene_triangle_bvh_topology_view(scene), scene_prim_map(scene),            \
                                            stack.data_ptr<int>(), overflow.data_ptr<int>(), lane_count)
...
void launch_diffraction_accumulation_cuda(const SceneCache& scene, const DfrAccumParams& params, int pipeline_variant,
    ...
    if (lane_count == 0)
        return;
    if (pipeline_variant != 6 && pipeline_variant != 7 && pipeline_variant != 8 && pipeline_variant != 9 &&
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream(scene.device_index).stream();
    const int blocks = (lane_count + kBlockSize - 1) / kBlockSize;
    diffraction_accumulation_kernel<<<blocks, kBlockSize, sizeof(DfrAccumParams), stream>>>(
        params, pipeline_variant, scene_triangle_view(scene), scene_triangle_bvh_bounds_view(scene),
        scene_triangle_bvh_topology_view(scene), scene_prim_map(scene), stack.data_ptr<int>(), overflow.data_ptr<int>(),
        lane_count);
