...
__global__ void triangle_bounds_kernel(const float* p0_x, const float* p0_y, const float* p0_z, const float* e1_x,
                                       const float* e1_y, const float* e1_z, const float* e2_x, const float* e2_y,
                                       const float* e2_z, int count, float* min_x, float* min_y, float* min_z,
                                       float* max_x, float* max_y, float* max_z, shared::bvh::BvhBounds3* packed) {
    ...
    shared::bvh::BvhBounds3 bounds = {{fminf(ax, fminf(bx, cx)), fminf(ay, fminf(by, cy)), fminf(az, fminf(bz, cz))},
...
__global__ void prepare_triangle_rays_kernel(const float* ray_o, const float* ray_d, const float* ray_tmax,
                                             const bool* active, int64_t ray_count, int64_t o_stride0,
                                             int64_t o_stride1, int64_t d_stride0, int64_t d_stride1, float* ox,
                                             float* oy, float* oz, float* dx, float* dy, float* dz, float* tmax,
                                             int* active_i32) {
...
__global__ void finalize_triangle_hits_kernel(const int* face_local_id, int64_t ray_count, const int* winner_global,
                                              const float* bary_u, const float* bary_v, int* out_local, int* out_global,
                                              float* out_bary_uv) {
    const int64_t ray = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (ray >= ray_count)
        return;
...
void compute_triangle_bvh_bounds_cuda(const SceneCache& scene, at::Tensor& primitive_min_x, at::Tensor& primitive_min_y,
                                      at::Tensor& primitive_min_z, at::Tensor& primitive_max_x,
                                      at::Tensor& primitive_max_y, at::Tensor& primitive_max_z,
                                      at::Tensor& packed_bounds, cudaStream_t stream) {
    ...
    triangle_bounds_kernel<<<blocks, kBlockSize, 0, stream>>>(
        scene.tri_p0_x.data_ptr<float>(), scene.tri_p0_y.data_ptr<float>(), scene.tri_p0_z.data_ptr<float>(),
        scene.tri_e1_x.data_ptr<float>(), scene.tri_e1_y.data_ptr<float>(), scene.tri_e1_z.data_ptr<float>(),
        scene.tri_e2_x.data_ptr<float>(), scene.tri_e2_y.data_ptr<float>(), scene.tri_e2_z.data_ptr<float>(),
        static_cast<int>(count), primitive_min_x.data_ptr<float>(), primitive_min_y.data_ptr<float>(),
        primitive_min_z.data_ptr<float>(), primitive_max_x.data_ptr<float>(), primitive_max_y.data_ptr<float>(),
        primitive_max_z.data_ptr<float>(),
...
void launch_intersect_cuda_bvh(const SceneCache& scene, const at::Tensor& ray_o, const at::Tensor& ray_d,
    ...
    at::Tensor stack = at::empty({shared::bvh::kBvhTraversalStackDepth, ray_count}, iopts);
    ...
    prepare_triangle_rays_kernel<<<blocks, kBlockSize, 0, stream>>>(
        ...
        !active.defined() || active.numel() == 0 ? nullptr : active.data_ptr<bool>(), ray_count, ray_o.stride(0),
        ray_o.stride(1), ray_d.stride(0), ray_d.stride(1), ox.data_ptr<float>(), oy.data_ptr<float>(),
        oz.data_ptr<float>(), dx.data_ptr<float>(), dy.data_ptr<float>(), dz.data_ptr<float>(), tmax.data_ptr<float>(),
        active_i32.data_ptr<int>());
    ...
    params.rays = {ox.data_ptr<float>(),   oy.data_ptr<float>(),       oz.data_ptr<float>(),
                   ...
                   tmax.data_ptr<float>(), active_i32.data_ptr<int>(), static_cast<size_t>(ray_count)};
    ...
    params.prim_map = {scene.face_shape_id.data_ptr<int>(), scene.primitive_identity.data_ptr<int>(),
                       static_cast<size_t>(scene.global_faces.size(0))};
    params.output = {out_t.data_ptr<float>(),       bary_u.data_ptr<float>(),
                     bary_v.data_ptr<float>(),      out_shape_id == nullptr ? shape.data_ptr<int>() : out_shape_id,
    ...
    finalize_triangle_hits_kernel<<<blocks, kBlockSize, 0, stream>>>(scene.face_local_id.data_ptr<int>(), ray_count,
                                                                     winner_global.data_ptr<int>(),
                                                                     bary_u.data_ptr<float>(), bary_v.data_ptr<float>(),
                                                                     out_local_prim_id, out_global_prim_id,
                                                                     out_bary_uv);
