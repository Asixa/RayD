...
TriBvhTrianglePtrs triangle_ptrs(const Vector3f& p0, const Vector3f& e1, const Vector3f& e2) {
    return {p0[0].data(), p0[1].data(), p0[2].data(), e1[0].data(), e1[1].data(),
            e1[2].data(), e2[0].data(), e2[1].data(), e2[2].data()};
}
...
void CudaTraceBackend::build_or_refit(const TriangleInfo& triangles, const Int& shape_id, const Int& local_prim_id,
                                      bool refit) {
    ...
    if (refit) {
        ...
        drjit::eval(node_bbox_min_, node_bbox_max_, left_child_, right_child_, leaf_primitives_, leaf_nodes_,
                    refit_level_nodes_);
        drjit::sync_thread();

        const int level_count = static_cast<int>(refit_level_offsets_.empty() ? 0 : refit_level_offsets_.size() - 1);
        refit_triangle_bvh_gpu(node_count_, triangle_ptrs(tri_p0_, tri_e1_, tri_e2_), left_child_.data(),
                               right_child_.data(), leaf_primitives_.data(), leaf_nodes_.data(),
                               static_cast<int>(leaf_nodes_.size()), refit_level_nodes_.data(),
                               refit_level_offsets_.data(), level_count, bounds_ptrs(node_bbox_min_, node_bbox_max_));
        ready_ = true;
        return;
    }
    ...
    build_triangle_bvh_gpu(primitive_count_, triangle_ptrs(tri_p0_, tri_e1_, tri_e2_),
                           bounds_ptrs(primitive_bbox_min, primitive_bbox_max), bounds_ptrs(raw_node_min, raw_node_max),
                           raw_left.data(), raw_right.data(), raw_leaf_primitive.data(), raw_is_leaf.data(),
    ...
    shared::bvh::compute_subtree_leaf_count(0, host_left, host_right, host_is_leaf, subtree_leaf_counts);
    ...
    shared::bvh::emit_compacted_preorder(0, host_left, host_right, host_leaf_primitive, host_is_leaf,
                                         subtree_leaf_counts, host_node_min, host_node_max, kLeafSize, compacted);
    ...
    const int max_height = shared::bvh::compute_node_height(0, final_left, final_right, final_is_leaf, heights);
    ...
    drjit::eval(tri_p0_, tri_e1_, tri_e2_, shape_id_, local_prim_id_, node_bbox_min_, node_bbox_max_, left_child_,
                right_child_, leaf_primitives_, primitive_leaf_node_, leaf_nodes_, refit_level_nodes_);
    drjit::sync_thread();
...
OptixIntersection CudaTraceBackend::intersect_impl(const RayT<Detached>& ray, MaskT<Detached>& active) const {
    ...
    drjit::eval(ox, oy, oz, dx, dy, dz, t_max, active_flags, tri_p0_, tri_e1_, tri_e2_, node_bbox_min_, node_bbox_max_,
                left_child_, right_child_, leaf_primitives_, shape_id_, local_prim_id_, intersection.t,
                intersection.barycentric, intersection.shape_id, intersection.local_prim_id, stack_nodes, overflow);
    ...
    (void)dz.data();
    (void)t_max.data();
    (void)active_flags.data();
    (void)intersection.t.data();
    (void)intersection.barycentric[0].data();
    (void)intersection.barycentric[1].data();
    (void)intersection.shape_id.data();
    (void)intersection.local_prim_id.data();
    (void)stack_nodes.data();
    (void)overflow.data();
    drjit::sync_thread();

    query_triangle_closest_hit_gpu(ray_count, primitive_count_, node_count_, static_cast<int>(leaf_primitives_.size()),
...
MaskT<Detached> CudaTraceBackend::shadow_test_impl(const RayT<Detached>& ray, MaskT<Detached> active) const {
    ...
    drjit::eval(ox, oy, oz, dx, dy, dz, t_max, active_flags, tri_p0_, tri_e1_, tri_e2_, node_bbox_min_, node_bbox_max_,
                left_child_, right_child_, leaf_primitives_, out_hit, stack_nodes, overflow);
    ...
    query_triangle_occluded_gpu(ray_count, primitive_count_, node_count_, static_cast<int>(leaf_primitives_.size()),
                                RayEpsilon, triangle_ptrs(tri_p0_, tri_e1_, tri_e2_),
                                {node_bbox_min_[0].data(), node_bbox_min_[1].data(), node_bbox_min_[2].data(),
                                 node_bbox_max_[0].data(), node_bbox_max_[1].data(), node_bbox_max_[2].data()},
                                left_child_.data(), right_child_.data(), leaf_primitives_.data(),
...
std::vector<int> CudaTraceBackend::first_blocker_selftest(const Vector3f& origin, const Vector3f& direction,
                                                          const Float& tmax,
    ...
    if (ignore_stride > 0) {
        ...
        for (int r = 0; r < ray_count; ++r) {
            for (int i = 0; i < ignore_stride; ++i) {
                tiled[static_cast<size_t>(r) * ignore_stride + i] = ignore_prim_ids[static_cast<size_t>(i)];
            }
        }
    ...
    drjit::eval(ox, oy, oz, dx, dy, dz, t_max, tri_p0_, tri_e1_, tri_e2_, node_bbox_min_, node_bbox_max_, left_child_,
                right_child_, leaf_primitives_, out_global, stack_nodes, overflow);
    // See intersect_impl: literal-backed arrays materialize on .data(), which
    // must happen before the stream sync to keep the native launch ordered.
    (void)ox.data();
    (void)oy.data();
    ...
    query_triangle_first_blocker_gpu(ray_count, primitive_count_, node_count_,
                                     static_cast<int>(leaf_primitives_.size()), RayEpsilon,
                                     triangle_ptrs(tri_p0_, tri_e1_, tri_e2_),
                                     {node_bbox_min_[0].data(), node_bbox_min_[1].data(), node_bbox_min_[2].data(),
                                      node_bbox_max_[0].data(), node_bbox_max_[1].data(), node_bbox_max_[2].data()},
                                     left_child_.data(), right_child_.data(), leaf_primitives_.data(),
                                     {ox.data(), oy.data(), oz.data(), dx.data(), dy.data(), dz.data(), t_max.data(),
...
void CudaTraceBackend::materialize_for_fused_launch() const {
    ...
    drjit::eval(tri_p0_, tri_e1_, tri_e2_, node_bbox_min_, node_bbox_max_, left_child_, right_child_, leaf_primitives_,
                shape_id_, local_prim_id_);
    (void)tri_p0_[0].data();
    (void)tri_p0_[1].data();
    (void)tri_p0_[2].data();
    (void)tri_e1_[0].data();
    ...
    (void)tri_e2_[1].data();
    (void)tri_e2_[2].data();
    (void)node_bbox_min_[0].data();
    (void)node_bbox_min_[1].data();
    (void)node_bbox_min_[2].data();
    (void)node_bbox_max_[0].data();
    (void)node_bbox_max_[1].data();
    (void)node_bbox_max_[2].data();
    (void)left_child_.data();
    (void)right_child_.data();
    (void)leaf_primitives_.data();
    (void)shape_id_.data();
    (void)local_prim_id_.data();
    drjit::sync_thread();
}

void CudaTraceBackend::run_reflection_trace(shared::optix::ReflectionTraceParams params, int lane_count) const {
...
void CudaTraceBackend::run_reflection_epc(shared::optix::ReflEpcParams params, bool direct_only,
    ...
    params.secondary_handle = 0;
    materialize_for_fused_launch();
    launch_reflection_epc_cuda(params, multipath_bvh(), direct_only, primary_visibility_only, lane_count);
}
...
void CudaTraceBackend::run_dfr_accum_direct(DfrAccumParams params, bool has_non_suffix_strategy,
    ...
    params.secondary_handle = 0;
    materialize_for_fused_launch();
    launch_dfr_accum_direct_cuda(params, multipath_bvh(), has_non_suffix_strategy, has_suffix_strategy, lane_count);
}
...
void CudaTraceBackend::run_dfr_accum_combined(DfrAccumParams params, bool has_non_suffix_strategy,
    ...
    params.secondary_handle = 0;
    materialize_for_fused_launch();
    launch_dfr_accum_combined_cuda(params, multipath_bvh(), has_non_suffix_strategy, has_suffix_strategy, lane_count);
}
...
enum class OptixSplitMode { Auto, Off, On };
...
OptixSplitMode active_optix_split_mode() {
    static const OptixSplitMode value = []() {
        ...
        throw std::runtime_error("Invalid RAYD_OPTIX_SPLIT_MODE. Expected one of: auto, off, on.");
    }();
    return value;
}

bool should_split_optix_scene(OptixSplitMode mode, int static_mesh_count, int dynamic_mesh_count) {
...
bool OptixTraceBackend::is_ready() const {
    if (split_active_) {
        return scene_ != nullptr && static_scene_ != nullptr && dynamic_scene_ != nullptr && scene_->is_ready() &&
    ...
    if (split_active_) {
        ...
        for (size_t mesh_index = 0; mesh_index < dynamic_flags.size(); ++mesh_index) {
            if (dynamic_flags[mesh_index]) {
                dynamic_mesh_local_index_[mesh_index] = static_cast<int>(dynamic_mesh_descs.size());
...
OptixTraceSyncResult OptixTraceBackend::sync(const std::vector<OptixSceneMeshDesc>& mesh_descs,
    ...
    if (split_active_) {
        ...
        for (const OptixSceneMeshUpdate& update : updates) {
            const int dynamic_local_index = dynamic_mesh_local_index_[static_cast<size_t>(update.mesh_id)];
            if (dynamic_local_index < 0) {
                continue;
            }
            dynamic_updates.push_back({dynamic_local_index, update.vertices_dirty, update.transform_dirty});
        }
