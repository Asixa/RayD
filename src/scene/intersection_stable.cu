...
__global__ void intersection_valid_from_t_kernel(const float* __restrict__ t, int64_t count, int64_t stride,
...
__global__ void intersection_valid_from_shape_kernel(const int* __restrict__ shape_id, int64_t count, int64_t stride,
                                                     bool* __restrict__ valid) {
    const int64_t index = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (index < count)
...
void require_rank1_cuda(const Tensor& tensor, torch::headeronly::ScalarType dtype, const char* name) {
...
Tensor intersection_valid(const Tensor& t, const Tensor& shape_id) {
    ...
    STD_TORCH_CHECK(shape_id.size(0) == 0 || shape_id.size(0) == t.size(0), "shape_id must be empty or match t length");
    STD_TORCH_CHECK(shape_id.size(0) == 0 || shape_id.get_device_index() == t.get_device_index(),
    ...
    Tensor valid = torch::stable::new_empty(t, torch::headeronly::IntHeaderOnlyArrayRef(shape),
    ...
    if (shape_id.size(0) == t.size(0)) {
        intersection_valid_from_shape_kernel<<<blocks, threads, 0, stream>>>(shape_id.const_data_ptr<int>(), t.size(0),
                                                                             shape_id.stride(0),
                                                                             valid.mutable_data_ptr<bool>());
    } else {
        intersection_valid_from_t_kernel<<<blocks, threads, 0, stream>>>(t.const_data_ptr<float>(), t.size(0),
                                                                         t.stride(0), valid.mutable_data_ptr<bool>());
    }
    const cudaError_t error = cudaGetLastError();
    STD_TORCH_CHECK(error == cudaSuccess, "CUDA error in intersection_valid: ", cudaGetErrorString(error));
    return valid;
}
...
STABLE_TORCH_LIBRARY_IMPL(rayd_torch_stable, CUDA, m) {
    m.impl("intersection_valid", TORCH_BOX(&rayd::torch_backend::stable::intersection_valid));
}
