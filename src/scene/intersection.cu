...
constexpr int64_t kRayFlagsGeometric = static_cast<int64_t>(shared::RayFlagBits::Geometric);
constexpr int64_t kRayFlagsShadingN = static_cast<int64_t>(shared::RayFlagBits::ShadingN);
...
__global__ void intersect_recompute_kernel(
    ...
    const float* __restrict__ ray_d, const bool* __restrict__ active, int64_t ray_count,
    const float* __restrict__ optix_t, const int* __restrict__ optix_shape_id,
    const int* __restrict__ optix_local_prim_id, const int* __restrict__ optix_global_prim_id,
    const float* __restrict__ optix_bary_uv, float* __restrict__ out_p, float* __restrict__ out_n,
    float* __restrict__ out_geo_n, float* __restrict__ out_uv, float* __restrict__ out_bary,
    int* __restrict__ out_shape_id, int* __restrict__ out_prim_id, int* __restrict__ out_local_prim_id,
    int* __restrict__ out_global_prim_id, int64_t flags) {
    ...
    const int local_prim_id = lane_active ? optix_local_prim_id[ray_idx] : shared::InvalidSignedId;
    const int global_prim_id = lane_active ? optix_global_prim_id[ray_idx] : shared::InvalidSignedId;
...
at::Tensor make_intersect_params_buffer(int device_index) {
    ...
    at::TensorOptions byte_options = at::TensorOptions().device(at::Device(at::kCUDA, device_index)).dtype(at::kByte);
    return at::empty({static_cast<int64_t>(sizeof(OptixIntersectParams))}, byte_options);
}

void launch_intersect_optix(const SceneCache& scene, const at::Tensor& ray_o, const at::Tensor& ray_d,
                            const at::Tensor& ray_tmax, const at::Tensor& active, at::Tensor& out_t, int* out_shape_id,
                            int* out_local_prim_id, int* out_global_prim_id, float* out_bary_uv, cudaStream_t stream) {
    ...
    params.ray_tmax = (!ray_tmax.defined() || ray_tmax.numel() == 0) ? nullptr : ray_tmax.data_ptr<float>();
    ...
    at::Tensor params_buffer = make_intersect_params_buffer(static_cast<int>(scene.device_index));
    ...
    cuda_check(cudaMemcpyAsync(params_buffer.data_ptr<uint8_t>(), &params, sizeof(OptixIntersectParams),
                               cudaMemcpyHostToDevice, stream),
               "cudaMemcpyAsync(OptiX intersect params)");

    rayd_torch_OPTIX_CHECK(optixLaunch(optix_entry.intersect_pipeline, stream,
                                       reinterpret_cast<CUdeviceptr>(params_buffer.data_ptr<uint8_t>()),
                                       sizeof(OptixIntersectParams), &optix_entry.intersect_sbt,
                                       static_cast<unsigned int>(ray_o.size(0)), 1, 1));
}

void launch_intersect_backend(const SceneCache& scene, const at::Tensor& ray_o, const at::Tensor& ray_d,
                              const at::Tensor& ray_tmax, const at::Tensor& active, at::Tensor& out_t,
                              int* out_shape_id, int* out_local_prim_id, int* out_global_prim_id, float* out_bary_uv,
                              cudaStream_t stream) {
    if (scene.trace_backend == TraceBackend::Cuda) {
        launch_intersect_cuda_bvh(scene, ray_o, ray_d, ray_tmax, active, out_t, out_shape_id, out_local_prim_id,
                                  out_global_prim_id, out_bary_uv, stream);
        return;
    }
    launch_intersect_optix(scene, ray_o, ray_d, ray_tmax, active, out_t, out_shape_id, out_local_prim_id,
                           out_global_prim_id, out_bary_uv, stream);
}

} // namespace

IntersectForwardOutputs intersect_forward_cuda(const SceneCache& scene, const at::Tensor& ray_o,
    ...
    launch_intersect_backend(scene, ray_o, ray_d, ray_tmax, active, out.t, out.shape_id.data_ptr<int>(),
                             out.local_prim_id.data_ptr<int>(), out.global_prim_id.data_ptr<int>(),
                             optix_bary_uv.data_ptr<float>(), torch_ctx.stream);
    ...
    intersect_recompute_kernel<<<blocks, threads, 0, torch_ctx.stream>>>(
        ...
        optional_mask_ptr(active), ray_count, out.t.data_ptr<float>(), out.shape_id.data_ptr<int>(),
        out.local_prim_id.data_ptr<int>(), out.global_prim_id.data_ptr<int>(), optix_bary_uv.data_ptr<float>(),
        out.p.data_ptr<float>(), out.n.data_ptr<float>(), out.geo_n.data_ptr<float>(), out.uv.data_ptr<float>(),
        out.barycentric.data_ptr<float>(), out.shape_id.data_ptr<int>(), out.prim_id.data_ptr<int>(),
        out.local_prim_id.data_ptr<int>(), out.global_prim_id.data_ptr<int>(), kRayFlagsAll);

    return out;
}

IntersectForwardOutputs intersect_forward_flags_cuda(const SceneCache& scene, const at::Tensor& ray_o,
                                                     const at::Tensor& ray_d, const at::Tensor& ray_tmax,
    ...
    at::Tensor optix_shape_id =
        want_geometric || want_shading || want_uv ? at::empty({ray_count}, iopts) : at::Tensor();
    at::Tensor optix_local_prim_id =
        want_geometric || want_shading || want_uv ? at::empty({ray_count}, iopts) : at::Tensor();
    at::Tensor optix_global_prim_id =
        want_geometric || want_shading || want_uv ? at::empty({ray_count}, iopts) : at::Tensor();
    at::Tensor optix_bary_uv =
        want_geometric || want_shading || want_uv ? at::empty({ray_count, 2}, fopts) : at::Tensor();

    TorchCudaContext torch_ctx = current_torch_cuda_context();
    launch_intersect_backend(scene, ray_o, ray_d, ray_tmax, active, out.t,
                             ...
                             optix_bary_uv.defined() ? optix_bary_uv.data_ptr<float>() : nullptr, torch_ctx.stream);
    if (flags != 0) {
        const int threads = 128;
        const int blocks = static_cast<int>((ray_count + threads - 1) / threads);
        intersect_recompute_kernel<<<blocks, threads, 0, torch_ctx.stream>>>(
            scene.global_vertices.data_ptr<float>(), scene.global_faces.data_ptr<int>(), ray_o.data_ptr<float>(),
            ray_d.data_ptr<float>(), optional_mask_ptr(active), ray_count, out.t.data_ptr<float>(),
            optix_shape_id.data_ptr<int>(), optix_local_prim_id.data_ptr<int>(), optix_global_prim_id.data_ptr<int>(),
            optix_bary_uv.data_ptr<float>(), want_geometric ? out.p.data_ptr<float>() : nullptr,
            want_shading ? out.n.data_ptr<float>() : nullptr, want_geometric ? out.geo_n.data_ptr<float>() : nullptr,
            want_uv ? out.uv.data_ptr<float>() : nullptr, want_geometric ? out.barycentric.data_ptr<float>() : nullptr,
            ...
            want_geometric ? out.global_prim_id.data_ptr<int>() : nullptr, flags);
    }
    return out;
}

IntersectForwardOutputs intersect_forward_ad_flags_cuda(const SceneCache& scene, const at::Tensor& ray_o,
                                                        const at::Tensor& ray_d, const at::Tensor& ray_tmax,
                                                        const at::Tensor& active, int64_t flags) {
    const int64_t ray_count = ray_o.size(0);
    auto fopts = scene.global_vertices.options();
    ...
    TorchCudaContext torch_ctx = current_torch_cuda_context();
    launch_intersect_backend(scene, ray_o, ray_d, ray_tmax, active, out.t, optix_shape_id.data_ptr<int>(),
                             optix_local_prim_id.data_ptr<int>(), optix_global_prim_id.data_ptr<int>(),
                             optix_bary_uv.data_ptr<float>(), torch_ctx.stream);
...
IntersectForwardOutputs intersect_forward_tape_cuda(const SceneCache& scene, const at::Tensor& ray_o,
                                                    const at::Tensor& ray_d, const at::Tensor& ray_tmax,
                                                    const at::Tensor& active) {
    const int64_t ray_count = ray_o.size(0);
    auto fopts = scene.global_vertices.options();
    ...
    launch_intersect_backend(scene, ray_o, ray_d, ray_tmax, active, out.t, nullptr, nullptr,
                             out.tape_prim_id.data_ptr<int>(), nullptr, torch_ctx.stream);
    return out;
}

at::Tensor intersect_forward_t_only_cuda(const SceneCache& scene, const at::Tensor& ray_o, const at::Tensor& ray_d,
    ...
    if (ray_count == 0)
        return out_t;
    launch_intersect_backend(scene, ray_o, ray_d, ray_tmax, active, out_t, nullptr, nullptr, nullptr, nullptr,
                             torch_ctx.stream);
    return out_t;
}
...
__device__ float2 read_vec2_or_zero(const float* base, int64_t index, int64_t stride0, int64_t stride1) {
    return base == nullptr ? make_float2(0.f, 0.f)
                           : make_float2(base[index * stride0 + 0 * stride1], base[index * stride0 + 1 * stride1]);
}
...
__global__ void intersect_backward_kernel(
    ...
    const float* __restrict__ tape_bary, int tape_bary_width, const float* __restrict__ grad_t,
    const float* __restrict__ grad_p, const float* __restrict__ grad_n, const float* __restrict__ grad_geo_n,
    const float* __restrict__ grad_uv, const float* __restrict__ grad_bary, int64_t grad_t_stride0,
    int64_t grad_p_stride0, int64_t grad_p_stride1, int64_t grad_n_stride0, int64_t grad_n_stride1,
    int64_t grad_geo_n_stride0, int64_t grad_geo_n_stride1, int64_t grad_uv_stride0, int64_t grad_uv_stride1,
    int64_t grad_bary_stride0, int64_t grad_bary_stride1, int64_t ray_count, float* __restrict__ grad_vertices,
...
__global__ void intersect_backward_t_kernel(const float* __restrict__ vertices, const int* __restrict__ faces,
                                            ...
                                            const float* __restrict__ grad_t, int64_t grad_t_stride,
                                            float grad_t_constant, bool grad_t_is_constant, int64_t ray_count,
                                            float* __restrict__ grad_vertices, float* __restrict__ grad_ray_o,
                                            float* __restrict__ grad_ray_d, bool need_grad_vertices,
                                            bool need_grad_ray_o, bool need_grad_ray_d) {
    const int ray_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (ray_idx >= ray_count)
        return;
    if (active != nullptr && !active[ray_idx])
...
__global__ void intersect_backward_t_vertices_coop_kernel(
    const float* __restrict__ vertices, const int* __restrict__ faces, const float* __restrict__ ray_o,
    const float* __restrict__ ray_d, const bool* __restrict__ active, const int* __restrict__ tape_prim_id,
    const float* __restrict__ tape_bary, int tape_bary_width, const float* __restrict__ grad_t, int64_t grad_t_stride,
    int64_t vertex_count, int64_t ray_count, float* __restrict__ grad_vertices) {
    namespace cg = cooperative_groups;
...
CoopLaunchConfig coop_launch_config_for_device(int device) {
    ...
    if (status == cudaSuccess && prop.cooperativeLaunch) {
        ...
        status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm,
                                                               intersect_backward_t_vertices_coop_kernel, threads, 0);
...
bool launch_intersect_backward_t_vertices_coop(const at::Tensor& vertices, const at::Tensor& faces,
                                               const at::Tensor& ray_o, const at::Tensor& ray_d,
                                               const at::Tensor& active, const at::Tensor& tape_prim_id,
                                               const at::Tensor& tape_barycentric, const at::Tensor& grad_t,
                                               int64_t grad_t_stride, at::Tensor& grad_vertices, cudaStream_t stream) {
    ...
    int tape_bary_width =
        (!tape_barycentric.defined() || tape_barycentric.numel() == 0) ? 0 : static_cast<int>(tape_barycentric.size(1));
    ...
    const float* tape_bary_ptr = tape_bary_width == 0 ? nullptr : tape_barycentric.data_ptr<float>();
    const float* grad_t_ptr = grad_t.data_ptr<float>();
    float* grad_vertices_ptr = grad_vertices.data_ptr<float>();
    void* kernel_args[] = {
        &vertices_ptr,    &faces_ptr,  &ray_o_ptr,     &ray_d_ptr,    &active_ptr, &tape_prim_id_ptr,  &tape_bary_ptr,
        &tape_bary_width, &grad_t_ptr, &grad_t_stride, &vertex_count, &ray_count,  &grad_vertices_ptr,
    };
    cudaError_t status = cudaLaunchCooperativeKernel(reinterpret_cast<void*>(intersect_backward_t_vertices_coop_kernel),
                                                     blocks, threads, kernel_args, 0, stream);
    if (status != cudaSuccess) {
        cudaGetLastError();
        return false;
    }
...
__global__ void intersect_jvp_kernel(
    const float* __restrict__ vertices, const int* __restrict__ faces, const float* __restrict__ ray_o,
    const float* __restrict__ ray_d, const bool* __restrict__ active, const int* __restrict__ tape_prim_id,
    const float* __restrict__ tape_bary, int tape_bary_width, const float* __restrict__ tangent_vertices,
    const float* __restrict__ tangent_ray_o, const float* __restrict__ tangent_ray_d, int64_t tangent_vertices_stride0,
    int64_t tangent_vertices_stride1, int64_t tangent_ray_o_stride0, int64_t tangent_ray_o_stride1,
    int64_t tangent_ray_d_stride0, int64_t tangent_ray_d_stride1, int64_t ray_count, float* __restrict__ tangent_t,
    float* __restrict__ tangent_p, float* __restrict__ tangent_n, float* __restrict__ tangent_geo_n,
    float* __restrict__ tangent_uv, float* __restrict__ tangent_barycentric, int64_t flags) {
    const int ray_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (ray_idx >= ray_count)
        return;
    const bool want_geometric = (flags & kRayFlagsGeometric) != 0;
    ...
    const float solved_t = solve_columns(mul3(-1.f, d), e1, e2, sub3(make_f3(ray_o + ray_idx * 3), v0)).x;

    const float3 rhs =
        sub3(add3(dorigin, mul3(solved_t, dd)), add3(add3(mul3(bary.x, dv0), mul3(bary.y, dv1)), mul3(bary.z, dv2)));
...
IntersectBackwardOutputs intersect_backward_cuda(const at::Tensor& vertices, const at::Tensor& faces,
                                                 const at::Tensor& ray_o, const at::Tensor& ray_d,
                                                 const at::Tensor& ray_tmax, const at::Tensor& active,
                                                 const at::Tensor& tape_prim_id, const at::Tensor& tape_barycentric,
                                                 const at::Tensor& grad_t, const at::Tensor& grad_p,
                                                 const at::Tensor& grad_n, const at::Tensor& grad_geo_n,
                                                 const at::Tensor& grad_uv, const at::Tensor& grad_barycentric) {
    return intersect_backward_optional_cuda(vertices, faces, ray_o, ray_d, ray_tmax, active, tape_prim_id,
                                            tape_barycentric, &grad_t, &grad_p, &grad_n, &grad_geo_n, &grad_uv,
                                            &grad_barycentric, true, true, true, true);
}
...
IntersectBackwardOutputs intersect_backward_optional_cuda(
    ...
    const at::Tensor& tape_barycentric, const at::Tensor* grad_t, const at::Tensor* grad_p, const at::Tensor* grad_n,
    const at::Tensor* grad_geo_n, const at::Tensor* grad_uv, const at::Tensor* grad_barycentric,
    bool need_grad_vertices, bool need_grad_ray_o, bool need_grad_ray_d, bool need_grad_ray_tmax) {
    (void)ray_tmax;
    ...
    if (ray_count == 0 || (!need_grad_vertices && !need_grad_ray_o && !need_grad_ray_d && !need_grad_ray_tmax)) {
        return out;
    }
    ...
    intersect_backward_kernel<<<blocks, threads, 0, stream>>>(
        vertices.data_ptr<float>(), faces.data_ptr<int>(), ray_o.data_ptr<float>(), ray_d.data_ptr<float>(),
        optional_mask_ptr(active), tape_prim_id.data_ptr<int>(),
        (!tape_barycentric.defined() || tape_barycentric.numel() == 0) ? nullptr : tape_barycentric.data_ptr<float>(),
        (!tape_barycentric.defined() || tape_barycentric.numel() == 0) ? 0 : static_cast<int>(tape_barycentric.size(1)),
        ...
        grad_barycentric == nullptr ? nullptr : grad_barycentric->data_ptr<float>(), optional_stride(grad_t, 0),
        optional_stride(grad_p, 0), optional_stride(grad_p, 1), optional_stride(grad_n, 0), optional_stride(grad_n, 1),
        optional_stride(grad_geo_n, 0), optional_stride(grad_geo_n, 1), optional_stride(grad_uv, 0),
        optional_stride(grad_uv, 1), optional_stride(grad_barycentric, 0), optional_stride(grad_barycentric, 1),
...
IntersectBackwardOutputs intersect_backward_t_cuda(const at::Tensor& vertices, const at::Tensor& faces,
                                                   const at::Tensor& ray_o, const at::Tensor& ray_d,
                                                   const at::Tensor& active, const at::Tensor& tape_prim_id,
                                                   const at::Tensor& tape_barycentric, const at::Tensor& grad_t,
                                                   int64_t grad_t_stride, bool need_grad_vertices, bool need_grad_ray_o,
    ...
    if (need_grad_vertices && !need_grad_ray_o && !need_grad_ray_d && !need_grad_ray_tmax) {
        if (launch_intersect_backward_t_vertices_coop(vertices, faces, ray_o, ray_d, active, tape_prim_id,
                                                      tape_barycentric, grad_t, grad_t_stride, out.grad_vertices,
                                                      stream)) {
            return out;
        }
    }
    ...
    intersect_backward_t_kernel<<<blocks, threads, 0, stream>>>(
        ...
        (!tape_barycentric.defined() || tape_barycentric.numel() == 0) ? nullptr : tape_barycentric.data_ptr<float>(),
        (!tape_barycentric.defined() || tape_barycentric.numel() == 0) ? 0 : static_cast<int>(tape_barycentric.size(1)),
        grad_t.data_ptr<float>(), grad_t_stride, 0.f, false, ray_count,
        need_grad_vertices ? out.grad_vertices.data_ptr<float>() : nullptr,
        need_grad_ray_o ? out.grad_ray_o.data_ptr<float>() : nullptr,
        need_grad_ray_d ? out.grad_ray_d.data_ptr<float>() : nullptr, need_grad_vertices, need_grad_ray_o,
        need_grad_ray_d);
    return out;
}

IntersectJvpOutputs intersect_jvp_cuda(const at::Tensor& vertices, const at::Tensor& faces, const at::Tensor& ray_o,
    ...
    return intersect_jvp_optional_cuda(vertices, faces, ray_o, ray_d, active, tape_prim_id, tape_barycentric,
                                       &tangent_vertices, &tangent_ray_o, &tangent_ray_d, kRayFlagsAll);
}

IntersectJvpOutputs intersect_jvp_optional_cuda(const at::Tensor& vertices, const at::Tensor& faces,
                                                const at::Tensor& ray_o, const at::Tensor& ray_d,
                                                const at::Tensor& active, const at::Tensor& tape_prim_id,
                                                const at::Tensor& tape_barycentric, const at::Tensor* tangent_vertices,
    ...
    intersect_jvp_kernel<<<blocks, threads, 0, stream>>>(
        vertices.data_ptr<float>(), faces.data_ptr<int>(), ray_o.data_ptr<float>(), ray_d.data_ptr<float>(),
        optional_mask_ptr(active), tape_prim_id.data_ptr<int>(), tape_barycentric.data_ptr<float>(),
        ...
        out.tangent_t.data_ptr<float>(), want_geometric ? out.tangent_p.data_ptr<float>() : nullptr,
        ...
        want_geometric ? out.tangent_barycentric.data_ptr<float>() : nullptr, flags);
    return out;
}
