  ...
  public:
    ...
    T* get() {
        if (ptr_ != nullptr) {
            const int device = current_cuda_device();
            if (device != device_) {
                throw_runtime_error_local("cuda_multipath: CudaBuffer was allocated on CUDA device " +
...
__device__ __forceinline__ bvh::CudaBvhView make_view(const CudaMultipathBvh& b) {
    ...
    view.triangles = {b.p0_x, b.p0_y, b.p0_z, b.e1_x, b.e1_y,
                      b.e1_z, b.e2_x, b.e2_y, b.e2_z, static_cast<size_t>(b.primitive_count)};
    view.node_bounds = {b.node_min_x,
                        b.node_min_y,
                        b.node_min_z,
                        b.node_max_x,
                        b.node_max_y,
...
__device__ __forceinline__ bvh::TriangleTraversalScratchView make_scratch(int* stack_nodes, int* overflow,
                                                                          int lane_count) {
...
__device__ __forceinline__ bvh::CudaBvhTraverser make_traverser(const bvh::CudaBvhView& view,
...
__global__ void reflection_trace_kernel(shared::optix::ReflectionTraceParams params, CudaMultipathBvh bvh,
...
__global__ void segment_visibility_kernel(shared::optix::SegmentVisibilityParams params, CudaMultipathBvh bvh,
                                          int variant, int* stack_nodes, int* overflow, int lane_count) {
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    if (lane >= static_cast<unsigned int>(lane_count)) {
        return;
    }
...
__global__ void reflection_accumulation_kernel(AccumParams params, CudaMultipathBvh bvh, int* stack_nodes,
    ...
    const bvh::TriangleTraversalScratchView scratch = make_scratch(stack_nodes, overflow, lane_count);
    const bvh::CudaBvhTraverser traverser = make_traverser(view, scratch, lane);
    multipath::reflection_accumulation_algo<AccumParams, CudaReflectionAccumulationPolicy, bvh::CudaBvhTraverser>(
...
__device__ __forceinline__ bool epc_prim_ignored(const ReflEpcParams& params, int prim, int ig0, int ig1, int ig2) {
    int cand = prim;
...
__device__ __forceinline__ int epc_first_blocker(const bvh::CudaBvhView& view,
                                                 const bvh::TriangleTraversalScratchView& scratch, std::size_t lane,
                                                 const ReflEpcParams& params, math::Vec3f origin, math::Vec3f direction,
                                                 float tmin, float tmax, int ig0, int ig1, int ig2) {
    ...
    for (;;) {
        while (node >= 0 && !bvh::is_leaf_node(view.topology, node)) {
            ...
            float t_left = 0.0f;
            float t_right = 0.0f;
            const bool hit_left = bvh::intersect_node_bounds(view.node_bounds, left, ox, oy, oz, inv_dx, inv_dy, inv_dz,
                                                             tmin, best_t, t_left);
            const bool hit_right = bvh::intersect_node_bounds(view.node_bounds, right, ox, oy, oz, inv_dx, inv_dy,
                                                              inv_dz, tmin, best_t, t_right);
        ...
        if (node >= 0) {
            ...
            for (int slot = 0; slot < leaf_count; ++slot) {
                ...
                const bvh::WatertightTriangleHit hit =
                    bvh::intersect_triangle_watertight(ox, oy, oz, dx, dy, dz, v.ax, v.ay, v.az, v.bx, v.by, v.bz, v.cx,
                                                       v.cy, v.cz, tmin, best_t);
...
struct CudaEpcTraverser {
    ...
    __device__ __forceinline__ rt::TriangleHit trace_first_blocker(math::Vec3f origin, math::Vec3f direction,
        ...
        const int prim = epc_first_blocker(view, scratch, lane, *params, origin, direction, tmin, tmax, ig0, ig1, ig2);
        rt::TriangleHit hit;
        hit.t = tmax;
        hit.bary_u = 0.0f;
        hit.bary_v = 0.0f;
...
static_assert(rt::is_traverser_v<CudaEpcTraverser>, "CudaEpcTraverser must satisfy the rt::Traverser concept.");

template <bool DirectOnly, bool PrimaryOnly>
__global__ void reflection_epc_kernel(ReflEpcParams params, CudaMultipathBvh bvh, int* stack_nodes, int* overflow,
                                      int lane_count) {
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    if (lane >= static_cast<unsigned int>(lane_count)) {
        return;
    ...
    multipath::run_reflection_epc_algo<Config, DirectOnly, PrimaryOnly>(params, lane, traverser, traverser);
}
...
__global__ void dfr_paths_source_visibility_kernel(DfrPathParams params, CudaMultipathBvh bvh, int* stack_nodes,
                                                   int* overflow, int lane_count) {
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    if (lane >= static_cast<unsigned int>(lane_count)) {
        return;
    ...
    const bvh::TriangleTraversalScratchView scratch = make_scratch(stack_nodes, overflow, lane_count);
    const bvh::CudaBvhTraverser traverser = make_traverser(view, scratch, lane);
    multipath::trace_paths_source_visibility_algo<DfrPathParams, bvh::CudaBvhTraverser>(params, lane, traverser,
                                                                                        traverser);
}

__global__ void dfr_paths_target_export_kernel(DfrPathParams params, CudaMultipathBvh bvh, int* stack_nodes,
                                               int* overflow, int lane_count) {
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    if (lane >= static_cast<unsigned int>(lane_count)) {
        return;
    ...
    const bvh::TriangleTraversalScratchView scratch = make_scratch(stack_nodes, overflow, lane_count);
    const bvh::CudaBvhTraverser traverser = make_traverser(view, scratch, lane);
    multipath::trace_paths_target_export_algo<DfrPathParams, bvh::CudaBvhTraverser>(params, lane, traverser, traverser);
}
...
struct CudaDiffractionAccumulationPolicy {
    static __forceinline__ __device__ const DfrAccumParams& params() { return *dfr_accum_params_slot(); }

    static constexpr int kDirect = static_cast<int>(shared::optix::DiffractionStrategyBit::Direct);
    static constexpr int kKeller = static_cast<int>(shared::optix::DiffractionStrategyBit::Keller);
    static constexpr int kSuffix = static_cast<int>(shared::optix::DiffractionStrategyBit::SuffixReflection);
    ...
    static __forceinline__ __device__ bool recursive_active_state(int i) {
        return params().recursive_active_mask == nullptr || params().recursive_active_mask[i] != 0u;
    }

    static __forceinline__ __device__ int state_edge_index_at(int i) { return params().state_edge_index[i]; }
    ...
    static __forceinline__ __device__ float state_edge_t_min_at(int i) { return params().state_edge_t_min[i]; }

    static __forceinline__ __device__ float state_edge_t_max_at(int i) { return params().state_edge_t_max[i]; }

    static __forceinline__ __device__ float sample_edge_weight_for_lane(int state_idx, unsigned int, int sample_count) {
        const float edge_length = fmaxf(state_edge_t_max_at(state_idx) - state_edge_t_min_at(state_idx), 0.f);
        return edge_length / fmaxf(static_cast<float>(sample_count), 1.f);
    }

    static __forceinline__ __device__ int state_prim0_at(int i) { return params().state_prim0[i]; }

    static __forceinline__ __device__ int state_prim1_at(int i) { return params().state_prim1[i]; }

    static __forceinline__ __device__ float state_exterior_angle_at(int i) { return params().state_exterior_angle[i]; }

    static __forceinline__ __device__ float state_src_power_at(int i) { return params().state_src_power[i]; }

    static __forceinline__ __device__ math::Vec3f state_src_at(int i) {
        return math::make_vec3(params().state_src_x[i], params().state_src_y[i], params().state_src_z[i]);
    }
    ...
    static __forceinline__ __device__ math::Vec3f recursive_state_edge_pos_at(int i) {
        return math::make_vec3(params().recursive_state_edge_pos_x[i], params().recursive_state_edge_pos_y[i],
                               params().recursive_state_edge_pos_z[i]);
    }

    static __forceinline__ __device__ math::Vec3f recursive_state_edge_dir_at(int i) {
        return math::make_vec3(params().recursive_state_edge_dir_x[i], params().recursive_state_edge_dir_y[i],
    ...
    static __forceinline__ __device__ int recursive_state_prim0_at(int i) { return params().recursive_state_prim0[i]; }

    static __forceinline__ __device__ int recursive_state_prim1_at(int i) { return params().recursive_state_prim1[i]; }
    ...
    static __forceinline__ __device__ float material_gain_at(int prim) { return params().material_gain[prim]; }

    static __forceinline__ __device__ CellGroup cell_group(int) { return {}; }

    static __forceinline__ __device__ void atomic_add_same_cell(float* base, int i, float value, CellGroup) {
        atomicAdd(base + i, value);
    }

    static __forceinline__ __device__ void atomic_add_same_cell(int* base, int i, int value, CellGroup) {
        atomicAdd(base + i, value);
    }

    static __forceinline__ __device__ void atomic_add_warp(float* base, float value) { atomicAdd(base, value); }

    static __forceinline__ __device__ void atomic_add_warp(int* base, int value) { atomicAdd(base, value); }

    static __forceinline__ __device__ bool stage_order1(unsigned int, int, float, float, bool, bool) { return false; }

    static __forceinline__ __device__ bool stage_coherent(int, int, bool, float, float, float, float, float, float) {
        return false;
    }
};
...
__device__ __forceinline__ CudaDiffractionAccumulationAlgo make_dfr_accum_algo(const CudaMultipathBvh& bvh,
                                                                               int* stack_nodes, int* overflow,
                                                                               int lane_count, unsigned int lane) {
    const bvh::CudaBvhView view = make_view(bvh);
    const bvh::TriangleTraversalScratchView scratch = make_scratch(stack_nodes, overflow, lane_count);
    const bvh::CudaBvhTraverser traverser = make_traverser(view, scratch, lane);
...
__global__ void dfr_accum_order1_kernel(__grid_constant__ const DfrAccumParams params, CudaMultipathBvh bvh,
                                        int* stack_nodes, int* overflow, int lane_count) {
    ...
    make_dfr_accum_algo(bvh, stack_nodes, overflow, lane_count, lane)
        .run_diffraction_order1_accumulation_algo<PrimaryOnly, IncludeCoherent, IncludeDirect, IncludeKeller,
                                                  IncludeSuffix>(lane);
}
...
__global__ void dfr_accum_phase_kernel(__grid_constant__ const DfrAccumParams params, CudaMultipathBvh bvh, int phase,
                                       int* stack_nodes, int* overflow, int lane_count) {
    publish_dfr_accum_params(params);
    const unsigned int lane = blockIdx.x * blockDim.x + threadIdx.x;
    if (lane >= static_cast<unsigned int>(lane_count)) {
    ...
    CudaDiffractionAccumulationAlgo algo = make_dfr_accum_algo(bvh, stack_nodes, overflow, lane_count, lane);
...
void launch_reflection_trace_cuda(const shared::optix::ReflectionTraceParams& params, const CudaMultipathBvh& bvh,
                                  int lane_count) {
    if (lane_count == 0) {
        return;
    }
    try {
        ...
        reflection_trace_kernel<<<block_count(lane_count), kBlockSize, 0, stream_handle.get()>>>(params, bvh,
                                                                                                 scratch.stack.get(),
                                                                                                 scratch.overflow.get(),
                                                                                                 lane_count);
        audit_cuda_kernel_launch("reflection_trace_cuda_kernel", static_cast<uint32_t>(block_count(lane_count)), 1, 1,
...
void launch_segment_visibility_cuda(const shared::optix::SegmentVisibilityParams& params, const CudaMultipathBvh& bvh,
    ...
    try {
        ...
        segment_visibility_kernel<<<block_count(lane_count), kBlockSize, 0, stream_handle.get()>>>(
            params, bvh, static_cast<int>(variant), scratch.stack.get(), scratch.overflow.get(), lane_count);
        audit_cuda_kernel_launch("segment_visibility_cuda_kernel", static_cast<uint32_t>(block_count(lane_count)), 1, 1,
                                 kBlockSize, 1, 1, static_cast<uint64_t>(lane_count));
...
void launch_reflection_accumulation_cuda(const AccumParams& params, const CudaMultipathBvh& bvh, int lane_count) {
    if (lane_count == 0) {
        return;
    }
    try {
        CudaStreamHandle stream_handle;
        ScratchBuffers scratch(lane_count);
        reflection_accumulation_kernel<<<block_count(lane_count), kBlockSize, 0, stream_handle.get()>>>(
            params, bvh, scratch.stack.get(), scratch.overflow.get(), lane_count);
        audit_cuda_kernel_launch("reflection_accumulation_cuda_kernel", static_cast<uint32_t>(block_count(lane_count)),
                                 1, 1, kBlockSize, 1, 1, static_cast<uint64_t>(lane_count));
        ...
        throw_runtime_error_local(std::string("launch_reflection_accumulation_cuda(): ") + error.what());
    }
}

void launch_reflection_epc_cuda(const shared::optix::ReflEpcParams& params, const CudaMultipathBvh& bvh,
                                bool direct_only, bool primary_visibility_only, int lane_count) {
    if (lane_count == 0) {
        return;
    }
    try {
        ...
        if (direct_only && primary_visibility_only) {
            reflection_epc_kernel<true, true><<<blocks, kBlockSize, 0, s>>>(params, bvh, stack, ovf, lane_count);
        } else if (direct_only) {
            reflection_epc_kernel<true, false><<<blocks, kBlockSize, 0, s>>>(params, bvh, stack, ovf, lane_count);
        } else {
            reflection_epc_kernel<false, false><<<blocks, kBlockSize, 0, s>>>(params, bvh, stack, ovf, lane_count);
        }
        audit_cuda_kernel_launch("reflection_epc_cuda_kernel", static_cast<uint32_t>(blocks), 1, 1, kBlockSize, 1, 1,
                                 static_cast<uint64_t>(lane_count));
        check_cuda_last_error("launch_reflection_epc_cuda(): kernel launch failed");
        audit_cuda_stream_synchronize();
        check_cuda_call(cudaStreamSynchronize(s), "launch_reflection_epc_cuda(): stream sync failed");
    } catch (const std::exception& error) {
        throw_runtime_error_local(std::string("launch_reflection_epc_cuda(): ") + error.what());
    }
}

void launch_dfr_paths_cuda(const DfrPathParams& params, const CudaMultipathBvh& bvh, int lane_count) {
    if (lane_count == 0) {
        return;
    }
    try {
        ...
        dfr_paths_source_visibility_kernel<<<blocks, kBlockSize, 0, s>>>(params, bvh, stack, ovf, lane_count);
        audit_cuda_kernel_launch("dfr_paths_source_visibility_cuda_kernel", static_cast<uint32_t>(blocks), 1, 1,
                                 kBlockSize, 1, 1, static_cast<uint64_t>(lane_count));
        check_cuda_last_error("launch_dfr_paths_cuda(): source-visibility launch failed");
        dfr_paths_target_export_kernel<<<blocks, kBlockSize, 0, s>>>(params, bvh, stack, ovf, lane_count);
        audit_cuda_kernel_launch("dfr_paths_target_export_cuda_kernel", static_cast<uint32_t>(blocks), 1, 1, kBlockSize,
                                 1, 1, static_cast<uint64_t>(lane_count));
...
void launch_dfr_accum_phase(const DfrAccumParams& params, DfrAccumPhase phase, const char* audit_name,
                            const CudaMultipathBvh& bvh, int* stack, int* ovf, int lane_count, cudaStream_t stream) {
    const int blocks = block_count(lane_count);
    dfr_accum_phase_kernel<<<blocks, kBlockSize, 0, stream>>>(params, bvh, static_cast<int>(phase), stack, ovf,
                                                              lane_count);
...
void launch_dfr_accum_direct_cuda(const DfrAccumParams& params, const CudaMultipathBvh& bvh,
                                  bool has_non_suffix_strategy, bool has_suffix_strategy, int lane_count) {
    if (lane_count == 0) {
        return;
    }
    try {
        ...
        // on one stream.
        launch_dfr_accum_phase(params, DfrAccumPhase::SourceVisibility, "dfr_accum_source_visibility_cuda_kernel", bvh,
                               stack, ovf, lane_count, s);
        if (has_non_suffix_strategy) {
            launch_dfr_accum_phase(params, DfrAccumPhase::NoSuffixTarget, "dfr_accum_no_suffix_target_cuda_kernel", bvh,
                                   stack, ovf, lane_count, s);
        }
        if (has_suffix_strategy) {
            launch_dfr_accum_phase(params, DfrAccumPhase::SuffixFirstVisibility,
                                   "dfr_accum_suffix_first_visibility_cuda_kernel", bvh, stack, ovf, lane_count, s);
            launch_dfr_accum_phase(params, DfrAccumPhase::SuffixTarget, "dfr_accum_suffix_target_cuda_kernel", bvh,
                                   stack, ovf, lane_count, s);
        }
        audit_cuda_stream_synchronize();
        check_cuda_call(cudaStreamSynchronize(s), "launch_dfr_accum_direct_cuda(): stream sync failed");
    } catch (const std::exception& error) {
        throw_runtime_error_local(std::string("launch_dfr_accum_direct_cuda(): ") + error.what());
    }
}

void launch_dfr_accum_coherent_cuda(const DfrAccumParams& params, const CudaMultipathBvh& bvh, int lane_count) {
    if (lane_count == 0) {
        return;
    }
    try {
        ...
        ScratchBuffers scratch(lane_count);
        const cudaStream_t s = stream_handle.get();
        launch_dfr_accum_phase(params, DfrAccumPhase::Coherent, "dfr_accum_coherent_cuda_kernel", bvh,
                               scratch.stack.get(), scratch.overflow.get(), lane_count, s);
        audit_cuda_stream_synchronize();
        check_cuda_call(cudaStreamSynchronize(s), "launch_dfr_accum_coherent_cuda(): stream sync failed");
    } catch (const std::exception& error) {
        throw_runtime_error_local(std::string("launch_dfr_accum_coherent_cuda(): ") + error.what());
    }
}

void launch_dfr_accum_chain_cuda(const DfrAccumParams& params, const CudaMultipathBvh& bvh, int lane_count) {
    if (lane_count == 0) {
        return;
    }
    try {
        ...
        ScratchBuffers scratch(lane_count);
        const cudaStream_t s = stream_handle.get();
        launch_dfr_accum_phase(params, DfrAccumPhase::Chain, "dfr_accum_chain_cuda_kernel", bvh, scratch.stack.get(),
                               scratch.overflow.get(), lane_count, s);
        audit_cuda_stream_synchronize();
        check_cuda_call(cudaStreamSynchronize(s), "launch_dfr_accum_chain_cuda(): stream sync failed");
    } catch (const std::exception& error) {
...
void launch_dfr_accum_combined_cuda(const DfrAccumParams& params, const CudaMultipathBvh& bvh,
    ...
    try {
        ...
        audit_cuda_kernel_launch("dfr_accum_order1_cuda_kernel", static_cast<uint32_t>(blocks), 1, 1, kBlockSize, 1, 1,
                                 static_cast<uint64_t>(lane_count));
        check_cuda_last_error("launch_dfr_accum_combined_cuda(): kernel launch failed");
        audit_cuda_stream_synchronize();
        check_cuda_call(cudaStreamSynchronize(s), "launch_dfr_accum_combined_cuda(): stream sync failed");
    } catch (const std::exception& error) {
