...
extern "C" __global__ void __raygen__intersect() {
    ...
    if (params.active == nullptr || params.active[ray_idx]) {
        const float3 origin =
            make_float3(params.ray_o[ray_idx * 3 + 0], params.ray_o[ray_idx * 3 + 1], params.ray_o[ray_idx * 3 + 2]);
        const float3 direction =
            make_float3(params.ray_d[ray_idx * 3 + 0], params.ray_d[ray_idx * 3 + 1], params.ray_d[ray_idx * 3 + 2]);
        rayd::shared::optix::SceneIntersectionPayload payload{
            __float_as_uint(t), static_cast<unsigned int>(shape_id),      __float_as_uint(u),
            __float_as_uint(v), static_cast<unsigned int>(local_prim_id),
        };
        const float trace_tmax = params.ray_tmax != nullptr ? params.ray_tmax[ray_idx] : __uint_as_float(0x7f7fffffu);
        optixTrace(params.traversable, origin, direction, rayd::shared::SmallEpsilon, trace_tmax, 0.0f,
                   OptixVisibilityMask(255), OPTIX_RAY_FLAG_DISABLE_ANYHIT, 0, 1, 0, payload.ray_t, payload.shape_id,
                   payload.barycentric_u, payload.barycentric_v, payload.local_primitive_id);
...
extern "C" __global__ void __closesthit__intersect() {
    ...
    rayd::shared::optix::set_scene_intersection_payload(optixGetRayTmax(), optixGetInstanceId(), bary.x, bary.y,
