...
at::Tensor visibility_active_mask(const c10::optional<at::Tensor>& active, int64_t count,
                                  const at::TensorOptions& options, const char* name) {
...
at::Tensor prepare_visibility_ignore_ids(const c10::optional<at::Tensor>& ignore_ids, int64_t row_count, int max_rank,
                                         const char* name, int32_t& ignore_k) {
    ignore_k = 0;
    ...
    if (row_count <= 0 || ignore_ids->numel() % row_count != 0)
        throw std::runtime_error(std::string(name) + " size must be a multiple of its visibility row count.");
...
void launch_segment_visibility_backend(SceneCache& scene, const SegmentVisibilityParams& params, int variant,
                                       unsigned int lane_count, cudaStream_t stream) {
    if (scene.trace_backend == TraceBackend::Cuda) {
        launch_segment_visibility_cuda(scene, params, static_cast<CudaVisibilityVariant>(variant),
                                       static_cast<int>(lane_count));
        return;
    }
    optix_pipeline_for_scene(scene, segment_visibility_pipeline_config())->launch(variant, params, lane_count, stream);
}
...
SegmentVisibilityNativeOutputs visibility_forward_native_impl(SceneCache& scene, at::Tensor start, at::Tensor end,
    ...
    launch_segment_visibility_backend(scene, params, 0, static_cast<unsigned int>(ray_count), torch_ctx.stream);
    return {visible, blocker_prim, tape_t};
}
...
py::tuple visibility_forward_op(int64_t scene_handle, at::Tensor start, at::Tensor end, py::object active_obj) {
    at::Tensor active_storage;
    const at::Tensor* active = optional_tensor(active_obj, active_storage);
    SegmentVisibilityNativeOutputs out = visibility_forward_native_impl(get_scene(scene_handle), start, end, active);
    return py::make_tuple(out.visible, out.blocker_prim, out.tape_t);
}

std::vector<at::Tensor> visible_pair_forward_impl(int64_t scene_handle, at::Tensor start, at::Tensor end_a,
                                                  at::Tensor end_b, c10::optional<at::Tensor> ignore_prim_ids,
    ...
    at::Tensor active_mask = visibility_active_mask(active, ray_count, start.options(), "active");
    active_mask =
        (active_mask & finite_vec3_rows(start) & finite_vec3_rows(end_a) & finite_vec3_rows(end_b)).contiguous();
    int32_t ignore_k = 0;
    at::Tensor ignore_ids = prepare_visibility_ignore_ids(ignore_prim_ids, ray_count, 2, "ignore_prim_ids", ignore_k);
    ...
    TorchCudaContext torch_ctx = current_torch_cuda_context();
    launch_segment_visibility_backend(scene, params, 1, static_cast<unsigned int>(ray_count), torch_ctx.stream);
    return {visible_a, visible_b};
}

std::vector<at::Tensor> visible_edge_forward_impl(int64_t scene_handle, at::Tensor source, at::Tensor edge_position,
    ...
    if (!std::all_of(sample_fractions.begin(), sample_fractions.end(),
    ...
    if (edge_t_min.size(0) != source.size(0) || edge_t_max.size(0) != source.size(0))
    ...
    at::Tensor any_visible = at::empty({state_count}, source.options().dtype(at::kBool));
    if (state_count == 0)
        return {any_visible};

    at::Tensor active_mask = visibility_active_mask(active, state_count, source.options(), "active");
    active_mask = (active_mask & finite_vec3_rows(source) & finite_vec3_rows(edge_position) &
                   finite_vec3_rows(edge_direction) & at::isfinite(edge_t_min) & at::isfinite(edge_t_max))
                      .contiguous();
    ...
    TorchCudaContext torch_ctx = current_torch_cuda_context();
    launch_segment_visibility_backend(scene, params, 2, static_cast<unsigned int>(state_count), torch_ctx.stream);
    return {any_visible};
}

std::vector<at::Tensor> visible_chain_forward_impl(int64_t scene_handle, at::Tensor points, at::Tensor chain_length,
    ...
    if (points.size(0) != chain_length.size(0))
        throw std::runtime_error("visible_chain points and chain_length must have the same batch size.");
    ...
    at::Tensor all_visible = at::empty({chain_count}, points.options().dtype(at::kBool));
    at::Tensor first_blocked_segment = at::empty({chain_count}, chain_length.options());
    at::Tensor first_blocked_prim = at::empty({chain_count}, chain_length.options());
    if (chain_count == 0)
    ...
    at::Tensor active_mask = visibility_active_mask(active, chain_count, points.options(), "active");
    active_mask = (active_mask & chain_length.ge(0) & at::isfinite(points).all(2).all(1)).contiguous();

    int32_t ignore_k = 0;
    at::Tensor ignore_ids = prepare_visibility_ignore_ids(ignore_prim_per_segment, chain_count * max_segments, 3,
                                                          "ignore_prim_per_segment", ignore_k);
    ...
    TorchCudaContext torch_ctx = current_torch_cuda_context();
    launch_segment_visibility_backend(scene, params, 3, static_cast<unsigned int>(chain_count), torch_ctx.stream);
    return {all_visible, first_blocked_segment, first_blocked_prim};
}

at::Tensor axial_edge_visibility_forward_native_impl(
    SceneCache& scene, const at::Tensor& tx, const at::Tensor& edge_position, const at::Tensor& edge_direction,
    const at::Tensor& edge_t_min, const at::Tensor& edge_t_max, const at::Tensor* active,
    const std::array<std::uint32_t, AxialEdgeVisibilitySampleCount>& sample_fraction_bits) {
    if (scene.trace_backend == TraceBackend::Cuda)
        throw std::runtime_error("ADR-0029 axial-edge visibility is unsupported by the CUDA ray-tracing backend; "
    ...
    if (edge_direction.size(0) != state_count || edge_t_min.size(0) != state_count ||
        edge_t_max.size(0) != state_count) {
        throw std::runtime_error("axial-edge visibility inputs must have the same state count.");
    }
    if (active != nullptr) {
        require_mask(*active, "active");
        require_contiguous(*active, "active");
        if (active->size(0) != state_count) {
            throw std::runtime_error("active must match the axial-edge state count.");
        }
    }
    ...
    for (std::size_t index = 0; index < sample_fractions.size(); ++index) {
        ...
        if (!std::isfinite(sample_fractions[index])) {
            throw std::runtime_error("axial-edge visibility fraction bits must encode finite float32 values.");
        }
    }

    c10::cuda::CUDAGuard guard(static_cast<c10::DeviceIndex>(scene.device_index));
    at::Tensor any_visible = at::empty({state_count}, edge_position.options().dtype(at::kBool));
    if (state_count == 0)
        return any_visible;
...
VisibilityResult visibility_forward(const SceneResource& scene, const VisibilityRequest& request) {
    ...
    auto out = torch_backend::visibility_forward_native_impl(cache, request.start, request.end,
...
AxialEdgeVisibilityResult axial_edge_visibility_forward(const SceneResource& scene,
    ...
    auto& cache = detail::IntegrationAccess::scene_cache(scene);
    return {torch_backend::axial_edge_visibility_forward_native_impl(cache, request.tx, request.edge_position,
                                                                     request.edge_direction, request.edge_t_min,
                                                                     request.edge_t_max,
                                                                     present_optional(request.active),
