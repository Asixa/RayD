...
__device__ float3 edge_start(const float* __restrict__ p0_x, const float* __restrict__ p0_y,
                             const float* __restrict__ p0_z, int edge) {
    return make_float3(p0_x[edge], p0_y[edge], p0_z[edge]);
}

__device__ float3 edge_vector(const float* __restrict__ e1_x, const float* __restrict__ e1_y,
                              const float* __restrict__ e1_z, int edge) {
...
void require_edge_cache_dtype(const at::Tensor& tensor, at::ScalarType dtype, const char* name) {
    if (tensor.scalar_type() != dtype)
        throw std::runtime_error(std::string("edge cache tensor ") + name + " has dtype " +
...
void fill_edge_tiers(const SceneCache& scene, EdgeOptixQueryParams& params) {
    if (scene.edge_accels.size() > static_cast<std::size_t>(EdgeOptixMaxTiers)) {
        throw std::runtime_error("edge query has more GAS tiers than EdgeOptixMaxTiers; increase the OptiX params "
                                 "tier array size.");
    }
...
void fill_edge_geometry_params(const SceneCache& scene, int64_t edge_count, EdgeOptixQueryParams& params) {
...
__global__ void init_edge_point_outputs_kernel(int64_t point_count, float* __restrict__ distance,
                                               float* __restrict__ edge_point, float* __restrict__ edge_t,
                                               int* __restrict__ shape_id, int* __restrict__ edge_id,
                                               int* __restrict__ global_edge_id, int* __restrict__ tape_edge_id,
                                               float* __restrict__ tape_s, float* __restrict__ tape_d,
...
__global__ void init_edge_point_public_outputs_kernel(int64_t point_count, float* __restrict__ distance,
                                                      float* __restrict__ edge_point, float* __restrict__ edge_t,
                                                      int* __restrict__ shape_id, int* __restrict__ edge_id,
                                                      int* __restrict__ global_edge_id, bool* __restrict__ unresolved) {
    const int point_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (point_idx >= point_count)
        return;
    distance[point_idx] = CUDART_INF_F;
...
__global__ void split_vec3_to_soa_kernel(const float* __restrict__ value, int64_t count, int64_t stride0,
                                         int64_t stride1, float* __restrict__ x, float* __restrict__ y,
...
__global__ void split_ray_vec3_to_soa_kernel(const float* __restrict__ ray_o, const float* __restrict__ ray_d,
                                             int64_t count, int64_t ray_o_stride0, int64_t ray_o_stride1,
                                             int64_t ray_d_stride0, int64_t ray_d_stride1, float* __restrict__ query_x,
                                             float* __restrict__ query_y, float* __restrict__ query_z,
                                             float* __restrict__ ray_dx, float* __restrict__ ray_dy,
...
__global__ void init_edge_ray_outputs_kernel(const bool* __restrict__ active, int64_t ray_count,
                                             float* __restrict__ distance, float* __restrict__ ray_t,
                                             float* __restrict__ point, float* __restrict__ edge_t,
                                             float* __restrict__ edge_point, int* __restrict__ shape_id,
                                             int* __restrict__ edge_id, int* __restrict__ global_edge_id,
                                             int* __restrict__ tape_edge_id, bool* __restrict__ unresolved) {
...
__global__ void finalize_edge_ray_stage_kernel(
    const float* __restrict__ p0_x, const float* __restrict__ p0_y, const float* __restrict__ p0_z,
    const float* __restrict__ e1_x, const float* __restrict__ e1_y, const float* __restrict__ e1_z,
    const int* __restrict__ edge_shape_id, const int* __restrict__ edge_local_id, const float* __restrict__ ray_o,
    const float* __restrict__ ray_d, const int* __restrict__ stage_edge_id, const float* __restrict__ stage_distance_sq,
    const float* __restrict__ stage_ray_t, const float* __restrict__ stage_edge_t, const bool* __restrict__ stage_valid,
    int64_t ray_count, float* __restrict__ distance, float* __restrict__ ray_t, float* __restrict__ point,
    float* __restrict__ edge_t, float* __restrict__ edge_point, int* __restrict__ shape_id, int* __restrict__ edge_id,
    int* __restrict__ global_edge_id, int* __restrict__ tape_edge_id, bool* __restrict__ unresolved) {
    const int ray_idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void edge_point_bruteforce_kernel(
    const float* __restrict__ query_x, const float* __restrict__ query_y, const float* __restrict__ query_z,
    const bool* __restrict__ unresolved_mask, int point_count, const float* __restrict__ p0_x,
    const float* __restrict__ p0_y, const float* __restrict__ p0_z, const float* __restrict__ e1_x,
    const float* __restrict__ e1_y, const float* __restrict__ e1_z, const uint8_t* __restrict__ edge_mask,
    const int* __restrict__ edge_shape_id, const int* __restrict__ edge_local_id, int edge_count, float max_radius,
    float* __restrict__ final_distance, float* __restrict__ final_edge_point, float* __restrict__ final_edge_t,
    int* __restrict__ final_shape_id, int* __restrict__ final_edge_id, int* __restrict__ final_global_edge_id,
    int* __restrict__ final_tape_edge_id, float* __restrict__ final_tape_s, float* __restrict__ final_tape_d,
        ...
        if (active) {
            for (int j = 0; j < tile_n; ++j) {
                ...
                const shared::edge::PointSegmentDistance candidate =
                    shared::edge::point_segment_distance(shared::math::make_vec3(qx, qy, qz),
...
void launch_edge_query(OptixDeviceContextEntry& optix_entry, cudaStream_t stream, const EdgeOptixQueryParams& params,
                       EdgeOptixLaunchKind kind, int64_t query_count) {
    at::Tensor params_buffer =
        at::empty({static_cast<int64_t>(sizeof(EdgeOptixQueryParams))},
                  at::TensorOptions().device(at::Device(at::kCUDA, optix_entry.device_index)).dtype(at::kByte));
    cuda_check(cudaMemcpyAsync(params_buffer.data_ptr<uint8_t>(), &params, sizeof(EdgeOptixQueryParams),
                               cudaMemcpyHostToDevice, stream),
               "cudaMemcpyAsync(edge OptiX params)");
    rayd_torch_OPTIX_CHECK(optixLaunch(edge_pipeline(optix_entry, kind), stream,
                                       reinterpret_cast<CUdeviceptr>(params_buffer.data_ptr<uint8_t>()),
                                       sizeof(EdgeOptixQueryParams), &edge_sbt(optix_entry, kind),
...
EdgeForwardOutputs edge_forward_cuda(const SceneCache& scene, const at::Tensor& point) {
    ...
    init_edge_point_outputs_kernel<<<blocks, threads, 0, torch_ctx.stream>>>(
        point_count, out.distance.data_ptr<float>(), out.edge_point.data_ptr<float>(), out.edge_t.data_ptr<float>(),
        out.shape_id.data_ptr<int>(), out.edge_id.data_ptr<int>(), out.global_edge_id.data_ptr<int>(),
        out.tape_edge_id.data_ptr<int>(), out.tape_s.data_ptr<float>(), out.tape_d.data_ptr<float>(),
    ...
    split_vec3_to_soa_kernel<<<blocks, threads, 0, torch_ctx.stream>>>(point.data_ptr<float>(), point_count,
                                                                       point.stride(0), point.stride(1),
    ...
    if (params.tier_count > 0) {
        launch_edge_query(optix_entry, torch_ctx.stream, params, EdgeOptixLaunchKind::Point, point_count);
    }
    const int brute_blocks = static_cast<int>((point_count + kEdgeBruteTile - 1) / kEdgeBruteTile);
    edge_point_bruteforce_kernel<<<brute_blocks, kEdgeBruteTile, 0, torch_ctx.stream>>>(
        query_x.data_ptr<float>(), query_y.data_ptr<float>(), query_z.data_ptr<float>(), unresolved.data_ptr<bool>(),
        static_cast<int>(point_count), scene.edge_p0_x.data_ptr<float>(), scene.edge_p0_y.data_ptr<float>(),
        scene.edge_p0_z.data_ptr<float>(), scene.edge_e1_x.data_ptr<float>(), scene.edge_e1_y.data_ptr<float>(),
        scene.edge_e1_z.data_ptr<float>(), scene.edge_mask.data_ptr<uint8_t>(), scene.edge_shape_id.data_ptr<int>(),
        scene.edge_local_id.data_ptr<int>(), static_cast<int>(edge_count), full_search_radius,
        out.distance.data_ptr<float>(), out.edge_point.data_ptr<float>(), out.edge_t.data_ptr<float>(),
        out.shape_id.data_ptr<int>(), out.edge_id.data_ptr<int>(), out.global_edge_id.data_ptr<int>(),
        out.tape_edge_id.data_ptr<int>(), out.tape_s.data_ptr<float>(), out.tape_d.data_ptr<float>(),
        unresolved.data_ptr<bool>());
    return out;
...
EdgeForwardPublicOutputs edge_forward_noad_cuda(const SceneCache& scene, const at::Tensor& point) {
    ...
    const int blocks = static_cast<int>((point_count + threads - 1) / threads);
    at::Tensor unresolved = at::empty({point_count}, bopts);
    init_edge_point_public_outputs_kernel<<<blocks, threads, 0, torch_ctx.stream>>>(point_count,
    ...
    edge_point_bruteforce_kernel<<<brute_blocks, kEdgeBruteTile, 0, torch_ctx.stream>>>(
        ...
        scene.edge_local_id.data_ptr<int>(), static_cast<int>(edge_count), full_search_radius,
        out.distance.data_ptr<float>(), out.edge_point.data_ptr<float>(), out.edge_t.data_ptr<float>(),
        out.shape_id.data_ptr<int>(), out.edge_id.data_ptr<int>(), out.global_edge_id.data_ptr<int>(), nullptr, nullptr,
        nullptr, unresolved.data_ptr<bool>());
    return out;
}

EdgeRayForwardOutputs edge_ray_forward_cuda(const SceneCache& scene, const at::Tensor& ray_o, const at::Tensor& ray_d,
    ...
    init_edge_ray_outputs_kernel<<<blocks, threads, 0, torch_ctx.stream>>>(
        active_c.numel() == 0 ? nullptr : active_c.data_ptr<bool>(), ray_count, out.distance.data_ptr<float>(),
        out.ray_t.data_ptr<float>(), out.point.data_ptr<float>(), out.edge_t.data_ptr<float>(),
        out.edge_point.data_ptr<float>(), out.shape_id.data_ptr<int>(), out.edge_id.data_ptr<int>(),
        out.global_edge_id.data_ptr<int>(), out.tape_edge_id.data_ptr<int>(), unresolved.data_ptr<bool>());
    ...
    split_ray_vec3_to_soa_kernel<<<blocks, threads, 0, torch_ctx.stream>>>(
        ray_o.data_ptr<float>(), ray_d.data_ptr<float>(), ray_count, ray_o.stride(0), ray_o.stride(1), ray_d.stride(0),
        ray_d.stride(1), query_x.data_ptr<float>(), query_y.data_ptr<float>(), query_z.data_ptr<float>(),
        ray_dx.data_ptr<float>(), ray_dy.data_ptr<float>(), ray_dz.data_ptr<float>());
    ...
    finalize_edge_ray_stage_kernel<<<blocks, threads, 0, torch_ctx.stream>>>(
        ...
        scene.edge_shape_id.data_ptr<int>(), scene.edge_local_id.data_ptr<int>(), ray_o.data_ptr<float>(),
        ray_d.data_ptr<float>(), stage_edge_id.data_ptr<int>(), stage_distance_sq.data_ptr<float>(),
        stage_ray_t.data_ptr<float>(), stage_edge_t.data_ptr<float>(), stage_valid.data_ptr<bool>(), ray_count,
        out.distance.data_ptr<float>(), out.ray_t.data_ptr<float>(), out.point.data_ptr<float>(),
        out.edge_t.data_ptr<float>(), out.edge_point.data_ptr<float>(), out.shape_id.data_ptr<int>(),
...
__global__ void edge_backward_kernel(const float* __restrict__ vertices, const int* __restrict__ edge_v0,
                                     const int* __restrict__ edge_v1, const float* __restrict__ point,
                                     const int* __restrict__ tape_edge_id, const float* __restrict__ tape_s,
                                     const float* __restrict__ tape_d, const float* __restrict__ grad_distance,
                                     const float* __restrict__ grad_edge_point, const float* __restrict__ grad_edge_t,
                                     const float* __restrict__ grad_edge_t_alias, int64_t grad_distance_stride0,
                                     int64_t grad_edge_point_stride0, int64_t grad_edge_point_stride1,
                                     int64_t grad_edge_t_stride0, int64_t grad_edge_t_alias_stride0,
                                     int64_t point_count, float* __restrict__ grad_vertices,
    ...
    const float edge_parameter_bar = read_scalar_or_zero(grad_edge_t, point_idx, grad_edge_t_stride0) +
                                     read_scalar_or_zero(grad_edge_t_alias, point_idx, grad_edge_t_alias_stride0);
    const shared::edge::PointSegmentVjp vjp =
        shared::edge::point_segment_vjp_fixed_winner(to_shared_vec3(p), to_shared_vec3(a), to_shared_vec3(b), s,
                                                     to_shared_vec3(d), distance_bar, to_shared_vec3(gep),
                                                     edge_parameter_bar);
...
__global__ void edge_jvp_kernel(const float* __restrict__ vertices, const int* __restrict__ edge_v0,
                                const int* __restrict__ edge_v1, const float* __restrict__ point,
                                const int* __restrict__ tape_edge_id, const float* __restrict__ tape_s,
                                const float* __restrict__ tape_d, const float* __restrict__ tangent_vertices,
                                const float* __restrict__ tangent_point, int64_t tangent_vertices_stride0,
                                int64_t tangent_vertices_stride1, int64_t tangent_point_stride0,
                                int64_t tangent_point_stride1, int64_t point_count,
                                float* __restrict__ tangent_distance, float* __restrict__ tangent_edge_point,
                                float* __restrict__ tangent_edge_t, float* __restrict__ tangent_tape_s,
    ...
    const shared::edge::PointSegmentJvp jvp =
        shared::edge::point_segment_jvp_fixed_winner(to_shared_vec3(p), to_shared_vec3(a), to_shared_vec3(b), s,
                                                     to_shared_vec3(d), to_shared_vec3(dp), to_shared_vec3(da),
                                                     to_shared_vec3(db));
...
__global__ void edge_ray_backward_kernel(
    const float* __restrict__ vertices, const int* __restrict__ edge_v0, const int* __restrict__ edge_v1,
    const float* __restrict__ ray_o, const float* __restrict__ ray_d, const float* __restrict__ ray_tmax,
    const int* __restrict__ tape_edge_id, const float* __restrict__ ray_t, const float* __restrict__ edge_t,
    const float* __restrict__ grad_distance, const float* __restrict__ grad_ray_t, const float* __restrict__ grad_point,
    const float* __restrict__ grad_edge_t, const float* __restrict__ grad_edge_point, int64_t grad_distance_stride0,
    int64_t grad_ray_t_stride0, int64_t grad_point_stride0, int64_t grad_point_stride1, int64_t grad_edge_t_stride0,
    int64_t grad_edge_point_stride0, int64_t grad_edge_point_stride1, int64_t ray_count,
    float* __restrict__ grad_vertices, float* __restrict__ grad_ray_o, float* __restrict__ grad_ray_d) {
    const int ray_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (ray_idx >= ray_count)
        return;
    for (int axis = 0; axis < 3; ++axis) {
    ...
    const float3 a = make_f3(vertices + i0 * 3);
    const float3 b = make_f3(vertices + i1 * 3);
    const bool has_max = ray_tmax != nullptr && isfinite(ray_tmax[ray_idx]) && ray_tmax[ray_idx] > 0.f;
    const shared::edge::RaySegmentVjp vjp = shared::edge::ray_segment_vjp_fixed_winner(
        to_shared_vec3(ro), to_shared_vec3(rd), to_shared_vec3(a), to_shared_vec3(b), ray_t[ray_idx], edge_t[ray_idx],
        has_max, has_max ? ray_tmax[ray_idx] : shared::edge::EdgeDistanceFloatMax,
        read_scalar_or_zero(grad_distance, ray_idx, grad_distance_stride0),
        read_scalar_or_zero(grad_ray_t, ray_idx, grad_ray_t_stride0),
        to_shared_vec3(read_vec3_or_zero(grad_point, ray_idx, grad_point_stride0, grad_point_stride1)),
        read_scalar_or_zero(grad_edge_t, ray_idx, grad_edge_t_stride0),
        to_shared_vec3(read_vec3_or_zero(grad_edge_point, ray_idx, grad_edge_point_stride0, grad_edge_point_stride1)));
...
__global__ void edge_ray_jvp_kernel(const float* __restrict__ vertices, const int* __restrict__ edge_v0,
                                    const int* __restrict__ edge_v1, const float* __restrict__ ray_o,
                                    const float* __restrict__ ray_d, const float* __restrict__ ray_tmax,
                                    const int* __restrict__ tape_edge_id, const float* __restrict__ ray_t,
                                    const float* __restrict__ edge_t, const float* __restrict__ tangent_vertices,
                                    const float* __restrict__ tangent_ray_o, const float* __restrict__ tangent_ray_d,
                                    int64_t tangent_vertices_stride0, int64_t tangent_vertices_stride1,
                                    int64_t tangent_ray_o_stride0, int64_t tangent_ray_o_stride1,
                                    int64_t tangent_ray_d_stride0, int64_t tangent_ray_d_stride1, int64_t ray_count,
                                    float* __restrict__ tangent_distance, float* __restrict__ tangent_ray_t,
                                    float* __restrict__ tangent_point, float* __restrict__ tangent_edge_t,
    ...
    const int i1 = edge_v1[edge_id];
    const bool has_max = ray_tmax != nullptr && isfinite(ray_tmax[ray_idx]) && ray_tmax[ray_idx] > 0.f;
    const shared::edge::RaySegmentJvp jvp = shared::edge::ray_segment_jvp_fixed_winner(
        to_shared_vec3(make_f3(ray_o + ray_idx * 3)), to_shared_vec3(make_f3(ray_d + ray_idx * 3)),
        to_shared_vec3(make_f3(vertices + i0 * 3)), to_shared_vec3(make_f3(vertices + i1 * 3)), ray_t[ray_idx],
        edge_t[ray_idx], has_max, has_max ? ray_tmax[ray_idx] : shared::edge::EdgeDistanceFloatMax,
        to_shared_vec3(read_vec3_or_zero(tangent_ray_o, ray_idx, tangent_ray_o_stride0, tangent_ray_o_stride1)),
        to_shared_vec3(read_vec3_or_zero(tangent_ray_d, ray_idx, tangent_ray_d_stride0, tangent_ray_d_stride1)),
        to_shared_vec3(read_vec3_or_zero(tangent_vertices, i0, tangent_vertices_stride0, tangent_vertices_stride1)),
        to_shared_vec3(read_vec3_or_zero(tangent_vertices, i1, tangent_vertices_stride0, tangent_vertices_stride1)));
...
// guard of its own.
EdgeBackwardOutputs edge_backward_cuda(const at::Tensor& vertices, const at::Tensor& edge_v0, const at::Tensor& edge_v1,
                                       const at::Tensor& point, const at::Tensor& tape_edge_id,
                                       const at::Tensor& tape_s, const at::Tensor& tape_d,
                                       const at::Tensor& grad_distance, const at::Tensor& grad_edge_point,
                                       const at::Tensor& grad_edge_t) {
    return edge_backward_optional_cuda(vertices, edge_v0, edge_v1, point, tape_edge_id, tape_s, tape_d, &grad_distance,
                                       &grad_edge_point, &grad_edge_t, nullptr);
}

EdgeBackwardOutputs edge_backward_optional_cuda(const at::Tensor& vertices, const at::Tensor& edge_v0,
                                                const at::Tensor& edge_v1, const at::Tensor& point,
                                                const at::Tensor& tape_edge_id, const at::Tensor& tape_s,
                                                const at::Tensor& tape_d, const at::Tensor* grad_distance,
                                                const at::Tensor* grad_edge_point, const at::Tensor* grad_edge_t,
    ...
    edge_backward_kernel<<<blocks, threads, 0, stream>>>(
        vertices.data_ptr<float>(), edge_v0.data_ptr<int>(), edge_v1.data_ptr<int>(), point.data_ptr<float>(),
        tape_edge_id.data_ptr<int>(), tape_s.data_ptr<float>(), tape_d.data_ptr<float>(),
        ...
        optional_stride(grad_distance, 0), optional_stride(grad_edge_point, 0), optional_stride(grad_edge_point, 1),
        optional_stride(grad_edge_t, 0), optional_stride(grad_edge_t_alias, 0), point_count,
        out.grad_vertices.data_ptr<float>(), out.grad_point.data_ptr<float>());
    return out;
}

EdgeJvpOutputs edge_jvp_cuda(const at::Tensor& vertices, const at::Tensor& edge_v0, const at::Tensor& edge_v1,
                             const at::Tensor& point, const at::Tensor& tape_edge_id, const at::Tensor& tape_s,
                             const at::Tensor& tape_d, const at::Tensor& tangent_vertices,
                             const at::Tensor& tangent_point) {
    return edge_jvp_optional_cuda(vertices, edge_v0, edge_v1, point, tape_edge_id, tape_s, tape_d, &tangent_vertices,
                                  &tangent_point);
}

EdgeJvpOutputs edge_jvp_optional_cuda(const at::Tensor& vertices, const at::Tensor& edge_v0, const at::Tensor& edge_v1,
                                      const at::Tensor& point, const at::Tensor& tape_edge_id, const at::Tensor& tape_s,
                                      const at::Tensor& tape_d, const at::Tensor* tangent_vertices,
                                      const at::Tensor* tangent_point) {
    const int64_t point_count = point.size(0);
    ...
    edge_jvp_kernel<<<blocks, threads, 0, stream>>>(
        ...
        tangent_point == nullptr ? nullptr : tangent_point->data_ptr<float>(), optional_stride(tangent_vertices, 0),
        optional_stride(tangent_vertices, 1), optional_stride(tangent_point, 0), optional_stride(tangent_point, 1),
        point_count, out.tangent_distance.data_ptr<float>(), out.tangent_edge_point.data_ptr<float>(),
        out.tangent_edge_t.data_ptr<float>(), out.tangent_tape_s.data_ptr<float>(),
        out.tangent_tape_d.data_ptr<float>());
    return out;
}

EdgeRayBackwardOutputs edge_ray_backward_optional_cuda(
    const at::Tensor& vertices, const at::Tensor& edge_v0, const at::Tensor& edge_v1, const at::Tensor& ray_o,
    const at::Tensor& ray_d, const at::Tensor& ray_tmax, const at::Tensor& tape_edge_id, const at::Tensor& ray_t,
    const at::Tensor& edge_t, const at::Tensor* grad_distance, const at::Tensor* grad_ray_t,
    const at::Tensor* grad_point, const at::Tensor* grad_edge_t, const at::Tensor* grad_edge_point) {
    const int64_t ray_count = ray_o.size(0);
    ...
    edge_ray_backward_kernel<<<blocks, threads, 0, stream>>>(
        vertices.data_ptr<float>(), edge_v0.data_ptr<int>(), edge_v1.data_ptr<int>(), ray_o.data_ptr<float>(),
        ray_d.data_ptr<float>(), ray_tmax.numel() == 0 ? nullptr : ray_tmax.data_ptr<float>(),
        tape_edge_id.data_ptr<int>(), ray_t.data_ptr<float>(), edge_t.data_ptr<float>(),
        grad_distance == nullptr ? nullptr : grad_distance->data_ptr<float>(),
        grad_ray_t == nullptr ? nullptr : grad_ray_t->data_ptr<float>(),
        grad_point == nullptr ? nullptr : grad_point->data_ptr<float>(),
        grad_edge_t == nullptr ? nullptr : grad_edge_t->data_ptr<float>(),
        grad_edge_point == nullptr ? nullptr : grad_edge_point->data_ptr<float>(), optional_stride(grad_distance, 0),
        optional_stride(grad_ray_t, 0), optional_stride(grad_point, 0), optional_stride(grad_point, 1),
        optional_stride(grad_edge_t, 0), optional_stride(grad_edge_point, 0), optional_stride(grad_edge_point, 1),
        ray_count, out.grad_vertices.data_ptr<float>(), out.grad_ray_o.data_ptr<float>(),
        out.grad_ray_d.data_ptr<float>());
    return out;
}

EdgeRayJvpOutputs edge_ray_jvp_optional_cuda(const at::Tensor& vertices, const at::Tensor& edge_v0,
                                             const at::Tensor& edge_v1, const at::Tensor& ray_o,
                                             const at::Tensor& ray_d, const at::Tensor& ray_tmax,
                                             const at::Tensor& tape_edge_id, const at::Tensor& ray_t,
                                             const at::Tensor& edge_t, const at::Tensor* tangent_vertices,
    ...
    edge_ray_jvp_kernel<<<blocks, threads, 0, stream>>>(
        ...
        tangent_ray_d == nullptr ? nullptr : tangent_ray_d->data_ptr<float>(), optional_stride(tangent_vertices, 0),
        optional_stride(tangent_vertices, 1), optional_stride(tangent_ray_o, 0), optional_stride(tangent_ray_o, 1),
        optional_stride(tangent_ray_d, 0), optional_stride(tangent_ray_d, 1), ray_count,
        out.tangent_distance.data_ptr<float>(), out.tangent_ray_t.data_ptr<float>(),
        out.tangent_point.data_ptr<float>(), out.tangent_edge_t.data_ptr<float>(),
...
__global__ void prepare_topk_queries_kernel(const float* __restrict__ point, const bool* __restrict__ active,
                                            int64_t query_count, int64_t point_stride0, int64_t point_stride1,
                                            float* __restrict__ query_x, float* __restrict__ query_y,
                                            float* __restrict__ query_z, std::uint8_t* __restrict__ query_active) {
    ...
    query_y[query] = y;
    query_z[query] = z;
    query_active[query] = (active == nullptr || active[query]) && isfinite(x) && isfinite(y) && isfinite(z) ? 1u : 0u;
}
...
__global__ void repair_topk_overflow_kernel(
    const float* __restrict__ query_x, const float* __restrict__ query_y, const float* __restrict__ query_z,
    const std::uint8_t* __restrict__ query_active, int64_t query_count, int k, const float* __restrict__ edge_p0_x,
    const float* __restrict__ edge_p0_y, const float* __restrict__ edge_p0_z, const float* __restrict__ edge_e1_x,
    const float* __restrict__ edge_e1_y, const float* __restrict__ edge_e1_z,
    const std::uint8_t* __restrict__ edge_mask, int64_t edge_count, std::uint8_t* __restrict__ overflow,
    int* __restrict__ candidate_edge, float* __restrict__ candidate_distance_sq, float* __restrict__ candidate_edge_t) {
    const int64_t query = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
...
#pragma unroll
    ...
    if (query_active[query] != 0u) {
        const shared::math::Vec3f point = shared::math::make_vec3(query_x[query], query_y[query], query_z[query]);
        for (int edge = 0; edge < edge_count; ++edge) {
            if (edge_mask != nullptr && edge_mask[edge] == 0u)
                continue;
            const shared::edge::PointSegmentDistance candidate =
                shared::edge::point_segment_distance(point,
                                                     shared::math::make_vec3(edge_p0_x[edge], edge_p0_y[edge],
                                                                             edge_p0_z[edge]),
                                                     shared::math::make_vec3(edge_e1_x[edge], edge_e1_y[edge],
                                                                             edge_e1_z[edge]));
            ...
            for (int rank = 0; rank < k; ++rank) {
                if (!candidate_precedes(displaced_distance, displaced_edge, distances[rank], edges[rank]))
                    continue;
...
__global__ void finalize_topk_kernel(
    const float* __restrict__ query_x, const float* __restrict__ query_y, const float* __restrict__ query_z,
    const std::uint8_t* __restrict__ query_active, int64_t query_count, int k, const int* __restrict__ candidate_edge,
    const float* __restrict__ candidate_distance_sq, const float* __restrict__ candidate_edge_t,
    const float* __restrict__ edge_p0_x, const float* __restrict__ edge_p0_y, const float* __restrict__ edge_p0_z,
    const float* __restrict__ edge_e1_x, const float* __restrict__ edge_e1_y, const float* __restrict__ edge_e1_z,
    const int* __restrict__ edge_face1, const int* __restrict__ edge_shape_id, const int* __restrict__ edge_local_id,
    int64_t edge_count, bool* __restrict__ is_valid, float* __restrict__ distances, float* __restrict__ points,
    float* __restrict__ edge_t, float* __restrict__ edge_points, int* __restrict__ shape_ids,
    int* __restrict__ edge_ids, int* __restrict__ global_edge_ids, bool* __restrict__ is_boundary,
    int* __restrict__ tape_edge_id, float* __restrict__ tape_s, float* __restrict__ tape_d) {
...
// host launcher below resolves its stream on the scene device and adds no
// guard of its own.
EdgeTopKForwardOutputs edge_topk_forward_cuda(const SceneCache& scene, const at::Tensor& point, int64_t k,
    ...
    const auto bool_options = at::TensorOptions().device(point.device()).dtype(at::kBool);
    const auto byte_options = at::TensorOptions().device(point.device()).dtype(at::kByte);

    EdgeTopKForwardOutputs out;
    out.is_valid = at::zeros({query_count, k}, bool_options);
    out.distances = at::full({query_count, k}, std::numeric_limits<float>::infinity(), float_options);
    ...
    prepare_topk_queries_kernel<<<query_blocks, kThreads, 0, stream>>>(point.data_ptr<float>(),
                                                                       active.numel() == 0 ? nullptr
                                                                                           : active.data_ptr<bool>(),
                                                                       query_count, point.stride(0), point.stride(1),
                                                                       query_x.data_ptr<float>(),
                                                                       query_y.data_ptr<float>(),
                                                                       query_z.data_ptr<float>(),
    ...
    at::Tensor traversal_stack = at::empty({shared::edge::kBvhTraversalStackDepth, query_count}, int_options);
    ...
    params.output = {
        candidate_edge.data_ptr<int>(),        candidate_distance_sq.data_ptr<float>(),
        candidate_edge_t.data_ptr<float>(),    nullptr,
        static_cast<std::size_t>(query_count), static_cast<std::size_t>(k),
        static_cast<std::size_t>(k),           static_cast<std::size_t>(output_count),
    };
    params.scratch = {
    ...
    params.edge_mask = scene.edge_mask.numel() == 0 ? nullptr : scene.edge_mask.data_ptr<std::uint8_t>();
    params.stream = stream;
    shared::edge::launch_point_bvh_query_async(params);

    repair_topk_overflow_kernel<<<query_blocks, kThreads, 0, stream>>>(
        query_x.data_ptr<float>(), query_y.data_ptr<float>(), query_z.data_ptr<float>(),
        query_active.data_ptr<std::uint8_t>(), query_count, static_cast<int>(k), scene.edge_p0_x.data_ptr<float>(),
        scene.edge_p0_y.data_ptr<float>(), scene.edge_p0_z.data_ptr<float>(), scene.edge_e1_x.data_ptr<float>(),
        scene.edge_e1_y.data_ptr<float>(), scene.edge_e1_z.data_ptr<float>(), params.edge_mask, edge_count,
        overflow.data_ptr<std::uint8_t>(), candidate_edge.data_ptr<int>(), candidate_distance_sq.data_ptr<float>(),
    ...
    finalize_topk_kernel<<<output_blocks, kThreads, 0, stream>>>(
        query_x.data_ptr<float>(), query_y.data_ptr<float>(), query_z.data_ptr<float>(),
        query_active.data_ptr<std::uint8_t>(), query_count, static_cast<int>(k), candidate_edge.data_ptr<int>(),
        candidate_distance_sq.data_ptr<float>(), candidate_edge_t.data_ptr<float>(), scene.edge_p0_x.data_ptr<float>(),
        scene.edge_p0_y.data_ptr<float>(), scene.edge_p0_z.data_ptr<float>(), scene.edge_e1_x.data_ptr<float>(),
        scene.edge_e1_y.data_ptr<float>(), scene.edge_e1_z.data_ptr<float>(), scene.edge_face1.data_ptr<int>(),
        scene.edge_shape_id.data_ptr<int>(), scene.edge_local_id.data_ptr<int>(), edge_count,
        out.is_valid.data_ptr<bool>(), out.distances.data_ptr<float>(), out.points.data_ptr<float>(),
        out.edge_t.data_ptr<float>(), out.edge_points.data_ptr<float>(), out.shape_ids.data_ptr<int>(),
        out.edge_ids.data_ptr<int>(), out.global_edge_ids.data_ptr<int>(), out.is_boundary.data_ptr<bool>(),
        out.tape_edge_id.data_ptr<int>(), out.tape_s.data_ptr<float>(), out.tape_d.data_ptr<float>());
    cuda_check(cudaGetLastError(), "edge_topk_forward_cuda() kernel launch");
    return out;
}

__global__ void prepare_ray_query_kernel(const float* ray_o, const float* ray_d, const float* ray_tmax,
                                         const bool* active, int64_t count, int64_t o_s0, int64_t o_s1, int64_t d_s0,
                                         int64_t d_s1, float* ox, float* oy, float* oz, float* dx, float* dy, float* dz,
    ...
    query_active[ray] = (active == nullptr || active[ray]) && isfinite(ox[ray]) && isfinite(oy[ray]) &&
                                isfinite(oz[ray]) && isfinite(dx[ray]) && isfinite(dy[ray]) && isfinite(dz[ray])
                            ? 1u
                            : 0u;
}

__global__ void repair_ray_overflow_kernel(const float* ox, const float* oy, const float* oz, const float* dx,
                                           const float* dy, const float* dz, const float* tmax,
                                           const std::uint8_t* active, int64_t query_count, const float* p0x,
                                           const float* p0y, const float* p0z, const float* e1x, const float* e1y,
                                           const float* e1z, const std::uint8_t* edge_mask, int64_t edge_count,
    ...
    if (active[ray] != 0u) {
        ...
        for (int edge = 0; edge < edge_count; ++edge) {
            ...
            if (isinf(tmax[ray])) {
                ...
                const auto candidate = shared::edge::segment_segment_distance(o, shared::math::scale(d, limit), p0, e1);
                distance = candidate.squared_distance;
                edge_t = candidate.edge_parameter;
...
__global__ void finalize_ray_query_kernel(const float* ox, const float* oy, const float* oz, const float* dx,
                                          const float* dy, const float* dz, const std::uint8_t* active, int64_t count,
                                          const int* candidate_edge, const float* distance_sq,
                                          const float* candidate_ray_t, const float* candidate_edge_t, const float* p0x,
                                          const float* p0y, const float* p0z, const float* e1x, const float* e1y,
                                          const float* e1z, const int* edge_shape, const int* edge_local,
                                          float* out_distance, float* out_ray_t, float* out_point, float* out_edge_t,
                                          float* out_edge_point, int* out_shape, int* out_local, int* out_global,
                                          int* out_tape) {
    const int64_t ray = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (ray >= count)
        return;
    const int edge = candidate_edge[ray];
    out_distance[ray] = CUDART_INF_F;
    out_ray_t[ray] = 0.0f;
    out_edge_t[ray] = 0.0f;
    out_shape[ray] = -1;
    out_local[ray] = -1;
    ...
    out_ray_t[ray] = rt;
    out_edge_t[ray] = et;
    out_shape[ray] = edge_shape[edge];
    out_local[ray] = edge_local[edge];
    out_global[ray] = edge;
    out_tape[ray] = edge;
...
EdgeForwardOutputs edge_forward_bvh_cuda(SceneCache& scene, const at::Tensor& point) {
    ...
    return {
        top.distances.select(1, 0),    top.edge_points.select(1, 0), top.edge_t.select(1, 0),
        top.shape_ids.select(1, 0),    top.edge_ids.select(1, 0),    top.global_edge_ids.select(1, 0),
        top.tape_edge_id.select(1, 0), top.tape_s.select(1, 0),      top.tape_d.select(1, 0),
    };
}
...
EdgeForwardPublicOutputs edge_forward_noad_bvh_cuda(SceneCache& scene, const at::Tensor& point) {
    ...
    return {out.distance, out.edge_point, out.edge_t, out.shape_id, out.edge_id, out.global_edge_id};
}

EdgeRayForwardOutputs edge_ray_forward_bvh_cuda(SceneCache& scene, const at::Tensor& ray_o, const at::Tensor& ray_d,
                                                const at::Tensor& ray_tmax, const at::Tensor& active) {
    ...
    EdgeRayForwardOutputs out;
    out.distance = at::empty({count}, fopts);
    out.ray_t = at::empty({count}, fopts);
    out.point = at::empty({count, 3}, fopts);
    out.edge_t = at::empty({count}, fopts);
    out.edge_point = at::empty({count, 3}, fopts);
    out.shape_id = at::empty({count}, iopts);
    out.edge_id = at::empty({count}, iopts);
    out.global_edge_id = at::empty({count}, iopts);
    ...
    prepare_ray_query_kernel<<<blocks, kThreads, 0, stream>>>(ray_o.data_ptr<float>(), ray_d.data_ptr<float>(),
                                                              ray_tmax.numel() == 0 ? nullptr
                                                                                    : ray_tmax.data_ptr<float>(),
                                                              active.numel() == 0 ? nullptr : active.data_ptr<bool>(),
                                                              count, ray_o.stride(0), ray_o.stride(1), ray_d.stride(0),
                                                              ray_d.stride(1), ox.data_ptr<float>(),
                                                              oy.data_ptr<float>(), oz.data_ptr<float>(),
                                                              dx.data_ptr<float>(), dy.data_ptr<float>(),
                                                              dz.data_ptr<float>(), tmax.data_ptr<float>(),
                                                              query_active.data_ptr<uint8_t>());
    ...
    params.node_bounds = scene_edge_bvh_bounds_view(scene);
    params.topology = scene_edge_bvh_topology_view(scene);
    params.rays = {ox.data_ptr<float>(), oy.data_ptr<float>(), oz.data_ptr<float>(),   dx.data_ptr<float>(),
                   dy.data_ptr<float>(), dz.data_ptr<float>(), tmax.data_ptr<float>(), static_cast<size_t>(count)};
    params.output = {edge.data_ptr<int>(),
                     dist.data_ptr<float>(),
                     et.data_ptr<float>(),
                     rt.data_ptr<float>(),
                     ...
                     1,
                     static_cast<size_t>(count)};
    params.scratch = {stack.data_ptr<int>(),
                      overflow.data_ptr<uint8_t>(),
                      static_cast<size_t>(count),
                      static_cast<size_t>(shared::edge::kBvhTraversalStackDepth),
                      static_cast<size_t>(stack.numel()),
                      static_cast<size_t>(overflow.numel())};
    params.active_mask = query_active.data_ptr<uint8_t>();
    params.edge_mask = scene.edge_mask.data_ptr<uint8_t>();
    params.stream = stream;
    shared::edge::launch_ray_bvh_query_async(params);
    repair_ray_overflow_kernel<<<blocks, kThreads, 0, stream>>>(
        ox.data_ptr<float>(), oy.data_ptr<float>(), oz.data_ptr<float>(), dx.data_ptr<float>(), dy.data_ptr<float>(),
        dz.data_ptr<float>(), tmax.data_ptr<float>(), query_active.data_ptr<uint8_t>(), count,
        scene.edge_p0_x.data_ptr<float>(), scene.edge_p0_y.data_ptr<float>(), scene.edge_p0_z.data_ptr<float>(),
        scene.edge_e1_x.data_ptr<float>(), scene.edge_e1_y.data_ptr<float>(), scene.edge_e1_z.data_ptr<float>(),
        params.edge_mask, scene.edge_v0.numel(), overflow.data_ptr<uint8_t>(), edge.data_ptr<int>(),
        dist.data_ptr<float>(), et.data_ptr<float>(), rt.data_ptr<float>());
    finalize_ray_query_kernel<<<blocks, kThreads, 0, stream>>>(
        ox.data_ptr<float>(), oy.data_ptr<float>(), oz.data_ptr<float>(), dx.data_ptr<float>(), dy.data_ptr<float>(),
        dz.data_ptr<float>(), query_active.data_ptr<uint8_t>(), count, edge.data_ptr<int>(), dist.data_ptr<float>(),
        rt.data_ptr<float>(), et.data_ptr<float>(), scene.edge_p0_x.data_ptr<float>(),
        scene.edge_p0_y.data_ptr<float>(), scene.edge_p0_z.data_ptr<float>(), scene.edge_e1_x.data_ptr<float>(),
        scene.edge_e1_y.data_ptr<float>(), scene.edge_e1_z.data_ptr<float>(), scene.edge_shape_id.data_ptr<int>(),
        scene.edge_local_id.data_ptr<int>(), out.distance.data_ptr<float>(), out.ray_t.data_ptr<float>(),
        out.point.data_ptr<float>(), out.edge_t.data_ptr<float>(), out.edge_point.data_ptr<float>(),
        out.shape_id.data_ptr<int>(), out.edge_id.data_ptr<int>(), out.global_edge_id.data_ptr<int>(),
