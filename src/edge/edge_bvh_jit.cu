  ...
  public:
    explicit CudaDeviceGuard(int device) : previous_device_(current_cuda_device()) {
        if (previous_device_ != device) {
            check_cuda_call(cudaSetDevice(device), "edge_bvh: failed to bind the requested CUDA device");
            restore_ = true;
        }
    }
...
template <typename T> class CudaBuffer {
  public:
    ...
    CudaBuffer(CudaBuffer&& other) noexcept : ptr_(other.ptr_), count_(other.count_), device_(other.device_) {
        other.ptr_ = nullptr;
        other.count_ = 0;
        other.device_ = -1;
    }
    ...
    void allocate(size_t count) {
        ...
        require_local(error == cudaSuccess, std::string("CudaBuffer::allocate(): ") + cudaGetErrorString(error));
        device_ = device;
    }

    T* get() {
        require_owning_device();
        return ptr_;
    }
    const T* get() const {
        require_owning_device();
  ...
  private:
    ...
    void require_owning_device() const {
        ...
        const int device = current_cuda_device();
        require_local(device == device_, "CudaBuffer::get(): buffer was allocated on CUDA device " +
...
void check_cuda_call(cudaError_t error, const char* message) {
    require_local(error == cudaSuccess, std::string(message) + ": " + cudaGetErrorString(error));
}
...
class CudaEventHandle {
  public:
    explicit CudaEventHandle(unsigned flags = cudaEventDisableTiming) {
        check_cuda_call(cudaEventCreateWithFlags(&event_, flags), "build_edge_lbvh_gpu(): failed to create CUDA event");
    }
...
void memset_int_async(int* ptr, int value, size_t count, cudaStream_t stream, const char* message) {
    if (count == 0) {
        return;
    }
...
std::vector<int> copy_int_buffer_to_host(const int* device_ptr, size_t count, const char* message) {
    ...
    check_cuda_call(cudaMemcpy(result.data(), device_ptr, count * sizeof(int), cudaMemcpyDeviceToHost), message);
    audit_cuda_memcpy();
    return result;
}

int compute_node_height_host(int node_index, const std::vector<int>& left_child, const std::vector<int>& right_child,
                             const std::vector<int>& is_leaf, std::vector<int>& node_heights) {
    ...
    height = 1 + std::max(compute_node_height_host(left_child[static_cast<size_t>(node_index)], left_child, right_child,
                                                   is_leaf, node_heights),
                          compute_node_height_host(right_child[static_cast<size_t>(node_index)], left_child,
                                                   right_child, is_leaf, node_heights));
    return height;
}
...
HostTreeLevels build_host_tree_levels_from_topology(const std::vector<int>& left_child,
    ...
    for (int node_index = 0; node_index < node_count; ++node_index) {
        ...
        const int height = compute_node_height_host(node_index, left_child, right_child, is_leaf, result.node_heights);
...
void compute_edge_optix_aabbs_gpu(const EdgeBvhCudaContext& context, int primitive_count, const float* edge_p0_x,
    ...
    try {
        ...
        shared::edge::launch_edge_aabb(primitive_count, edge_p0_x, edge_p0_y, edge_p0_z, edge_e1_x, edge_e1_y,
                                       edge_e1_z, inflation, out_aabbs, context.stream);
        audit_cuda_kernel_launch("compute_edge_optix_aabbs_kernel", static_cast<uint32_t>(block_count), 1, 1,
                                 block_size, 1, 1, static_cast<uint64_t>(primitive_count));
        check_cuda_last_error("compute_edge_optix_aabbs_gpu(): failed to launch AABB kernel");
        synchronize_cuda(context.stream, "compute_edge_optix_aabbs_gpu(): failed to finish AABB kernel");
...
void build_edge_bvh_gpu(const EdgeBvhCudaContext& context, int primitive_count, const float* edge_p0_x,
                        ...
                        int* left_child, int* right_child, int* leaf_primitive, int* is_leaf,
    ...
    try {
        ...
        const bool overlap_build_streams = build_stream_mode == EdgeBVHBuildStreamMode::Overlap;
        const bool treelet_enabled = post_build_strategy == EdgeBVHPostBuildStrategy::GpuTreelet &&
                                     primitive_count >= EdgeBVHTreeletMinPrimitives &&
                                     primitive_count <= EdgeBVHTreeletMaxPrimitives && internal_count > 0;
        ...
        const cudaStream_t sequence_stream = overlap_build_streams ? sequence_stream_handle.get() : bounds_stream;
        ...
        shared::edge::launch_compute_primitive_bounds_async(
            {{edge_p0_x, edge_p0_y, edge_p0_z, edge_e1_x, edge_e1_y, edge_e1_z, static_cast<size_t>(primitive_count)},
             {primitive_bbox_min_x, primitive_bbox_min_y, primitive_bbox_min_z, primitive_bbox_max_x,
              primitive_bbox_max_y, primitive_bbox_max_z, static_cast<size_t>(primitive_count)},
             primitive_bounds.get(),
             bounds_stream});
        audit_cuda_kernel_launch("compute_primitive_bounds_kernel", static_cast<uint32_t>(primitive_blocks), 1, 1,
                                 block_size, 1, 1, static_cast<uint64_t>(primitive_count));
        check_cuda_last_error("build_edge_lbvh_gpu(): failed to launch primitive-bounds kernel");

        size_t reduce_temp_size = 0;
        audit_cub_reduce();
        check_cuda_call(cub::DeviceReduce::Reduce(nullptr, reduce_temp_size, primitive_bounds.get(),
                                                  reduced_bounds.get(), primitive_count, BoundsUnion(), empty_bounds(),
                                                  bounds_stream),
                        "build_edge_lbvh_gpu(): failed to size scene-bound reduction");
        CudaBuffer<char> reduce_temp(reduce_temp_size);
        audit_cub_reduce();
        check_cuda_call(cub::DeviceReduce::Reduce(reduce_temp.get(), reduce_temp_size, primitive_bounds.get(),
                                                  reduced_bounds.get(), primitive_count, BoundsUnion(), empty_bounds(),
                                                  bounds_stream),
        ...
        check_cuda_call(cudaMemcpyAsync(&scene_bounds, reduced_bounds.get(), sizeof(Bounds3), cudaMemcpyDeviceToHost,
                                        bounds_stream),
                        "build_edge_lbvh_gpu(): failed to copy scene bounds");
        shared::edge::launch_init_sequence_async({primitive_indices_in.get(), primitive_count, sequence_stream});
        audit_cuda_kernel_launch("init_sequence_kernel", static_cast<uint32_t>(primitive_blocks), 1, 1, block_size, 1,
                                 1, static_cast<uint64_t>(primitive_count));
        ...
        audit_cuda_stream_synchronize();
        check_cuda_call(cudaStreamSynchronize(bounds_stream), "build_edge_lbvh_gpu(): failed to reduce scene bounds");
        ...
        shared::edge::launch_compute_morton_codes_async(
            {{primitive_bbox_min_x, primitive_bbox_min_y, primitive_bbox_min_z, primitive_bbox_max_x,
             ...
             morton_codes_in.get(),
             bounds_stream});
        audit_cuda_kernel_launch("compute_morton_codes_kernel", static_cast<uint32_t>(primitive_blocks), 1, 1,
                                 block_size, 1, 1, static_cast<uint64_t>(primitive_count));
        ...
        audit_cub_sort();
        check_cuda_call(cub::DeviceRadixSort::SortPairs(nullptr, sort_temp_size, morton_codes_in.get(),
                                                        morton_codes_out.get(), primitive_indices_in.get(),
                                                        primitive_indices_out.get(), primitive_count, 0, 32,
                                                        bounds_stream),
                        "build_edge_lbvh_gpu(): failed to size radix sort");
        CudaBuffer<char> sort_temp(sort_temp_size);
        audit_cub_sort();
        check_cuda_call(cub::DeviceRadixSort::SortPairs(sort_temp.get(), sort_temp_size, morton_codes_in.get(),
                                                        morton_codes_out.get(), primitive_indices_in.get(),
                                                        primitive_indices_out.get(), primitive_count, 0, 32,
                                                        bounds_stream),
        ...
        memset_int_async(left_child, -1, static_cast<size_t>(node_count), bounds_stream,
                         "build_edge_lbvh_gpu(): failed to init left_child");
        memset_int_async(right_child, -1, static_cast<size_t>(node_count), bounds_stream,
                         "build_edge_lbvh_gpu(): failed to init right_child");
        memset_int_async(leaf_primitive, -1, static_cast<size_t>(node_count), bounds_stream,
                         "build_edge_lbvh_gpu(): failed to init leaf_primitive");
        memset_int_async(is_leaf, 0, static_cast<size_t>(node_count), bounds_stream,
                         "build_edge_lbvh_gpu(): failed to init is_leaf");
        memset_int_async(primitive_leaf_node, -1, static_cast<size_t>(primitive_count), bounds_stream,
                         "build_edge_lbvh_gpu(): failed to init primitive_leaf_node");
        memset_int_async(parent.get(), -1, static_cast<size_t>(node_count), bounds_stream,
                         "build_edge_lbvh_gpu(): failed to init parent");
        memset_int_async(merge_counters.get(), 0, static_cast<size_t>(std::max(internal_count, 1)), bounds_stream,
                         "build_edge_lbvh_gpu(): failed to init merge counters");

        if (internal_count > 0) {
            shared::edge::launch_build_radix_tree_async({morton_codes_out.get(), primitive_indices_out.get(),
                                                         left_child, right_child, parent.get(), primitive_count,
                                                         bounds_stream});
            audit_cuda_kernel_launch("build_radix_tree_kernel", static_cast<uint32_t>(internal_blocks), 1, 1,
                                     block_size, 1, 1, static_cast<uint64_t>(internal_count));
        ...
        if (internal_count > 0 && treelet_enabled) {
            ...
            host_left_child = copy_int_buffer_to_host(left_child, static_cast<size_t>(node_count),
                                                      "build_edge_lbvh_gpu(): failed to copy left_child");
            host_right_child = copy_int_buffer_to_host(right_child, static_cast<size_t>(node_count),
            ...
            const HostTreeLevels host_levels =
                build_host_tree_levels_from_topology(host_left_child, host_right_child, host_is_leaf);
        ...
        shared::edge::launch_finalize_leaves_and_bounds_async(
            {primitive_indices_out.get(),
             ...
             left_child,
             right_child,
             {node_bbox_min_x, node_bbox_min_y, node_bbox_min_z, node_bbox_max_x, node_bbox_max_y, node_bbox_max_z,
             ...
             primitive_count,
             bounds_stream});
        audit_cuda_kernel_launch("finalize_leaves_and_bounds_kernel", static_cast<uint32_t>(primitive_blocks), 1, 1,
                                 block_size, 1, 1, static_cast<uint64_t>(primitive_count));
        ...
        if (treelet_enabled) {
            const float scene_scale =
                fmaxf(scene_bounds.max.x - scene_bounds.min.x,
                      fmaxf(scene_bounds.max.y - scene_bounds.min.y, scene_bounds.max.z - scene_bounds.min.z));
            const float inflation = fmaxf(scene_scale * EdgeBVHTreeletCostInflationRatio, 1e-6f);
            shared::edge::launch_initialize_leaf_costs_async(
                {{node_bbox_min_x, node_bbox_min_y, node_bbox_min_z, node_bbox_max_x, node_bbox_max_y, node_bbox_max_z,
                 ...
                 primitive_count,
                 bounds_stream});
            audit_cuda_kernel_launch("initialize_leaf_costs_kernel", static_cast<uint32_t>(primitive_blocks), 1, 1,
                                     block_size, 1, 1, static_cast<uint64_t>(primitive_count));
            ...
            memset_int_async(internal_cost_arrival_counter.get(), 0, static_cast<size_t>(node_count), bounds_stream,
            ...
            if (internal_count > 0) {
                shared::edge::launch_initialize_internal_costs_async(
                    {left_child,
                     right_child,
                     parent.get(),
                     {node_bbox_min_x, node_bbox_min_y, node_bbox_min_z, node_bbox_max_x, node_bbox_max_y,
                      node_bbox_max_z, static_cast<size_t>(node_count)},
                     node_costs.get(),
                     internal_cost_arrival_counter.get(),
                     inflation,
                     primitive_count,
                     bounds_stream});
                audit_cuda_kernel_launch("initialize_internal_costs_kernel", static_cast<uint32_t>(primitive_blocks), 1,
                                         1, block_size, 1, 1, static_cast<uint64_t>(primitive_count));
                check_cuda_last_error("build_edge_lbvh_gpu(): failed to launch internal-cost initialization");
            }

            {
                ...
                if (!optimize_schedule.nodes.empty()) {
                    ...
                    audit_cuda_memcpy_async();
                    check_cuda_call(cudaMemcpyAsync(optimize_nodes_device.get(), optimize_schedule.nodes.data(),
                                                    optimize_schedule.nodes.size() * sizeof(int),
                                                    cudaMemcpyHostToDevice, bounds_stream),
                    ...
                    audit_cuda_stream_wait_event();
                    check_cuda_call(cudaStreamWaitEvent(bounds_stream, schedule_uploaded_event.get(), 0),
                                    "build_edge_lbvh_gpu(): failed to order the treelet schedule upload");
                }

                for (int height = 1; height <= max_height; ++height) {
                    const int optimize_start = optimize_schedule.level_offsets[static_cast<size_t>(height)];
                    const int optimize_end = optimize_schedule.level_offsets[static_cast<size_t>(height + 1)];
                    ...
                    if (optimize_count > 0) {
                        ...
                        audit_cuda_kernel_launch("optimize_selected_treelets_kernel",
                                                 static_cast<uint32_t>(level_blocks), 1, 1, block_size, 1, 1,
        ...
        audit_cuda_stream_synchronize();
        check_cuda_call(cudaStreamSynchronize(bounds_stream), "build_edge_lbvh_gpu(): failed to complete build");
    } catch (const std::exception& e) {
...
void mark_edge_bvh_dirty_ancestors_gpu(const EdgeBvhCudaContext& context, int node_count, int leaf_count,
                                       ...
                                       bool clear_marks) {
    require_local(node_count >= 0, "mark_edge_bvh_dirty_ancestors_gpu(): node_count must be non-negative.");
    require_local(leaf_count >= 0, "mark_edge_bvh_dirty_ancestors_gpu(): leaf_count must be non-negative.");
    ...
    try {
        ...
        if (clear_marks) {
            memset_int_async(out_dirty_marks, 0, static_cast<size_t>(node_count), context.stream,
        ...
        shared::edge::launch_mark_dirty_ancestors_async(
            {leaf_nodes, node_parent, out_dirty_marks, leaf_count, context.stream});
        audit_cuda_kernel_launch("mark_dirty_ancestors_kernel", static_cast<uint32_t>(block_count), 1, 1, block_size, 1,
                                 1, static_cast<uint64_t>(leaf_count));
        check_cuda_last_error("mark_edge_bvh_dirty_ancestors_gpu(): failed to launch dirty-ancestor kernel");
    } catch (const std::exception& e) {
...
void compact_and_refit_edge_bvh_level_gpu(const EdgeBvhCudaContext& context, int level_count, const int* level_nodes,
    ...
    require_local(level_count >= 0, "compact_and_refit_edge_bvh_level_gpu(): level_count must be non-negative.");
    ...
    try {
        ...
        memset_int_async(scratch_selected_count, 0, 1, context.stream,
        ...
        shared::edge::launch_compact_dirty_level_async(
            {level_nodes, dirty_marks, scratch_selected_nodes, scratch_selected_count, level_count, context.stream});
        audit_cuda_kernel_launch("compact_dirty_level_kernel", static_cast<uint32_t>(block_count), 1, 1, block_size, 1,
                                 1, static_cast<uint64_t>(level_count));
        check_cuda_last_error("compact_and_refit_edge_bvh_level_gpu(): failed to launch dirty-level compaction");
        ...
        shared::edge::launch_refit_selected_internal_nodes_async(
            {scratch_selected_count,
             ...
             left_child,
             right_child,
             {node_bbox_min_x, node_bbox_min_y, node_bbox_min_z, node_bbox_max_x, node_bbox_max_y, node_bbox_max_z, 0},
             level_count,
             context.stream});
        audit_cuda_kernel_launch("refit_selected_internal_nodes_kernel", static_cast<uint32_t>(block_count), 1, 1,
                                 block_size, 1, 1, static_cast<uint64_t>(level_count));
        check_cuda_last_error("compact_and_refit_edge_bvh_level_gpu(): failed to launch dirty-level refit");
    } catch (const std::exception& e) {
