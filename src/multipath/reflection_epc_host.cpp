...
ReflectionEpcPipeline::~ReflectionEpcPipeline() {
...
void ReflectionEpcPipeline::build(OptixDeviceContext context,
                                  int hitgroup_record_count) {
    require(context != nullptr, "ReflectionEpcPipeline::build(): invalid OptiX context.");
    require(hitgroup_record_count > 0,
            "ReflectionEpcPipeline::build(): hitgroup_record_count must be positive.");
    init_optix_api();
    ...
    check_optix(
        optixModuleCreate(context,
                          ...
                          &pipeline_options,
                          reflection_epc_ptx,
                          reflection_epc_ptx_size,
                          log,
                          &log_size,
                          &module_),
        "optixModuleCreate(reflection_epc)");
    ...
    raygen_desc.raygen.entryFunctionName = "__raygen__reflection_epc";
    log_size = sizeof(log);
    check_optix(optixProgramGroupCreate(context,
                                        ...
                                        &log_size,
                                        &pg_raygen_),
                "optixProgramGroupCreate(reflection_epc raygen)");

    OptixProgramGroupDesc miss_desc = {};
    miss_desc.kind = OPTIX_PROGRAM_GROUP_KIND_MISS;
    miss_desc.miss.module = module_;
    miss_desc.miss.entryFunctionName = "__miss__reflection_epc";
    log_size = sizeof(log);
    check_optix(optixProgramGroupCreate(context,
                                        &miss_desc,
                                        1,
                                        ...
                                        &log_size,
                                        &pg_miss_),
                "optixProgramGroupCreate(reflection_epc miss)");

    OptixProgramGroupDesc hitgroup_desc = {};
    hitgroup_desc.kind = OPTIX_PROGRAM_GROUP_KIND_HITGROUP;
    hitgroup_desc.hitgroup.moduleCH = module_;
    hitgroup_desc.hitgroup.entryFunctionNameCH = "__closesthit__reflection_epc";
    hitgroup_desc.hitgroup.moduleAH = module_;
    hitgroup_desc.hitgroup.entryFunctionNameAH = "__anyhit__reflection_epc";
    log_size = sizeof(log);
    check_optix(optixProgramGroupCreate(context,
                                        &hitgroup_desc,
                                        1,
                                        ...
                                        &log_size,
                                        &pg_hitgroup_),
                "optixProgramGroupCreate(reflection_epc hitgroup)");
    ...
    check_optix(optixPipelineCreate(context,
                                    ...
                                    &log_size,
                                    &pipeline_),
                "optixPipelineCreate(reflection_epc)");
    ...
    check_optix(optixSbtRecordPackHeader(pg_miss_, &miss_record),
                "optixSbtRecordPackHeader(reflection_epc miss)");
    ...
    for (EmptySbtRecord &record : hitgroup_records) {
        check_optix(optixSbtRecordPackHeader(pg_hitgroup_, &record),
                    "optixSbtRecordPackHeader(reflection_epc hitgroup)");
    }
    ...
    params_buffer_ = jit_malloc(AllocType::Device, sizeof(ReflectionEpcParams));
    hitgroup_record_count_ = hitgroup_record_count;
    ready_ = true;
}

void ReflectionEpcPipeline::launch(const ReflectionEpcParams &params) const {
    require(ready_, "ReflectionEpcPipeline::launch(): pipeline is not ready.");

    audit_jit_memcpy_async();
    jit_memcpy_async(JitBackend::CUDA,
                     params_buffer_,
                     &params,
                     sizeof(ReflectionEpcParams));
