...
ReflectionAccumulationPipeline::~ReflectionAccumulationPipeline() {
...
void ReflectionAccumulationPipeline::build(OptixDeviceContext context,
                                           int hitgroup_record_count) {
    require(context != nullptr,
            "ReflectionAccumulationPipeline::build(): invalid OptiX context.");
    require(hitgroup_record_count > 0,
            "ReflectionAccumulationPipeline::build(): hitgroup_record_count must be positive.");
    init_optix_api();
    ...
    check_optix(
        optixModuleCreate(context,
                          &module_options,
                          &pipeline_options,
                          reflection_accumulation_ptx,
                          reflection_accumulation_ptx_size,
                          log,
                          &log_size,
                          &module_),
        "optixModuleCreate(reflection_accumulation)");
    ...
    raygen_desc.raygen.entryFunctionName = "__raygen__reflection_accumulation";
    log_size = sizeof(log);
    check_optix(optixProgramGroupCreate(context,
                                        ...
                                        &log_size,
                                        &pg_raygen_),
                "optixProgramGroupCreate(reflection_accumulation raygen)");

    OptixProgramGroupDesc miss_desc = {};
    miss_desc.kind = OPTIX_PROGRAM_GROUP_KIND_MISS;
    miss_desc.miss.module = module_;
    miss_desc.miss.entryFunctionName = "__miss__reflection_accumulation";
    log_size = sizeof(log);
    check_optix(optixProgramGroupCreate(context,
                                        &miss_desc,
                                        1,
                                        ...
                                        &log_size,
                                        &pg_miss_),
                "optixProgramGroupCreate(reflection_accumulation miss)");

    OptixProgramGroupDesc hitgroup_desc = {};
    hitgroup_desc.kind = OPTIX_PROGRAM_GROUP_KIND_HITGROUP;
    hitgroup_desc.hitgroup.moduleCH = module_;
    hitgroup_desc.hitgroup.entryFunctionNameCH = "__closesthit__reflection_accumulation";
    log_size = sizeof(log);
    check_optix(optixProgramGroupCreate(context,
                                        &hitgroup_desc,
                                        1,
                                        ...
                                        &log_size,
                                        &pg_hitgroup_),
                "optixProgramGroupCreate(reflection_accumulation hitgroup)");
    ...
    check_optix(optixPipelineCreate(context,
                                    ...
                                    &log_size,
                                    &pipeline_),
                "optixPipelineCreate(reflection_accumulation)");

    check_optix(optixPipelineSetStackSize(pipeline_, 0, 0, 4096, 2),
                "optixPipelineSetStackSize(reflection_accumulation)");

    EmptySbtRecord raygen_record = {};
    check_optix(optixSbtRecordPackHeader(pg_raygen_, &raygen_record),
                "optixSbtRecordPackHeader(reflection_accumulation raygen)");
    ...
    check_optix(optixSbtRecordPackHeader(pg_miss_, &miss_record),
                "optixSbtRecordPackHeader(reflection_accumulation miss)");
    ...
    for (EmptySbtRecord &record : hitgroup_records) {
        check_optix(optixSbtRecordPackHeader(pg_hitgroup_, &record),
                    "optixSbtRecordPackHeader(reflection_accumulation hitgroup)");
    }
    ...
    params_buffer_ = jit_malloc(AllocType::Device, sizeof(ReflectionAccumulationParams));
    hitgroup_record_count_ = hitgroup_record_count;
    ready_ = true;
}

void ReflectionAccumulationPipeline::launch(const ReflectionAccumulationParams &params) const {
    require(ready_, "ReflectionAccumulationPipeline::launch(): pipeline is not ready.");

    audit_jit_memcpy_async();
    jit_memcpy_async(JitBackend::CUDA,
                     params_buffer_,
                     &params,
                     sizeof(ReflectionAccumulationParams));
    ...
    check_optix(optixLaunch(pipeline_,
                            ...
                            1,
                            1),
                "optixLaunch(reflection_accumulation)");
}
