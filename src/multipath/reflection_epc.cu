...
static __forceinline__ __device__ VisibilityPayload trace_visibility(float3 start,
    ...
    VisibilityPayload primary =
        trace_visibility_handle(params.primary_handle, start, end, ignore0_u, ignore1_u);
    if (primary.visible == 0u || params.split_mode == 0) {
        return primary;
    }

    return trace_visibility_handle(params.secondary_handle, start, end, ignore0_u, ignore1_u);
}
...
extern "C" __global__ void __anyhit__reflection_epc() {
    ...
    if ((ignore0 != kInvalidPrim && global_prim == static_cast<int>(ignore0)) ||
        (ignore1 != kInvalidPrim && global_prim == static_cast<int>(ignore1))) {
...
extern "C" __global__ void __raygen__reflection_epc() {
    ...
    int prim_ids[ReflectionEpcMaxBounces];
    ...
    float3 trace_direction = direction;
    ...
    if (bounce_count != B) {
        store_invalid(ray_index, bounce_count, -1, -1);
        return;
    }
    ...
    for (int bounce = B - 1; bounce >= 0; --bounce) {
        ...
        valid = valid &&
                intersect_line_plane(image_sources[bounce + 1],
                                     ...
                                     point);
        valid = valid && point_inside_triangle(prim_ids[bounce], point);
    ...
    for (int segment = 0; segment <= B; ++segment) {
        ...
        const int ignore0 = segment > 0 ? prim_ids[segment - 1] : -1;
        const int ignore1 = segment < B ? prim_ids[segment] : -1;
        const VisibilityPayload visibility = trace_visibility(start, end, ignore0, ignore1);
    ...
    if (!valid) {
        store_invalid(ray_index, bounce_count, first_blocked_segment, first_blocked_prim);
        return;
    }
    ...
    for (int bounce = 0; bounce < B; ++bounce) {
        ...
        params.out_prim_ids[slot] = prim_ids[bounce];
    }
}
