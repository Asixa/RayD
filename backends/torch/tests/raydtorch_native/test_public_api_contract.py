...
class PublicApiContractTests(unittest.TestCase):
    ...
    def test_acceptance_benchmarks_use_public_raydtorch_api(self):
        stress = (ROOT / "tests" / "benchmark_raydtorch_rayd_mitsuba_stress.py").read_text()
        multipath = (ROOT / "tests" / "benchmark_raydtorch_rayd_mitsuba_multipath.py").read_text()
    ...
    def test_intersection_is_valid_uses_native_kernel(self):
        source = (ROOT / "raydtorch" / "types.py").read_text()
        ...
        self.assertIn("_C.intersection_valid", intersection_source)
        self.assertLess(intersection_source.index("_C.intersection_valid"), intersection_source.index("torch.isfinite"))

    def test_camera_public_path_does_not_stage_contiguous_copies(self):
        source = (ROOT / "raydtorch" / "camera.py").read_text()
        forbidden = (
            "sample.contiguous()",
            "point.contiguous()",
    ...
    def test_diffraction_active_masks_are_not_expanded_or_contiguous_staged(self):
        python_source = (ROOT / "raydtorch" / "autograd.py").read_text()
    ...
    def test_diffraction_accumulation_forward_does_not_split_vec3_with_aten_copies(self):
        ...
        coherent_end = source.index("void bind_diffraction_ops")
