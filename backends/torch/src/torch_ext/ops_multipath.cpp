...
#include <raydtorch/diffraction_accumulation_params.h>
#include <raydtorch/diffraction_paths_params.h>
...
#include <raydtorch/reflection_dedup.h>
#include <raydtorch/reflection_epc_field.h>
#include <raydtorch/reflection_epc_params.h>
#include <raydtorch/reflection_trace_params.h>
#include <raydtorch/segment_visibility_params.h>
...
py::tuple diffraction_accumulation_forward_op(
    ...
    SceneCache &scene = get_scene(scene_handle);
    if (scene.meshes.size() != 1)
        throw std::runtime_error("diffraction_accumulation_forward: first milestone supports exactly one mesh.");
    const MeshRecord &mesh = scene.meshes[0];
    const int64_t state_count = state_edge_index.size(0);
    require_state_width(state_edge_pos, state_count, "state_edge_pos");
    require_state_width(state_edge_dir, state_count, "state_edge_dir");
    ...
    const int32_t launch_count = checked_i32(direct_samples + keller_samples, "launch_count");
    ...
    params.collect_edge_use = 0;
    params.collect_debug_counts = 0;
