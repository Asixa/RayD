...
TORCH_LIBRARY_FRAGMENT(rayd_torch, m) {
    ...
    m.def("reflection_accumulation_forward(" RAYD_TORCH_SCHEMA_SCENE " scene, Tensor ray_o, Tensor ray_d, Tensor ray_tmax, Tensor active, Tensor tx, Tensor tx_pol, int max_bounces, int grid_axis, float grid_position, float grid_coord0_min, float grid_coord0_max, float grid_coord1_min, float grid_coord1_max, int grid_resolution0, int grid_resolution1, float wavelength) -> Tensor?[]");
    ...
    m.def("diffraction_paths_order1_forward(" RAYD_TORCH_SCHEMA_SCENE " scene, Tensor tx_pos, Tensor rx_pos, Tensor? active, Tensor state_edge_index, Tensor state_edge_pos, Tensor state_edge_dir, Tensor state_edge_t_min, Tensor state_edge_t_max, Tensor state_n0, Tensor state_n1, Tensor state_prim0, Tensor state_prim1, Tensor state_exterior_angle, Tensor state_src, Tensor state_src_power, Tensor material_gain, Tensor material_valid, int state_limit, int capacity, float wavelength) -> Tensor?[]");
