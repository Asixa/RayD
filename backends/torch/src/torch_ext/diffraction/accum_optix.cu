...
static __forceinline__ __device__ float sample_edge_weight_for_lane(
    int state_idx,
    unsigned int lane,
    int sample_count) {
...
static __forceinline__ __device__ bool keller_grid_hit(int state_idx,
    ...
    return keller_grid_hit_from_incident(
        incident, lane, 1u, edge_point, edge_dir, target, cell, measure_scale);
}
...
static __forceinline__ __device__ void run_diffraction_order1_accumulation_raygen() {
    ...
    const int direct_limit = IncludeDirect && (params.strategy_mask & RAYD_TORCH_DFR_DIRECT) != 0
        ? params.direct_samples
        : 0;
    const int keller_limit = IncludeKeller && (params.strategy_mask & RAYD_TORCH_DFR_KELLER) != 0
        ? params.keller_samples
        : 0;
    const int suffix_limit = IncludeSuffix && (params.strategy_mask & RAYD_TORCH_DFR_SUFFIX_REFL) != 0
    ...
    if (state_idx < 0 || !active_for_state(params.active_mask, params.active_width, params.active_stride, state_idx)) {
        return;
    }
    ...
    if constexpr (IncludeKeller) {
        if (is_keller && !keller_grid_hit(
                state_idx, lane, edge_point, edge_dir, target, cell, keller_measure_scale)) {
...
static __forceinline__ __device__ void run_diffraction_order1_source_visibility_raygen() {
    ...
    const int direct_limit =
        (params.strategy_mask & RAYD_TORCH_DFR_DIRECT) != 0 ? params.direct_samples : 0;
    const int keller_limit =
        (params.strategy_mask & RAYD_TORCH_DFR_KELLER) != 0 ? params.keller_samples : 0;
    const int suffix_limit =
        (params.strategy_mask & RAYD_TORCH_DFR_SUFFIX_REFL) != 0 ? params.suffix_samples : 0;
    const int total_samples = direct_limit + keller_limit + suffix_limit;
...
static __forceinline__ __device__ void run_diffraction_chain_accumulation_raygen() {
    ...
    if (lane >= static_cast<unsigned int>(params.n_rays) ||
        ...
        (params.strategy_mask &
         (RAYD_TORCH_DFR_DIRECT | RAYD_TORCH_DFR_KELLER | RAYD_TORCH_DFR_SUFFIX_REFL)) == 0) {
        return;
    }
