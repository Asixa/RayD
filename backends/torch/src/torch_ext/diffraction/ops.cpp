...
py::tuple diffraction_paths_order1_forward_op(
    ...
    if (active_contig.size(0) != 1 && active_contig.size(0) != state_count)
        throw std::runtime_error("diffraction_paths_order1_forward active width must be 1 or match state_count.");
    ...
    params.active_width = checked_i32(active_contig.size(0), "active_width");
