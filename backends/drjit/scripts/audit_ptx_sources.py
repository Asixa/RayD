...
``*_ptx.h`` headers under ``backends/drjit/include/rayd`` are checked in so a
wheel build needs no OptiX SDK. Regeneration is opt-in
...
``${CMAKE_CURRENT_BINARY_DIR}/rayd/...`` and that directory is prepended to the
include path, so a fresh header only shadows the committed one. Refreshing the
committed PTX is a three-step manual operation:

1. configure with ``-DRAYD_REGENERATE_<MODULE>_PTX=ON`` and an OptiX SDK present,
2. copy ``<build>/rayd/<header>`` over ``backends/drjit/include/rayd/<header>``,
3. re-run ``python backends/drjit/scripts/audit_ptx_sources.py --write``,
...
Usage::
    ...
    python backends/drjit/scripts/audit_ptx_sources.py --check
    python backends/drjit/scripts/audit_ptx_sources.py --write
    python backends/drjit/scripts/audit_ptx_sources.py --git-drift
"""
...
# repository-relative paths. Mirrors backends/drjit/CMakeLists.txt.
CMAKE_VARIABLES = {
    "CMAKE_CURRENT_SOURCE_DIR": "backends/drjit",
    "RAYD_SOURCE_DIR": "src",
    "RAYD_INCLUDE_DIR": "include/rayd",
    "RAYD_SHARED_INCLUDE_DIR": "shared/include",
}
...
def module_record(call: dict[str, object]) -> dict[str, object]:
    ...
    header = (BACKEND / "include" / "rayd" / str(call["header"])).resolve()
...
def check(output: Path) -> None:
    ...
    if not output.is_file():
        raise SystemExit(
            ...
            "Run: python backends/drjit/scripts/audit_ptx_sources.py --write"
        )
    drift = depends_drift()
    if drift:
    ...
    lines.append("Then: python backends/drjit/scripts/audit_ptx_sources.py --write")
    raise SystemExit("\n".join(lines))
