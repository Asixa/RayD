...
from scripts.verify_cuda_binary_arches import _cuobjdump
...
class VerifyCudaBinaryArchesTests(TestCase):
    def test_linux_elf_falls_back_to_extracted_fatbin(self):
        ...
        with patch("scripts.verify_cuda_binary_arches.subprocess.run", side_effect=run):
    ...
    def test_failure_reports_cuobjdump_stderr(self):
        ...
        with patch(
            "scripts.verify_cuda_binary_arches.subprocess.run",
            return_value=failed,
        ):
