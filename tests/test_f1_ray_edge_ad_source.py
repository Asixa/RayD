...
class F1RayEdgeAdSourceTests(unittest.TestCase):
    def test_autograd_calls_fixed_winner_ray_edge_ops(self):
        source = (ROOT / "python/rayd/_impl/multipath.py").read_text(encoding="utf-8")
        self.assertIn("nearest_edge_ray_backward_optional", source)
        self.assertIn("nearest_edge_ray_jvp_optional", source)
        self.assertNotIn("def backward(ctx, *grad_outputs):\n        return None, None, None, None, None, None", source)

    def test_dispatcher_registers_ray_edge_derivative_ops(self):
        source = (ROOT / "src/bindings/library.cpp").read_text(encoding="utf-8")
        for operation in ("nearest_edge_ray_backward_optional", "nearest_edge_ray_jvp_optional"):
    ...
    def test_cuda_adapter_uses_shared_ray_segment_derivatives(self):
        source = (ROOT / "src/edge/edge_queries.cu").read_text(encoding="utf-8")
    ...
    def test_tmax_is_a_detached_domain_boundary(self):
        ...
        self.assertIn("return None, grad_vertices, grad_ray_o, grad_ray_d, None, None", ray_ad)
