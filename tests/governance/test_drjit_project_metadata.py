...
class ProjectMetadataTests(unittest.TestCase):
    def test_torch_and_slang_frontends_are_not_shipped(self):
        removed_paths = [
            ...
            ROOT / "python" / "rayd" / "slang",
            ROOT / "tests" / "torch",
            ROOT / "tests" / "slang",
            WORKSPACE_ROOT / "include" / "rayd" / "slang",
            WORKSPACE_ROOT / "include" / "rayd_slang.slang",
            WORKSPACE_ROOT / "src" / "slang_interop.cpp",
        ]
    ...
    def test_core_camera_api_is_not_shipped(self):
        removed_paths = [
            WORKSPACE_ROOT / "include" / "rayd" / "camera.h",
            WORKSPACE_ROOT / "src" / "camera" / "camera_jit.cpp",
        ]
    ...
    def test_cornell_renderer_uses_example_local_camera(self):
        renderer_camera = WORKSPACE_ROOT / "examples" / "drjit" / "renderer" / "camera.py"
        cornell_box = WORKSPACE_ROOT / "examples" / "drjit" / "renderer" / "cornell_box.py"
    ...
    def test_release_ci_covers_supported_python_and_cuda_architectures(self):
        ...
        cmake = (ROOT / "CMakeLists.txt").read_text(encoding="utf-8")
        cuda_helper = (WORKSPACE_ROOT / "cmake" / "RayDCompilePolicy.cmake").read_text(encoding="utf-8")
        pyproject = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
        ...
        # The consolidated Dr.Jit CUDA roles are compiled by rayd_cuda_object(), which
        # is the single site that applies the gencode flags to both platforms.
        self.assertGreaterEqual(cmake.count("rayd_cuda_object("), 9)
    ...
    def test_public_optix_cold_create_matrix_covers_multipath_apis(self):
        source = (WORKSPACE_ROOT / "tests" / "native" / "test_optix_pipeline_cold_create_jit.py").read_text(
            encoding="utf-8"
        )
        expected_apis = {
            "trace_reflections",
