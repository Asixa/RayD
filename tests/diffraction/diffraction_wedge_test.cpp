...
at::TensorOptions float_options(int device = 0) {
    return at::TensorOptions().dtype(at::kFloat).device(at::Device(at::kCUDA, device));
}

at::TensorOptions bool_options(int device = 0) {
    return at::TensorOptions().dtype(at::kBool).device(at::Device(at::kCUDA, device));
}

rayd::torch::DiffractionWedgeRequest wedge_request(bool with_vertices = true, int device = 0) {
...
void require_result_schema(const rayd::torch::DiffractionWedgeResult& result, int64_t rows, int device = 0) {
    ...
    for (const auto& tensor : {result.field_vector, result.direction})
        require(tensor.is_cuda() && tensor.get_device() == device && tensor.is_contiguous(),
                "wedge output must be contiguous CUDA storage");
}

void require_jvp_schema(const rayd::torch::DiffractionWedgeJvpResult& result, int64_t rows, int device = 0) {
    require(result.tangent_field_vector.sizes() == at::IntArrayRef({rows, 3}) &&
                result.tangent_field_vector.scalar_type() == at::kComplexFloat,
            "wedge JVP field-vector schema differs");
    require(result.tangent_direction.sizes() == at::IntArrayRef({rows, 3}) &&
                result.tangent_direction.scalar_type() == at::kFloat,
            "wedge JVP direction schema differs");
    for (const auto& tensor : {result.tangent_field_vector, result.tangent_direction})
        require(tensor.is_cuda() && tensor.get_device() == device && tensor.is_contiguous(),
...
std::array<const std::optional<at::Tensor>*, 13> gradient_slots(
    ...
    return {&result.grad_source,     &result.grad_target,      &result.grad_face0_eps_r,   &result.grad_face0_sigma_e,
            &result.grad_face0_gain, &result.grad_face1_eps_r, &result.grad_face1_sigma_e, &result.grad_face1_gain,
            &result.grad_frequency,  &result.grad_vertex_v0,   &result.grad_vertex_v1,     &result.grad_vertex_opp0,
            &result.grad_vertex_opp1};
}

void require_all_gradients_present(const rayd::torch::DiffractionWedgeBackwardResult& result) {
    for (const auto* slot : gradient_slots(result))
        require(slot->has_value(), "requested wedge gradient must be present");
}

void require_all_gradients_absent(const rayd::torch::DiffractionWedgeBackwardResult& result) {
    for (const auto* slot : gradient_slots(result))
        require(!slot->has_value(), "disabled wedge gradient must be absent");
}

void require_all_gradients_zero(const rayd::torch::DiffractionWedgeBackwardResult& result) {
    require_all_gradients_present(result);
    for (const auto* slot : gradient_slots(result))
        require(at::count_nonzero(**slot).item<int64_t>() == 0, "zero-cotangent wedge gradient must be exactly zero");
}

void enable_all_gradients(rayd::torch::DiffractionWedgeBackwardRequest& request) {
...
void test_empty_schema_and_optional_bundle() {
    ...
    const auto empty_gradients = rayd::torch::field_diffraction_wedge_backward(empty_backward);
    require_all_gradients_zero(empty_gradients);
    require(empty_gradients.grad_frequency->sizes() == at::IntArrayRef({1}),
            "empty wedge frequency gradient schema differs");

    auto partial = wedge_request(true);
    partial.edge_boundary.reset();
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(partial); },
    ...
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge_backward(vertex_need); },
    ...
    vertex_tangent.tangent_vertex_v0 = at::ones({1, 3}, vertex_tangent.primal.source.options());
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge_jvp(vertex_tangent); },
...
void test_primal_ad_duality_need_and_zero() {
    ...
    require(at::isfinite(at::view_as_real(forward.field_vector)).all().item<bool>() &&
    ...
    jvp.tangent_target = at::tensor({-0.02F, 0.01F, 0.015F}, floats).reshape({1, 3});
    ...
    jvp.tangent_vertex_v0 = at::tensor({0.002F, -0.001F, 0.003F}, floats).reshape({1, 3});
    jvp.tangent_vertex_v1 = at::tensor({-0.001F, 0.002F, -0.002F}, floats).reshape({1, 3});
    jvp.tangent_vertex_opp0 = at::tensor({0.003F, 0.001F, -0.001F}, floats).reshape({1, 3});
    jvp.tangent_vertex_opp1 = at::tensor({-0.002F, 0.003F, 0.001F}, floats).reshape({1, 3});
    ...
    backward.grad_field_vector = at::ones({1, 3}, floats.dtype(at::kComplexFloat));
    backward.grad_direction = at::full({1, 3}, 0.25F, floats);
    enable_all_gradients(backward);
    const auto gradients = rayd::torch::field_diffraction_wedge_backward(backward);
    require_all_gradients_present(gradients);
    at::cuda::getCurrentCUDAStream(0).synchronize();

    const double lhs = at::view_as_real(tangents.tangent_field_vector).select(-1, 0).sum().item<double>() +
                       0.25 * tangents.tangent_direction.sum().item<double>();
    const double rhs = ((*gradients.grad_source) * (*jvp.tangent_source)).sum().item<double>() +
                       ((*gradients.grad_target) * (*jvp.tangent_target)).sum().item<double>() +
                       ((*gradients.grad_face0_eps_r) * (*jvp.tangent_face0_eps_r)).sum().item<double>() +
                       ((*gradients.grad_face0_sigma_e) * (*jvp.tangent_face0_sigma_e)).sum().item<double>() +
                       ((*gradients.grad_face0_gain) * (*jvp.tangent_face0_gain)).sum().item<double>() +
                       ((*gradients.grad_face1_eps_r) * (*jvp.tangent_face1_eps_r)).sum().item<double>() +
                       ((*gradients.grad_face1_sigma_e) * (*jvp.tangent_face1_sigma_e)).sum().item<double>() +
                       ((*gradients.grad_face1_gain) * (*jvp.tangent_face1_gain)).sum().item<double>() +
                       gradients.grad_frequency->item<double>() * jvp.tangent_frequency +
                       ((*gradients.grad_vertex_v0) * (*jvp.tangent_vertex_v0)).sum().item<double>() +
                       ((*gradients.grad_vertex_v1) * (*jvp.tangent_vertex_v1)).sum().item<double>() +
                       ((*gradients.grad_vertex_opp0) * (*jvp.tangent_vertex_opp0)).sum().item<double>() +
                       ((*gradients.grad_vertex_opp1) * (*jvp.tangent_vertex_opp1)).sum().item<double>();
    const double scale = std::max({1.0, std::fabs(lhs), std::fabs(rhs)});
    require(std::isfinite(lhs) && std::isfinite(rhs) && std::fabs(lhs - rhs) <= 5.0e-3 * scale,
            "wedge JVP/VJP dot-product duality differs");

    rayd::torch::DiffractionWedgeBackwardRequest disabled;
    disabled.primal = primal;
    require_all_gradients_absent(rayd::torch::field_diffraction_wedge_backward(disabled));

    rayd::torch::DiffractionWedgeBackwardRequest zero;
    zero.primal = primal;
    enable_all_gradients(zero);
    require_all_gradients_zero(rayd::torch::field_diffraction_wedge_backward(zero));
}
...
void test_negative_contracts() {
    ...
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(bad_valid_dtype); },
    ...
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(bad_valid_shape); },
                   "wrong wedge valid shape must fail");

    auto bad_shape = wedge_request(true);
    bad_shape.target = at::zeros({1, 2}, bad_shape.target.options());
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(bad_shape); }, "wrong wedge shape must fail");

    auto bad_dtype = wedge_request(true);
    bad_dtype.face0_eps_r = bad_dtype.face0_eps_r.to(at::kDouble);
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(bad_dtype); }, "wrong wedge dtype must fail");

    auto bad_device = wedge_request(true);
    bad_device.source = bad_device.source.cpu();
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(bad_device); }, "CPU wedge input must fail");

    auto noncontiguous = wedge_request(true);
    noncontiguous.source = at::ones({1, 6}, noncontiguous.source.options()).slice(1, 0, 6, 2);
    require(!noncontiguous.source.is_contiguous(), "fixture must be noncontiguous");
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(noncontiguous); },
                   "noncontiguous wedge input must fail");

    auto noncontiguous_optional = wedge_request(true);
    noncontiguous_optional.vertex_v0 = at::ones({1, 6}, noncontiguous_optional.source.options()).slice(1, 0, 6, 2);
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(noncontiguous_optional); },
                   "noncontiguous wedge optional input must fail");

    auto zero_frequency = wedge_request(true);
    zero_frequency.frequency_hz = 0.0;
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(zero_frequency); },
                   "zero wedge frequency must fail");
    auto negative_frequency = wedge_request(true);
    negative_frequency.frequency_hz = -1.0;
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge(negative_frequency); },
    ...
    require_result_schema(rayd::torch::field_diffraction_wedge(infinite_empty), 0);

    rayd::torch::DiffractionWedgeBackwardRequest bad_grad;
    bad_grad.primal = wedge_request(true);
    bad_grad.grad_direction = at::ones({1, 6}, bad_grad.primal.source.options()).slice(1, 0, 6, 2);
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge_backward(bad_grad); },
    ...
    bad_tangent.tangent_face0_eps_r = at::ones({1}, bad_tangent.primal.source.options().dtype(at::kDouble));
    require_throws([&] { (void)rayd::torch::field_diffraction_wedge_jvp(bad_tangent); },
    ...
    if (at::cuda::device_count() > 1) {
        ...
        require_throws([&] { (void)rayd::torch::field_diffraction_wedge(mixed_device); },
...
void test_nondefault_stream_dependency() {
    ...
    reference_jvp.tangent_source = at::ones({1, 3}, reference_primal.source.options());
    rayd::torch::DiffractionWedgeBackwardRequest reference_backward;
    reference_backward.primal = reference_primal;
    reference_backward.grad_field_vector = at::ones({1, 3}, reference_primal.source.options().dtype(at::kComplexFloat));
    reference_backward.need_grad_material = true;
    const auto reference = rayd::torch::field_diffraction_wedge(reference_primal);
    const auto reference_tangent = rayd::torch::field_diffraction_wedge_jvp(reference_jvp);
    const auto reference_gradient = rayd::torch::field_diffraction_wedge_backward(reference_backward);
    ...
    require(at::allclose(result.field_vector, reference.field_vector, 1.0e-5, 1.0e-8) &&
                at::allclose(result.direction, reference.direction, 1.0e-6, 1.0e-8),
            "wedge primal ignored stream dependency");
    require(at::allclose(tangent.tangent_field_vector, reference_tangent.tangent_field_vector, 1.0e-5, 1.0e-8) &&
                at::allclose(tangent.tangent_direction, reference_tangent.tangent_direction, 1.0e-5, 1.0e-8),
            "wedge JVP ignored stream dependency");
    require(gradient.grad_face0_eps_r.has_value() && reference_gradient.grad_face0_eps_r.has_value() &&
                at::allclose(*gradient.grad_face0_eps_r, *reference_gradient.grad_face0_eps_r, 1.0e-5, 1.0e-8),
...
void test_invalid_row_short_circuits_poison() {
    ...
    const auto forward = rayd::torch::field_diffraction_wedge(primal);
    require(at::count_nonzero(forward.field_vector).item<int64_t>() == 0 &&
    ...
    require(at::count_nonzero(tangent.tangent_field_vector).item<int64_t>() == 0 &&
    ...
    enable_all_gradients(backward);
    require_all_gradients_zero(rayd::torch::field_diffraction_wedge_backward(backward));
}
...
int main() {
    try {
        ...
        std::cerr << "rayd::torch pure-wedge direct contract failure: " << error.what() << '\n';
        return 1;
    }
}
