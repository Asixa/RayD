...
# Tests torch public api contract.
...
class PublicApiContractTests(unittest.TestCase):
    ...
    def test_acceptance_benchmarks_use_public_torch_api(self):
        stress = (ROOT / "benchmarks" / "torch" / "benchmark_rayd_torch_drjit_mitsuba_stress.py").read_text()
        multipath = (ROOT / "benchmarks" / "torch" / "benchmark_rayd_torch_drjit_mitsuba_multipath.py").read_text()
    ...
    def test_full_intersection_backward_does_not_contiguous_upstream_grads(self):
        source = (ROOT / "src" / "scene" / "intersection.cpp").read_text()
        forbidden = (
    ...
    def test_camera_public_path_does_not_stage_contiguous_copies(self):
        source = (ROOT / "python" / "rayd" / "_impl" / "camera.py").read_text()
        forbidden = (
            "sample.contiguous()",
            "point.contiguous()",
    ...
    def test_diffraction_active_masks_are_not_expanded_or_contiguous_staged(self):
        python_source = (ROOT / "python" / "rayd" / "_impl" / "multipath.py").read_text()
        cpp_source = (ROOT / "src" / "diffraction" / "diffraction.cpp").read_text()
        forbidden = (
            "active.contiguous()",
    ...
    def test_trace_dfr_paths_public_wrapper_does_not_stage_contiguous_inputs(self):
        source = (ROOT / "python" / "rayd" / "_impl" / "multipath.py").read_text()
    ...
    def test_diffraction_accumulation_forward_does_not_split_vec3_with_aten_copies(self):
        source = (ROOT / "src" / "diffraction" / "diffraction.cpp").read_text()
    ...
    def test_nearest_edge_ad_does_not_contiguous_upstream_grads_or_tangents(self):
        source = (ROOT / "src" / "edge" / "edge.cpp").read_text()
        forbidden = (
            "optional_contiguous_tensor",
    ...
    def test_reflection_epc_ad_does_not_contiguous_upstream_grads_or_tangents(self):
        source = (ROOT / "src" / "reflection" / "reflection.cpp").read_text()
    ...
    def test_reflection_chain_ad_uses_fused_kernels_not_aten_bounce_loop(self):
        source = (ROOT / "src" / "reflection" / "reflection_kernels.cu").read_text()
