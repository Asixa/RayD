...
SCENE_CONTRACT = ROOT / "src/scene/scene_internal.h"
EDGE_CONTRACT = ROOT / "src/edge/optix_contracts.h"
SBT_CONTRACT = ROOT / "include/rayd/contracts.h"
PIPELINE_CONTRACT = ROOT / "src/runtime/rt_internal.h"
EDGE_DEVICE = ROOT / "src/edge/edge_optix_device.cuh"
...
class Share4SceneEdgeOptixContractsTests(unittest.TestCase):
    ...
    def test_edge_programs_use_shared_device_helpers(self):
        for path in (DRJIT_EDGE, TORCH_EDGE):
            ...
            self.assertIn("src/edge/edge_optix_device.cuh", source)
            for symbol in (
    ...
    def test_backend_specific_edge_outer_params_remain_adapters(self):
        drjit_params = (ROOT / "include/rayd/jit/edge.h").read_text(encoding="utf-8")
    ...
    def test_scene_payload_and_hitobject_order_share_one_contract(self):
        ...
        self.assertIn("void set_scene_intersection_payload(", torch_scene)
