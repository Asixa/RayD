...
T *managed(std::size_t count) {
    T *pointer = nullptr;
    return cudaMallocManaged(&pointer, sizeof(T) * count) == cudaSuccess
        ? pointer
        : nullptr;
}
...
int main() {
    ...
    if (!edge || !bounds || !topology || !leaf_primitives || !query_data ||
        !out_ids || !out_values || !stack || !overflow || !active || !mask) {
        return 2;
    }
    ...
    float *d_y = edge + 12;
    float *d_z = edge + 15;
    p0_x[0] = -1.0f; p0_y[0] = 1.0f; p0_z[0] = 0.0f;
    d_x[0] = 2.0f; d_y[0] = 0.0f; d_z[0] = 0.0f;
    p0_x[1] = -1.0f; p0_y[1] = -1.0f; p0_z[1] = 0.0f;
    d_x[1] = 2.0f; d_y[1] = 0.0f; d_z[1] = 0.0f;
    p0_x[2] = 2.0f; p0_y[2] = -1.0f; p0_z[2] = 0.0f;
    d_x[2] = 0.0f; d_y[2] = 2.0f; d_z[2] = 0.0f;
    ...
    min_x[0] = -1.0f; min_y[0] = -1.0f; min_z[0] = 0.0f;
    max_x[0] = 2.0f; max_y[0] = 1.0f; max_z[0] = 0.0f;
    min_x[1] = -1.0f; min_y[1] = -1.0f; min_z[1] = 0.0f;
    max_x[1] = 2.0f; max_y[1] = 1.0f; max_z[1] = 0.0f;
    min_x[2] = -1.0f; min_y[2] = 1.0f; min_z[2] = 0.0f;
    max_x[2] = 1.0f; max_y[2] = 1.0f; max_z[2] = 0.0f;
    ...
    const AabbSoAView node_bounds = {
        min_x, min_y, min_z, max_x, max_y, max_z, 3};
    const CompactBvhTopologyView compact = {
        left, right, leaf_primitives, nullptr, 3, 3, 3};
    EdgeQueryOutputView output = {
        out_ids, out_values, out_values + 2, out_values + 4, 1, 2, 2, 2};
    const BvhTraversalScratchView scratch = {stack, overflow, 1, 1, 1, 1};
    PointBvhQueryParams point_params = {
        edges,
        node_bounds,
        compact,
        {query_data, query_data + 1, query_data + 2, 1},
        output,
        scratch,
        active,
        mask,
    ...
    if (cudaDeviceSynchronize() != cudaSuccess || out_ids[0] != 0 ||
        out_ids[1] != 1 || std::fabs(out_values[0] - 1.0f) > 1.0e-5f) {
        return 3;
    }
    ...
    launch_point_bvh_query_async(point_params);
    if (cudaDeviceSynchronize() != cudaSuccess || overflow[0] != 1u ||
        out_ids[0] != -1) {
        return 5;
    }
    ...
    ray[3] = 1.0f; ray[4] = 0.0f; ray[5] = 0.0f; ray[6] = 0.5f;
    output.result_count = 1;
    RayBvhQueryParams ray_params = {
        edges,
        node_bounds,
        compact,
        {ray, ray + 1, ray + 2, ray + 3, ray + 4, ray + 5, ray + 6, 1},
        output,
        scratch,
        active,
        mask,
    ...
    launch_ray_bvh_query_async(ray_params);
    if (cudaDeviceSynchronize() != cudaSuccess || out_ids[0] != 2 ||
        std::fabs(out_values[0]) > 1.0e-5f ||
