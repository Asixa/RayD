            ...
            for (int j = 0; j < samples; ++j) {
                for (int k = 0; k < samples; ++k) {
                    ...
                    const std::size_t linear =
                        (static_cast<std::size_t>(i) * samples + j) * samples + k;
                    values[linear] =
                        strength * (std::sqrt(x * x + y * y + z * z) - radius);
                }
            }
        }
    }
...
struct GridSampler {
    ...
    float operator()(float t) {
        const Vec3f world = rayd::shared::math::add(
            origin, rayd::shared::math::scale(direction, t));
        ...
        const TrilinearCell cell =
            rayd::shared::sdf::trilinear_cell(u, base, grid.extent);
        const GridSample sample =
            rayd::shared::sdf::sample_cell(grid.values.data(), cell);
    ...
    Vec3f world_gradient() const {
        const Vec3f cells = rayd::shared::sdf::grid_cells(grid.extent);
        return rayd::shared::sdf::local_to_world_direction(
            placement,
            rayd::shared::sdf::local_gradient(index_gradient, cells, grid.scale));
    }
};
...
MarchResult trace(GridSampler &sampler, float tmax) {
    const Vec3f local_origin =
        rayd::shared::sdf::world_to_local_point(sampler.placement, sampler.origin);
    const Vec3f local_direction = rayd::shared::sdf::world_to_local_direction(
        sampler.placement, sampler.direction);
    const Interval interval = rayd::shared::sdf::clip_ray_to_box(
        local_origin, local_direction, sampler.grid.scale, tmax);
    if (!interval.valid) {
...
int check_trilinear_interpolation() {
    ...
    if (!close(weight_sum, 1.0f, 1.0e-6f) ||
        !close(sample.value, 16.75f, 1.0e-5f) ||
        !close(sample.index_gradient.x, 2.0f, 1.0e-5f) ||
        !close(sample.index_gradient.y, 3.0f, 1.0e-5f) ||
    ...
    if (cell.index[0] != origin || cell.index[7] != origin + extent.ny * extent.nz +
                                                        extent.nz + 1) {
        return 3;
    }
    return 0;
...
int check_grid_coordinate_clamp() {
    ...
    const Vec3f u = rayd::shared::sdf::grid_coord(
        make_vec3(-7.0f, 7.0f, 0.0f), scale, cells);
...
int check_placement() {
    ...
    const Placement placement =
        rayd::shared::sdf::make_placement(half, 0.0f, 0.0f, half,
                                          make_vec3(5.0f, 0.0f, 0.0f));
    const Vec3f mapped = rayd::shared::sdf::local_to_world_direction(
        placement, make_vec3(1.0f, 0.0f, 0.0f));
    if (!close(mapped.x, 0.0f, 1.0e-6f) || !close(mapped.y, 1.0f, 1.0e-6f) ||
        !close(mapped.z, 0.0f, 1.0e-6f)) {
        return 6;
    }
    const Vec3f local = rayd::shared::sdf::world_to_local_point(
        placement, make_vec3(5.0f, 3.0f, -1.0f));
    if (!close(local.x, 3.0f, 1.0e-6f) || !close(local.y, 0.0f, 1.0e-6f) ||
        !close(local.z, -1.0f, 1.0e-6f)) {
        return 7;
    }
    // An unnormalized quaternion is normalized internally, not rejected.
    const Placement scaled =
        rayd::shared::sdf::make_placement(3.0f, 0.0f, 0.0f, 0.0f, make_vec3(0, 0, 0));
    if (!close(scaled.axis_x.x, 1.0f, 1.0e-6f) ||
        !close(scaled.axis_y.y, 1.0f, 1.0e-6f) ||
...
int check_slab_clip() {
    ...
    const Interval forward = rayd::shared::sdf::clip_ray_to_box(
        make_vec3(0.0f, 0.0f, -10.0f), make_vec3(0.0f, 0.0f, 1.0f), scale,
        std::numeric_limits<float>::infinity());
    if (!forward.valid || !close(forward.t_lo, 7.0f, 1.0e-6f) ||
        !close(forward.t_hi, 13.0f, 1.0e-6f)) {
        return 9;
    }
    // tmax truncates the far end; the interval never reports past it.
    const Interval bounded = rayd::shared::sdf::clip_ray_to_box(
        make_vec3(0.0f, 0.0f, -10.0f), make_vec3(0.0f, 0.0f, 1.0f), scale, 8.0f);
    if (!bounded.valid || !close(bounded.t_hi, 8.0f, 1.0e-6f)) {
        return 10;
    }
    // A ray parallel to a slab it lies outside of misses without dividing.
    const Interval parallel = rayd::shared::sdf::clip_ray_to_box(
        make_vec3(0.0f, 5.0f, -10.0f), make_vec3(0.0f, 0.0f, 1.0f), scale,
        std::numeric_limits<float>::infinity());
    if (parallel.valid) {
        return 11;
    }
    // A ray pointing away from the box misses: the overlap is empty.
    const Interval behind = rayd::shared::sdf::clip_ray_to_box(
        make_vec3(0.0f, 0.0f, -10.0f), make_vec3(0.0f, 0.0f, -1.0f), scale,
        std::numeric_limits<float>::infinity());
    if (behind.valid) {
        return 12;
    ...
    const Interval inside = rayd::shared::sdf::clip_ray_to_box(
        make_vec3(0.0f, 0.0f, 0.0f), make_vec3(0.0f, 0.0f, 1.0f), scale,
        std::numeric_limits<float>::infinity());
    if (!inside.valid || !close(inside.t_lo, 0.0f, 0.0f) ||
        !close(inside.t_hi, 3.0f, 1.0e-6f)) {
        return 13;
    }
    return 0;
}
...
int check_eps_hit_derivation() {
    ...
    if (!close(rayd::shared::sdf::resolve_eps_hit(-1.0f, scale, cells),
               1.0e-3f * 0.125f, 0.0f)) {
        return 14;
    }
...
int check_eikonal_march() {
    ...
    const Placement placement = rayd::shared::sdf::make_placement(
        1.0f, 0.0f, 0.0f, 0.0f, make_vec3(0.0f, 0.0f, 0.0f));
    GridSampler sampler{grid, placement, make_vec3(-3.0f, 0.0f, 0.0f),
                        make_vec3(1.0f, 0.0f, 0.0f)};
    const MarchResult result =
        trace(sampler, std::numeric_limits<float>::infinity());
    ...
    if (!close(gradient.x, -1.0f, 0.15f) || std::fabs(gradient.y) > 0.15f ||
        std::fabs(gradient.z) > 0.15f) {
        return 19;
    }
    return 0;
}
...
int check_placed_march_is_invariant() {
    ...
    const float half = std::sqrt(0.5f);
    const Placement placement = rayd::shared::sdf::make_placement(
        half, 0.0f, 0.0f, half, make_vec3(5.0f, 0.0f, 0.0f));
    GridSampler sampler{grid, placement, make_vec3(5.0f, -3.0f, 0.0f),
...
int check_inside_start() {
    ...
    const Placement placement = rayd::shared::sdf::make_placement(
        1.0f, 0.0f, 0.0f, 0.0f, make_vec3(0.0f, 0.0f, 0.0f));
    GridSampler sampler{grid, placement, make_vec3(0.0f, 0.0f, 0.0f),
                        make_vec3(1.0f, 0.0f, 0.0f)};
    const MarchResult result =
        trace(sampler, std::numeric_limits<float>::infinity());
...
int check_misses() {
    ...
    GridSampler clipped{grid, placement, make_vec3(-3.0f, 0.0f, 0.0f),
                        make_vec3(1.0f, 0.0f, 0.0f)};
...
int check_bisection_contract() {
    ...
    const MarchResult found =
        rayd::shared::sdf::bisect_bracket(linear, 0.0f, 1.0f, -1.0f, 1.0e-5f, 0);
    ...
    const MarchResult exhausted =
        rayd::shared::sdf::bisect_bracket(step, 0.0f, 1.0f, -1.0f, 1.0e-9f, 0);
    if (!exhausted.hit || !close(exhausted.t, 0.5f, 1.0e-6f) ||
        exhausted.steps != 32) {
        return 29;
    }
