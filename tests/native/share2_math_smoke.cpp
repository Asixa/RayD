...
int main() {
    ...
    const auto ray_jvp = rayd::shared::edge::ray_segment_jvp_fixed_winner(
        make_vec3(0.5f, -0.25f, 1.0f),
        ...
        0.5f,
        false,
        rayd::shared::edge::EdgeDistanceFloatMax,
        make_vec3(0.0f, 0.0f, 0.2f),
        make_vec3(0.0f, 0.0f, 0.0f),
        make_vec3(0.0f, 0.0f, 0.3f),
        make_vec3(0.0f, 0.0f, 0.3f));
    if (!close(ray_jvp.distance, 0.0f) ||
        !close(ray_jvp.ray_parameter, -0.1f) ||
        !close(ray_jvp.ray_point, make_vec3(0.0f, 0.0f, 0.3f)) ||
        !close(ray_jvp.edge_parameter, 0.0f) ||
        !close(ray_jvp.edge_point, make_vec3(0.0f, 0.0f, 0.3f))) {
        return 6;
    }

    const auto ray_vjp = rayd::shared::edge::ray_segment_vjp_fixed_winner(
        make_vec3(0.5f, -0.25f, 1.0f),
        make_vec3(0.0f, 0.0f, -1.0f),
        make_vec3(0.0f, 0.0f, 0.0f),
        make_vec3(1.0f, 0.0f, 0.0f),
    ...
    const bool plane_hit_valid = rayd::shared::reflection::intersect_segment_plane(
        make_vec3(0.0f, 0.0f, -1.0f),
        make_vec3(0.0f, 0.0f, 1.0f),
        make_vec3(0.0f, 0.0f, 0.0f),
        make_vec3(0.0f, 0.0f, 1.0f),
    ...
    if (!close(normal, make_vec3(0.0f, 0.0f, -1.0f)) ||
        ...
        !plane_hit_valid || !close(plane_hit, make_vec3(0.0f, 0.0f, 0.0f))) {
        return 8;
    }
    return 0;
}
