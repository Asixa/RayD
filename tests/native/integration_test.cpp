...
void require_tensor_exact(const at::Tensor& actual, const at::Tensor& expected, const std::string& name) {
...
void require_tensor_contract(const at::Tensor& tensor, at::IntArrayRef sizes, at::ScalarType dtype,
                             const at::Device& device, const std::string& name) {
...
void require_finite(const at::Tensor& tensor, const std::string& name) {
    require(at::isfinite(tensor).all().item<bool>(), name + ": contains a non-finite value");
}
...
MeshFixture make_triangle(int device_index = 0) {
    const auto float_options = at::TensorOptions().dtype(at::kFloat).device(at::Device(at::kCUDA, device_index));
    const auto int_options = at::TensorOptions().dtype(at::kInt).device(at::Device(at::kCUDA, device_index));
    return {
        at::tensor({0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F}, float_options).reshape({3, 3}),
...
rayd::torch::MeshInput mesh_input(const MeshFixture& mesh) {
    return {
        mesh.vertices, mesh.faces, mesh.uv, mesh.face_uv, mesh.to_world_left, mesh.to_world_right, false, true, false,
    };
}
...
rayd::torch::RayBatch empty_rays_with_present_empty_mask(int device_index = 0) {
    const auto float_options = at::TensorOptions().dtype(at::kFloat).device(at::Device(at::kCUDA, device_index));
    const auto bool_options = at::TensorOptions().dtype(at::kBool).device(at::Device(at::kCUDA, device_index));
    return {
...
EmptyDiffractionFixture make_empty_diffraction_fixture(int device_index = 0) {
    ...
    rayd::torch::MaterialPayload material = {
        at::ones({1}, float_options), at::zeros({1}, float_options), at::ones({1}, float_options),
        at::ones({1}, float_options), at::ones({1}, bool_options),
    };
    rayd::torch::Grid2D grid = {
        2, 0.0, -1.0, 1.0, -1.0, 1.0, 2, 2, 1.0,
    };
    return {
        std::move(state),
        std::move(material),
...
EmptyDiffractionFixture make_one_diffraction_fixture(int device_index = 0) {
    ...
    const auto int_options = at::TensorOptions().dtype(at::kInt).device(at::Device(at::kCUDA, device_index));
    const auto bool_options = at::TensorOptions().dtype(at::kBool).device(at::Device(at::kCUDA, device_index));
    const at::Tensor tx_pos = at::tensor({0.0F, -1.0F, 0.25F}, float_options).reshape({1, 3});
    ...
    rayd::torch::Grid2D grid = {
        2, 0.0, -1.0, 1.0, -1.0, 1.0, 4, 4, 0.25,
    };
    return {
        std::move(state),
        std::move(material),
...
void test_scene_and_intersection_typed_contracts() {
    ...
    for (const auto& entry : {edges.edge_v0, edges.edge_v1, edges.edge_face0_global, edges.edge_face1_global,
                              edges.edge_shape_id, edges.edge_local_id, edges.edge_opposite})
        require_tensor_contract(entry, {3}, at::kInt, device, "scene edge record");
    require(at::all(edges.edge_shape_id == 0).item<bool>(), "single-mesh edge records must use shape id zero");
    ...
    require_tensor_exact(hit.p, at::tensor({0.25F, 0.25F, 0.0F}, floats).reshape({1, 3}), "intersect point");
    ...
    for (const auto& entry : {hit.shape_id, hit.prim_id, hit.local_prim_id, hit.global_prim_id}) {
    ...
    require_tensor_contract(gradients.grad_vertices, {3, 3}, at::kFloat, device, "intersect vertex gradient");
    require_tensor_exact(gradients.grad_ray_o, at::tensor({0.0F, 0.0F, -1.0F}, floats).reshape({1, 3}),
                         "intersect ray-origin gradient");
    require_tensor_exact(gradients.grad_ray_d, at::tensor({0.0F, 0.0F, -1.0F}, floats).reshape({1, 3}),
                         "intersect ray-direction gradient");
    require_tensor_exact(gradients.grad_ray_tmax, at::zeros({1}, floats), "intersect ray-tmax gradient");
    require(at::allclose(gradients.grad_vertices.sum(0), at::tensor({0.0F, 0.0F, 1.0F}, floats), 0.0, 0.0),
    ...
    require_tensor_exact(tangents.tangent_t, at::full({1}, -1.0F, floats), "intersect tangent t");
    require_tensor_exact(tangents.tangent_p, at::tensor({1.0F, 1.0F, 0.0F}, floats).reshape({1, 3}),
                         "intersect tangent point");
    require_tensor_contract(tangents.tangent_n, {1, 3}, at::kFloat, device, "intersect tangent normal");
    require_tensor_contract(tangents.tangent_geo_n, {1, 3}, at::kFloat, device, "intersect tangent geo-normal");
    require_tensor_contract(tangents.tangent_uv, {1, 2}, at::kFloat, device, "intersect tangent uv");
    require_tensor_contract(tangents.tangent_barycentric, {1, 3}, at::kFloat, device, "intersect tangent barycentric");
...
void test_empty_and_stream_contracts() {
    ...
    const c10::cuda::CUDAStream stream = c10::cuda::getStreamFromPool(false, 0);
    {
        ...
        require(c10::cuda::getCurrentCUDAStream(0).stream() == stream.stream(),
                "typed intersection changed the caller's active CUDA stream");
        const std::array<at::Tensor, 10> typed_values = {
            typed.t,           typed.p,        typed.n,       typed.geo_n,         typed.uv,
            typed.barycentric, typed.shape_id, typed.prim_id, typed.local_prim_id, typed.global_prim_id,
        };
...
void test_visibility_trace_and_face_normal_typed_contracts() {
    ...
    const auto visibility = rayd::torch::visibility_forward(scene, visibility_request);
    require_tensor_exact(visibility.visible, at::zeros({1}, bools), "blocked segment visibility");
    require_tensor_exact(visibility.blocker_prim, at::zeros({1}, mesh.faces.options()), "visibility blocker primitive");
    require_tensor_exact(visibility.tape_t, at::full({1}, std::numeric_limits<float>::infinity(), floats),
    ...
    const auto trace = rayd::torch::trace_reflections_forward(scene, trace_request);
    require_tensor_exact(trace.valid, at::ones({1, 1}, bools), "reflection trace validity");
    require_tensor_exact(trace.t, at::ones({1, 1}, floats), "reflection trace distance");
    require_tensor_exact(trace.prim_ids, at::zeros({1, 1}, mesh.faces.options()), "reflection trace primitive");

    const auto tape = rayd::torch::trace_reflections_forward_tape(scene, trace_request);
    ...
    require(tape.prim_ids.unsafeGetTensorImpl() == tape.tape_prim_id.unsafeGetTensorImpl(),
            "reflection tape must preserve prim-id tensor aliasing");
    require_tensor_contract(tape.image_sources, {1, 1, 3}, at::kFloat, device, "reflection image sources");
    require_tensor_contract(tape.tape_barycentric, {1, 1, 3}, at::kFloat, device, "reflection barycentric tape");
    require_tensor_contract(tape.tape_hit_points, {1, 1, 3}, at::kFloat, device, "reflection hit-point tape");
    require_tensor_contract(tape.tape_normals, {1, 1, 3}, at::kFloat, device, "reflection normal tape");
    ...
    const auto gradients = rayd::torch::trace_reflections_backward(scene, backward);
    require_tensor_contract(gradients.grad_vertices, {3, 3}, at::kFloat, device, "reflection vertex gradient");
    require_tensor_exact(gradients.grad_ray_o, at::tensor({0.0F, 0.0F, -1.0F}, floats).reshape({1, 3}),
                         "reflection ray-origin gradient");
    require_tensor_exact(gradients.grad_ray_d, at::tensor({0.0F, 0.0F, -1.0F}, floats).reshape({1, 3}),
                         "reflection ray-direction gradient");
    require_tensor_exact(gradients.grad_ray_tmax, at::zeros({1}, floats), "reflection ray-tmax gradient");
    ...
    require_tensor_exact(tangents.tangent_t, at::full({1, 1}, -1.0F, floats), "reflection trace tangent distance");
    require_tensor_contract(tangents.tangent_image_sources, {1, 1, 3}, at::kFloat, device,
                            "reflection image-source tangent");
    require_finite(tangents.tangent_image_sources, "reflection image-source tangent");

    const auto edges = rayd::torch::scene_edge_records(scene);
    const at::Tensor grad_face_normals = at::ones({1, 3}, floats);
    const auto face_gradients = rayd::torch::scene_face_normals_backward(scene, grad_face_normals);
    require_tensor_contract(face_gradients, {3, 3}, at::kFloat, device, "face-normal vertex gradient");
    require_finite(face_gradients, "face-normal vertex gradient");
    require(at::allclose(face_gradients.sum(0), at::zeros({3}, floats), 0.0, 1.0e-6),
            "face-normal gradients must be translation invariant");

    const at::Tensor rigid_translation = at::ones_like(edges.global_vertices);
    const auto face_tangent = rayd::torch::scene_face_normals_jvp(scene, rigid_translation);
    require_tensor_exact(face_tangent, at::zeros({1, 3}, floats), "face-normal rigid-translation tangent");
}
...
void test_axial_edge_visibility_typed_contracts() {
    ...
    const auto result = rayd::torch::axial_edge_visibility_forward(scene, request);
    require_tensor_exact(result.any_visible, at::tensor({1, 0, 1}, mesh.faces.options()).to(at::kBool),
    ...
    active_request.active = at::tensor({1, 1, 0}, mesh.faces.options()).to(at::kBool);
    require_tensor_exact(rayd::torch::axial_edge_visibility_forward(scene, active_request).any_visible,
                         at::tensor({1, 0, 0}, mesh.faces.options()).to(at::kBool), "axial-edge active mask");
    ...
    const auto empty = rayd::torch::axial_edge_visibility_forward(scene, empty_request);
    require_tensor_contract(empty.any_visible, {0}, at::kBool, mesh.vertices.device(), "empty axial-edge visibility");
    ...
    require_tensor_exact(rayd::torch::axial_edge_visibility_forward(scene, nonfinite_request).any_visible,
                         at::zeros({3}, bools), "axial-edge nonfinite lanes");

    auto nonfinite_tx = request;
    nonfinite_tx.tx = at::tensor({nan, 0.25F, -1.0F}, floats);
    require_tensor_exact(rayd::torch::axial_edge_visibility_forward(scene, nonfinite_tx).any_visible,
                         at::zeros({3}, bools), "axial-edge nonfinite transmitter");

    const std::array<bool, 4> fraction_visibility = {false, true, true, true};
    for (std::size_t fraction_index = 0; fraction_index < fraction_visibility.size(); ++fraction_index) {
        ...
        require_tensor_exact(rayd::torch::axial_edge_visibility_forward(scene, fraction_request).any_visible,
                             fraction_visibility[fraction_index] ? at::ones({1}, bools) : at::zeros({1}, bools),
    ...
    require_throws([&] { (void)rayd::torch::axial_edge_visibility_forward(scene, bad_tx_view); },
    ...
    require(!bad_edge_view.edge_position.is_contiguous(), "edge noncontiguous fixture is contiguous");
    require_throws([&] { (void)rayd::torch::axial_edge_visibility_forward(scene, bad_edge_view); },
    ...
    require(!bad_scalar_view.edge_t_min.is_contiguous(), "scalar noncontiguous fixture is contiguous");
    require_throws([&] { (void)rayd::torch::axial_edge_visibility_forward(scene, bad_scalar_view); },
    ...
    require(!bad_active_view.active->is_contiguous(), "active noncontiguous fixture is contiguous");
    require_throws([&] { (void)rayd::torch::axial_edge_visibility_forward(scene, bad_active_view); },
    ...
    require_throws([&] { (void)rayd::torch::axial_edge_visibility_forward(scene, bad_fraction); },
                   "axial-edge nonfinite fraction must fail loudly");

    auto cpu_tx = request;
    cpu_tx.tx = cpu_tx.tx.cpu();
    require_throws([&] { (void)rayd::torch::axial_edge_visibility_forward(scene, cpu_tx); },
                   "axial-edge CPU tx must fail loudly");

    const auto stream = c10::cuda::getStreamFromPool(false, 0);
    {
        c10::cuda::CUDAStreamGuard guard(stream);
        const auto streamed = rayd::torch::axial_edge_visibility_forward(scene, request);
        require(c10::cuda::getCurrentCUDAStream(0).stream() == stream.stream(),
                "axial-edge visibility changed the caller's CUDA stream");
        stream.synchronize();
        require_tensor_exact(streamed.any_visible, at::tensor({1, 0, 1}, mesh.faces.options()).to(at::kBool),
...
std::array<float, 3> exact_axial_sample_host(const std::array<float, 3>& position,
                                             const std::array<float, 3>& direction, float t_min, float t_max,
                                             float fraction) {
...
void test_axial_edge_visibility_legacy_parity() {
    ...
    const auto to_device_matrix = [&](std::vector<float>& values) {
        return at::from_blob(values.data(), {static_cast<int64_t>(state_count), 3}, cpu_floats).clone().to(device);
    };
    const auto to_device_vector = [&](std::vector<float>& values) {
        return at::from_blob(values.data(), {static_cast<int64_t>(state_count)}, cpu_floats).clone().to(device);
    };
    const at::Tensor tx = at::tensor({0.25F, 0.25F, -1.0F}, cpu_floats).to(device);
    rayd::torch::AxialEdgeVisibilityRequest request = {
        tx,
    ...
    const at::Tensor single_launch = rayd::torch::axial_edge_visibility_forward(scene, request).any_visible;
    ...
    at::Tensor four_launches = at::zeros({static_cast<int64_t>(state_count)}, mesh.vertices.options().dtype(at::kBool));
    for (const std::uint32_t fraction_bits : rayd::torch::kDiffractionTxAxialEdgeFractionBits) {
        const float fraction = test_float_from_bits(fraction_bits);
        std::vector<float> ends_aos;
        ends_aos.reserve(state_count * 3);
        for (std::size_t row = 0; row < state_count; ++row) {
            const auto point =
                exact_axial_sample_host(positions[row], directions[row], t_min[row], t_max[row], fraction);
            ends_aos.insert(ends_aos.end(), point.begin(), point.end());
        }
        const auto visibility =
            rayd::torch::visibility_forward(scene, {starts, to_device_matrix(ends_aos), std::nullopt});
        four_launches.bitwise_or_(visibility.visible);
    }
    require_tensor_exact(single_launch, four_launches,
...
void test_reflection_accumulation_and_epc_typed_contracts() {
    ...
    const rayd::torch::MaterialPayload material = {
        at::full({1}, 4.0F, floats), at::zeros({1}, floats), at::ones({1}, floats),
        at::ones({1}, floats),       at::ones({1}, bools),
    };
    const rayd::torch::Grid2D grid = {
        2, 0.5, -1.0, 1.0, -1.0, 1.0, 4, 4, 0.25,
    };
    ...
    rayd::torch::ReflectionAccumulationConfig accumulation = {
        rays,     rays.ray_o, at::tensor({1.0F, 0.0F, 0.0F}, floats).reshape({1, 3}),
        material, 1,          grid,
        1.0,      1.0,        false,
        false,    0,          1,
        0,        0,          0,
    ...
    const auto accumulated = rayd::torch::reflection_accumulation_forward(scene, accumulation);
    for (const auto& field : {accumulated.power, accumulated.field_x_re, accumulated.field_x_im, accumulated.field_y_re,
                              accumulated.field_y_im, accumulated.field_z_re, accumulated.field_z_im}) {
        require_tensor_contract(field, {4, 4}, at::kFloat, device, "reflection accumulation grid");
        require_finite(field, "reflection accumulation grid");
    }
    require(at::all(accumulated.power >= 0).item<bool>(), "reflection accumulated power must be non-negative");
    require_tensor_contract(accumulated.reflection_count, {1}, at::kInt, device, "reflection accumulation count");
    const int reflection_count = accumulated.reflection_count.item<int>();
    require(reflection_count > 0,
            "nonempty reflection ray produced no reflected-grid hit; count=" + std::to_string(reflection_count));
    require_tensor_exact(accumulated.wedge_count, at::zeros({1}, ints), "disabled wedge count");
    for (const auto& wedge :
         {accumulated.wedge_ray_index, accumulated.wedge_hit, accumulated.wedge_normal, accumulated.wedge_prim_id,
          accumulated.wedge_direction, accumulated.wedge_source, accumulated.wedge_source_power,
          accumulated.wedge_initial_direction, accumulated.wedge_bounce_depth})
    ...
    require_tensor_exact(path.path_length, at::full({1}, 2.0F, floats), "reflection EPC path length");
    require_tensor_exact(path.resolved_prim_ids, at::zeros({1, 1}, ints), "reflection EPC primitive");
    require_tensor_exact(path.surface_group_ids, at::zeros({1, 1}, ints), "reflection EPC group");
    require_tensor_exact(path.hit_positions, at::zeros({1, 1, 3}, floats), "reflection EPC hit");
    require_tensor_exact(path.normals, at::tensor({0.0F, 0.0F, 1.0F}, floats).reshape({1, 1, 3}),
    ...
    const auto gradients = rayd::torch::reflection_epc_paths_backward(scene, backward);
    require_tensor_contract(gradients.grad_vertices, {3, 3}, at::kFloat, device, "reflection EPC vertex gradient");
    require_tensor_exact(gradients.grad_source, at::tensor({0.0F, 0.0F, -1.0F}, floats).reshape({1, 3}),
                         "reflection EPC source gradient");
    require_tensor_exact(gradients.grad_receiver, at::tensor({0.0F, 0.0F, -1.0F}, floats).reshape({1, 3}),
                         "reflection EPC receiver gradient");
    require(at::allclose(gradients.grad_vertices.sum(0), at::tensor({0.0F, 0.0F, 2.0F}, floats), 0.0, 1.0e-6),
    ...
    require_tensor_exact(tangents.tangent_points, at::ones({1, 1, 3}, floats),
                         "reflection EPC rigid-translation point tangent");
    require_tensor_exact(tangents.tangent_normals, at::zeros({1, 1, 3}, floats),
                         "reflection EPC rigid-translation normal tangent");
    require_tensor_exact(tangents.tangent_path_length, at::zeros({1}, floats),
                         "reflection EPC rigid-translation length tangent");
}

void test_error_and_lifecycle_contracts() {
    require_throws([] { (void)rayd::torch::create_scene({}); }, "empty typed scene construction must fail");

    MeshFixture bad_mesh = make_triangle();
    bad_mesh.vertices = bad_mesh.vertices.to(at::kInt);
    require_throws([&] { (void)rayd::torch::create_scene({mesh_input(bad_mesh)}); },
    ...
    require_throws([&] { (void)rayd::torch::scene_edge_records(scene); }, "operation on moved-from scene must fail");

    auto invalid_rays = one_hit_ray();
    invalid_rays.ray_o = invalid_rays.ray_o.to(at::kDouble);
    require_throws([&] { (void)rayd::torch::intersect_forward(moved, invalid_rays, 0); },
                   "invalid ray dtype must fail");
    auto invalid_shape = one_hit_ray();
    invalid_shape.ray_o = at::empty({1, 2}, mesh.vertices.options());
    require_throws([&] { (void)rayd::torch::intersect_forward(moved, invalid_shape, 0); },
    ...
    require(rayd::torch::intersect_forward(simultaneous[0], one_hit_ray(), 0).t.numel() == 1 &&
    ...
    if (at::cuda::device_count() > 1) {
        ...
        require_throws([&] { (void)rayd::torch::intersect_forward(moved, rays_on_second_device, 0); },
...
void test_diffraction_paths_typed_contracts() {
    ...
    rayd::torch::DiffractionPathConfig config = {
        fixture.tx_pos, fixture.tx_pol, fixture.rx_pos, fixture.active, fixture.state, fixture.material, 1, 8, 1.0, 0.0,
    };
    const auto paths = rayd::torch::diffraction_paths_order1_forward(scene, config);
    ...
    for (const auto& entry : {paths.tx_id, paths.rx_id, paths.order, paths.edge0, paths.edge1, paths.edge2})
        require_tensor_contract(entry, {8}, at::kInt, device, "diffraction path index");
    for (const auto& entry : {paths.delay, paths.field_x_re, paths.field_x_im, paths.field_y_re, paths.field_y_im,
                              paths.field_z_re, paths.field_z_im}) {
    ...
    require(paths.valid.sum().item<int64_t>() == count, "diffraction valid rows must equal the exported count");
    if (count > 0) {
        const auto rows = at::arange(count, ints.dtype(at::kLong));
        require(at::all(paths.tx_id.index_select(0, rows) == 0).item<bool>() &&
        ...
        require(at::all(paths.delay.index_select(0, rows) >= 0).item<bool>(),
    ...
    const auto lane_paths = rayd::torch::diffraction_paths_order1_forward(scene, source_lane);
    require(lane_paths.count.item<int>() == 1, "source-lane count must track valid paths");
    require(!lane_paths.valid.select(0, 0).item<bool>() && lane_paths.valid.select(0, 1).item<bool>(),
            "source-lane export must preserve the pair/state lane index");
    require(lane_paths.tx_id.select(0, 1).item<int>() == 0 && lane_paths.rx_id.select(0, 1).item<int>() == 0 &&
                lane_paths.edge0.select(0, 1).item<int>() == 0,
            "source-lane path identity differs");
    require(lane_paths.tx_id.select(0, 0).item<int>() == -1 && lane_paths.rx_id.select(0, 0).item<int>() == -1 &&
                lane_paths.order.select(0, 0).item<int>() == 0 && lane_paths.edge0.select(0, 0).item<int>() == -1 &&
                lane_paths.edge1.select(0, 0).item<int>() == -1 && lane_paths.edge2.select(0, 0).item<int>() == -1,
            "source-lane inactive identity must remain canonical");
    for (const auto& entry : {lane_paths.delay, lane_paths.field_x_re, lane_paths.field_x_im, lane_paths.field_y_re,
                              lane_paths.field_y_im, lane_paths.field_z_re, lane_paths.field_z_im}) {
        require(entry.select(0, 0).item<float>() == 0.0f,
    ...
    for (const auto& entry : {lane_paths.p0, lane_paths.p1, lane_paths.p2}) {
        require(at::equal(entry.select(0, 0), at::zeros_like(entry.select(0, 0))),
    ...
    const auto compact_paths = rayd::torch::diffraction_paths_order1_forward(scene, compact_lane);
    require(compact_paths.count.item<int>() == 1 && compact_paths.valid.select(0, 0).item<bool>(),
            "compact parity fixture must export one row");
    for (const auto& pair : {std::pair<at::Tensor, at::Tensor>{compact_paths.tx_id, lane_paths.tx_id},
        ...
        require(at::equal(pair.first.select(0, 0), pair.second.select(0, 1)),
    ...
    const auto multi_paths = rayd::torch::diffraction_paths_order1_forward(scene, multi_lane);
    ...
    require(!pair_lanes.select(1, 0).any().item<bool>() && pair_lanes.select(1, 1).all().item<bool>(),
            "source-lane state must be the fastest-varying coordinate");
    const auto odd_rows = at::tensor({1, 3, 5, 7}, ints.dtype(at::kLong));
    require(at::equal(multi_paths.tx_id.index_select(0, odd_rows), at::tensor({0, 0, 1, 1}, ints)) &&
                at::equal(multi_paths.rx_id.index_select(0, odd_rows), at::tensor({0, 1, 0, 1}, ints)),
    ...
    require_throws([&] { (void)rayd::torch::diffraction_paths_order1_forward(scene, invalid_layout); },
                   "invalid diffraction path layout must fail loudly");

    auto missing_active = config;
    missing_active.active = at::Tensor();
    require_throws([&] { (void)rayd::torch::diffraction_paths_order1_forward(scene, missing_active); },
                   "missing diffraction path active tensor must fail loudly");

    auto wrong_shape = config;
    wrong_shape.active = at::ones({2}, bools);
    require_throws([&] { (void)rayd::torch::diffraction_paths_order1_forward(scene, wrong_shape); },
                   "diffraction path active shape must equal state_limit");

    auto wrong_dtype = config;
    wrong_dtype.active = at::ones({1}, ints);
    require_throws([&] { (void)rayd::torch::diffraction_paths_order1_forward(scene, wrong_dtype); },
    ...
    require(!noncontiguous_active.active.is_contiguous(),
            "diffraction path noncontiguous active fixture is contiguous");
    require_throws([&] { (void)rayd::torch::diffraction_paths_order1_forward(scene, noncontiguous_active); },
    ...
    require_throws([&] { (void)rayd::torch::diffraction_paths_order1_forward(scene, cpu_active); },
    ...
    if (device_count > 1) {
        ...
        other_device_active.active =
            at::ones({1}, at::TensorOptions().dtype(at::kBool).device(at::Device(at::kCUDA, 1)));
        require_throws([&] { (void)rayd::torch::diffraction_paths_order1_forward(scene, other_device_active); },
    ...
    for (const auto& entry :
         {poisoned.delay, poisoned.field_x_re, poisoned.field_x_im, poisoned.field_y_re, poisoned.field_y_im,
          poisoned.field_z_re, poisoned.field_z_im, poisoned.p0, poisoned.p1, poisoned.p2}) {
        require(at::equal(entry, at::zeros_like(entry)),
...
void test_diffraction_accumulation_typed_contracts() {
    ...
    const auto accumulated = rayd::torch::diffraction_accumulation_forward(scene, config);
    for (const auto& entry : {accumulated.power, accumulated.field_x_re, accumulated.field_x_im, accumulated.field_y_re,
                              accumulated.field_y_im, accumulated.field_z_re, accumulated.field_z_im}) {
        require_tensor_contract(entry, {4, 4}, at::kFloat, device, "diffraction accumulation grid");
        require_finite(entry, "diffraction accumulation grid");
    }
    require(at::all(accumulated.power >= 0).item<bool>(), "diffraction accumulated power must be non-negative");
    for (const auto& counter :
         {accumulated.direct_count, accumulated.keller_count, accumulated.suffix_count, accumulated.visibility_rejects,
          accumulated.edge_visibility_rejects, accumulated.utd_rejects, accumulated.edge_uses}) {
        require_tensor_contract(counter, {1}, at::kInt, device, "diffraction accumulation counter");
        require(counter.item<int>() >= 0, "diffraction accumulation counter must be non-negative");
    }
    require(accumulated.direct_count.item<int>() + accumulated.visibility_rejects.item<int>() +
                    accumulated.edge_visibility_rejects.item<int>() + accumulated.utd_rejects.item<int>() >
                0,
            "nonempty direct diffraction samples produced no accounting outcome");
    for (const auto& tape : {accumulated.tape_active, accumulated.tape_state_idx, accumulated.tape_cell,
                             accumulated.tape_material_idx, accumulated.tape_edge_u})
...
void test_coherent_diffraction_typed_contracts() {
    ...
    rayd::torch::CoherentDiffractionConfig config = {
        fixture.active, fixture.state, fixture.material, 1, fixture.grid, 1.0, true, true,
    };
    const auto coherent = rayd::torch::diffraction_coherent_accumulation_forward(scene, config);
    for (const auto& entry : {coherent.direct_x_re, coherent.direct_x_im, coherent.direct_y_re, coherent.direct_y_im,
                              coherent.direct_z_re, coherent.direct_z_im, coherent.multi_x_re, coherent.multi_x_im,
                              coherent.multi_y_re, coherent.multi_y_im, coherent.multi_z_re, coherent.multi_z_im}) {
        require_tensor_contract(entry, {4, 4}, at::kFloat, device, "coherent diffraction field");
        require_finite(entry, "coherent diffraction field");
    }
    for (const auto& counter :
         {coherent.direct_count, coherent.multi_count, coherent.visibility_reject_count, coherent.utd_reject_count}) {
        require_tensor_contract(counter, {4, 4}, at::kInt, device, "coherent diffraction counter");
        require(at::all(counter >= 0).item<bool>(), "coherent diffraction counters must be non-negative");
    }
    require(coherent.direct_count.sum().item<int64_t>() + coherent.visibility_reject_count.sum().item<int64_t>() +
                    coherent.utd_reject_count.sum().item<int64_t>() >
                0,
            "nonempty coherent diffraction state produced no accounting outcome");
}

std::array<at::Tensor, 12> layer_stack_values(const rayd::torch::LayerStackResult& result) {
    return {result.r_te_real, result.r_te_imag, result.r_tm_real, result.r_tm_imag, result.t_te_real, result.t_te_imag,
            result.t_tm_real, result.t_tm_imag, result.cap_r_te,  result.cap_r_tm,  result.cap_t_te,  result.cap_t_tm};
}
...
void test_layer_stack_empty_and_contracts() {
    ...
    rayd::torch::LayerStackRequest primal = {
        at::empty({0}, float_options), at::empty({0}, int_options),   at::zeros({1}, int_options),
        at::zeros({1}, int_options),   at::empty({0}, float_options), at::empty({0}, float_options),
        at::empty({0}, float_options), at::empty({0}, float_options), 3.5e9,
    };
    ...
    require(gradients.grad_frequency.dim() == 1 && gradients.grad_frequency.size(0) == 1,
            "frequency gradient schema differs");

    auto invalid = primal;
    invalid.cos_theta = invalid.cos_theta.to(at::kDouble);
    require_throws([&] { (void)rayd::torch::em_layer_stack_eval(invalid); },
...
void test_layer_stack_nonempty_ad_and_stream() {
    ...
    rayd::torch::LayerStackRequest primal = {
        at::tensor({0.65F}, float_options), at::tensor({0}, int_options),  at::tensor({0}, int_options),
        at::tensor({0}, int_options),       at::empty({0}, float_options), at::empty({0}, float_options),
        at::empty({0}, float_options),      at::empty({0}, float_options), 3.5e9,
    };
    ...
    const std::array<float, 12> identity_expected = {
        0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 1.0F, 1.0F,
    };
    ...
    for (std::size_t index = 0; index < identity_values.size(); ++index) {
        ...
        require(std::fabs(value - identity_expected[index]) <= 1.0e-6F,
    ...
    for (std::size_t index = 0; index < backward.grad_outputs.size(); ++index)
        backward.grad_outputs[index] = at::full({1}, 0.125F * static_cast<float>(index + 1), float_options);
    backward.need_cos_theta = true;
    backward.need_layers = true;
    backward.need_frequency = true;
    const auto gradients = rayd::torch::em_layer_stack_backward(backward);
    ...
    double rhs = static_cast<double>(gradients.grad_cos_theta.item<float>()) * 0.1 +
    ...
    require(std::fabs(lhs - rhs) <= 5.0e-4 * (duality_scale > 1.0 ? duality_scale : 1.0),
    ...
    stream.synchronize();
    require(disabled_gradients.grad_cos_theta.item<float>() == 0.0F, "disabled cos gradient must be zero");
    require(disabled_gradients.grad_layer_thickness_m.item<float>() == 0.0F,
            "disabled thickness gradient must be zero");
    require(disabled_gradients.grad_layer_eps_r.item<float>() == 0.0F, "disabled eps gradient must be zero");
    require(disabled_gradients.grad_layer_sigma_e.item<float>() == 0.0F, "disabled sigma gradient must be zero");
    require(disabled_gradients.grad_frequency.item<float>() == 0.0F, "disabled frequency gradient must be zero");

    auto invalid_jvp = jvp;
    invalid_jvp.tangent_cos_theta = at::tensor({0.1}, at::TensorOptions().dtype(at::kDouble).device(at::kCUDA));
    require_throws([&] { (void)rayd::torch::em_layer_stack_jvp(invalid_jvp); },
...
void require_layer_stack_scalar(const at::Tensor& tensor, float expected, const std::string& name) {
    const float value = tensor.item<float>();
    require(std::isfinite(value) && std::fabs(value - expected) <= 3.0e-5F,
...
void test_layer_stack_lossy_primal_and_negative_contracts() {
    ...
    const std::array<float, 12> expected = {
        -0.35482694F, -0.14114616F, -0.05475237F, -0.02465952F, -0.55237210F, 0.22183056F,
        -0.69762940F, 0.21103882F,  0.14582440F,  0.00360591F,  0.35432373F,  0.53122417F,
    };
    const auto values = layer_stack_values(result);
    for (std::size_t index = 0; index < values.size(); ++index)
        require_layer_stack_scalar(values[index], expected[index], "lossy layer-stack field " + std::to_string(index));
    ...
    require_throws([&] { (void)rayd::torch::em_layer_stack_eval(bad_shape); },
                   "layer-stack row-shape mismatch must fail loudly");

    auto bad_contiguous = request;
    bad_contiguous.cos_theta = at::ones({2, 2}, request.cos_theta.options()).select(1, 0);
    bad_contiguous.material_id = at::zeros({2}, request.material_id.options());
    require(!bad_contiguous.cos_theta.is_contiguous(), "test fixture must be non-contiguous");
    require_throws([&] { (void)rayd::torch::em_layer_stack_eval(bad_contiguous); },
                   "layer-stack non-contiguous input must fail loudly");

    rayd::torch::LayerStackBackwardRequest backward;
    backward.primal = request;
    backward.grad_outputs[0] = at::ones({1}, request.cos_theta.options().dtype(at::kDouble));
    require_throws([&] { (void)rayd::torch::em_layer_stack_backward(backward); },
    ...
    if (at::cuda::device_count() > 1) {
        ...
        require_throws([&] { (void)rayd::torch::em_layer_stack_eval(bad_device); },
...
void test_layer_stack_nondefault_stream_dependency() {
    ...
    auto scratch = at::empty({64 * 1024 * 1024}, request.cos_theta.options().dtype(at::kByte));
    cudaEvent_t ready = nullptr;
    C10_CUDA_CHECK(cudaEventCreateWithFlags(&ready, cudaEventDisableTiming));
    for (int iteration = 0; iteration < 32; ++iteration) {
        C10_CUDA_CHECK(cudaMemsetAsync(scratch.data_ptr(), iteration, static_cast<std::size_t>(scratch.numel()),
                                       producer.stream()));
    }
    {
...
rayd::torch::TransmissionSequenceRequest transmission_request(bool active, float eps_r = 4.0F, float sigma_e = 0.05F,
...
void require_transmission_schema(const rayd::torch::TransmissionSequenceResult& result, int64_t rows,
                                 int expected_device = 0) {
    require(result.field_vector.sizes() == at::IntArrayRef({rows, 3}) &&
    ...
    for (const auto& tensor : {result.coefficient, result.path_field})
        require(tensor.sizes() == at::IntArrayRef({rows}) && tensor.scalar_type() == at::kComplexFloat,
                "transmission complex scalar schema differs");
    for (const auto& tensor : {result.path_gain, result.path_length_m, result.delay_s})
        require(tensor.sizes() == at::IntArrayRef({rows}) && tensor.scalar_type() == at::kFloat,
                "transmission real scalar schema differs");
    require(result.direction.sizes() == at::IntArrayRef({rows, 3}) && result.direction.scalar_type() == at::kFloat,
            "transmission direction schema differs");
    for (const auto& tensor : {result.field_vector, result.coefficient, result.path_field, result.path_gain,
                               result.path_length_m, result.delay_s, result.direction})
        require(tensor.is_cuda() && tensor.get_device() == expected_device && tensor.is_contiguous(),
                "transmission output must be contiguous CUDA storage");
}

void require_transmission_jvp_schema(const rayd::torch::TransmissionSequenceJvpResult& result, int64_t rows,
                                     int expected_device = 0) {
    require(result.field_vector.sizes() == at::IntArrayRef({rows, 3}) &&
                result.field_vector.scalar_type() == at::kComplexFloat,
    ...
    for (const auto& tensor : {result.field_vector, result.coefficient, result.path_field, result.path_gain,
                               result.path_length_m, result.delay_s})
        require(tensor.is_cuda() && tensor.get_device() == expected_device && tensor.is_contiguous(),
...
void test_transmission_sequence_primal_and_depth_contracts() {
    ...
    require_transmission_jvp_schema(rayd::torch::field_transmission_sequence_jvp(empty_jvp), 0);
    rayd::torch::TransmissionSequenceBackwardRequest empty_backward;
    empty_backward.primal = empty;
    const auto empty_absent = rayd::torch::field_transmission_sequence_backward(empty_backward);
    require(!empty_absent.grad_layer_thickness_m.has_value() && !empty_absent.grad_layer_eps_r.has_value() &&
                !empty_absent.grad_layer_sigma_e.has_value() && !empty_absent.grad_frequency.has_value() &&
                !empty_absent.grad_source.has_value() && !empty_absent.grad_target.has_value() &&
    ...
    const auto empty_defined = rayd::torch::field_transmission_sequence_backward(empty_backward);
    require(empty_defined.grad_layer_thickness_m.has_value() && empty_defined.grad_layer_eps_r.has_value() &&
                empty_defined.grad_layer_sigma_e.has_value() && empty_defined.grad_frequency.has_value() &&
                empty_defined.grad_source.has_value() && empty_defined.grad_target.has_value() &&
                !empty_defined.grad_interaction_positions.has_value() &&
                empty_defined.grad_interaction_normals.has_value() &&
                empty_defined.grad_layer_thickness_m->sizes() == at::IntArrayRef({empty.layer_thickness_m.size(0)}) &&
                empty_defined.grad_layer_eps_r->sizes() == at::IntArrayRef({empty.layer_eps_r.size(0)}) &&
                empty_defined.grad_layer_sigma_e->sizes() == at::IntArrayRef({empty.layer_sigma_e.size(0)}) &&
                ...
                empty_defined.grad_interaction_normals->sizes() == at::IntArrayRef({0, 1, 3}),
            "empty transmission backward requested-output schema differs");
    for (const auto& tensor :
         {*empty_defined.grad_layer_thickness_m, *empty_defined.grad_layer_eps_r, *empty_defined.grad_layer_sigma_e,
          *empty_defined.grad_frequency, *empty_defined.grad_source, *empty_defined.grad_target,
          *empty_defined.grad_interaction_normals})
        require(tensor.is_cuda() && tensor.get_device() == empty.source.get_device() &&
    ...
    const auto inactive = rayd::torch::field_transmission_sequence(transmission_request(false));
    const auto lossy = rayd::torch::field_transmission_sequence(transmission_request(true));
    at::cuda::getCurrentCUDAStream(0).synchronize();
    require_transmission_schema(lossy, 1);
    require(std::fabs(lossy.path_length_m.item<float>() - 4.0F) <= 1.0e-6F,
            "transmission full straight path length differs");
    require(std::fabs(lossy.delay_s.item<float>() - 4.0F / 299792458.0F) <= 1.0e-12F, "transmission delay differs");
    require(at::allclose(lossy.direction, at::tensor({0.0F, 0.0F, -1.0F}, lossy.direction.options()).reshape({1, 3}),
                         0.0, 0.0),
            "transmission direction differs");
    require(lossy.path_gain.item<float>() < inactive.path_gain.item<float>(),
            "lossy transmission must attenuate the inactive-wall field");

    const auto vacuum = rayd::torch::field_transmission_sequence(transmission_request(true, 1.0F, 0.0F, 0.3F));
    at::cuda::getCurrentCUDAStream(0).synchronize();
    require(at::allclose(vacuum.field_vector, inactive.field_vector, 1.0e-4, 1.0e-9),
            "vacuum wall must reproduce the no-wall field");

    auto depth_nine = transmission_request(false);
    depth_nine.interaction_positions = at::zeros({1, 9, 3}, depth_nine.source.options());
    depth_nine.interaction_normals = at::zeros({1, 9, 3}, depth_nine.source.options());
    depth_nine.interaction_material_id = at::zeros({1, 9}, depth_nine.layer_offset.options());
    depth_nine.interaction_valid = at::zeros({1, 9}, depth_nine.interaction_valid.options());
    require_transmission_schema(rayd::torch::field_transmission_sequence(depth_nine), 1);
    rayd::torch::TransmissionSequenceJvpRequest too_deep_jvp;
    too_deep_jvp.primal = depth_nine;
    require_throws([&] { (void)rayd::torch::field_transmission_sequence_jvp(too_deep_jvp); },
...
void test_transmission_sequence_ad_duality_and_optional_schema() {
    ...
    jvp.tangent_frequency = 1.0e6;
    jvp.tangent_source = at::tensor({0.01F, -0.02F, 0.03F}, floats).reshape({1, 3});
    jvp.tangent_target = at::tensor({-0.01F, 0.01F, -0.02F}, floats).reshape({1, 3});
    jvp.tangent_interaction_positions = at::ones({1, 1, 3}, floats);
    jvp.tangent_interaction_normals = at::tensor({0.02F, 0.01F, 0.0F}, floats).reshape({1, 1, 3});
    ...
    const auto gradients = rayd::torch::field_transmission_sequence_backward(backward);
    at::cuda::getCurrentCUDAStream(0).synchronize();

    const double lhs = at::view_as_real(tangents.field_vector).select(-1, 0).sum().item<double>() +
                       ...
                       tangents.path_gain.sum().item<double>() + tangents.path_length_m.sum().item<double>() +
                       tangents.delay_s.sum().item<double>();
    double rhs = ((*gradients.grad_layer_thickness_m) * (*jvp.tangent_layer_thickness_m)).sum().item<double>() +
                 ((*gradients.grad_layer_eps_r) * (*jvp.tangent_layer_eps_r)).sum().item<double>() +
                 ((*gradients.grad_layer_sigma_e) * (*jvp.tangent_layer_sigma_e)).sum().item<double>() +
                 ...
                 ((*gradients.grad_interaction_normals) * (*jvp.tangent_interaction_normals)).sum().item<double>();
    const double scale = std::max({1.0, std::fabs(lhs), std::fabs(rhs)});
    require(std::isfinite(lhs) && std::isfinite(rhs) && std::fabs(lhs - rhs) <= 2.0e-3 * scale,
            "transmission JVP/VJP dot-product duality differs");
    require(!gradients.grad_interaction_positions.has_value(),
    ...
    const auto disabled_result = rayd::torch::field_transmission_sequence_backward(disabled);
    require(!disabled_result.grad_layer_thickness_m.has_value() && !disabled_result.grad_layer_eps_r.has_value() &&
                !disabled_result.grad_layer_sigma_e.has_value() && !disabled_result.grad_frequency.has_value() &&
                !disabled_result.grad_source.has_value() && !disabled_result.grad_target.has_value() &&
    ...
    const auto zero_gradients = rayd::torch::field_transmission_sequence_backward(zero_cotangents);
    require(zero_gradients.grad_layer_thickness_m.has_value() && zero_gradients.grad_layer_eps_r.has_value() &&
                zero_gradients.grad_layer_sigma_e.has_value() && zero_gradients.grad_frequency.has_value() &&
                zero_gradients.grad_source.has_value() && zero_gradients.grad_target.has_value() &&
    ...
    for (const auto& tensor :
         {*zero_gradients.grad_layer_thickness_m, *zero_gradients.grad_layer_eps_r, *zero_gradients.grad_layer_sigma_e,
          *zero_gradients.grad_frequency, *zero_gradients.grad_source, *zero_gradients.grad_target,
          *zero_gradients.grad_interaction_normals})
        require(at::count_nonzero(tensor).item<int64_t>() == 0,
...
void test_transmission_sequence_negative_and_stream_contracts() {
    ...
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(bad_path_valid_dtype); },
    ...
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(bad_path_valid_shape); },
    ...
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(bad_dtype); },
                   "transmission wrong dtype must fail loudly");
    auto cpu_primal = primal;
    cpu_primal.source = cpu_primal.source.cpu();
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(cpu_primal); },
                   "transmission CPU primal must fail loudly");
    auto noncontiguous_primal = primal;
    noncontiguous_primal.source = at::zeros({1, 3, 2}, primal.source.options()).select(2, 0);
    require(!noncontiguous_primal.source.is_contiguous(),
            "noncontiguous transmission fixture unexpectedly became contiguous");
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(noncontiguous_primal); },
                   "transmission noncontiguous primal must fail loudly");
    auto bad_rows = primal;
    bad_rows.target = at::zeros({2, 3}, primal.target.options());
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(bad_rows); },
                   "transmission row mismatch must fail loudly");
    auto bad_csr = primal;
    bad_csr.layer_count = at::ones({2}, primal.layer_count.options());
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(bad_csr); },
                   "transmission CSR material-count mismatch must fail loudly");
    auto bad_depth = primal;
    bad_depth.interaction_positions = at::empty({1, 0, 3}, primal.source.options());
    bad_depth.interaction_normals = at::empty({1, 0, 3}, primal.source.options());
    bad_depth.interaction_material_id = at::empty({1, 0}, primal.layer_offset.options());
    bad_depth.interaction_valid = at::empty({1, 0}, primal.interaction_valid.options());
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(bad_depth); },
                   "transmission D=0 must fail loudly");
    auto bad_frequency = primal;
    bad_frequency.frequency_hz = 0.0;
    require_throws([&] { (void)rayd::torch::field_transmission_sequence(bad_frequency); },
                   "transmission non-positive frequency must fail loudly");
    auto bad_material = primal;
    bad_material.interaction_material_id = bad_material.interaction_material_id.clone();
    bad_material.interaction_material_id.fill_(1);
    const auto invalid_material = rayd::torch::field_transmission_sequence(bad_material);
    require(invalid_material.path_gain.item<float>() == 0.0F,
            "transmission out-of-range material id must invalidate the path");
    auto inactive_invalid = transmission_request(false);
    const auto inactive_reference = rayd::torch::field_transmission_sequence(inactive_invalid);
    inactive_invalid.interaction_material_id = inactive_invalid.interaction_material_id.clone();
    inactive_invalid.interaction_material_id.fill_(1);
    const auto inactive_with_invalid_material = rayd::torch::field_transmission_sequence(inactive_invalid);
    require(at::allclose(inactive_with_invalid_material.coefficient, inactive_reference.coefficient, 0.0, 0.0),
            "inactive transmission slot must ignore its material id");
    auto bad_jvp = jvp;
    bad_jvp.tangent_source = at::ones({2, 3}, primal.source.options());
    require_throws([&] { (void)rayd::torch::field_transmission_sequence_jvp(bad_jvp); },
                   "transmission tangent shape mismatch must fail loudly");
    auto noncontiguous_jvp = jvp;
    noncontiguous_jvp.tangent_source = at::ones({1, 3, 2}, primal.source.options()).select(2, 0);
    require(!noncontiguous_jvp.tangent_source->is_contiguous(),
            "noncontiguous tangent fixture unexpectedly became contiguous");
    require_transmission_jvp_schema(rayd::torch::field_transmission_sequence_jvp(noncontiguous_jvp), 1);
    auto bad_backward = backward;
    bad_backward.grad_path_gain = at::ones({1}, primal.source.options().dtype(at::kDouble));
    require_throws([&] { (void)rayd::torch::field_transmission_sequence_backward(bad_backward); },
                   "transmission cotangent dtype mismatch must fail loudly");
    auto noncontiguous_backward = backward;
    noncontiguous_backward.grad_field_vector =
        at::ones({1, 3, 2}, primal.source.options().dtype(at::kComplexFloat)).select(2, 0);
    noncontiguous_backward.need_grad_layer_thickness_m = true;
    require(!noncontiguous_backward.grad_field_vector->is_contiguous(),
            "noncontiguous cotangent fixture unexpectedly became contiguous");
    require(rayd::torch::field_transmission_sequence_backward(noncontiguous_backward)
                .grad_layer_thickness_m.has_value(),
            "noncontiguous transmission cotangent must be accepted");
    auto too_deep = primal;
    too_deep.interaction_positions = at::zeros({1, 9, 3}, primal.interaction_positions.options());
    too_deep.interaction_normals = at::zeros({1, 9, 3}, primal.interaction_normals.options());
    too_deep.interaction_material_id = at::zeros({1, 9}, primal.interaction_material_id.options());
    too_deep.interaction_valid = at::zeros({1, 9}, primal.interaction_valid.options());
    auto too_deep_backward = backward;
    too_deep_backward.primal = too_deep;
    require_throws([&] { (void)rayd::torch::field_transmission_sequence_backward(too_deep_backward); },
                   "transmission backward must reject D > 8");
    if (at::cuda::device_count() > 1) {
        auto bad_device = primal;
        bad_device.layer_eps_r = bad_device.layer_eps_r.to(at::Device(at::kCUDA, 1));
        require_throws([&] { (void)rayd::torch::field_transmission_sequence(bad_device); },
                       "transmission cross-device primal must fail loudly");
        auto bad_tangent_device = jvp;
        bad_tangent_device.tangent_source = at::ones({1, 3}, primal.source.options().device(at::Device(at::kCUDA, 1)));
        require_throws([&] { (void)rayd::torch::field_transmission_sequence_jvp(bad_tangent_device); },
                       "transmission cross-device tangent must fail loudly");
        auto bad_cotangent_device = backward;
        bad_cotangent_device.grad_path_gain = at::ones({1}, primal.source.options().device(at::Device(at::kCUDA, 1)));
        require_throws([&] { (void)rayd::torch::field_transmission_sequence_backward(bad_cotangent_device); },
...
void test_transmission_sequence_nondefault_stream_dependency() {
    ...
    reference_jvp.tangent_layer_thickness_m = at::ones({1}, reference_request.layer_thickness_m.options());
    rayd::torch::TransmissionSequenceBackwardRequest reference_backward;
    reference_backward.primal = reference_request;
    reference_backward.grad_path_gain = at::ones({1}, reference_request.tx_power.options());
    reference_backward.need_grad_layer_thickness_m = true;
    const auto reference = rayd::torch::field_transmission_sequence(reference_request);
    const auto reference_tangent = rayd::torch::field_transmission_sequence_jvp(reference_jvp);
    const auto reference_gradient = rayd::torch::field_transmission_sequence_backward(reference_backward);
    at::cuda::getDefaultCUDAStream().synchronize();

    const auto producer = c10::cuda::getStreamFromPool(false, 0);
    const auto consumer = c10::cuda::getStreamFromPool(false, 0);
    require(producer.stream() != consumer.stream(), "stream-pool fixtures must differ");
    auto scratch = at::empty({64 * 1024 * 1024}, request.source.options().dtype(at::kByte));
    cudaEvent_t ready = nullptr;
    C10_CUDA_CHECK(cudaEventCreateWithFlags(&ready, cudaEventDisableTiming));
    for (int iteration = 0; iteration < 32; ++iteration) {
        C10_CUDA_CHECK(cudaMemsetAsync(scratch.data_ptr(), iteration, static_cast<std::size_t>(scratch.numel()),
    ...
    rayd::torch::TransmissionSequenceBackwardResult gradient;
    {
        ...
        rayd::torch::TransmissionSequenceJvpRequest jvp;
        jvp.primal = request;
        jvp.tangent_layer_thickness_m = at::ones({1}, request.layer_thickness_m.options());
    ...
    require(at::allclose(result.path_gain, reference.path_gain, 1.0e-6, 1.0e-12),
            "transmission primal ignored producer-consumer stream dependency");
    require(at::allclose(tangent.path_gain, reference_tangent.path_gain, 1.0e-6, 1.0e-12),
            "transmission JVP ignored producer-consumer stream dependency");
    require(gradient.grad_layer_thickness_m.has_value() &&
                at::allclose(*gradient.grad_layer_thickness_m, *reference_gradient.grad_layer_thickness_m, 1.0e-6,
                             1.0e-12),
...
void test_transmission_path_valid_short_circuits_poison() {
    ...
    for (const auto& tensor : {forward.field_vector, forward.coefficient, forward.path_field, forward.path_gain,
                               forward.path_length_m, forward.delay_s, forward.direction})
        require(at::count_nonzero(tensor).item<int64_t>() == 0,
    ...
    const auto tangent = rayd::torch::field_transmission_sequence_jvp(jvp);
    at::cuda::getCurrentCUDAStream().synchronize();
    for (const auto& tensor : {tangent.field_vector, tangent.coefficient, tangent.path_field, tangent.path_gain,
                               tangent.path_length_m, tangent.delay_s})
        require(at::count_nonzero(tensor).item<int64_t>() == 0,
                "invalid poisoned transmission JVP output must be exactly zero");

    rayd::torch::TransmissionSequenceBackwardRequest backward;
    backward.primal = primal;
    backward.grad_field_vector = at::ones({1, 3}, primal.source.options().dtype(at::kComplexFloat));
    backward.grad_path_gain = at::ones({1}, primal.source.options());
    backward.need_grad_layer_thickness_m = true;
    backward.need_grad_layer_eps_r = true;
    backward.need_grad_layer_sigma_e = true;
    ...
    for (const auto* tensor : {&*gradients.grad_layer_thickness_m, &*gradients.grad_layer_eps_r,
                               &*gradients.grad_layer_sigma_e, &*gradients.grad_frequency, &*gradients.grad_source,
                               &*gradients.grad_target, &*gradients.grad_interaction_normals})
        require(at::count_nonzero(*tensor).item<int64_t>() == 0,
