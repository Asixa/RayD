...
static_assert(std::is_same_v<decltype(&scattering_chain_ensemble_eval),
                             ScatteringChainEnsembleEvalResult (*)(const ScatteringChainEnsembleEvalRequest&)>);
static_assert(
    std::is_same_v<decltype(&scattering_chain_ensemble_eval_backward),
                   ScatteringChainEnsembleEvalBackwardResult (*)(const ScatteringChainEnsembleEvalBackwardRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_chain_ensemble_eval_jvp),
                             ScatteringChainEnsembleEvalJvpResult (*)(const ScatteringChainEnsembleEvalJvpRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_chain_realization_eval),
                             ScatteringChainRealizationEvalResult (*)(const ScatteringChainRealizationEvalRequest&)>);
static_assert(std::is_same_v<
              decltype(&scattering_chain_realization_eval_backward),
              ScatteringChainRealizationEvalBackwardResult (*)(const ScatteringChainRealizationEvalBackwardRequest&)>);
static_assert(
    std::is_same_v<decltype(&scattering_chain_realization_eval_jvp),
                   ScatteringChainRealizationEvalJvpResult (*)(const ScatteringChainRealizationEvalJvpRequest&)>);
...
at::Tensor repeated_vec3(std::initializer_list<float> values, int64_t rows) {
    return at::tensor(values, cuda_options(at::kFloat)).reshape({1, 3}).repeat({rows, 1}).contiguous();
}
...
ScatteringChainRealizationEvalRequest realization_request(int64_t rows = 1) {
    ...
    const auto depth = at::zeros({rows}, i32);
    const auto scalar_one = at::ones({rows}, f32);
    const auto patch_tris = at::tensor({0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F}, f32).reshape({1, 3, 3});
    const auto patch_uvs = at::tensor({0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F}, f32).reshape({1, 3, 2});
    return {
        at::ones({rows}, cuda_options(at::kBool)),
        patch_tris,
        patch_uvs,
...
void set_full_depth(ScatteringChainEnsembleEvalRequest& request) {
    ...
    request.c1_positions = at::stack({zero, zero, 1.0F - z}, 1).unsqueeze(0).contiguous();
    request.c2_positions = at::stack({zero, zero, z}, 1).unsqueeze(0).contiguous();
    request.c1_normals = repeated_vec3({0.0F, 0.0F, 1.0F}, 8).reshape({1, 8, 3});
...
void test_nonempty_six_typed_operations() {
    ...
    require(ensemble_forward.gain.sizes() == at::IntArrayRef({1}), "ensemble gain shape differs");
    require(ensemble_forward.keep.scalar_type() == at::kBool, "ensemble keep dtype differs");
    ...
    const auto ensemble_gradient = scattering_chain_ensemble_eval_backward(ensemble_backward);
    require(ensemble_gradient.grad_c1_eps_r && ensemble_gradient.grad_c1_sigma_e && ensemble_gradient.grad_c1_gain &&
                ensemble_gradient.grad_c1_thickness && ensemble_gradient.grad_c2_eps_r &&
                ensemble_gradient.grad_c2_sigma_e && ensemble_gradient.grad_c2_gain &&
                ensemble_gradient.grad_c2_thickness && ensemble_gradient.grad_f_te && ensemble_gradient.grad_f_tm &&
                ensemble_gradient.grad_coefficient && ensemble_gradient.grad_frequency,
            "ensemble backward omitted a requested field");
    require_finite(*ensemble_gradient.grad_c1_eps_r, "ensemble chain gradient is not finite");
    ...
    require_finite(ensemble_tangent.tangent_gain, "ensemble gain tangent is not finite");
    require_finite(ensemble_tangent.tangent_amplitude, "ensemble amplitude tangent is not finite");
    require_finite(ensemble_tangent.tangent_length, "ensemble length tangent is not finite");

    auto realization = realization_request();
    const auto realization_forward = scattering_chain_realization_eval(realization);
    require(realization_forward.total.dim() == 0, "realization total must be scalar");
    require(realization_forward.path_field.scalar_type() == at::kComplexFloat, "realization path field dtype differs");
    require_finite(realization_forward.total, "realization total is not finite");
    require_finite(realization_forward.path_field, "realization path field is not finite");
    require_finite(realization_forward.path_gain, "realization path gain is not finite");

    ScatteringChainRealizationEvalBackwardRequest realization_backward;
    realization_backward.primal = realization;
    realization_backward.grad_total = at::ones({}, realization.patch_tris.options().dtype(at::kComplexFloat));
    ...
    const auto realization_gradient = scattering_chain_realization_eval_backward(realization_backward);
    require(realization_gradient.grad_d_i && realization_gradient.grad_d_o && realization_gradient.grad_c1_positions &&
                realization_gradient.grad_c1_normals && realization_gradient.grad_c2_positions &&
                realization_gradient.grad_c2_normals && realization_gradient.grad_l1 && realization_gradient.grad_l2 &&
                realization_gradient.grad_sp1 && realization_gradient.grad_sp2 && realization_gradient.grad_centroids,
            "realization backward omitted one of 11 geometry gradients");
    require(realization_gradient.grad_heights && realization_gradient.grad_layer_thickness &&
                realization_gradient.grad_layer_eps_r && realization_gradient.grad_layer_sigma_e &&
                realization_gradient.grad_c1_eps_r && realization_gradient.grad_c1_sigma_e &&
                realization_gradient.grad_c1_gain && realization_gradient.grad_c1_thickness &&
                realization_gradient.grad_c2_eps_r && realization_gradient.grad_c2_sigma_e &&
                realization_gradient.grad_c2_gain && realization_gradient.grad_c2_thickness &&
                realization_gradient.grad_k0 && realization_gradient.grad_frequency,
            "realization backward omitted another requested field");
    require_finite(*realization_gradient.grad_d_i, "realization geometry gradient is not finite");
    ...
    require_finite(realization_tangent.tangent_total, "realization total tangent is not finite");
    require_finite(realization_tangent.tangent_path_field, "realization field tangent is not finite");
    require_finite(realization_tangent.tangent_path_gain, "realization gain tangent is not finite");
}
...
void test_ad_capability_and_optional_gating() {
    ...
    require_throws([&] { (void)scattering_chain_ensemble_eval_backward(unsupported); },
    ...
    const auto ensemble_gated = scattering_chain_ensemble_eval_backward(ensemble_none);
    require(!ensemble_gated.grad_c1_eps_r && !ensemble_gated.grad_c2_eps_r && !ensemble_gated.grad_f_te &&
                !ensemble_gated.grad_coefficient && !ensemble_gated.grad_frequency,
    ...
    realization_none.grad_total = at::ones({}, realization_none.primal.patch_tris.options().dtype(at::kComplexFloat));
    const auto realization_gated = scattering_chain_realization_eval_backward(realization_none);
    require(!realization_gated.grad_heights && !realization_gated.grad_layer_thickness &&
                !realization_gated.grad_c1_eps_r && !realization_gated.grad_c2_eps_r && !realization_gated.grad_d_i &&
                !realization_gated.grad_centroids && !realization_gated.grad_k0 && !realization_gated.grad_frequency,
...
void test_max_depth_primal_and_ad() {
    ...
    const auto ensemble_forward = scattering_chain_ensemble_eval(ensemble);
    require(ensemble_forward.gain.sizes() == at::IntArrayRef({1}), "max-depth ensemble gain shape differs");
    require_finite(ensemble_forward.gain, "max-depth ensemble primal is not finite");
    ...
    require_finite(*ensemble_gradient.grad_c1_eps_r, "max-depth ensemble gradient is not finite");

    ScatteringChainEnsembleEvalJvpRequest ensemble_jvp;
    ensemble_jvp.primal = ensemble;
    ensemble_jvp.tangent_c1_eps_r = at::zeros_like(ensemble.c1_eps_r);
    const auto ensemble_tangent = scattering_chain_ensemble_eval_jvp(ensemble_jvp);
    require_finite(ensemble_tangent.tangent_gain, "max-depth ensemble tangent is not finite");
    ...
    require_finite(realization_forward.total, "max-depth realization primal is not finite");
    require_finite(realization_forward.path_field, "max-depth realization field is not finite");
    ...
    require_finite(*realization_gradient.grad_c1_eps_r, "max-depth realization gradient is not finite");

    ScatteringChainRealizationEvalJvpRequest realization_jvp;
    realization_jvp.primal = realization;
    realization_jvp.tangent_c1_eps_r = at::zeros_like(realization.c1_eps_r);
    const auto realization_tangent = scattering_chain_realization_eval_jvp(realization_jvp);
    require_finite(realization_tangent.tangent_total, "max-depth realization tangent is not finite");
}

void test_zero_rows() {
    auto ensemble = ensemble_request(0);
    const auto forward = scattering_chain_ensemble_eval(ensemble);
    require(forward.gain.numel() == 0 && forward.keep.numel() == 0, "zero-row ensemble outputs must be empty");
    ...
    require(tangent.tangent_gain.numel() == 0, "zero-row ensemble tangent must be empty");

    auto realization = realization_request(0);
    const auto realization_forward = scattering_chain_realization_eval(realization);
    require(realization_forward.path_field.numel() == 0, "zero-row realization per-row output must be empty");
    require(at::abs(realization_forward.total).item<float>() == 0.0F, "zero-row realization total must be zero");
    ScatteringChainRealizationEvalBackwardRequest realization_backward;
    realization_backward.primal = realization;
    realization_backward.grad_total = at::ones({}, realization.patch_tris.options().dtype(at::kComplexFloat));
    realization_backward.need_grad_geometry = true;
    const auto realization_gradient = scattering_chain_realization_eval_backward(realization_backward);
    require(realization_gradient.grad_d_i && realization_gradient.grad_d_i->numel() == 0,
...
void test_dmax_and_optional_contracts_fail_loudly() {
    ...
    bad_ensemble_valid_dtype.valid = at::ones({1}, bad_ensemble_valid_dtype.tx_pol.options());
    require_throws([&] { (void)scattering_chain_ensemble_eval(bad_ensemble_valid_dtype); },
                   "wrong chain ensemble valid dtype must fail loudly");
    auto bad_realization_valid_shape = realization_request();
    bad_realization_valid_shape.valid = at::ones({2}, bad_realization_valid_shape.valid.options());
    require_throws([&] { (void)scattering_chain_realization_eval(bad_realization_valid_shape); },
                   "wrong chain realization valid shape must fail loudly");

    auto ensemble = ensemble_request();
    ensemble.c1_positions = at::zeros({1, 9, 3}, ensemble.tx_pol.options());
    require_throws([&] { (void)scattering_chain_ensemble_eval(ensemble); }, "ensemble padded Dmax=9 must fail loudly");

    auto realization = realization_request();
    realization.c2_positions = at::zeros({1, 9, 3}, realization.patch_tris.options());
    require_throws([&] { (void)scattering_chain_realization_eval(realization); },
    ...
    bad_tangent.tangent_c1_eps_r = at::zeros({1, 9}, bad_tangent.primal.tx_pol.options());
    require_throws([&] { (void)scattering_chain_ensemble_eval_jvp(bad_tangent); },
    ...
    bad_ensemble_backward_primal.primal.c1_eps_r =
        at::zeros({1, 7}, bad_ensemble_backward_primal.primal.tx_pol.options());
    require_throws([&] { (void)scattering_chain_ensemble_eval_backward(bad_ensemble_backward_primal); },
    ...
    bad_ensemble_jvp_primal.primal.c1_sigma_e = at::zeros({1, 7}, bad_ensemble_jvp_primal.primal.tx_pol.options());
    require_throws([&] { (void)scattering_chain_ensemble_eval_jvp(bad_ensemble_jvp_primal); },
    ...
    bad_cotangent.grad_total = at::ones({}, bad_cotangent.primal.patch_tris.options());
    require_throws([&] { (void)scattering_chain_realization_eval_backward(bad_cotangent); },
    ...
    bad_realization_leg.primal.c1_normals = bad_realization_leg.primal.c1_normals.to(at::kDouble);
    bad_realization_leg.grad_total =
        at::ones({}, bad_realization_leg.primal.patch_tris.options().dtype(at::kComplexFloat));
    require_throws([&] { (void)scattering_chain_realization_eval_backward(bad_realization_leg); },
    ...
    bad_realization_layers.primal.layer_eps_r = at::ones({2}, bad_realization_layers.primal.patch_tris.options());
    require_throws([&] { (void)scattering_chain_realization_eval_jvp(bad_realization_layers); },
                   "realization JVP must validate the complete primal layer contract");

    auto bad_realization_quad_device = realization_request();
    bad_realization_quad_device.quad_w = bad_realization_quad_device.quad_w.cpu();
    require_throws([&] { (void)scattering_chain_realization_eval(bad_realization_quad_device); },
...
void test_sparse_invalid_rows_short_circuit_poison() {
    ...
    for (const auto* tensor :
         {&*ensemble_grad.grad_c1_eps_r, &*ensemble_grad.grad_c2_eps_r, &*ensemble_grad.grad_f_te,
          &*ensemble_grad.grad_f_tm, &*ensemble_grad.grad_coefficient, &*ensemble_grad.grad_frequency})
        require_finite(*tensor, "invalid sparse ensemble row contaminated a shared gradient");
    require_exact_zero(ensemble_grad.grad_c1_eps_r->select(0, 1),
    ...
    require_exact_zero(ensemble_tangent.tangent_amplitude.select(0, 1),
                       "invalid sparse ensemble amplitude JVP must be zero");
    ...
    for (const auto* tensor :
         {&*realization_grad.grad_heights, &*realization_grad.grad_layer_eps_r, &*realization_grad.grad_c1_eps_r,
          &*realization_grad.grad_c2_eps_r, &*realization_grad.grad_d_i, &*realization_grad.grad_k0,
          &*realization_grad.grad_frequency})
        require_finite(*tensor, "invalid sparse realization row contaminated a gradient");
    require_exact_zero(realization_grad.grad_d_i->select(0, 1),
                       "invalid sparse realization geometry gradient must be zero");
    require_exact_zero(realization_grad.grad_l1->select(0, 1),
                       "invalid sparse realization length gradient must be zero");
    ScatteringChainRealizationEvalJvpRequest realization_jvp;
    realization_jvp.primal = realization;
    ...
    require_exact_zero(realization_tangent.tangent_path_field.select(0, 1),
                       "invalid sparse realization field JVP must be zero");
    require_exact_zero(realization_tangent.tangent_path_gain.select(0, 1),
                       "invalid sparse realization gain JVP must be zero");
}
...
int main() {
    try {
        ...
        std::cerr << "rayd::torch chain scattering direct contract failure: " << error.what() << '\n';
        return 1;
    }
}
