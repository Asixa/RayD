...
static_assert(kIntegrationHeaderIdentity == std::string_view("rayd.torch.integration"));

static_assert(
    std::is_same_v<decltype(&scattering_table_eval), ScatteringTableEvalResult (*)(const ScatteringTableEvalRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_table_eval_backward),
                             ScatteringTableEvalBackwardResult (*)(const ScatteringTableEvalBackwardRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_table_eval_jvp),
                             ScatteringTableEvalJvpResult (*)(const ScatteringTableEvalJvpRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_table_sample),
                             ScatteringTableSampleResult (*)(const ScatteringTableSampleRequest&)>);
static_assert(
    std::is_same_v<decltype(&scattering_table_pdf), ScatteringTablePdfResult (*)(const ScatteringTablePdfRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_ensemble_eval),
                             ScatteringEnsembleEvalResult (*)(const ScatteringEnsembleEvalRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_ensemble_eval_backward),
                             ScatteringEnsembleEvalBackwardResult (*)(const ScatteringEnsembleEvalBackwardRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_ensemble_eval_jvp),
                             ScatteringEnsembleEvalJvpResult (*)(const ScatteringEnsembleEvalJvpRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_patch_integral_eval),
                             ScatteringPatchIntegralEvalResult (*)(const ScatteringPatchIntegralEvalRequest&)>);
static_assert(
    std::is_same_v<decltype(&scattering_patch_integral_eval_backward),
                   ScatteringPatchIntegralEvalBackwardResult (*)(const ScatteringPatchIntegralEvalBackwardRequest&)>);
static_assert(std::is_same_v<decltype(&scattering_patch_integral_eval_jvp),
                             ScatteringPatchIntegralEvalJvpResult (*)(const ScatteringPatchIntegralEvalJvpRequest&)>);
...
void require_close(float actual, float expected, const std::string& message) {
    require(std::isfinite(actual) && std::fabs(actual - expected) <= 2.0e-5F, message);
}
...
ScatteringTableEvalRequest empty_table_request() {
    ...
    return {
        at::empty({0}, cuda_options(at::kBool)), at::empty({0, 3}, options),       at::empty({0, 3}, options),
        at::zeros({2, 2, 2, 2}, options),        at::zeros({2, 2, 2, 2}, options),
    };
}
...
ScatteringPatchIntegralEvalRequest nonempty_patch_request() {
    ...
    return {
        at::ones({1}, cuda_options(at::kBool)),
        at::tensor({0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F}, floats).reshape({1, 3, 3}),
        at::tensor({0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F}, floats).reshape({1, 3, 2}),
        at::zeros({1}, longs),
        at::tensor({0.0F, 0.0F, 1.0F}, floats).reshape({1, 3}),
        at::tensor({0.0F, 0.0F, 1.0F}, floats).reshape({1, 3}),
        at::tensor({0.0F, 0.0F, 1.0F}, floats).reshape({1, 3}),
...
ScatteringTableSampleRequest nonempty_table_sample_request() {
    ...
    return {
        primal.valid, primal.wi,   at::tensor({0.25F, 0.25F}, floats).reshape({1, 2}),
        marginal,     conditional, at::full({2, 2, 2, 2}, 0.25F, floats),
    };
}
...
ScatteringTablePdfRequest nonempty_table_pdf_request() {
    const auto primal = nonempty_table_request();
    return {
        primal.valid, primal.wi, primal.wo, at::full({2, 2, 2, 2}, 0.25F, cuda_options(at::kFloat)), false,
    };
}
...
LayerStackRequest nonempty_layer_stack_request() {
    ...
    const auto ints = cuda_options(at::kInt);
    return {
        at::tensor({0.55F}, floats),  at::tensor({0}, ints),       at::tensor({0}, ints),
        at::tensor({1}, ints),        at::tensor({0.12F}, floats), at::tensor({4.0F}, floats),
        at::tensor({0.025F}, floats), at::tensor({1.0F}, floats),  3.5e9,
    };
}
...
DiffractionWedgeRequest nonempty_wedge_request() {
    ...
    request.edge_direction = at::tensor({0.0F, 0.0F, 1.0F}, floats).reshape({1, 3});
...
void test_table_empty_contracts() {
    ...
    require(result.f_te.sizes() == at::IntArrayRef({0}), "empty table TE shape differs");
    require(result.f_tm.sizes() == at::IntArrayRef({0}), "empty table TM shape differs");
    ...
    require(gradients.grad_wi.has_value() && gradients.grad_wi->sizes() == at::IntArrayRef({0, 3}) &&
                gradients.grad_wo.has_value() && gradients.grad_f_te.has_value() &&
                gradients.grad_f_te->sizes() == primal.f_te.sizes() && gradients.grad_f_tm.has_value(),
    ...
    require(sampled.wo.sizes() == at::IntArrayRef({0, 3}) && sampled.pdf_forward.sizes() == at::IntArrayRef({0}) &&
                sampled.pdf_reverse.sizes() == at::IntArrayRef({0}),
            "empty table sample schema differs");

    const auto pdf = scattering_table_pdf({primal.valid, primal.wi, primal.wo, sample.sample_density, false});
    require(pdf.pdf.sizes() == at::IntArrayRef({0}), "empty table PDF schema differs");
}
...
void test_ensemble_empty_contract() {
    ...
    require(result.gain.sizes() == at::IntArrayRef({0}) && result.amplitude.sizes() == at::IntArrayRef({0}) &&
                result.length.sizes() == at::IntArrayRef({0}) && result.keep.sizes() == at::IntArrayRef({0}) &&
    ...
    require(gradients.grad_wo_rows.has_value() && gradients.grad_wo_rows->sizes() == at::IntArrayRef({0, 3}) &&
                gradients.grad_n_o.has_value() && gradients.grad_n_o->sizes() == at::IntArrayRef({0, 3}) &&
                gradients.grad_f_te.has_value() && gradients.grad_f_te->sizes() == at::IntArrayRef({0}) &&
                gradients.grad_coefficient.has_value() && gradients.grad_coefficient->sizes() == at::IntArrayRef({1}),
...
void test_patch_empty_contract() {
    ...
    require(result.total.dim() == 0 && result.total.scalar_type() == at::kComplexFloat &&
                result.integral.sizes() == at::IntArrayRef({0}) && result.row_value.sizes() == at::IntArrayRef({0}),
            "empty patch-integral schema differs");
    at::cuda::getCurrentCUDAStream(0).synchronize();
    require(at::count_nonzero(result.total).item<std::int64_t>() == 0, "empty patch-integral total must be zero");
    ...
    require(gradients.grad_heights.has_value() && gradients.grad_heights->sizes() == request.heights.sizes() &&
                gradients.grad_r_te.has_value() && gradients.grad_r_te->sizes() == at::IntArrayRef({0}) &&
                gradients.grad_d_i.has_value() && gradients.grad_d_i->sizes() == at::IntArrayRef({0, 3}) &&
                gradients.grad_k0.has_value() && gradients.grad_k0->sizes() == at::IntArrayRef({1}),
    ...
    at::cuda::getCurrentCUDAStream(0).synchronize();
    require(tangent.tangent_total.dim() == 0 && tangent.tangent_total.scalar_type() == at::kComplexFloat &&
...
void test_table_nonempty_primal_ad_sample_pdf() {
    ...
    const auto uniforms = at::tensor({0.25F, 0.25F}, primal.wi.options()).reshape({1, 2});
    const auto sampled = scattering_table_sample({primal.valid, primal.wi, uniforms, marginal, conditional, density});
    const auto pdf = scattering_table_pdf({primal.valid, primal.wi, primal.wo, density, false});
    ...
    require(gradients.grad_wi.has_value() && gradients.grad_wo.has_value() && gradients.grad_f_te.has_value() &&
    ...
    require_close(gradients.grad_f_te->sum().item<float>(), 1.0F, "nonempty table gradient weights do not sum to one");
    require_close(tangent.tangent_f_te.item<float>(), 1.0F, "nonempty table JVP TE differs");
    require_close(tangent.tangent_f_tm.item<float>(), 0.0F, "nonempty table JVP TM differs");
    require_finite(sampled.wo, "nonempty table sample direction is not finite");
    require_close(sampled.pdf_forward.item<float>(), 0.25F, "nonempty table sample forward PDF differs");
    require_close(sampled.pdf_reverse.item<float>(), 0.25F, "nonempty table sample reverse PDF differs");
...
void test_ensemble_nonempty_primal_ad() {
    ...
    require_close(forward.amplitude.item<float>(), 1.0F, "nonempty ensemble amplitude differs");
    ...
    require(gradients.grad_wo_rows.has_value() && gradients.grad_n_o.has_value() && gradients.grad_f_te.has_value() &&
    ...
    require_close(gradients.grad_coefficient->item<float>(), 1.0F, "nonempty ensemble coefficient gradient differs");
    require_close(tangent.tangent_gain.item<float>(), 1.0F, "nonempty ensemble gain JVP differs");
    require_close(tangent.tangent_amplitude.item<float>(), 0.5F, "nonempty ensemble amplitude JVP differs");
    require_close(tangent.tangent_length.item<float>(), 0.0F, "nonempty ensemble length JVP differs");
}
...
void test_patch_nonempty_primal_ad() {
    ...
    require_close(at::abs(forward.integral).item<float>(), 0.0F, "nonempty patch integral differs");
    require_close(at::abs(forward.row_value).item<float>(), 0.0F, "nonempty patch row value differs");
    require(gradients.grad_heights.has_value() && gradients.grad_r_te.has_value() && gradients.grad_d_i.has_value() &&
    ...
    require_close(at::abs(tangent.tangent_total).item<float>(), 0.0F, "nonempty patch JVP differs");
}
...
void test_nondefault_stream_dependency() {
    ...
    stream.synchronize();
    require_close(result.f_te.item<float>(), 4.0F, "scattering table eval ignored a non-default-stream dependency");
}
...
void test_invalid_rows_short_circuit_poison() {
    ...
    const auto sampled = scattering_table_sample({table.valid, table.wi, uniforms, marginal, density, density});
    ...
    const auto pdf = scattering_table_pdf({table.valid, table.wi, table.wo, density, false});
...
void test_invalid_contracts_fail_loudly() {
    ...
    require_throws([&] { (void)scattering_table_eval(bad_table_valid_dtype); },
                   "wrong table valid dtype must fail loudly");
    auto bad_ensemble_valid_shape = nonempty_ensemble_request();
    bad_ensemble_valid_shape.valid = at::ones({2}, bad_ensemble_valid_shape.valid.options());
    require_throws([&] { (void)scattering_ensemble_eval(bad_ensemble_valid_shape); },
                   "wrong ensemble valid shape must fail loudly");
    auto bad_patch_valid_shape = nonempty_patch_request();
    bad_patch_valid_shape.valid = at::ones({2}, bad_patch_valid_shape.valid.options());
    require_throws([&] { (void)scattering_patch_integral_eval(bad_patch_valid_shape); },
                   "wrong patch valid shape must fail loudly");

    auto cpu = empty_table_request();
    cpu.wi = cpu.wi.cpu();
    require_throws([&] { (void)scattering_table_eval(cpu); }, "CPU table input must fail loudly");

    auto dtype = empty_table_request();
    dtype.wi = dtype.wi.to(at::kDouble);
    require_throws([&] { (void)scattering_table_eval(dtype); }, "wrong table dtype must fail loudly");

    auto shape = empty_table_request();
    shape.wi = at::empty({0, 2}, shape.wi.options());
    require_throws([&] { (void)scattering_table_eval(shape); }, "wrong table shape must fail loudly");
    ...
    require_throws([&] { (void)scattering_table_eval_backward(bad_cotangent); },
    ...
    bad_tangent.tangent_wi = at::ones({1, 3}, bad_tangent.primal.wi.options().dtype(at::kDouble));
    require_throws([&] { (void)scattering_table_eval_jvp(bad_tangent); },
                   "wrong optional table tangent dtype must fail loudly");

    auto ensemble = nonempty_ensemble_request();
    auto noncontiguous_rows = at::ones({1, 3, 2}, ensemble.wo_rows.options()).select(2, 0);
    require(!noncontiguous_rows.is_contiguous(), "ensemble fixture must be non-contiguous");
    ensemble.wo_rows = noncontiguous_rows;
    require_throws([&] { (void)scattering_ensemble_eval(ensemble); }, "non-contiguous ensemble input must fail loudly");

    ScatteringEnsembleEvalBackwardRequest ensemble_backward;
    ensemble_backward.primal = nonempty_ensemble_request();
    ensemble_backward.grad_gain = at::ones({1}, ensemble_backward.primal.wo_rows.options().dtype(at::kDouble));
    ensemble_backward.need_grad_rows = true;
    require_throws([&] { (void)scattering_ensemble_eval_backward(ensemble_backward); },
                   "wrong optional ensemble cotangent dtype must fail loudly");

    ScatteringEnsembleEvalJvpRequest ensemble_jvp;
    ensemble_jvp.primal = nonempty_ensemble_request();
    ensemble_jvp.tangent_r2_rows = at::ones({2}, ensemble_jvp.primal.r2_rows.options());
    require_throws([&] { (void)scattering_ensemble_eval_jvp(ensemble_jvp); },
                   "wrong optional ensemble tangent shape must fail loudly");

    auto patch = nonempty_patch_request();
    auto noncontiguous_tris = at::ones({1, 3, 3, 2}, patch.patch_tris.options()).select(3, 0);
    require(!noncontiguous_tris.is_contiguous(), "patch fixture must be non-contiguous");
    patch.patch_tris = noncontiguous_tris;
    require_throws([&] { (void)scattering_patch_integral_eval(patch); }, "non-contiguous patch input must fail loudly");

    ScatteringPatchIntegralEvalBackwardRequest patch_backward;
    patch_backward.primal = nonempty_patch_request();
    patch_backward.grad_total = at::ones({}, cuda_options(at::kFloat));
    patch_backward.need_grad_heights = true;
    require_throws([&] { (void)scattering_patch_integral_eval_backward(patch_backward); },
                   "wrong patch cotangent dtype must fail loudly");

    ScatteringPatchIntegralEvalJvpRequest patch_jvp;
    patch_jvp.primal = nonempty_patch_request();
    patch_jvp.tangent_heights =
        at::ones(patch_jvp.primal.heights.sizes(), patch_jvp.primal.heights.options().dtype(at::kDouble));
    require_throws([&] { (void)scattering_patch_integral_eval_jvp(patch_jvp); },
...
void test_cross_device_inputs_fail_loudly() {
    ...
    require_throws([&] { (void)scattering_table_eval(bad_table_direction_device); },
    ...
    require_throws([&] { (void)scattering_table_eval(bad_table_payload_device); },
    ...
    table_backward.need_grad_directions = true;
    require_throws([&] { (void)scattering_table_eval_backward(table_backward); },
                   "cross-device table cotangent must fail loudly");

    ScatteringTableEvalJvpRequest table_jvp;
    table_jvp.primal = table;
    table_jvp.tangent_wi = at::ones({1, 3}, table.wi.options().device(second));
    require_throws([&] { (void)scattering_table_eval_jvp(table_jvp); }, "cross-device table tangent must fail loudly");
    ...
    require_throws([&] { (void)scattering_table_sample(bad_sample_device); },
    ...
    require_throws([&] { (void)scattering_table_pdf(bad_pdf_device); },
    ...
    require_throws([&] { (void)scattering_ensemble_eval(bad_ensemble_device); },
    ...
    require_throws([&] { (void)scattering_patch_integral_eval(bad_patch_device); },
    ...
    ScatteringPatchIntegralEvalJvpRequest patch_jvp;
    patch_jvp.primal = patch;
    patch_jvp.tangent_heights = at::ones(patch.heights.sizes(), patch.heights.options().device(second));
    require_throws([&] { (void)scattering_patch_integral_eval_jvp(patch_jvp); },
    ...
    bad_transmission_device.layer_eps_r = bad_transmission_device.layer_eps_r.to(second);
    require_throws([&] { (void)field_transmission_sequence(bad_transmission_device); },
    ...
    transmission_jvp.tangent_source = at::ones({1, 3}, transmission.source.options().device(second));
    require_throws([&] { (void)field_transmission_sequence_jvp(transmission_jvp); },
    ...
    bad_layer_stack_device.layer_eps_r = bad_layer_stack_device.layer_eps_r.to(second);
    require_throws([&] { (void)em_layer_stack_eval(bad_layer_stack_device); },
    ...
    layer_stack_backward.grad_outputs[0] = at::ones({1}, layer_stack.cos_theta.options().device(second));
    layer_stack_backward.need_cos_theta = true;
    require_throws([&] { (void)em_layer_stack_backward(layer_stack_backward); },
    ...
    require_throws([&] { (void)field_diffraction_wedge(bad_wedge_device); },
                   "cross-device wedge input must fail loudly");
    auto bad_wedge_vertex_device = wedge;
    bad_wedge_vertex_device.vertex_opp1 = bad_wedge_vertex_device.vertex_opp1->to(second);
    require_throws([&] { (void)field_diffraction_wedge(bad_wedge_vertex_device); },
                   "cross-device wedge vertex must fail loudly");

    DiffractionWedgeJvpRequest wedge_jvp;
    wedge_jvp.primal = wedge;
    wedge_jvp.tangent_source = at::ones({1, 3}, wedge.source.options().device(second));
    require_throws([&] { (void)field_diffraction_wedge_jvp(wedge_jvp); },
...
int main() {
    try {
        ...
        std::cerr << "rayd::torch scattering direct contract failure: " << error.what() << '\n';
        return 1;
    }
}
