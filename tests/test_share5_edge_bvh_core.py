...
BVH_CORE_INCLUDE = ROOT / "src"
BVH_CORE_SOURCE = ROOT / "src"


class Share5EdgeBvhCoreTests(unittest.TestCase):
    def test_raw_and_compact_topologies_are_distinct_pods(self):
        topology = (BVH_CORE_INCLUDE / "bvh_topology.h").read_text(encoding="utf-8")
    ...
    def test_product_treelet_constants_have_one_shared_definition(self):
        topology = (BVH_CORE_INCLUDE / "bvh_topology.h").read_text(encoding="utf-8")
        shared = (ROOT / "include/rayd/contracts.h").read_text(encoding="utf-8")
        edge = (SHARED_INCLUDE / "bvh_types.h").read_text(encoding="utf-8")
        adapter = (ROOT / "include/rayd/jit/edge.h").read_text(encoding="utf-8")
        for token in (
            "BvhTreeletMaxLeaves = 7",
            "BvhTreeletMinPrimitives = 65536",
            "BvhTreeletMaxPrimitives = 500000",
            "BvhTreeletMinSubtreeLeaves = 32",
            "BvhTreeletCostInflationRatio = 1e-4f",
            "BvhLeafSize = 4",
            "BvhTraversalStackDepth = 64",
            "BvhTopKMax = 16",
        ):
            # Exactly one literal definition, in the shared core.
            self.assertIn(token, shared)
            self.assertNotIn(token, edge)
        self.assertIn("::rayd::shared::BvhTreeletMaxLeaves", topology)
        ...
        self.assertGreaterEqual(adapter.count("shared::BvhTreelet"), 4)
        self.assertIn("shared::BvhLeafSize", adapter)
        scene_edge = (ROOT / "src/edge/edge_jit.cpp").read_text(encoding="utf-8")
        self.assertIn("shared::bvh::kBvhTraversalStackDepth", scene_edge)
    ...
    def test_build_stages_are_shared_and_drjit_is_an_adapter(self):
        ...
        core = (BVH_CORE_SOURCE / "bvh_build_shared.cu").read_text(encoding="utf-8")
    ...
    def test_shared_cuda_owns_no_resources_or_host_barriers(self):
        ...
        combined += "\n" + (BVH_CORE_SOURCE / "bvh_build_shared.cu").read_text(encoding="utf-8")
        for forbidden in (
            "cudaMalloc",
            "cudaFree",
            "cudaMemcpy",
    ...
    def test_query_contract_freezes_masks_topk_stack_and_tie_break(self):
        ...
        traversal = (BVH_CORE_INCLUDE / "bvh_query_device.cuh").read_text(encoding="utf-8")
        for token in (
        ...
        self.assertIn("<src/bvh_query_device.cuh>", source)
    ...
    def test_both_backends_compile_the_shared_units(self):
        ...
        self.assertIn("bvh_build_shared.cu", drjit)
        self.assertIn("bvh_build_shared.cu", torch)
