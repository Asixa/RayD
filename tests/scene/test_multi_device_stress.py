...
def _query_inputs(device: torch.device, rays: int, points: int):
    ...
    origin = torch.stack((1.8 * u, 1.8 * v, torch.full_like(u, -1.0)), dim=1).contiguous()
...
class _DeviceWorkload:
    ...
    def __init__(self, index: int, cells: int, rays: int, points: int, own_stream: bool = True) -> None:
        self.index = index
    ...
    def collect(self, iterations: int):
        ...
        devices = {value.device.index for iteration in device_results for value in iteration.values()}
        host_results = [{name: _bits(value) for name, value in iteration.items()} for iteration in device_results]
...
class MultiDeviceStressTests(unittest.TestCase):
    ...
    def assert_same_bits(self, reference, observed, context: str) -> None:
        ...
        for step, (left, right) in enumerate(zip(reference, observed)):
            self.assertEqual(sorted(left), sorted(right), f"{context}: iteration {step}")
            for name, value in left.items():
                other = right[name]
                self.assertEqual(value.dtype, other.dtype, f"{context}: {name} dtype at {step}")
                self.assertEqual(value.shape, other.shape, f"{context}: {name} shape at {step}")
                self.assertTrue(
                    torch.equal(value, other), f"{context}: {name} is not bitwise equal at iteration {step}"
                )
    ...
    def assert_non_degenerate(self, results, context: str) -> None:
        ...
        self.assertGreater(hit.float().mean().item(), 0.9, f"{context}: intersect mostly missed")
        self.assertTrue(bool((t[hit] > 0.0).all()), f"{context}: intersect returned a degenerate t")
        self.assertGreater(
            first["intersect.prim_id"][hit].unique().numel(), 1, f"{context}: intersect resolved a single primitive"
        )
        self.assertGreater(
        ...
        self.assertTrue(bool(torch.isfinite(distance).all()), f"{context}: nearest_edge found no edge")
        self.assertTrue(
            bool((first["nearest_edge.global_edge_id"] >= 0).all()), f"{context}: nearest_edge returned no edge id"
        )
    ...
    def run_two_threads(self, body, concurrent: bool) -> None:
        ...
        if errors:
            raise AssertionError(
                "worker thread failed: " + "; ".join(f"cuda:{index}: {error!r}" for index, error in errors.items())
            )
    ...
    def test_two_threads_on_two_devices_reproduce_single_device_results(self):
        ...
        for index in (0, 1):
            torch.cuda.set_device(index)
            workload = _DeviceWorkload(index, _STRESS_CELLS, _STRESS_RAYS, _STRESS_POINTS, own_stream=False)
            results, devices = workload.collect(_STRESS_ITERATIONS)
            self.assertEqual(devices, {index}, f"cuda:{index} reference left its device")
        ...
        def body(index: int, barrier) -> None:
            ...
            workload = _DeviceWorkload(index, _STRESS_CELLS, _STRESS_RAYS, _STRESS_POINTS)
        ...
        for index in (0, 1):
            self.assertEqual(seen[index], {index}, f"cuda:{index} thread left its device")
            self.assert_same_bits(reference[index], observed[index], f"cuda:{index} concurrent thread")
        self.assertEqual(torch.cuda.current_device(), 0, "a worker thread leaked the main device")
    ...
    def test_cold_create_on_a_non_zero_device_in_a_fresh_process(self):
        ...
        env["PYTHONPATH"] = os.pathsep.join(part for part in (str(_PACKAGE_ROOT), env.get("PYTHONPATH", "")) if part)
        ...
        self.assertEqual(result.returncode, 0, f"cold-create subprocess failed.\n{combined}")
    ...
    def test_two_devices_overlap_instead_of_serializing(self):
        ...
        for index in (0, 1):
            torch.cuda.set_device(index)
            prepared[index] = _DeviceWorkload(index, _TIMED_CELLS, _TIMED_RAYS, _TIMED_POINTS)
        torch.cuda.set_device(0)
        ...
        self.assertGreater(serialized, 0.02, "workload too small to time; raise the iteration count")
        self.assertLess(
            concurrent,
