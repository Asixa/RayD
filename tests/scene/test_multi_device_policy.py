...
def _planned(options: rt.MultiDeviceOptions) -> _ReplicatedScene:
    ...
    with (
        ...
        mock.patch.object(torch.cuda, "can_device_access_peer", return_value=True, create=True),
        mock.patch.object(
...
class PeerTopologyPolicyTests(unittest.TestCase):
    ...
    def test_peer_access_is_required_by_default(self) -> None:
        with (
            mock.patch.object(torch.cuda, "is_available", return_value=True),
            mock.patch.object(torch.cuda, "device_count", return_value=2),
            mock.patch.object(torch.cuda, "can_device_access_peer", return_value=False, create=True),
            mock.patch.object(
                torch.cuda, "get_device_properties", return_value=SimpleNamespace(name="Test GPU", major=9, minor=0)
            ),
        ):
            with self.assertRaisesRegex(RuntimeError, "(?i)peer|p2p"):
                plan_multi_device(
                    [0, 1], rt.MultiDeviceOptions(warm_up=False), trace_backend="optix", edge_bvh_backend="optix"
                )

    def test_the_caller_can_explicitly_accept_a_non_peer_link(self) -> None:
        options = rt.MultiDeviceOptions(warm_up=False, require_peer_access=False)
        with (
            mock.patch.object(torch.cuda, "is_available", return_value=True),
            mock.patch.object(torch.cuda, "device_count", return_value=2),
            mock.patch.object(torch.cuda, "can_device_access_peer", return_value=False, create=True),
    ...
    def test_heterogeneous_devices_are_an_explicit_opt_in(self) -> None:
        ...
        with common[0], common[1], common[2], common[3]:
            layer = plan_multi_device(
                [0, 1],
                rt.MultiDeviceOptions(warm_up=False, require_homogeneous_devices=False),
                trace_backend="optix",
...
class OperationPolicyTests(unittest.TestCase):
    def test_default_intersection_probe_uses_the_private_dispatch_signature(self) -> None:
        tree = ast.parse(textwrap.dedent(inspect.getsource(_ReplicatedScene._default_probe)))
        calls = [
            node
            for node in ast.walk(tree)
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.attr == "intersect"
        ]
        self.assertEqual(len(calls), 1)
        self.assertEqual(len(calls[0].args), 3, "the built-in probe must work for both Scene and _ReplicatedScene")
    ...
    def test_builtin_probe_cannot_be_mislabeled_as_another_operation(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "(?i)built-in|custom probe"):
            layer.calibrate(operation="visible", rows=100, repeats=1, warm_up=0)

    def test_operation_weights_are_isolated(self) -> None:
        layer = _planned(
            rt.MultiDeviceOptions(
                warm_up=False, min_rays_per_device=1, operation_weights={"intersect": [1.0, 0.0], "visible": [1.0, 3.0]}
            )
        )
        ...
        self.assertEqual([(start, stop) for _r, _d, start, stop in visible], [(0, 25), (25, 100)])
        self.assertEqual([(start, stop) for _r, _d, start, stop in intersect_again], [(0, 100)])

    def test_calibration_updates_only_the_named_operation(self) -> None:
        layer = _planned(rt.MultiDeviceOptions(warm_up=False, operation_weights={"intersect": [1.0, 0.0]}))
        with (
            mock.patch.object(torch.cuda, "synchronize"),
            mock.patch.object(torch.cuda, "device", side_effect=lambda _d: nullcontext()),
            mock.patch("rayd._impl.multi.time.perf_counter", side_effect=[0.0, 1.0, 2.0, 4.0]),
        ):
            record = layer.calibrate(
                operation="visible", rows=100, probe=lambda _scene, _device: None, repeats=1, warm_up=0, refine=False
            )
    ...
    def test_dispatch_floor_uses_the_actual_remote_shard(self) -> None:
        layer = _planned(rt.MultiDeviceOptions(warm_up=False, min_rays_per_device=10, weights=[91.0, 9.0]))
        ...
        enough_remote_work = _planned(rt.MultiDeviceOptions(warm_up=False, min_rays_per_device=10, weights=[9.0, 1.0]))
        self.assertEqual(enough_remote_work._dispatch_mode("visible", 100, 100), "pipelined")

    def test_wide_rows_raise_the_copy_amortization_floor(self) -> None:
        layer = _planned(rt.MultiDeviceOptions(warm_up=False, min_rays_per_device=10, weights=[1.0, 1.0]))
    ...
    def test_small_grid_reduce_window_runs_on_the_master(self) -> None:
        layer = _planned(
            rt.MultiDeviceOptions(
                warm_up=False, min_lanes_per_device=128, operation_weights={"accum_dfr_direct": [1.0, 1.0]}
            )
        )
        ...
        result = layer._run_lane_shards("accum_dfr_direct", 64, 0, -1, scatter, call)
    ...
    def test_grid_reduce_master_only_weight_never_launches_a_remote(self) -> None:
        layer = _planned(
            rt.MultiDeviceOptions(
                warm_up=False, min_lanes_per_device=1, operation_weights={"accum_dfr_direct": [1.0, 0.0]}
            )
        )
        calls: list[object] = []
        answer = object()
...
class MemoryBudgetPolicyTests(unittest.TestCase):
    ...
    def _accum(*, requires_grad: bool) -> DfrAccum:
        values = [torch.ones(1, requires_grad=requires_grad) for _ in range(14)]
    ...
    def test_budget_accounts_for_three_resident_pipeline_chunks(self) -> None:
        ...
        plan = calibrate_chunk_size(
            "trace_reflections", 1_000_000, row_bytes=row_bytes, budget_bytes=budget, resident_chunks=3
        )
    ...
    def test_fixed_concatenated_output_is_reserved_before_chunk_sizing(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "(?i)fixed|returned output|budget"):
            calibrate_chunk_size(
                "intersect", 100, row_bytes=76, budget_bytes=8192, resident_chunks=3, fixed_output_bytes=8192
            )
    ...
    def test_grid_accumulation_autograd_graph_grows_with_chunk_count(self) -> None:
        ...
        two = _add_accum(self._accum(requires_grad=True), self._accum(requires_grad=True))
    ...
    def test_in_place_grid_merge_refuses_autograd(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "(?i)autograd"):
            _add_accum_in_place(self._accum(requires_grad=True), self._accum(requires_grad=True))
...
class CudaPeakMemoryBudgetTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        vertices = torch.tensor(
            [[-2.0, -2.0, 0.0], [2.0, -2.0, 0.0], [0.0, 2.0, 0.0]], dtype=torch.float32, device=self.device
        )
    ...
    def _scene(self, **options) -> rt.Scene:
        scene = rt.Scene(devices=[0], options=rt.MultiDeviceOptions(warm_up=False, **options))
    ...
    def test_streaming_peak_increment_respects_the_budget(self) -> None:
        budget = 16 << 20
        scene = self._scene(tape_memory_budget_bytes=budget, offload=lambda _start, _chunk: None)
