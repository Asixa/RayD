...
// Defines shared edge support for optix device.
...
static __forceinline__ __device__ float3 edge_load_vec3(const float* x, const float* y, const float* z,
                                                        unsigned int index) {
    return make_float3(x[index], y[index], z[index]);
}

static __forceinline__ __device__ bool edge_query_active(const EdgeQuerySoAView& view, unsigned int query) {
    return view.active == nullptr || view.active[query] != 0u;
}

static __forceinline__ __device__ bool edge_geometry_active(const EdgeGeometrySoAView& view, unsigned int edge) {
    return view.active == nullptr || view.active[edge] != 0u;
}

static __forceinline__ __device__ float3 load_edge_query_origin(const EdgeQuerySoAView& view, unsigned int query) {
    return edge_load_vec3(view.origin_x, view.origin_y, view.origin_z, query);
}

static __forceinline__ __device__ float3 load_edge_query_direction(const EdgeQuerySoAView& view, unsigned int query) {
    return edge_load_vec3(view.direction_x, view.direction_y, view.direction_z, query);
}

static __forceinline__ __device__ float3 load_edge_start(const EdgeGeometrySoAView& view, unsigned int edge) {
    return edge_load_vec3(view.p0_x, view.p0_y, view.p0_z, edge);
}

static __forceinline__ __device__ float3 load_edge_vector(const EdgeGeometrySoAView& view, unsigned int edge) {
    return edge_load_vec3(view.e1_x, view.e1_y, view.e1_z, edge);
}

static __forceinline__ __device__ float safe_edge_search_radius(const EdgeGeometrySoAView& view) {
    return fmaxf(view.search_radius, 0.0f);
}

static __forceinline__ __device__ void write_invalid_edge_result(const EdgeQueryOutputView& output,
                                                                 unsigned int query) {
...
static __forceinline__ __device__ void set_edge_point_payload(float distance, unsigned int edge_id,
...
static __forceinline__ __device__ void set_edge_topk_payload_slot(int slot, std::uint32_t edge_id,
                                                                  std::uint32_t squared_distance) {
    switch (slot) {
    case 0:
        optixSetPayload_0(edge_id);
        optixSetPayload_8(squared_distance);
        break;
    case 1:
        optixSetPayload_1(edge_id);
        optixSetPayload_9(squared_distance);
        break;
    case 2:
        optixSetPayload_2(edge_id);
        optixSetPayload_10(squared_distance);
        break;
    case 3:
        optixSetPayload_3(edge_id);
        optixSetPayload_11(squared_distance);
        break;
    case 4:
        optixSetPayload_4(edge_id);
        optixSetPayload_12(squared_distance);
        break;
    case 5:
        optixSetPayload_5(edge_id);
        optixSetPayload_13(squared_distance);
        break;
    case 6:
        optixSetPayload_6(edge_id);
        optixSetPayload_14(squared_distance);
        break;
    default:
        optixSetPayload_7(edge_id);
        optixSetPayload_15(squared_distance);
        break;
    }
}

static __forceinline__ __device__ void insert_edge_topk_payload_candidate(int k, int edge_id, float squared_distance) {
    ...
    int insert = k - 1;
    while (insert > 0 && squared_distance < __uint_as_float(get_edge_topk_payload_distance(insert - 1))) {
        set_edge_topk_payload_slot(insert, get_edge_topk_payload_id(insert - 1),
                                   get_edge_topk_payload_distance(insert - 1));
        --insert;
    }
    set_edge_topk_payload_slot(insert, static_cast<std::uint32_t>(edge_id), candidate_distance);
}
