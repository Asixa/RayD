...
// Defines shared edge support for bvh query.
...
inline constexpr std::size_t EdgeBvhTopKMax = static_cast<std::size_t>(kBvhTopKMax);
...
constexpr std::size_t edge_bvh_topk_capacity(std::size_t k) noexcept {
    return k == 0 || k > EdgeBvhTopKMax ? 0 : k <= 1 ? 1 : k <= 2 ? 2 : k <= 4 ? 4 : k <= 8 ? 8 : 16;
}
