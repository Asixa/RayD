...
// Backend-neutral trilinear sampling of a dense signed distance grid
// (ADR-0037 sections 1 and 2). The grid is caller-owned, vertex-centred, stored
// row-major as `values[(i * Ny + j) * Nz + k]`, and signed negative inside.
//
// Host-compilable: this header spells only RAYD_HOST_DEVICE, pulls in no CUDA or
// OptiX SDK header, and uses no device-only intrinsic, so the same expressions
// compile into a CUDA kernel and into a plain host translation unit. It is
// deliberately outside every committed-PTX include closure (ADR-0037 section 9).
//
// Nothing here allocates, branches on a runtime backend, or reads memory the
// caller did not hand it. The eight corner reads of `sample_cell` are the only
// memory traffic.
