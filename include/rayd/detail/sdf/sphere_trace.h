...
// Backend-neutral sphere-trace core for the ADR-0037 SDF intersection: the
// oriented-box placement (sections 2), the slab clip that fixes the traced
// interval (section 3), and the relaxed march with sign-flip bisection
// (section 4). The march is templated on a sampler so the same body serves a
// CUDA kernel, a host test, and any later backend; this header knows nothing
// about grids and does not include `grid_sdf.cuh`.
//
// Host-compilable and outside every committed-PTX include closure, like
// `grid_sdf.cuh`.
//
// The march is the frozen-winner forward pass: it is fully detached, takes no
// derivative, and produces exactly the discrete decisions the derivative passes
// consume (ADR-0037 section 6).
