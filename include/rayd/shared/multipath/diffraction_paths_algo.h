...
inline constexpr int kStrategyDirect =
    static_cast<int>(::rayd::shared::optix::DiffractionStrategyBit::Direct);
...
RAYD_HOST_DEVICE float norm3(Vec3f value) {
    return sqrtf(fmaxf(math::dot(value, value), 0.f));
}
...
RAYD_HOST_DEVICE ::rayd::shared::rt::TriangleHit choose_hit(
    ::rayd::shared::rt::TriangleHit a, ::rayd::shared::rt::TriangleHit b) {
    if (a.hit == 0u)
        return b;
    if (b.hit == 0u)
        return a;
...
RAYD_DEVICE ::rayd::shared::rt::TriangleHit trace_scene(
    ...
    Vec3f direction,
    float tmax) {
    const ::rayd::shared::rt::TriangleHit p =
        primary.trace_closest(origin, direction, kTraceTMin, tmax);
    if (!SplitScene)
        return p;
    const ::rayd::shared::rt::TriangleHit s =
        secondary.trace_closest(origin, direction, kTraceTMin, tmax);
    return choose_hit(p, s);
}

template <bool SplitScene, typename Traverser>
RAYD_DEVICE bool visible_segment(
    const Traverser &primary, const Traverser &secondary, Vec3f start, Vec3f end) {
    ...
    const ::rayd::shared::rt::TriangleHit hit = trace_scene<SplitScene>(
        primary,
        secondary,
        math::add(start, math::scale(dir, kRayBias)),
        dir,
        fmaxf(dist - 2.f * kRayBias, 0.f));
    return hit.hit == 0u;
}

RAYD_HOST_DEVICE Vec3f state_vec(
...
template <typename Params>
RAYD_HOST_DEVICE utd::PairInputs direct_pair_inputs(
    const Params &params,
    int state_idx,
    Vec3f source,
    Vec3f edge_pos,
...
template <typename Params>
RAYD_HOST_DEVICE utd::MaterialParams paths_material_params(const Params &params) {
...
RAYD_HOST_DEVICE void write_point(
    float *x, float *y, float *z, int idx, Vec3f value) {
...
template <typename Params>
RAYD_HOST_DEVICE bool paths_order1_lane(
    const Params &params, unsigned int lane, int &state_idx, int &rx_idx, int &tx_idx) {
    if (lane >= static_cast<unsigned int>(params.n_rays) ||
        ...
        params.max_order != 1 ||
        (params.strategy_mask & kStrategyDirect) == 0 ||
        params.receiver_model != kReceiverMatchedIso) {
        return false;
    }
    ...
    return tx_idx < params.tx_count &&
           ...
           state_active(params, state_idx);
}

template <typename Params>
RAYD_HOST_DEVICE Vec3f paths_edge_point(const Params &params, int state_idx, int rx_idx) {
    const Vec3f edge_pos = state_vec(
        params.state_edge_pos_x, params.state_edge_pos_y, params.state_edge_pos_z, state_idx);
    const Vec3f edge_dir = normalize3(state_vec(
        params.state_edge_dir_x, params.state_edge_dir_y, params.state_edge_dir_z, state_idx));
    ...
    const Vec3f source = state_vec(
        params.state_src_x, params.state_src_y, params.state_src_z, state_idx);
    const Vec3f receiver = math::make_vec3(
        params.rx_pos_x[rx_idx], params.rx_pos_y[rx_idx], params.rx_pos_z[rx_idx]);
...
RAYD_HOST_DEVICE bool finite_paths_points(Vec3f source, Vec3f edge_point, Vec3f receiver) {
    return is_finite(source.x) && is_finite(source.y) && is_finite(source.z) &&
           is_finite(edge_point.x) && is_finite(edge_point.y) && is_finite(edge_point.z) &&
...
RAYD_DEVICE void trace_paths_order1_algo(
    const Params &params,
    std::uint32_t lane,
    const Traverser &primary,
    const Traverser &secondary) {
    ...
    if (tx_idx >= params.tx_count || state_idx >= params.state_count ||
        !state_active(params, state_idx)) {
        return;
    }
    ...
    if (!is_finite(source.x) || !is_finite(source.y) || !is_finite(source.z) ||
        !is_finite(edge_pos.x) || !is_finite(edge_pos.y) || !is_finite(edge_pos.z) ||
        !is_finite(receiver.x) || !is_finite(receiver.y) || !is_finite(receiver.z) ||
    ...
    const utd::PairInputs pair =
        direct_pair_inputs(params, state_idx, source, edge_pos, edge_dir, t_min, t_max);
    const utd::PairOutputs utd_out = utd::compute_pair_contribution(
        pair, to_utd(receiver), params.k, paths_material_params(params));
    ...
    const float path_length =
        norm3(math::subtract(edge_point, source)) + norm3(math::subtract(receiver, edge_point));
    ...
    write_point(params.out_p1_x, params.out_p1_y, params.out_p1_z,
                out_idx, math::make_vec3(0.f, 0.f, 0.f));
...
RAYD_DEVICE void trace_paths_source_visibility_algo(
    const Params &params,
    std::uint32_t lane,
    const Traverser &primary,
    const Traverser &secondary) {
