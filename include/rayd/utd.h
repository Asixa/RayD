...
// Defines the shared UTD domain types and algorithms.
...
template <typename T> struct Jones2T {
    ComplexT<T> u, v;
};
using Jones2 = Jones2T<float>;

template <typename T> struct JonesOperatorT {
    ComplexT<T> m00, m01, m10, m11;
};
using JonesOperator = JonesOperatorT<float>;

template <typename T> struct Basis3T {
    Vec3T<T> u, v, k;
};
...
template <typename T> struct FaceMaterialParamsT {
    T etaR;
    T muR;
    T sigma;
    T gain;
...
template <typename T> struct PairInputsT {
    Vec3T<T> edgePos;
    Vec3T<T> edgeDir;
    Vec3T<T> n0;
    Vec3T<T> nn;
...
template <typename T> struct PairOutputsT {
    ComplexT<T> field;
    Complex3T<T> vectorField;
};
using PairOutputs = PairOutputsT<float>;

template <typename T> struct DiffractionOperatorTermsT {
    ComplexT<T> direct;
    ComplexT<T> face0;
    ComplexT<T> face1;
...
template <typename T> struct MaterialParamsT {
    int useFresnel;
    T etaR;
    T muR;
    T sigma;
...
// ---------------------------------------------------------------------------
template <typename T = float> UTD_DINLINE Jones2T<T> jones_zero() {
    return {cplx_zero<T>(), cplx_zero<T>()};
}
template <typename T> UTD_DINLINE Jones2T<T> jones_add(Jones2T<T> a, Jones2T<T> b) {
    return {cplx_add(a.u, b.u), cplx_add(a.v, b.v)};
}
template <typename T> UTD_DINLINE Jones2T<T> jones_scale(Jones2T<T> v, ComplexT<T> c) {
    return {cplx_mul(v.u, c), cplx_mul(v.v, c)};
}

template <typename T = float> UTD_DINLINE JonesOperatorT<T> jop_zero() {
    return {cplx_zero<T>(), cplx_zero<T>(), cplx_zero<T>(), cplx_zero<T>()};
}
template <typename T = float> UTD_DINLINE JonesOperatorT<T> jop_identity() {
    return {c_const<T>(1.f, 0.f), cplx_zero<T>(), cplx_zero<T>(), c_const<T>(1.f, 0.f)};
}
template <typename T> UTD_DINLINE JonesOperatorT<T> jop_add(JonesOperatorT<T> a, JonesOperatorT<T> b) {
    return {cplx_add(a.m00, b.m00), cplx_add(a.m01, b.m01), cplx_add(a.m10, b.m10), cplx_add(a.m11, b.m11)};
}
template <typename T> UTD_DINLINE JonesOperatorT<T> jop_scale(JonesOperatorT<T> v, ComplexT<T> c) {
    return {cplx_mul(v.m00, c), cplx_mul(v.m01, c), cplx_mul(v.m10, c), cplx_mul(v.m11, c)};
}
template <typename T> UTD_DINLINE Jones2T<T> apply_jop(Jones2T<T> v, JonesOperatorT<T> op) {
    return {cplx_add(cplx_mul(op.m00, v.u), cplx_mul(op.m01, v.v)),
            cplx_add(cplx_mul(op.m10, v.u), cplx_mul(op.m11, v.v))};
}
template <typename T> UTD_DINLINE Complex3T<T> vector_from_jones(Jones2T<T> v, Basis3T<T> b) {
    return c3_add(cplx_scale_real(b.u, v.u), cplx_scale_real(b.v, v.v));
}
template <typename T> UTD_DINLINE Jones2T<T> jones_from_vector(Complex3T<T> v, Basis3T<T> b) {
...
UTD_DINLINE PairInputsGrad pig_zero() {
    ...
    g.edgeLineMax = 0.f;
    g.sourcePos = f3_zero();
    g.incidentField = cplx_zero();
    g.incidentNormalDerivative = cplx_zero();
    g.r0 = cplx_zero();
    g.rn = cplx_zero();
    g.incidentVector = c3_zero();
    g.incidentDerivativeVector = c3_zero();
    g.incidentJones = jones_zero();
    g.incidentDerivativeJones = jones_zero();
    g.incidentBasis = {f3_zero(), f3_zero(), f3_zero()};
    g.face0Operator = jop_zero();
    g.face1Operator = jop_zero();
    g.face0Material = {0, 0, 0, 0, 0, 0};
    g.face1Material = {0, 0, 0, 0, 0, 0};
    return g;
}
...
// ---------------------------------------------------------------------------
UTD_DINLINE Dual dual_seed(float value, float tangent) {
...
UTD_DINLINE Complex3T<Dual> dual_seed(Complex3 value, Complex3 tangent) {
    return {dual_seed(value.x, tangent.x), dual_seed(value.y, tangent.y), dual_seed(value.z, tangent.z)};
}
...
UTD_DINLINE JonesOperatorT<Dual> dual_seed(JonesOperator value, JonesOperator tangent) {
    return {dual_seed(value.m00, tangent.m00), dual_seed(value.m01, tangent.m01), dual_seed(value.m10, tangent.m10),
            dual_seed(value.m11, tangent.m11)};
}
UTD_DINLINE Basis3T<Dual> dual_seed(Basis3 value, Basis3 tangent) {
    return {dual_seed(value.u, tangent.u), dual_seed(value.v, tangent.v), dual_seed(value.k, tangent.k)};
}
UTD_DINLINE FaceMaterialParamsT<Dual> dual_seed(FaceMaterialParams value, FaceMaterialParams tangent) {
    return {{value.etaR, tangent.etaR}, {value.muR, tangent.muR}, {value.sigma, tangent.sigma},
            {value.gain, tangent.gain}, value.useFresnel,         value.present};
}
...
UTD_DINLINE PairInputsT<Dual> pair_inputs_seed(const PairInputs& value, const PairInputsGrad& tangent) {
    ...
    out.incidentNormalDerivative = dual_seed(value.incidentNormalDerivative, tangent.incidentNormalDerivative);
    ...
    out.incidentDerivativeVector = dual_seed(value.incidentDerivativeVector, tangent.incidentDerivativeVector);
    out.incidentJones = dual_seed(value.incidentJones, tangent.incidentJones);
    out.incidentDerivativeJones = dual_seed(value.incidentDerivativeJones, tangent.incidentDerivativeJones);
...
UTD_DINLINE MaterialParamsT<Dual> material_params_seed(const MaterialParams& value, float tangentOmega) {
...
// ---------------------------------------------------------------------------
UTD_DINLINE void adj_cplx_mul(Complex a, Complex b, Complex gO, Complex& gA, Complex& gB) {
    gA.re += gO.re * b.re + gO.im * b.im;
    gA.im += -gO.re * b.im + gO.im * b.re;
    gB.re += gO.re * a.re + gO.im * a.im;
    gB.im += -gO.re * a.im + gO.im * a.re;
}
UTD_DINLINE void adj_cplx_mul_real(Complex a, float b, Complex gO, Complex& gA, float& gB) {
    gA.re += gO.re * b;
    gA.im += gO.im * b;
    gB += cplx_adj_dot(gO, a);
}
UTD_DINLINE void adj_cplx_scale_real(float3a basis, Complex coeff, Complex3 gO, float3a& gBasis, Complex& gCoeff) {
...
UTD_DINLINE void adj_cplx_dot_real(Complex3 v, float3a b, Complex gO, Complex3& gV, float3a& gB) {
    gV.x.re += gO.re * b.x;
    gV.x.im += gO.im * b.x;
    gV.y.re += gO.re * b.y;
    gV.y.im += gO.im * b.y;
...
// ===================================================================
template <typename T> UTD_DINLINE T safe_length(Vec3T<T> v) {
    return sqrtf(fmaxf(f3_dot(v, v), T(0.f)));
}

template <typename T> UTD_DINLINE Vec3T<T> safe_normalize(Vec3T<T> v, Vec3T<T> fallback) {
    T n = safe_length(v);
    if (n > UTD_SMALL_EPS)
        return f3_div(v, n + T(UTD_EPS));
    T fn = safe_length(fallback);
    return f3_div(fallback, fn + T(UTD_EPS));
}

template <typename T> UTD_DINLINE T safe_acos(T v) {
    T c = fminf(fmaxf(v, T(-1.f)), T(1.f));
    T s = sqrtf(fmaxf(T(1.f) - c * c, T(0.f)));
    return atan2f(s, c);
}

template <typename T> UTD_DINLINE T cot_val(T v) {
    T s, c;
    sincosf(v, &s, &c);
    T d = (fabsf(s) < UTD_SMALL_EPS) ? T((scalar_value(s) + UTD_SMALL_EPS) >= 0.f ? UTD_SMALL_EPS : -UTD_SMALL_EPS) : s;
    T r = c / d;
...
// ===================================================================
template <typename T> UTD_DINLINE Vec3T<T> project_to_wedge_plane(Vec3T<T> v, Vec3T<T> e) {
    return f3_sub(v, f3_mul(e, f3_dot(v, e)));
}

template <typename T> UTD_DINLINE Vec3T<T> rotate_vector_around_axis(Vec3T<T> v, Vec3T<T> axis, T angle) {
    T s, c;
    sincosf(angle, &s, &c);
...
template <typename T> UTD_DINLINE Vec3T<T> normalize_in_wedge_plane(Vec3T<T> v, Vec3T<T> e) {
    return safe_normalize(project_to_wedge_plane(v, e), v3_const<T>(1, 0, 0));
}

template <typename T> UTD_DINLINE Vec3T<T> stable_perp_basis(Vec3T<T> rayDir, Vec3T<T> preferred) {
...
template <typename T> UTD_DINLINE Basis3T<T> diffraction_edge_basis(Vec3T<T> rayDir, Vec3T<T> edgeDir, bool outgoing) {
    Vec3T<T> rayHat = safe_normalize(rayDir, v3_const<T>(0, 0, 1));
    Vec3T<T> edgeHat = safe_normalize(edgeDir, v3_const<T>(0, 0, 1));
    Vec3T<T> phiHat = f3_cross(rayHat, edgeHat);
    if (outgoing)
        phiHat = f3_neg(phiHat);
    Vec3T<T> fallback = stable_perp_basis(rayHat, edgeHat);
    return basis_from_first_vector(rayHat, phiHat, fallback);
}

template <typename T>
UTD_DINLINE JonesOperatorT<T> jop_in_basis(JonesOperatorT<T> op, Basis3T<T> srcIn, Basis3T<T> srcOut, Basis3T<T> dstIn,
                                           Basis3T<T> dstOut) {
...
template <typename T = float> UTD_DINLINE Basis3T<T> basis_zero() {
...
// ===================================================================
template <typename T>
UTD_DINLINE bool wedge_exterior_mask(Vec3T<T> dirFromEdge, Vec3T<T> edgeDir, Vec3T<T> n0, Vec3T<T> nn) {
    Vec3T<T> dp = project_to_wedge_plane(dirFromEdge, edgeDir);
    T sd0 = f3_dot(dp, n0);
    T sdn = f3_dot(dp, nn);
    return (safe_length(dp) > UTD_SMALL_EPS) && ((sd0 >= -UTD_SMALL_EPS) || (sdn >= -UTD_SMALL_EPS));
}

template <typename T> UTD_DINLINE T distance_to_cot_pole(T v) {
    T np = roundf(v / UTD_PI) * UTD_PI;
    return fabsf(v - np);
}

template <typename T> UTD_DINLINE bool cot_pole_safe_mask(T phi, T phiP, T n, float guard) {
    T twoN = 2.f * n;
    T args[4] = {(UTD_PI + phi - phiP) / twoN, (UTD_PI - phi + phiP) / twoN, (UTD_PI + phi + phiP) / twoN,
                 (UTD_PI - phi - phiP) / twoN};
    for (int i = 0; i < 4; ++i)
        if (distance_to_cot_pole(args[i]) <= guard)
            return false;
    return true;
}

template <typename T> UTD_DINLINE bool slope_safe_mask(T phi, T phiP, T n, float step) {
    T npi = n * UTD_PI;
    bool interior = (phi >= step) && (phi <= npi - T(step)) && (phiP >= step) && (phiP <= npi - T(step));
    T guard = step / (2.f * n);
...
// ===================================================================
template <typename T>
UTD_DINLINE void poly12(T x, float c0, float c1, float c2, float c3, float c4, float c5, float c6, float c7, float c8,
                        float c9, float c10, float c11, T& val, T& fst, T& snd) {
    val = c11;
    fst = 0.f;
    snd = 0.f;
#define POLY_STEP(ci)                                                                                                  \
...
template <typename T> UTD_DINLINE void fresnel_boersma(T x, ComplexT<T>& val, ComplexT<T>& fst, ComplexT<T>& snd) {
    ...
    T rS, rS1, rS2;
    poly12(argS, +1.595769140f, -0.000001702f, -6.808568854f, -0.000576361f, +6.920691902f, -0.016898657f,
           -3.050485660f, -0.075752419f, +0.850663781f, -0.025639041f, -0.150230960f, +0.034404779f, rS, rS1, rS2);
    T iS, iS1, iS2;
    poly12(argS, -0.000000033f, +4.255387524f, -0.000092810f, -7.780020400f, -0.009520895f, +5.075161298f,
           -0.138341947f, -1.363729124f, -0.403349276f, +0.702222016f, -0.216195929f, +0.019547031f, iS, iS1, iS2);
    T rL, rL1, rL2;
    poly12(argL, +0.000000000f, -0.024933975f, +0.000003936f, +0.005770956f, +0.000689892f, -0.009497136f,
           +0.011948809f, -0.006748873f, +0.000246420f, +0.002102967f, -0.001217930f, +0.000233939f, rL, rL1, rL2);
    T iL, iL1, iL2;
    poly12(argL, +0.199471140f, +0.000000023f, -0.009351341f, +0.000023006f, +0.004851466f, +0.001903218f,
           -0.017122914f, +0.029064067f, -0.027928955f, +0.016497308f, -0.005598515f, +0.000838386f, iL, iL1, iL2);
...
template <typename T>
UTD_DINLINE T first_order_diffraction_parameter(Vec3T<T> sourcePos, Vec3T<T> targetPos, Vec3T<T> edgeOrigin,
                                                Vec3T<T> edgeDir) {
    ...
    rotationAxis = rotationAxisNorm > UTD_SMALL_EPS ? f3_div(rotationAxis, rotationAxisNorm + T(UTD_EPS)) : zeta;
...
template <typename T> struct FiniteEdgePointSelectionT {
    Vec3T<T> point;
    T edgeLineMin;
    T edgeLineMax;
    bool valid;
...
template <typename T>
UTD_DINLINE FiniteEdgePointSelectionT<T> finite_edge_diffraction_point(PairInputsT<T> state, Vec3T<T> targetPos) {
    ...
    T parameter = first_order_diffraction_parameter(state.sourcePos, targetPos, edgeOrigin, edgeHat);
    bool valid = (edgeLength > UTD_SMALL_EPS) && isfinite(parameter);
    bool inside = valid && (parameter > 0.f) && (parameter < edgeLength);
    return {
        f3_add(edgeOrigin, f3_mul(edgeHat, parameter)), -parameter, edgeLength - parameter, valid, inside,
    };
}

template <typename T>
UTD_DINLINE PairInputsT<T> pair_state_at_stationary_point(PairInputsT<T> state, Vec3T<T> targetPos, bool& selected,
                                                          bool& inside, bool& valid) {
    selected = false;
    inside = false;
    valid = true;
...
template <typename T> UTD_DINLINE ComplexT<T> direct_source_field(Vec3T<T> sourcePos, Vec3T<T> targetPos, T k) {
...
template <typename T>
UTD_DINLINE Complex3T<T> direct_source_vector(Vec3T<T> sourcePos, Vec3T<T> targetPos, T k, MaterialParamsT<T> mat) {
...
template <typename T>
UTD_DINLINE T poly12_value(T x, float c0, float c1, float c2, float c3, float c4, float c5, float c6, float c7,
...
template <typename T> UTD_DINLINE ComplexT<T> fresnel_boersma_value(T x) {
    bool xPos = x > 0.f;
    T xA = fabsf(x);
    bool cond = xA < 4.f;
    ...
    T rS = poly12_value(arg, +1.595769140f, -0.000001702f, -6.808568854f, -0.000576361f, +6.920691902f, -0.016898657f,
                        -3.050485660f, -0.075752419f, +0.850663781f, -0.025639041f, -0.150230960f, +0.034404779f);
    T iS = poly12_value(arg, -0.000000033f, +4.255387524f, -0.000092810f, -7.780020400f, -0.009520895f, +5.075161298f,
                        -0.138341947f, -1.363729124f, -0.403349276f, +0.702222016f, -0.216195929f, +0.019547031f);
    T rL = poly12_value(arg, +0.000000000f, -0.024933975f, +0.000003936f, +0.005770956f, +0.000689892f, -0.009497136f,
                        +0.011948809f, -0.006748873f, +0.000246420f, +0.002102967f, -0.001217930f, +0.000233939f);
    T iL = poly12_value(arg, +0.199471140f, +0.000000023f, -0.009351341f, +0.000023006f, +0.004851466f, +0.001903218f,
                        -0.017122914f, +0.029064067f, -0.027928955f, +0.016497308f, -0.005598515f, +0.000838386f);
...
UTD_DINLINE void f_utd_with_derivatives(T x, ComplexT<T>& val, ComplexT<T>& fst, ComplexT<T>& snd) {
    ...
    fst = cplx_add(cplx_add(cplx_mul_real(cplx_mul(ph, br), pf1), cplx_mul_real(cplx_mul(ph1, br), pf)),
                   cplx_mul_real(cplx_mul(ph, br1), pf));
    snd = cplx_add(cplx_add(cplx_add(cplx_mul_real(cplx_mul(ph, br), pf2), cplx_mul_real(cplx_mul(ph1, br), 2.f * pf1)),
                            cplx_mul_real(cplx_mul(ph, br1), 2.f * pf1)),
                   cplx_add(cplx_add(cplx_mul_real(cplx_mul(ph2, br), pf), cplx_mul_real(cplx_mul(ph1, br1), 2.f * pf)),
                            cplx_mul_real(cplx_mul(ph, br2), pf)));
}

template <typename T> UTD_DINLINE ComplexT<T> f_utd_value(T x) {
    T sx = fmaxf(x, T(0.0f));
...
// ===================================================================
template <typename T> UTD_DINLINE T shadow_a_threshold(T n) {
    return 8.0e-12f * fmaxf(n * n, T(1.0f));
}

template <typename T>
UTD_DINLINE ComplexT<T> cot_transition_product_value(T cotV, ComplexT<T> transition, T x, T x1, T kL, T n,
                                                     float cotSign) {
...
template <typename T>
UTD_DINLINE void beta_term_values(T beta, T n, T kL, float cotSign, bool plusBranch, T& cotV, T& c1, T& c2, T& xo,
                                  T& x1, T& x2) {
    T twoN = 2.f * n;
...
template <typename T>
UTD_DINLINE void assemble_beta_term(T cotV, T c1, T c2, T x, T x1, T x2, T kL, T n, float cotSign, ComplexT<T> tr,
                                    ComplexT<T> tr1, ComplexT<T> tr2, ComplexT<T>& val, ComplexT<T>& fst,
                                    ComplexT<T>& snd) {
    ...
    snd = cplx_add(cplx_add(cplx_mul_real(tr, c2), cplx_mul_real(tr1, 2.f * c1 * x1)),
...
template <typename T> UTD_DINLINE T incident_nearest_boundary(T beta) {
...
template <typename T> UTD_DINLINE T reflection_nearest_boundary(T beta, T n) {
    // Reflection shadow boundaries lie at beta = pi and beta = (2n-1)*pi.
    float b = scalar_value(beta);
    float bFar = (2.f * scalar_value(n) - 1.f) * UTD_PI;
    return (fabsf(b - UTD_PI) <= fabsf(b - bFar)) ? T(UTD_PI) : (2.f * n - 1.f) * T(UTD_PI);
}

template <typename T>
UTD_DINLINE ComplexT<T> mend_beta_term_value(ComplexT<T> termValue, T beta, T bStar, T n, T kL, float cotSign,
                                             bool plusBranch, T gammaOdd = T(1.f),
                                             ComplexT<T> truncEven = c_const<T>(1, 0), T isbWindow = T(0)) {
    ...
    const bool truncIsOne = scalar_value(truncEven.re) == 1.f && scalar_value(truncEven.im) == 0.f;
    if (scalar_value(gammaOdd) >= 1.f && truncIsOne && !(scalar_value(isbWindow) > 0.f))
        return termValue;
    T delta = beta - bStar;
    ...
    assemble_beta_term(cvM, c1M, c2M, xoM, x1M, x2M, kL, n, cotSign, trM, tr1M, tr2M, tvM, tfM, tsM);
    ...
    return cplx_add(cplx_mul(even, truncEven), cplx_mul_real(odd, gammaOdd * blend * isbNotch));
}
...
// ===================================================================
template <typename T>
UTD_DINLINE void diffraction_beta_groups_from_betas(T dP, T sP2, T n, T k, T s, T sP, ComplexT<T> r0, ComplexT<T> rn,
                                                    ComplexT<T>& factor, ComplexT<T>& dG, ComplexT<T>& dG1,
                                                    ComplexT<T>& sG, ComplexT<T>& sG1, ComplexT<T>& dG2,
                                                    ComplexT<T>& sG2, T gammaOdd = T(1.f),
    ...
    factor = cplx_mul_real(cplx_exp_phase(T(-0.25f * UTD_PI)), -1.f / (2.f * n * sqrtf(UTD_TWO_PI * k + T(UTD_EPS))));
    ...
    for (int i = 0; i < 4; ++i)
        f_utd_with_derivatives(xv[i], tr[i], tr1[i], tr2[i]);
    ComplexT<T> tv[4], tf[4], ts[4];
    for (int i = 0; i < 4; ++i) {
        float cotSign = (i == 0 || i == 2) ? +1.f : -1.f;
        assemble_beta_term(cv[i], c1[i], c2[i], xv[i], x1[i], x2[i], kL, n, cotSign, tr[i], tr1[i], tr2[i], tv[i],
                           tf[i], ts[i]);
    }
...
template <typename T>
UTD_DINLINE void diffraction_beta_groups(T phi, T phiP, T n, T k, T s, T sP, ComplexT<T> r0, ComplexT<T> rn,
                                         ComplexT<T>& factor, ComplexT<T>& dG, ComplexT<T>& dG1, ComplexT<T>& sG,
                                         ComplexT<T>& sG1, ComplexT<T>& dG2, ComplexT<T>& sG2, T gammaOdd = T(1.f),
                                         ComplexT<T> truncEven = c_const<T>(1, 0), T isbWindowScale = T(0)) {
    diffraction_beta_groups_from_betas(phi - phiP, phi + phiP, n, k, s, sP, r0, rn, factor, dG, dG1, sG, sG1, dG2, sG2,
                                       gammaOdd, truncEven, isbWindowScale);
}

template <typename T>
UTD_DINLINE void diffraction_beta_groups_3d_from_betas(T dP, T sP2, T n, T k, T s, T sP, T sinBeta0, ComplexT<T> r0,
                                                       ComplexT<T> rn, ComplexT<T>& factor, ComplexT<T>& dG,
                                                       ComplexT<T>& dG1, ComplexT<T>& sG, ComplexT<T>& sG1,
                                                       ComplexT<T>& dG2, ComplexT<T>& sG2, T gammaOdd = T(1.f),
    ...
    factor =
        cplx_mul_real(cplx_exp_phase(T(-0.25f * UTD_PI)), -1.f / (2.f * n * sqrtf(UTD_TWO_PI * k + T(UTD_EPS)) * sb));
    T cv[4], c1[4], c2[4], xv[4], x1[4], x2[4];
    beta_term_values(dP, n, kL, +1.f, true, cv[0], c1[0], c2[0], xv[0], x1[0], x2[0]);
    beta_term_values(dP, n, kL, -1.f, false, cv[1], c1[1], c2[1], xv[1], x1[1], x2[1]);
    beta_term_values(sP2, n, kL, +1.f, true, cv[2], c1[2], c2[2], xv[2], x1[2], x2[2]);
...
template <typename T>
UTD_DINLINE void diffraction_beta_groups_3d(T phi, T phiP, T n, T k, T s, T sP, T sinBeta0, ComplexT<T> r0,
                                            ComplexT<T> rn, ComplexT<T>& factor, ComplexT<T>& dG, ComplexT<T>& dG1,
                                            ComplexT<T>& sG, ComplexT<T>& sG1, ComplexT<T>& dG2, ComplexT<T>& sG2,
                                            T gammaOdd = T(1.f), ComplexT<T> truncEven = c_const<T>(1, 0),
                                            T isbWindowScale = T(0)) {
    diffraction_beta_groups_3d_from_betas(phi - phiP, phi + phiP, n, k, s, sP, sinBeta0, r0, rn, factor, dG, dG1, sG,
                                          sG1, dG2, sG2, gammaOdd, truncEven, isbWindowScale);
}
...
// ===================================================================
template <typename T>
UTD_DINLINE ComplexT<T> diff_coeff_2d(T phi, T phiP, T n, T k, T s, T sP, ComplexT<T> r0, ComplexT<T> rn,
                                      T gammaOdd = T(1.f), ComplexT<T> truncEven = c_const<T>(1, 0),
                                      T isbWindowScale = T(0)) {
...
template <typename T>
UTD_DINLINE ComplexT<T> diff_coeff_3d(T phi, T phiP, T n, T k, T s, T sP, T sb, ComplexT<T> r0, ComplexT<T> rn,
                                      T gammaOdd = T(1.f), ComplexT<T> truncEven = c_const<T>(1, 0),
                                      T isbWindowScale = T(0)) {
    ComplexT<T> fac, dG, dG1, sG, sG1, dG2, sG2;
...
template <typename T>
UTD_DINLINE ComplexT<T> diff_coeff_2d_angle_deriv(T phi, T phiP, T n, T k, T s, T sP, bool wrtPhi, ComplexT<T> r0,
                                                  ComplexT<T> rn) {
    ComplexT<T> fac, dG, dG1, sG, sG1, dG2, sG2;
    diffraction_beta_groups(phi, phiP, n, k, s, sP, r0, rn, fac, dG, dG1, sG, sG1, dG2, sG2);
    ComplexT<T> combined = wrtPhi ? cplx_add(dG1, sG1) : cplx_add(cplx_mul_real(dG1, -1.f), sG1);
    return cplx_mul(fac, combined);
}

template <typename T>
UTD_DINLINE ComplexT<T> diff_coeff_3d_angle_deriv(T phi, T phiP, T n, T k, T s, T sP, T sb, bool wrtPhi, ComplexT<T> r0,
                                                  ComplexT<T> rn) {
    ComplexT<T> fac, dG, dG1, sG, sG1, dG2, sG2;
...
template <typename T>
UTD_DINLINE ComplexT<T> slope_diff_2d(T phi, T phiP, T n, T k, T s, T sP, ComplexT<T> r0, ComplexT<T> rn) {
    ComplexT<T> d = diff_coeff_2d_angle_deriv(phi, phiP, n, k, s, sP, false, r0, rn);
    return cplx_div_real(cplx_mul(c_const<T>(0, -1), d), k);
}

template <typename T>
UTD_DINLINE ComplexT<T> slope_diff_3d(T phi, T phiP, T n, T k, T s, T sP, T sb, ComplexT<T> r0, ComplexT<T> rn) {
...
// ===================================================================
template <typename T> UTD_DINLINE T oriented_angle_positive(T y, T x) {
    T a = atan2f(y, x);
    return a < 0.f ? a + T(UTD_TWO_PI) : a;
}

template <typename T>
UTD_DINLINE void compute_edge_angles(Vec3T<T> srcPos, Vec3T<T> edgePos, Vec3T<T> edgeDir, Vec3T<T> n0, Vec3T<T> tgtPos,
...
template <typename T>
UTD_DINLINE void compute_edge_geometry_3d(Vec3T<T> srcPos, Vec3T<T> edgePos, Vec3T<T> edgeDir, Vec3T<T> n0,
                                          Vec3T<T> tgtPos, T& phi, T& phiP, T& s, T& sP, T& sinBeta0) {
    T sProj, sPProj;
...
UTD_DINLINE void adj_safe_normalize(float3a v, float3a fallback, float3a gO, float3a& gV, float3a& gFallback) {
    float vn = safe_length(v);
    if (vn > UTD_SMALL_EPS) {
...
UTD_DINLINE void adj_stable_perp_basis(float3a rayDir, float3a preferred, float3a gO, float3a& gRayDir,
                                       float3a& gPreferred) {
...
// ===================================================================
template <typename T> UTD_DINLINE ComplexT<T> cplx_sqrt(ComplexT<T> z) {
    T x = z.re, y = z.im;
    T r = sqrtf(x * x + y * y);
    bool nz = r > 0.f;
    bool xnn = x >= 0.f;
...
// ===================================================================
template <typename T>
UTD_DINLINE void fresnel_reflection_face(T cosTheta, T etaR, T muR, T sigma, T omega, ComplexT<T>& rTE,
                                         ComplexT<T>& rTM) {
    ...
    rTM = cplx_div(cplx_sub(cplx_mul_real(eta, ct), a), cplx_add(cplx_mul_real(eta, ct), a));
}

template <typename T>
UTD_DINLINE JonesOperatorT<T> face_reflection_operator(FaceMaterialParamsT<T> fm, T cosTheta, Vec3T<T> normal,
                                                       Vec3T<T> inHat, Vec3T<T> outHat, Basis3T<T> inEdgeBasis,
                                                       Basis3T<T> outEdgeBasis, T omega) {
    ...
    JonesOperatorT<T> diagOp =
        useFr ? JonesOperatorT<T>{cplx_mul(gain, rTE), cplx_zero<T>(), cplx_zero<T>(), cplx_mul(gain, rTM)}
    ...
    Vec3T<T> faceSOut = f3_dot(faceSOutRaw, fallbackOut) < 0.0f ? f3_neg(faceSOutRaw) : faceSOutRaw;
...
template <typename T>
UTD_DINLINE JonesOperatorT<T> fallback_face_operator(JonesOperatorT<T> stored, Vec3T<T> normal, Vec3T<T> inHat,
                                                     Vec3T<T> outHat, Basis3T<T> inEdgeBasis, Basis3T<T> outEdgeBasis) {
    (void)normal;
    (void)inHat;
    (void)outHat;
    (void)inEdgeBasis;
...
// ===================================================================
template <typename T>
UTD_DINLINE DiffractionOperatorTermsT<T> compute_op_terms_3d(T phi, T phiP, T wedgeN, T k, T s, T sP, T sinBeta0,
                                                             T gammaOdd = T(1.f),
                                                             ComplexT<T> truncEven = c_const<T>(1, 0),
                                                             T isbWindowScale = T(0)) {
    ...
    ComplexT<T> fac, dG, dG1, sG, sG1, dG2, sG2;
    diffraction_beta_groups_3d(phi, phiP, wedgeN, k, s, sP, sinBeta0, z, z, fac, dG, dG1, sG, sG1, dG2, sG2, gammaOdd,
                               truncEven, isbWindowScale);
    ...
    diffraction_beta_groups_3d(phi, phiP, wedgeN, k, s, sP, sinBeta0, z, one, fac1, dF1, dF11, sF1, sF11, dF12, sF12,
                               gammaOdd, truncEven, isbWindowScale);
    return {cplx_mul(fac, dG),    cplx_mul(fac0, sF0), cplx_mul(fac1, sF1), cplx_mul(fac, cplx_mul_real(dG1, -1.f)),
            cplx_mul(fac0, sF01), cplx_mul(fac1, sF11)};
}

template <typename T>
UTD_DINLINE DiffractionOperatorTermsT<T> compute_op_terms_2d(T phi, T phiP, T wedgeN, T k, T s, T sP,
                                                             T gammaOdd = T(1.f),
                                                             ComplexT<T> truncEven = c_const<T>(1, 0),
                                                             T isbWindowScale = T(0)) {
    T l = s * sP / (s + sP + T(UTD_EPS));
    ...
    for (int i = 0; i < 4; ++i) {
        float cotSign = (i == 0 || i == 2) ? +1.f : -1.f;
        assemble_beta_term(cv[i], c1v[i], c2v[i], xv[i], x1v[i], x2v[i], kL, wedgeN, cotSign, tr[i], tr1[i], tr2[i],
                           tv[i], tf[i], ts[i]);
    }
    ...
    ComplexT<T> factor =
        cplx_mul_real(cplx_exp_phase(T(-0.25f * UTD_PI)), -1.f / (2.f * wedgeN * sqrtf(UTD_TWO_PI * k + T(UTD_EPS))));
    ...
    return {cplx_mul(factor, difV),  cplx_mul(factor, tv[3]),
            cplx_mul(factor, tv[2]), cplx_mul(factor, cplx_mul_real(difF, -1.f)),
            cplx_mul(factor, f0F),   cplx_mul(factor, f1F)};
}
...
// ===================================================================
template <typename T>
UTD_DINLINE JonesOperatorT<T> assemble_diff_operator(ComplexT<T> free_term, ComplexT<T> face0_term,
                                                     ComplexT<T> face1_term, JonesOperatorT<T> face0Op,
                                                     JonesOperatorT<T> face1Op) {
...
template <typename T>
UTD_DINLINE ComplexT<T> finite_wedge_truncation_factor_bounds(PairInputsT<T> state, Vec3T<T> tgtPos, T k, T lineMin,
                                                              T lineMax, bool stationaryAtOrigin) {
    Vec3T<T> edgeHat = safe_normalize(state.edgeDir, v3_const<T>(0.f, 0.f, 1.f));
    ...
    T sourceRange = sqrtf(sPrimeProj * sPrimeProj + sourceOffset * sourceOffset + T(UTD_EPS));
    T targetRange = sqrtf(sProj * sProj + targetOffset * targetOffset + T(UTD_EPS));
    T curvature = sPrimeProj * sPrimeProj / (sourceRange * sourceRange * sourceRange + T(UTD_EPS)) +
...
UTD_DINLINE ComplexT<T> finite_wedge_truncation_factor_bounds(PairInputsT<T> state, Vec3T<T> tgtPos, T k, T lineMin,
                                                              T lineMax) {
    return finite_wedge_truncation_factor_bounds(state, tgtPos, k, lineMin, lineMax, false);
}

template <typename T>
UTD_DINLINE ComplexT<T> finite_wedge_truncation_factor(PairInputsT<T> state, Vec3T<T> tgtPos, T k) {
    return finite_wedge_truncation_factor_bounds(state, tgtPos, k, state.edgeLineMin, state.edgeLineMax);
}
...
template <typename T>
UTD_DINLINE T finite_wedge_monotone_truncation(PairInputsT<T> state, Vec3T<T> tgtPos, T k, T lineMin, T lineMax) {
    Vec3T<T> edgeHat = safe_normalize(state.edgeDir, v3_const<T>(0.f, 0.f, 1.f));
    Vec3T<T> edgePos = state.edgePos;
    Vec3T<T> sourcePos = state.sourcePos;
...
template <typename T>
UTD_DINLINE T corner_mend_gamma(PairInputsT<T> state, Vec3T<T> tgtPos, T k, T lineMin, T lineMax) {
    Vec3T<T> edgeHat = safe_normalize(state.edgeDir, v3_const<T>(0.f, 0.f, 1.f));
    Vec3T<T> edgePos = state.edgePos;
    Vec3T<T> sourcePos = state.sourcePos;
...
template <typename T>
UTD_DINLINE void compute_pair_field_terms(PairInputsT<T> state, Vec3T<T> tgtPos, T k, MaterialParamsT<T> mat,
                                          bool& geomValid, ComplexT<T>& field, ComplexT<T>& directGain,
                                          ComplexT<T>& derivativeGain) {
    geomValid = false;
    field = cplx_zero<T>();
    directGain = cplx_zero<T>();
    derivativeGain = cplx_zero<T>();
    ...
    state = pair_state_at_stationary_point(state, tgtPos, selectedStationary, selectedInside, selectedValid);
    if (!selectedValid)
        return;
    (void)selectedInside;
    ...
    T npi = w * UTD_PI;
    if (phi > npi)
        phi = (phi - npi < 2.f * UTD_PI - phi) ? npi : T(0);
    if (phiP > npi)
        phiP = (phiP - npi < 2.f * UTD_PI - phiP) ? npi : T(0);
    ...
    if (selectedStationary) {
        ...
        T tMono = finite_wedge_monotone_truncation(state, tgtPos, k, state.edgeLineMin, state.edgeLineMax);
    ...
    ComplexT<T> d =
        useFace ? diff_coeff_3d(phi, phiP, w, k, s, sP, sb, r0, rn, gammaOdd, truncEven, T(state.isbTaperWidthScale))
                : diff_coeff_2d(phi, phiP, w, k, s, sP, r0, rn, gammaOdd, truncEven, T(state.isbTaperWidthScale));
    if (!poleSafe) {
        d.re = d.re;
        d.im = d.im;
    } // detach (no AD in CUDA anyway)
    ...
    ComplexT<T> incidentField =
        selectedStationary ? direct_source_field(state.sourcePos, state.edgePos, k) : state.incidentField;
    ComplexT<T> incidentNormalDerivative = selectedStationary ? cplx_zero<T>() : state.incidentNormalDerivative;
    field = cplx_add(cplx_mul(incidentField, directGain), cplx_mul(incidentNormalDerivative, derivativeGain));
}

// ===================================================================
// Vector field contribution (mega-kernel core)
// ===================================================================
template <typename T> UTD_DINLINE Complex3T<T> c3_scale_real(Complex3T<T> value, T scale) {
...
UTD_DINLINE Complex3T<T> compute_pair_vector_at_angles(
    PairInputsT<T> state, Vec3T<T> tgtPos, T k, MaterialParamsT<T> mat, T phi, T phiP, T s, T sP, T sb, Basis3T<T> inEB,
    Basis3T<T> outEB, ComplexT<T> finiteFactor, T gammaOdd = T(1.f), ComplexT<T> truncEven = c_const<T>(1, 0),
    ...
    bool externalIncident = selectedStationary && (state.stationaryExternalIncident > 0.5f);
    ...
    if (externalIncident) {
        ...
        Complex3T<T> frozenInc = vector_from_jones(state.incidentJones, state.incidentBasis);
        T sPrimeNew = safe_length(f3_sub(state.edgePos, state.sourcePos)) + T(UTD_EPS);
        T sPrimeOld = fmaxf(sPrimeFrozen, T(UTD_SMALL_EPS));
        ComplexT<T> reexScale = cplx_mul_real(cplx_exp_phase(-k * (sPrimeNew - sPrimeOld)), sPrimeOld / sPrimeNew);
        ...
        incidentDerivativeVector = vector_from_jones(state.incidentDerivativeJones, state.incidentBasis);
    }
    ...
    JonesOperatorT<T> f0Op =
        (f0HasMat && !useStoredFaceOps)
            ? face_reflection_operator(state.face0Material, fminf(fmaxf(fabsf(sinf(phiP)), T(1.0e-6f)), T(1.f)),
    ...
    JonesOperatorT<T> f1Op =
        (f1HasMat && !useStoredFaceOps)
    ...
    DiffractionOperatorTermsT<T> terms = useFace ? compute_op_terms_3d(phi, phiP, state.wedgeN, k, s, sP, sb, gammaOdd,
                                                                       truncEven, T(state.isbTaperWidthScale))
                                                 : compute_op_terms_2d(phi, phiP, state.wedgeN, k, s, sP, gammaOdd,
                                                                       truncEven, T(state.isbTaperWidthScale));
    ...
    DiffractionOperatorTermsT<T> slopeTerms =
        useFace ? compute_op_terms_3d(safePhi, safePhiP, state.wedgeN, k, s, sP, sb, T(1.f), truncEven)
                : compute_op_terms_2d(safePhi, safePhiP, state.wedgeN, k, s, sP, T(1.f), truncEven);
    JonesOperatorT<T> directOp =
        assemble_diff_operator(cplx_mul_real(terms.direct, -1.f), terms.face0, terms.face1, f0Op, f1Op);
    ComplexT<T> slopeFactor = cplx(T(0), -1.f / k);
    JonesOperatorT<T> slopeOp =
        hasSlope ? assemble_diff_operator(cplx_mul(slopeFactor, cplx_mul_real(slopeTerms.directDphiPrime, -1.f)),
                                          cplx_mul(slopeFactor, slopeTerms.face0DphiPrime),
                                          cplx_mul(slopeFactor, slopeTerms.face1DphiPrime), f0Op, f1Op)
                 : jop_zero<T>();
...
template <typename T>
UTD_DINLINE Complex3T<T> compute_pair_vector_contribution_no_completion(PairInputsT<T> state, Vec3T<T> tgtPos, T k,
    ...
    return compute_pair_vector_at_angles(state, tgtPos, k, mat, phi, phiP, s, sP, sb, inEB, outEB, outerFinite,
                                         gammaOdd, truncEven, sPrimeFrozen);
}

template <typename T>
UTD_DINLINE Complex3T<T> compute_pair_vector_contribution(PairInputsT<T> state, Vec3T<T> tgtPos, T k,
                                                          MaterialParamsT<T> mat) {
...
// ===================================================================
template <typename T>
UTD_DINLINE PairOutputsT<T> compute_pair_contribution(PairInputsT<T> state, Vec3T<T> tgtPos, T k,
                                                      MaterialParamsT<T> mat) {
    PairOutputsT<T> out;
    out.field = cplx_zero<T>();
    out.vectorField = c3_zero<T>();
    bool gv;
    ComplexT<T> dg, dvg;
...
// ===================================================================
UTD_DINLINE Complex3 pair_vector_output_jvp(const PairInputs& pi, const PairInputsGrad& tangentState, float3a tgt,
                                            float3a tangentTgt, float k, float tangentK, const MaterialParams& mat,
                                            float tangentOmega) {
    ...
    const Complex3T<Dual> out = compute_pair_vector_contribution(state, target, Dual(k, tangentK), material);
...
UTD_DINLINE float pair_vjp_contract(Complex3 vecGrad, Complex3 tangent) {
    return cplx_adj_dot(vecGrad.x, tangent.x) + cplx_adj_dot(vecGrad.y, tangent.y) + cplx_adj_dot(vecGrad.z, tangent.z);
}
...
UTD_DINLINE Complex pair_vjp_complex_linear(Complex3 vecGrad, Complex3 probe) {
    const Complex3 rotated = {cplx(-probe.x.im, probe.x.re), cplx(-probe.y.im, probe.y.re),
                              cplx(-probe.z.im, probe.z.re)};
    return cplx(pair_vjp_contract(vecGrad, probe), pair_vjp_contract(vecGrad, rotated));
}

} // namespace detail

UTD_DINLINE void pair_vector_output_vjp(const PairInputs& pi, float3a tgt, float k, const MaterialParams& mat,
                                        Complex3 vecGrad, PairInputsGrad& sg, float3a& gTgt, float& gK, float& gOmega) {
    ...
    float* slots[] = {
        ...
        &seed.edgeLineMin,
        &seed.edgeLineMax,
        &seed.incidentBasis.u.x,
        &seed.incidentBasis.u.y,
        &seed.incidentBasis.u.z,
        &seed.incidentBasis.v.x,
        &seed.incidentBasis.v.y,
        &seed.incidentBasis.v.z,
        &seed.incidentBasis.k.x,
        &seed.incidentBasis.k.y,
        &seed.incidentBasis.k.z,
    };
    float* outputs[] = {
        &sg.sourcePos.x,
        &sg.sourcePos.y,
        ...
        &sg.edgeLineMin,
        &sg.edgeLineMax,
        &sg.incidentBasis.u.x,
        &sg.incidentBasis.u.y,
        &sg.incidentBasis.u.z,
        &sg.incidentBasis.v.x,
        &sg.incidentBasis.v.y,
        &sg.incidentBasis.v.z,
        &sg.incidentBasis.k.x,
        &sg.incidentBasis.k.y,
        &sg.incidentBasis.k.z,
    };
    ...
    for (int slot = 0; slot < kRealSlots; ++slot) {
        ...
        if (stationary && slot >= 18)
            break;
        *slots[slot] = 1.f;
        const Complex3 tangent = pair_vector_output_jvp(pi, seed, tgt, zero3, k, 0.f, mat, 0.f);
        *slots[slot] = 0.f;
    ...
    for (int axis = 0; axis < 3; ++axis) {
        ...
        const Complex3 tangent = pair_vector_output_jvp(pi, seed, tgt, tgtSeed, k, 0.f, mat, 0.f);
    ...
    {
        const Complex3 tangent = pair_vector_output_jvp(pi, seed, tgt, zero3, k, 1.f, mat, 0.f);
        gK += detail::pair_vjp_contract(vecGrad, tangent);
    }
    const bool storedOps = mat.omega <= 0.f;
    if (!storedOps) {
        const Complex3 tangent = pair_vector_output_jvp(pi, seed, tgt, zero3, k, 0.f, mat, 1.f);
    ...
    if (f0Mat || f1Mat) {
        float* materialSlots[] = {
            &seed.face0Material.etaR, &seed.face0Material.sigma, &seed.face0Material.gain, &seed.face0Material.muR,
            &seed.face1Material.etaR, &seed.face1Material.sigma, &seed.face1Material.gain, &seed.face1Material.muR,
        };
        float* materialOutputs[] = {
            &sg.face0Material.etaR, &sg.face0Material.sigma, &sg.face0Material.gain, &sg.face0Material.muR,
            &sg.face1Material.etaR, &sg.face1Material.sigma, &sg.face1Material.gain, &sg.face1Material.muR,
        };
    ...
    if (!f0Mat) {
        Complex* opSeeds[] = {&seed.face0Operator.m00, &seed.face0Operator.m01, &seed.face0Operator.m10,
                              &seed.face0Operator.m11};
        Complex* opOutputs[] = {&sg.face0Operator.m00, &sg.face0Operator.m01, &sg.face0Operator.m10,
                                &sg.face0Operator.m11};
        for (int slot = 0; slot < 4; ++slot) {
            *opSeeds[slot] = cplx(1.f, 0.f);
            const Complex3 probe = pair_vector_output_jvp(pi, seed, tgt, zero3, k, 0.f, mat, 0.f);
            *opSeeds[slot] = cplx_zero();
            *opOutputs[slot] = cplx_add(*opOutputs[slot], detail::pair_vjp_complex_linear(vecGrad, probe));
        }
    }
    if (!f1Mat) {
        Complex* opSeeds[] = {&seed.face1Operator.m00, &seed.face1Operator.m01, &seed.face1Operator.m10,
                              &seed.face1Operator.m11};
        Complex* opOutputs[] = {&sg.face1Operator.m00, &sg.face1Operator.m01, &sg.face1Operator.m10,
                                &sg.face1Operator.m11};
        for (int slot = 0; slot < 4; ++slot) {
            *opSeeds[slot] = cplx(1.f, 0.f);
            const Complex3 probe = pair_vector_output_jvp(pi, seed, tgt, zero3, k, 0.f, mat, 0.f);
            *opSeeds[slot] = cplx_zero();
    ...
    if (!stationary) {
        Complex* jonesSeeds[] = {&seed.incidentJones.u, &seed.incidentJones.v, &seed.incidentDerivativeJones.u,
                                 &seed.incidentDerivativeJones.v};
        Complex* jonesOutputs[] = {&sg.incidentJones.u, &sg.incidentJones.v, &sg.incidentDerivativeJones.u,
                                   &sg.incidentDerivativeJones.v};
        const bool hasDerivative =
            cplx_abs_sqr(pi.incidentDerivativeJones.u) + cplx_abs_sqr(pi.incidentDerivativeJones.v) > 0.f;
        for (int slot = 0; slot < 4; ++slot) {
            ...
            *jonesOutputs[slot] = cplx_add(*jonesOutputs[slot], detail::pair_vjp_complex_linear(vecGrad, probe));
        }
    }
}
