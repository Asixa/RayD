...
RAYD_HOST_DEVICE Vec3f load_soa_vec3(
    const float *x, const float *y, const float *z, unsigned int index) {
...
RAYD_HOST_DEVICE Vec3f load_chain_point(
    const SegmentVisibilityParams &params, unsigned int chain, int point_index) {
    const int slot = static_cast<int>(chain) * params.max_points + point_index;
    return math::make_vec3(
        params.chain_point_x[slot], params.chain_point_y[slot], params.chain_point_z[slot]);
}
...
RAYD_DEVICE std::uint32_t trace_segment(
    const SegmentVisibilityParams &params,
    Vec3f start,
    Vec3f end,
    bool active,
    unsigned int ignore_base,
    const typename Config::Traverser &traverser,
    ...
    const std::int32_t *ignore_ptr =
        params.ignore_prim_ids != nullptr ? params.ignore_prim_ids + ignore_base : nullptr;

    if (blocker_prim == nullptr) {
        return traverser.trace_occluded_ignore(
                   origin, direction, kTraceTMin, tmax, ignore_ptr, ignore_count)
            ? 0u
            : 1u;
    }

    const ::rayd::shared::rt::TriangleHit hit = traverser.trace_first_blocker(
        origin, direction, kTraceTMin, tmax, ignore_ptr, ignore_count);
    *blocker_prim = hit.hit != 0u
        ? static_cast<std::uint32_t>(global_primitive_id(
              hit.instance, hit.prim, params.face_offsets, params.n_meshes))
        : 0xFFFFFFFFu;
...
template <typename Config>
RAYD_DEVICE void segment_visibility_algo(
    const ::rayd::shared::optix::SegmentVisibilityParams &params,
    std::uint32_t ray,
    const typename Config::Traverser &traverser) {
...
template <typename Config>
RAYD_DEVICE void segment_pair_visibility_algo(
    const ::rayd::shared::optix::SegmentVisibilityParams &params,
    std::uint32_t ray,
    const typename Config::Traverser &traverser) {
    using namespace segment_visibility_algo_detail;
    ...
    params.out_visible[ray] = trace_segment<Config>(
        params, start, load_end_a(params, ray), active, ignore_base, traverser, nullptr) != 0u;
    params.out_visible_b[ray] = trace_segment<Config>(
        params, start, load_end_b(params, ray), active, ignore_base, traverser, nullptr) != 0u;
}
...
template <typename Config>
RAYD_DEVICE void axial_edge_visibility_algo(
    const ::rayd::shared::optix::SegmentVisibilityParams &params,
    std::uint32_t ray,
    const typename Config::Traverser &traverser) {
    using namespace segment_visibility_algo_detail;
    ...
    const Vec3f edge_dir = load_soa_vec3(
...
#pragma unroll
    ...
    for (int i = 0; i < ::rayd::shared::optix::SegmentVisibilityMaxSamples; ++i) {
        if (i < params.sample_count) {
            ...
            any_visible |= trace_segment<Config>(
                params, source, sample, active, 0u, traverser, nullptr);
        }
    }
...
template <typename Config>
RAYD_DEVICE void segment_chain_visibility_algo(
    const ::rayd::shared::optix::SegmentVisibilityParams &params,
    std::uint32_t chain,
    const typename Config::Traverser &traverser) {
    using namespace segment_visibility_algo_detail;
    ...
    for (int segment = 0; segment < segment_count; ++segment) {
        ...
        if (trace_segment<Config>(
                params, start, end, true, ignore_base, traverser, &blocker_prim) == 0u) {
            all_visible = 0u;
