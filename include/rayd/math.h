...
template <typename V>
RAYD_MATH_INLINE constexpr V add(V a, V b) {
    return {a.x + b.x, a.y + b.y, a.z + b.z};
}

template <typename V>
RAYD_MATH_INLINE constexpr V subtract(V a, V b) {
    return {a.x - b.x, a.y - b.y, a.z - b.z};
}

template <typename V>
RAYD_MATH_INLINE constexpr V negate(V value) {
    return {-value.x, -value.y, -value.z};
}

template <typename V, typename S>
RAYD_MATH_INLINE constexpr V scale(V value, S factor) {
    return {value.x * factor, value.y * factor, value.z * factor};
}

template <typename V, typename S>
RAYD_MATH_INLINE constexpr V divide(V value, S divisor) {
    return {value.x / divisor, value.y / divisor, value.z / divisor};
}

template <typename V>
RAYD_MATH_INLINE constexpr auto dot(V a, V b) {
    return a.x * b.x + a.y * b.y + a.z * b.z;
}

template <typename V>
RAYD_MATH_INLINE constexpr auto squared_norm(V value) {
...
RAYD_MATH_INLINE Vec3f normalize_f32(
    Vec3f value,
    float minimum_squared_norm = 1.0e-12f) {
...
template <typename V>
RAYD_MATH_INLINE constexpr V cross(V a, V b) {
    return {
        a.y * b.z - a.z * b.y,
        a.z * b.x - a.x * b.z,
        a.x * b.y - a.y * b.x,
...
template <typename T>
RAYD_MATH_INLINE Vec3<T> load_vec3(const T *values, std::int64_t index) {
...
template <typename T>
RAYD_MATH_INLINE T length(Vec3<T> value) {
    const T squared = dot(value, value);
    return sqrt(squared > T(0) ? squared : T(0));
}

template <typename T>
RAYD_MATH_INLINE Vec3<T> safe_normalize(
    Vec3<T> value,
    Vec3<T> alternate,
    T threshold,
    T epsilon) {
...
template <typename T>
RAYD_MATH_INLINE Vec3<T> stable_perpendicular_basis(
    Vec3<T> direction,
    Vec3<T> preferred,
    T threshold,
    T epsilon) {
    const Vec3<T> projected = subtract(
        preferred, scale(direction, dot(preferred, direction)));
    const Vec3<T> alternate_axis = fabs(direction.z) < T(0.9)
        ? Vec3<T>{T(0), T(0), T(1)}
        : Vec3<T>{T(0), T(1), T(0)};
    const Vec3<T> alternate_projected = subtract(
        alternate_axis, scale(direction, dot(alternate_axis, direction)));
    return safe_normalize(projected, alternate_projected, threshold, epsilon);
}

template <typename T>
RAYD_MATH_INLINE Vec3<T> transverse_project(
    Vec3<T> direction,
    Vec3<T> preferred) {
...
template <typename T>
RAYD_MATH_INLINE constexpr Complex<T> make_complex(T re, T im) {
    return {re, im};
}

template <typename T>
RAYD_MATH_INLINE constexpr Complex<T> complex_add(Complex<T> a, Complex<T> b) {
    return {a.re + b.re, a.im + b.im};
}

template <typename T>
RAYD_MATH_INLINE constexpr Complex<T> complex_subtract(Complex<T> a, Complex<T> b) {
    return {a.re - b.re, a.im - b.im};
}

template <typename T>
RAYD_MATH_INLINE constexpr Complex<T> complex_multiply(Complex<T> a, Complex<T> b) {
    return {
        a.re * b.re - a.im * b.im,
        a.re * b.im + a.im * b.re,
    };
...
RAYD_MATH_INLINE constexpr Complex<T> complex_scale(Complex<T> value, S factor) {
    return {value.re * factor, value.im * factor};
}

template <typename T>
RAYD_MATH_INLINE constexpr T complex_abs_squared(Complex<T> value) {
...
struct Dual {
    ...
    RAYD_MATH_INLINE constexpr Dual(float value, float tangent)
        : v(value), d(tangent) {}
};
...
RAYD_MATH_INLINE constexpr bool operator<(Dual a, Dual b) { return a.v < b.v; }
RAYD_MATH_INLINE constexpr bool operator>(Dual a, Dual b) { return a.v > b.v; }
RAYD_MATH_INLINE constexpr bool operator<=(Dual a, Dual b) { return a.v <= b.v; }
RAYD_MATH_INLINE constexpr bool operator>=(Dual a, Dual b) { return a.v >= b.v; }
RAYD_MATH_INLINE constexpr bool operator==(Dual a, Dual b) { return a.v == b.v; }
...
RAYD_MATH_INLINE float sqrtf(float value) { return ::sqrtf(value); }
RAYD_MATH_INLINE float fmaxf(float a, float b) { return ::fmaxf(a, b); }
RAYD_MATH_INLINE float fminf(float a, float b) { return ::fminf(a, b); }
RAYD_MATH_INLINE float fabsf(float value) { return ::fabsf(value); }
RAYD_MATH_INLINE float sinf(float value) { return ::sinf(value); }
RAYD_MATH_INLINE float cosf(float value) { return ::cosf(value); }
RAYD_MATH_INLINE float expf(float value) { return ::expf(value); }
RAYD_MATH_INLINE float atan2f(float y, float x) { return ::atan2f(y, x); }
RAYD_MATH_INLINE float roundf(float value) { return ::roundf(value); }
RAYD_MATH_INLINE float floorf(float value) { return ::floorf(value); }
RAYD_MATH_INLINE bool isfinite(float value) { return ::isfinite(value); }
...
RAYD_MATH_INLINE Dual fmaxf(Dual a, Dual b) { return a.v >= b.v ? a : b; }
RAYD_MATH_INLINE Dual fminf(Dual a, Dual b) { return a.v <= b.v ? a : b; }
...
RAYD_MATH_INLINE Dual roundf(Dual value) { return {::roundf(value.v), 0.0f}; }
RAYD_MATH_INLINE Dual floorf(Dual value) { return {::floorf(value.v), 0.0f}; }
RAYD_MATH_INLINE bool isfinite(Dual value) { return ::isfinite(value.v); }
...
RAYD_MATH_INLINE constexpr float scalar_value(float value) { return value; }
RAYD_MATH_INLINE constexpr float scalar_value(Dual value) { return value.v; }
RAYD_MATH_INLINE constexpr float scalar_tangent(float) { return 0.0f; }
RAYD_MATH_INLINE constexpr float scalar_tangent(Dual value) { return value.d; }
...
template <typename T>
using Complex3T = math::Complex3<T>;
...
template <typename T>
RAYD_MATH_INLINE Vec3T<T> v3_const(float x, float y, float z) {
    return {T(x), T(y), T(z)};
}

template <typename T = float>
RAYD_MATH_INLINE Vec3T<T> f3_zero() {
    return {T(0.0f), T(0.0f), T(0.0f)};
}

template <typename T>
RAYD_MATH_INLINE Vec3T<T> f3_add(Vec3T<T> a, Vec3T<T> b) {
    return math::add(a, b);
}

template <typename T>
RAYD_MATH_INLINE Vec3T<T> f3_sub(Vec3T<T> a, Vec3T<T> b) {
    return math::subtract(a, b);
}

template <typename T, typename S>
RAYD_MATH_INLINE Vec3T<T> f3_mul(Vec3T<T> value, S factor) {
    const T scale = T(factor);
    return {value.x * scale, value.y * scale, value.z * scale};
}

template <typename T>
RAYD_MATH_INLINE Vec3T<T> f3_neg(Vec3T<T> value) {
    return math::negate(value);
}

template <typename T>
RAYD_MATH_INLINE T f3_dot(Vec3T<T> a, Vec3T<T> b) {
    return math::dot(a, b);
}

template <typename T>
RAYD_MATH_INLINE Vec3T<T> f3_cross(Vec3T<T> a, Vec3T<T> b) {
    return math::cross(a, b);
}

template <typename T>
RAYD_MATH_INLINE T f3_len(Vec3T<T> value) {
    return sqrtf(fmaxf(f3_dot(value, value), T(0.0f)));
}

template <typename T, typename S>
RAYD_MATH_INLINE Vec3T<T> f3_div(Vec3T<T> value, S divisor) {
...
template <typename T>
RAYD_MATH_INLINE ComplexT<T> c_const(float re, float im) {
    return {T(re), T(im)};
}

template <typename T = float>
RAYD_MATH_INLINE ComplexT<T> cplx_zero() {
    return {T(0.0f), T(0.0f)};
}

template <typename T>
RAYD_MATH_INLINE ComplexT<T> cplx_add(ComplexT<T> a, ComplexT<T> b) {
    return math::complex_add(a, b);
}

template <typename T>
RAYD_MATH_INLINE ComplexT<T> cplx_sub(ComplexT<T> a, ComplexT<T> b) {
    return math::complex_subtract(a, b);
}

template <typename T>
RAYD_MATH_INLINE ComplexT<T> cplx_mul(ComplexT<T> a, ComplexT<T> b) {
    return math::complex_multiply(a, b);
}

template <typename T, typename S>
RAYD_MATH_INLINE ComplexT<T> cplx_mul_real(ComplexT<T> value, S factor) {
    const T scale = T(factor);
    return {value.re * scale, value.im * scale};
}

template <typename T, typename S>
RAYD_MATH_INLINE ComplexT<T> cplx_div_real(ComplexT<T> value, S divisor) {
    const T scale = T(divisor);
    return {value.re / scale, value.im / scale};
}

template <typename T>
RAYD_MATH_INLINE ComplexT<T> cplx_div(ComplexT<T> a, ComplexT<T> b) {
...
template <typename T>
RAYD_MATH_INLINE ComplexT<T> cplx_conj(ComplexT<T> value) {
    return {value.re, -value.im};
}

template <typename T>
RAYD_MATH_INLINE ComplexT<T> cplx_exp_phase(T phase) {
    T sine;
    T cosine;
...
template <typename T>
RAYD_MATH_INLINE T cplx_abs_sqr(ComplexT<T> value) {
...
template <typename T = float>
RAYD_MATH_INLINE Complex3T<T> c3_zero() {
    return {cplx_zero<T>(), cplx_zero<T>(), cplx_zero<T>()};
}

template <typename T>
RAYD_MATH_INLINE Complex3T<T> c3_add(Complex3T<T> a, Complex3T<T> b) {
    return {cplx_add(a.x, b.x), cplx_add(a.y, b.y), cplx_add(a.z, b.z)};
}

template <typename T>
RAYD_MATH_INLINE Complex3T<T> c3_scale(Complex3T<T> value, ComplexT<T> factor) {
    return {
...
template <typename T>
RAYD_MATH_INLINE ComplexT<T> cplx_dot_real(Complex3T<T> value, Vec3T<T> basis) {
...
template <typename T>
RAYD_MATH_INLINE Complex3T<T> cplx_scale_real(Vec3T<T> basis, ComplexT<T> factor) {
    return {
...
RAYD_MATH_INLINE bool c3_grad_any_nonzero(Complex3 value) {
    return cplx_any_nonzero(value.x) ||
           cplx_any_nonzero(value.y) ||
           cplx_any_nonzero(value.z);
}
...
RAYD_MATH_INLINE Complex c_sqrt(Complex value) {
    ...
    if (radius <= 0.0f)
        return c_make(0.0f);
    const float real_magnitude =
        sqrtf(fmaxf(0.0f, 0.5f * (radius + value.re)));
    const float imag_magnitude =
        sqrtf(fmaxf(0.0f, 0.5f * (radius - value.re)));
...
RAYD_MATH_INLINE Complex c_exp_neg_i_product(float lhs, float rhs) {
    ...
    const double reduced =
        fmod(static_cast<double>(lhs) * static_cast<double>(rhs), TwoPi);
#else
    const double reduced =
        std::fmod(static_cast<double>(lhs) * static_cast<double>(rhs), TwoPi);
#endif
...
RAYD_MATH_INLINE Complex3 c3_from_real(Vec3 value) {
...
template <typename Vec3>
RAYD_MATH_INLINE Complex3 c3_scale_complex(Vec3 basis, Complex coefficient) {
    return {
...
template <typename Vec3>
RAYD_MATH_INLINE Complex c3_dot_real(Complex3 value, Vec3 basis) {
    return c_add(
        c_add(c_mul_real(value.x, basis.x), c_mul_real(value.y, basis.y)),
...
#if defined(__CUDA_ARCH__)
    return isfinite(value.x.re) && isfinite(value.x.im) &&
           isfinite(value.y.re) && isfinite(value.y.im) &&
           isfinite(value.z.re) && isfinite(value.z.im);
#else
    return std::isfinite(value.x.re) && std::isfinite(value.x.im) &&
           std::isfinite(value.y.re) && std::isfinite(value.y.im) &&
           std::isfinite(value.z.re) && std::isfinite(value.z.im);
#endif
}

RAYD_MATH_INLINE bool fresnel_reflection_coefficients(
    float eta_r_value,
    float sigma_value,
    float mu_r_value,
    float gain,
    ...
    const Complex eta =
        c_make(eta_r, -sigma / (omega * VacuumPermittivity));
    const Complex mu = c_make(mu_r);
    const float cosine =
        fminf(fmaxf(fabsf(cos_theta), epsilon), 1.0f);
    const float sine_squared =
        fmaxf(0.0f, 1.0f - cosine * cosine);
    const Complex root =
        c_sqrt(c_sub(c_mul(mu, eta), c_make(sine_squared)));
    const Complex mu_cosine = c_make(mu_r * cosine);
    const Complex eta_cosine =
        c_make(eta.re * cosine, eta.im * cosine);
    r_te = c_scale(
        c_div(c_sub(mu_cosine, root), c_add(mu_cosine, root)),
        gain);
    r_tm = c_scale(
        c_div(c_sub(eta_cosine, root), c_add(eta_cosine, root)),
        gain);
#if defined(__CUDA_ARCH__)
...
RAYD_MATH_INLINE float free_space_amplitude(
    float wavelength,
    float distance,
    float epsilon = SmallEpsilon) {
...
RAYD_MATH_INLINE LegacySlabComplex legacy_add(
    LegacySlabComplex a, LegacySlabComplex b) {
    return math::complex_add(a, b);
}

RAYD_MATH_INLINE LegacySlabComplex legacy_sub(
    LegacySlabComplex a, LegacySlabComplex b) {
    return math::complex_subtract(a, b);
}

RAYD_MATH_INLINE LegacySlabComplex legacy_mul(
    LegacySlabComplex a, LegacySlabComplex b) {
    return math::complex_multiply(a, b);
}

RAYD_MATH_INLINE LegacySlabComplex legacy_scale(
    LegacySlabComplex value, float factor) {
    return math::complex_scale(value, factor);
}

RAYD_MATH_INLINE LegacySlabComplex legacy_div_floor(
    LegacySlabComplex a,
    LegacySlabComplex b,
...
RAYD_MATH_INLINE LegacySlabComplex legacy_div(
    LegacySlabComplex a, LegacySlabComplex b) {
    return legacy_div_floor(a, b, 1.0e-30f);
}

RAYD_MATH_INLINE LegacySlabComplex legacy_sqrt(
    ...
    const float imaginary = copysignf(
        sqrtf(fmaxf(0.0f, 0.5f * (magnitude - value.re))), value.im);
    return {real, imaginary};
}

RAYD_MATH_INLINE LegacySlabComplex legacy_interface_sqrt(
    LegacySlabComplex value) {
    const float magnitude = hypotf(value.re, value.im);
    const float real = sqrtf(fmaxf(0.0f, 0.5f * (magnitude + value.re)));
    const float imaginary_sign = value.im < 0.0f ? -1.0f : 1.0f;
    const float imaginary = imaginary_sign * sqrtf(
        fmaxf(0.0f, 0.5f * (magnitude - value.re)));
    return {real, imaginary};
}

RAYD_MATH_INLINE LegacySlabComplex legacy_exp_neg_2i(
    LegacySlabComplex value) {
...
RAYD_MATH_INLINE utd::Complex c_sqrt_passive(utd::Complex value) {
    const float magnitude = hypotf(value.re, value.im);
    const float real =
        sqrtf(fmaxf(0.5f * (magnitude + value.re), 0.0f));
...
#if defined(__CUDA_ARCH__)
    const double reduced =
        fmod(phase, 6.283185307179586476925287);
#else
    const double reduced =
        std::fmod(phase, 6.283185307179586476925287);
#endif
    float sine;
    float cosine;
#if defined(__CUDA_ARCH__)
...
RAYD_MATH_INLINE utd::Complex c_div(
    utd::Complex a,
    utd::Complex b) {
    const float denominator =
        fmaxf(b.re * b.re + b.im * b.im, 1.0e-30f);
    return utd::cplx(
        (a.re * b.re + a.im * b.im) / denominator,
        (a.im * b.re - a.re * b.im) / denominator);
}
