...
// Defines shared reflection support for epc algo.
...
RAYD_HOST_DEVICE Vec3f load_triangle_normal(const ReflEpcParams &params, int prim) {
    return reflection::epc_normalize(math::make_vec3(
...
RAYD_DEVICE void run_reflection_epc_algo(
    ...
    if (direct_plane_mode(params)) {
        for (int bounce = 0; bounce < B; ++bounce) {
            ...
            const Vec3f plane_normal = reflection::epc_normalize(math::make_vec3(
        ...
        if constexpr (DirectOnly) {
            ...
            Vec3f trace_direction = reflection::epc_normalize(math::make_vec3(
            ...
            for (int bounce = 0; bounce < B; ++bounce) {
                ...
                trace_direction = reflection::epc_normalize(math::subtract(
