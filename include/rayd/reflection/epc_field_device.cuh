...
// Defines shared reflection support for epc field device.

#pragma once

static __forceinline__ __device__ float3 fallback_axis(float3 direction) {
    return fabsf(direction.z) < 0.9f ? RAYD_REFL_EPC_MAKE3(0.f, 0.f, 1.f) : RAYD_REFL_EPC_MAKE3(0.f, 1.f, 0.f);
}

static __forceinline__ __device__ float3 stable_perpendicular(float3 direction, float3 preferred) {
...
static __forceinline__ __device__ Complex3 reflect_field_vector(const ReflEpcFieldParams params, int slot,
                                                                Complex3 field, float3 incident_dir) {
    const float3 incident_hat = normalize3(incident_dir);
    float3 normal_hat = normalize3(
        RAYD_REFL_EPC_MAKE3(params.slot_normal_x[slot], params.slot_normal_y[slot], params.slot_normal_z[slot]));
    ...
    const float3 reflected_dir = normalize3(incident_hat - 2.f * dot_dn * normal_hat);
    ...
    return c3_add(c3_scale_complex(s_hat, c_mul(r_te, e_s)), c3_scale_complex(p_out_hat, c_mul(r_tm, e_p)));
}
...
__global__ void reflection_epc_field_kernel(ReflEpcFieldParams params) {
    ...
    const bool epc_valid = params.epc_valid != nullptr && params.epc_valid[ray_index] != 0u;
    const int bounce_count = params.epc_bounce_count != nullptr ? params.epc_bounce_count[ray_index] : 0;
    const int clamped_bounce_count = min(max(bounce_count, 0), params.max_bounces);
    ...
    if (params.out_path_length != nullptr) {
        params.out_path_length[ray_index] =
            params.epc_path_length != nullptr ? params.epc_path_length[ray_index] : __uint_as_float(0x7f800000u);
    }

    if (params.out_hit_x != nullptr || params.out_normal_x != nullptr || params.out_resolved_prim_ids != nullptr ||
        ...
        for (int b = 0; b < params.max_bounces; ++b) {
            ...
            if (params.out_resolved_prim_ids != nullptr && params.resolved_prim_ids != nullptr) {
                params.out_resolved_prim_ids[slot] = params.resolved_prim_ids[slot];
            }
            if (params.out_surface_group_ids != nullptr && params.surface_group_ids != nullptr) {
                params.out_surface_group_ids[slot] = params.surface_group_ids[slot];
            }
        }
    }

    const float3 tx = RAYD_REFL_EPC_MAKE3(params.ray_ox[ray_index], params.ray_oy[ray_index], params.ray_oz[ray_index]);
    if (params.out_tx_x != nullptr) {
        ...
        if (clamped_bounce_count > 0) {
            first = RAYD_REFL_EPC_MAKE3(params.hit_x[base], params.hit_y[base], params.hit_z[base]);
            const int last_slot = base + clamped_bounce_count - 1;
            last = RAYD_REFL_EPC_MAKE3(params.hit_x[last_slot], params.hit_y[last_slot], params.hit_z[last_slot]);
        }
    ...
    float3 previous = tx;
    const float3 first_hit = RAYD_REFL_EPC_MAKE3(params.hit_x[base], params.hit_y[base], params.hit_z[base]);
    ...
    float3 transverse_polarization = tx_polarization - dot3(tx_polarization, first_dir) * first_dir;
    ...
    for (int b = 0; b < params.max_bounces; ++b) {
        const int slot = base + b;
        const float3 hit = RAYD_REFL_EPC_MAKE3(params.hit_x[slot], params.hit_y[slot], params.hit_z[slot]);
    ...
    const float3 rx = RAYD_REFL_EPC_MAKE3(params.rx_x[rx_id], params.rx_y[rx_id], params.rx_z[rx_id]);
    const float final_segment_length = norm3(rx - previous);
    const float path_length =
        params.epc_path_length != nullptr ? params.epc_path_length[ray_index] : final_segment_length;
    ...
    const float amplitude = shared::field::free_space_amplitude(params.wavelength, path_length, RAYD_REFL_EPC_EPS);
