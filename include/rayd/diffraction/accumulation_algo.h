...
// Defines shared diffraction support for accumulation algo.
...
template <typename Policy, typename Traverser> struct DiffractionAccumulationAlgo {
    ...
    RAYD_HOST_DEVICE static Vec3f grid_cell_center(int cell) {
        ...
        if (Policy::params().grid_axis == 0) {
            return ::rayd::shared::math::make_vec3(Policy::params().grid_position, c0, c1);
        }
        if (Policy::params().grid_axis == 1) {
            return ::rayd::shared::math::make_vec3(c0, Policy::params().grid_position, c1);
        }
        return ::rayd::shared::math::make_vec3(c0, c1, Policy::params().grid_position);
    }
    ...
    RAYD_HOST_DEVICE static Vec3f stable_perpendicular(Vec3f axis, Vec3f preferred) {
        Vec3f projected = preferred - ::rayd::shared::math::dot(preferred, axis) * axis;
    ...
    RAYD_HOST_DEVICE static bool keller_grid_hit_from_incident(Vec3f incident_vec, unsigned int lane,
        ...
        const Vec3f basis1 = ::rayd::shared::math::normalize_f32(::rayd::shared::math::cross(edge_dir, basis0));
        float s;
        float c;
        sincos_f(2.f * kPi * uniform01(lane, stream, static_cast<unsigned int>(Policy::params().seed)), &s, &c);
        const Vec3f ko = ::rayd::shared::math::normalize_f32(axial * edge_dir + radial * (c * basis0 + s * basis1));
    ...
    RAYD_HOST_DEVICE static bool load_triangle(int prim, Vec3f& p0, Vec3f& e1, Vec3f& e2, Vec3f& normal) {
        ...
        normal = ::rayd::shared::math::normalize_f32(normal);
    ...
    RAYD_HOST_DEVICE static bool intersect_reflection_triangle(Vec3f image_source, Vec3f target, int prim,
        ...
        const Vec3f dir = (1.f / dist) * delta;
        const Vec3f h = ::rayd::shared::math::cross(dir, e2);
        const float a = ::rayd::shared::math::dot(e1, h);
        if (fabsf(a) <= 1e-7f) {
            return false;
        }
        const float f = 1.f / a;
        const Vec3f s = image_source - p0;
        const float u = f * ::rayd::shared::math::dot(s, h);
        if (u < -1e-5f || u > 1.f + 1e-5f) {
            return false;
        }
        const Vec3f q = ::rayd::shared::math::cross(s, e1);
        const float v = f * ::rayd::shared::math::dot(dir, q);
        if (v < -1e-5f || u + v > 1.f + 1e-5f) {
            return false;
        }
        const float t = f * ::rayd::shared::math::dot(e2, q);
    ...
    RAYD_HOST_DEVICE static bool suffix_reflection_connection(Vec3f diff_point, Vec3f target, int face0_prim,
        ...
        const float plane_distance = ::rayd::shared::math::dot(diff_point - p0, normal);
        ...
        const float incoming_dist = ::rayd::shared::math::length_f32(incoming);
        const float outgoing_dist = ::rayd::shared::math::length_f32(outgoing);
        if (!(incoming_dist > kDfrEps) || !(outgoing_dist > kDfrEps)) {
            return false;
        }
        const Vec3f incoming_hat = (1.f / incoming_dist) * incoming;
        const Vec3f oriented_normal = ::rayd::shared::math::dot(incoming_hat, normal) > 0.f ? (-1.f * normal) : normal;
        const Vec3f reflected_hat =
            incoming_hat - 2.f * ::rayd::shared::math::dot(incoming_hat, oriented_normal) * oriented_normal;
        const Vec3f outgoing_hat = (1.f / outgoing_dist) * outgoing;
        if (::rayd::shared::math::dot(reflected_hat, outgoing_hat) <= 1.f - 1e-3f) {
            return false;
        }
    ...
    template <bool PrimaryOnly> RAYD_DEVICE void run_diffraction_chain_accumulation_algo(std::uint32_t lane) const {
        ...
        const Vec3f first_edge_dir = ::rayd::shared::math::normalize_f32(state_edge_dir_at(first_idx));
        ...
        const Vec3f second_edge_dir = ::rayd::shared::math::normalize_f32(recursive_state_edge_dir_at(second_idx));
        ...
        if (Policy::params().max_order == 3) {
            ...
            third_edge_dir = ::rayd::shared::math::normalize_f32(recursive_state_edge_dir_at(third_idx));
